...
class FileUpload extends \ConvertForms\Field
{
		...
		foreach ($value as $key => &$source_file)
		{
			...
			$this->app->registerEvent('onConvertFormsSubmissionBeforeSave', function(&$data) use ($key, $source_file, $destination_file)
			{
				try
				{
					...
					$this->maybeResizeImage($destination_file, $tmpData);
    ...
    /**
     * Maybe resize the uploaded image.
	 * 
	 * @param  string  $destination_file  The destination file path
	 * @param  array   $tmpData           The submission data.
	 * 
	 * @return void
     */
    public function maybeResizeImage(&$destination_file, $tmpData)
    {
		// Get file type
		$file_type = Mimes::detectFileType($destination_file);
        ...
        $form = \ConvertForms\Form::load($tmpData['form_id']);

        $resizeImages = isset($form['fields'][$tmpData['field_name']]['resizeimages']) ? $form['fields'][$tmpData['field_name']]['resizeimages'] : null;
        if (!$resizeImages)
        {
            return;
        }

		if (!isset($resizeImages['by']) || $resizeImages['by'] === 'disabled')
		{
			return;
		}

		$by 	= $resizeImages['by'];
		$width  = isset($resizeImages['width']) && $resizeImages['width'] ? $resizeImages['width'] : null;
		$height = isset($resizeImages['height']) && $resizeImages['height'] ? $resizeImages['height'] : null;

		$resizeWidth = in_array($by, ['width', 'custom']) ? $width : null;
		$resizeHeight = in_array($by, ['height', 'custom']) ? $height : null;
		...
		\NRFramework\Image::resizeByWidthOrHeight($destination_file, $resizeWidth, $resizeHeight);
    }
	...
	public function onAjax($form_id, $field_key)
	{
		...
		{
			case 'upload':
				return $this->actionUpload($form_id, $field_key);
				break;
	...
	private function actionUpload($form_id, $field_key)
	{
		...
		try {
			...
			return [
				...
				'file_encode' => base64_encode(Helper::pathTorelative($uploaded_filename))
			];
		} catch (\Throwable $th)
		{
	...
	{
		$file = implode(DIRECTORY_SEPARATOR, [JPATH_PLUGINS, 'system', 'nrframework', 'fields', 'tffileupload.php']);
		...
		$inst = new \JFormFieldTFFileUpload();
