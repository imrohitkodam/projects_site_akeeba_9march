...
class FieldChoice extends \ConvertForms\Field
{
	...
	protected function getChoices()
	{
        ...
        foreach ($field->choices['choices'] as $key => $choiceValue)
        {
            ...
            // Temporary workaround: strip_tags shouldn't be here in the first place
            // Only strip tags if choice value is empty and we have both opening and closing HTML tags
            $cleanLabel = $choiceValue['value'] == '' && preg_match('/<[^>]*>/', $label) ? strip_tags($label) : $label;
            $value = $choiceValue['value'] == '' ? $cleanLabel : $choiceValue['value'];
	...
	public function onBeforeFormSave($model, $form_data, &$field_options)
	{
        ...
        if (!$valid)
        {
            $model->setError(Text::sprintf('COM_CONVERTFORMS_NO_CHOICES', $field_options['label']));
        }
