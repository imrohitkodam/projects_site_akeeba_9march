...
class FieldsHelper
{
    ...
    public static function getFieldTypes()
    {
        ...
        foreach (self::$fields as $group => $fields)
        {
            ...
            $arr[$group] = array(
                'name'  => $group,
                'title' => Text::_('COM_CONVERTFORMS_FIELDGROUP_' . strtoupper($group))
            );
            ...
            foreach ($fields as $key => $field)
            {
                $arr[$group]['fields'][] = array(
                    'name'  => $field,
                    'title' => Text::_('COM_CONVERTFORMS_FIELD_' . strtoupper($field)),
                    'desc'  => Text::_('COM_CONVERTFORMS_FIELD_' . strtoupper($field) . '_DESC'),
    ...
    public static function prepare($form, $classPrefix = 'cf')
    {
        ...
        $cssVars = [
            ...
            // Label settings
            'label-color'			 => $params->get('labelscolor', '#585858'),
            'label-size'			 => (int) $params->get('labelsfontsize', 16) . 'px',
            'label-weight'           => (int) $params->get('labelweight', 400),
            
            // Input settings
            'input-color'			 => $params->get('inputcolor', '#585858'),
            'input-placeholder-color' => $params->get('inputcolor', '#585858') . '70',
        ...
        {
            $imgurl = intval($params->get("bgimage")) == 1 ? Uri::root() . Helper::cleanLocalImage($params->get('bgfile', '')) : $params->get("bgurl");
