...
class Helper
{
    ...
    public static function parseIfShortcode(&$subject, $submission = null)
    {
        ...
        // Prevent unnecessary load by quickly checking if the subject contains IF shortcodes.
        if (is_string($subject) && strpos($subject, '{if') === false)
        {
            return;
        }
        ...
        if ($submission)
        {
            ...
            $payload = [
                ...
                'submission.modified' => $submission->modified,
                'submission.form_id'  => $submission->form_id,
                'submission.user_id'  => $submission->user_id,
                'submission.state'    => $submission->state
            ];

            // This should be passed as an anonymous function 
            if (strpos($subject, 'submission.count') !== false)
            {
                $payload['submission.count'] = number_format(Form::getSubmissionsTotal($submission->form_id));
            }
    
            foreach ($submission->prepared_fields as $key => $field)
            {
                if (method_exists($field->class, 'prepareValueAsObject'))
                {
                    $field->value_raw = $field->class->prepareValueAsObject($field->value_raw);
                }

                $payload['field.' . $key] = $field->value_raw;
                $payload['field.' . $key . '.html'] = $field->value_html;
            }
        }
        
        $sc_helper = new \NRFramework\Parser\ShortcodeParserHelper($subject, $payload);
