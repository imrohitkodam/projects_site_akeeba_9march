...
class Plugin extends CMSPlugin
{
    ...
    {
        $service = array(
            'name'  => Text::_('PLG_CONVERTFORMS_' . strtoupper($this->getName()) . '_ALIAS'),
            'alias' => $this->getName()
        );
    ...
    protected function validateLead()
    {
        ...
        {
            throw new \Exception(Text::_('COM_CONVERTFORMS_INVALID_LEAD'));
        }
        ...
        if (!isset($this->lead->email) || empty($this->lead->email))
        {
            throw new \Exception(Text::_('COM_CONVERTFORMS_FORM_IS_MISSING_THE_EMAIL_FIELD'));
        }
    }
