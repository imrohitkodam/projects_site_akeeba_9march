...
class ConvertFormsModelForm extends AdminModel
{
    ...
    {
        Text::script('COM_CONVERTFORMS_FORM');
        Text::script('COM_CONVERTFORMS_FORM_NAME');
        ...
        Text::script('COM_CONVERTFORMS_SUBMISSION_DATE_MODIFIED');
    ...
    public function copy($id)
    {
        ...
        $newFormId = $this->getState('form.id');

        // Import plugins to allow them to handle duplication logic
        \Joomla\CMS\Plugin\PluginHelper::importPlugin('convertformstools');

        // Trigger event for plugins to handle duplication of additional data (e.g., tasks)
        Factory::getApplication()->triggerEvent('onConvertFormsDuplicate', [$newFormId, $id]);
