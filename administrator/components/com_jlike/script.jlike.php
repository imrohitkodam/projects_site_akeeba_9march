...
class com_jlikeInstallerScript
{
	 ...
	 * @return void
	 */
	public function preflight(/** @scrutinizer ignore-unused */ $type, /** @scrutinizer ignore-unused */ $parent)
	{
	...
	public function install(/** @scrutinizer ignore-unused */ $parent)
	{
		$this->migratelikes();
	}
...
<?php
	 ...
	 */
	public function postflight(/** @scrutinizer ignore-unused */ $type, $parent)
	{
...
*/
	 ...
	 */
	private function _uninstallSubextensions(/** @scrutinizer ignore-unused */ $parent)
	{
		$db =  Factory::getDBO();
...
<?php
	...
	public function update($parent)
	{
			...
			if (count($queries) != 0)
			{
				foreach ($queries as $query)
				{
					...
					if ($query != '' && $query[0] != '#')
					{
						...
						if (!$db->execute())
						{
							Factory::getApplication()->enqueueMessage(Text::sprintf('JLIB_INSTALLER_ERROR_SQL_ERROR', $db->stderr(true)), 'warning');
		...
		else
		{
			...
			$config->getvalue('config.dbprefix');
		}
	 ...
	 * @return CMSObject The subextension uninstallation status
	 */
	private function _uninstallObsoleteSubextensions(/** @scrutinizer ignore-unused */ $parent)
	{
	...
	public function deFaultPermissionsFix()
	{
		...
		$query->where($db->quoteName('name') . '= ' . $db->quote('com_jlike'));
		$db->setQuery($query);
	...
	public function setDefaultLayout($type)
	{
		if ($type == 'install' && JVERSION >= '4.0.0')
		{
			...
			$query->where($db->quoteName('element') . ' = ' . $db->quote('com_jlike'));
			$db->setQuery($query);
			$data = $db->loadObject();
