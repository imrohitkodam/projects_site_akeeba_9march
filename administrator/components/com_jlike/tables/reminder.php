 ...
 * @since  1.6
 */
class JlikeTablereminder extends Table
{
	/**
	 * Constructor
	 *
	...
	public function __construct(&$db)
	{
		if (JVERSION < 4.0)
		{
			JObserverMapper::addObserverClassToClass('JTableObserverContenthistory', 'JlikeTablereminder', array('typeAlias' => 'com_jlike.reminder'));
		}
		else
		{
		...
		parent::__construct('#__jlike_reminders', 'id', $db);
	}
	...
	public function bind($array, $ignore = '')
	{
		...
		if (!Factory::getUser()->authorise('core.admin', 'com_jlike.reminder.' . $array['id']))
		{
			$actions         = Access::getActionsFromFile(
				JPATH_ADMINISTRATOR . '/components/com_jlike/access.xml',
				"/access/section[@name='reminder']/"
			);
			$default_actions = Access::getAssetRules('com_jlike.reminder.' . $array['id'])->getData();
			$array_jaccess   = array();
	...
	protected function _getAssetName()
	{
		...
		return 'com_jlike.reminder.' . (int) $this->$k;
	}
	...
	public function delete($pk = null)
	{
		...
		// Delete contentids after reminder deleted
		$query->delete($this->_db->quoteName('#__jlike_reminder_contentids'));
