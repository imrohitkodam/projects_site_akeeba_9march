 ...
 * @since 5.0.0
 */
trait JPageBuilderFrameworkFonts {
	private function getInstalledFont(string $familyName, string $fontType) {
		...
		$query->select ( [ 
				'id',
				'data'
		] )->from ( $db->quoteName ( '#__jpagebuilder_fonts' ) )->where ( $db->quoteName ( 'family_name' ) . ' = ' . $db->quote ( $familyName ) )->where ( $db->quoteName ( 'type' ) . ' = ' . $db->quote ( $fontType ) );
	...
	private function loadInstalledFonts() {
		...
		$query->select ( '*' )->from ( $db->quoteName ( '#__jpagebuilder_fonts' ) );
	...
	private function loadGoogleFonts() {
		...
		$API_KEY = $cParams->get ( 'google_font_api_key', 'AIzaSyDksiuBas8gVxA8_nxLUw69sq3FmJQR1wU' );
		...
		$cachePath = JPATH_CACHE . '/jpagebuilder';
	...
	private function saveFontData(string $familyName, string $type, $fontData) {
		...
		try {
			...
			if (! empty ( $installedFont->id )) {
				...
				$db->updateObject ( '#__jpagebuilder_fonts', $data, 'id', true );
				$response = ( object ) [ 
						'id' => $data->id
				];
				...
				$db->insertObject ( '#__jpagebuilder_fonts', $data, 'id' );
				$response = ( object ) [ 
						'id' => $data->id
				];
	...
	private function generateFamily($data) {
		...
		$linkFamily = str_replace ( ' ', '+', $family ) . ($hasItalic ? ':ital' : '') . ($hasItalic && $hasWeight ? ',' : '') . (! $hasItalic && $hasWeight ? ':' : '') . ($hasWeight ? 'wght' : '') . (($hasWeight || $hasItalic) && $individualStylesString ? '@' . $individualStylesString : '');
	...
	private function installGoogleFont($fontData) {
		...
		$baseDirectory = JPATH_ROOT . '/media/com_jpagebuilder/assets/google-fonts/' . $fontData->family;
	...
	private function uninstallFont() {
		...
		$baseDirectory = JPATH_ROOT . '/media/com_jpagebuilder/assets/';
		$directory = $type === 'google' ? 'google-fonts' : 'custom-fonts';

		if (! $this->isInstalledFont ( $familyName, $type )) {
			$this->sendResponse ( [ 
					'message' => 'Do not find the font to uninstall.'
			], 400 );
		}
		...
		$query->delete ( '#__jpagebuilder_fonts' )
