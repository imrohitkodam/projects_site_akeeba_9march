 ...
 * @since 5.0.0
 */
trait JPageBuilderFrameworkUploadFont {
	...
	private function uploadFontFiles() {
		...
		$fileExtension = strtolower ( pathinfo($fontFile ['name'], PATHINFO_EXTENSION) );
		...
		if (File::upload ( $fontFile ['tmp_name'], $zipPath, false, true )) {
			...
			if (! empty ( $fontName )) {
				$mediaDirectory = JPATH_ROOT . '/media/com_jpagebuilder/assets/custom-fonts/' . $fontName;
