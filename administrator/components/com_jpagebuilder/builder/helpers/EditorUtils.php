 ...
 * Utils helper class
 */
final class JpagebuilderEditorUtils {
	...
	public static function cleanXSS($pageData) {
		...
		array_walk_recursive ( $pageData, function (&$value) {
			if (is_string ( $value )) {
				...
				$blockedTags = array_filter ( $tagBlacklist, function ($tag) use ($allowedTags) {
					if (! in_array ( $tag, $allowedTags )) {
						return $tag;
					}
				} );
				$value = InputFilter::getInstance ( $blockedTags, [ ], InputFilter::ONLY_BLOCK_DEFINED_TAGS, InputFilter::ONLY_BLOCK_DEFINED_ATTRIBUTES, 0 )->clean ( $value, 'html' );
			}
		} );
	...
	public static function stringifyLinkItem($value) {
		...
		switch ($value->type) {
			...
			case 'menu' :
				...
				break;
			case 'page' :
				$response->url = Route::_ ( 'index.php?option=com_jpagebuilder&view=page&id=' . $value->page );
				break;
		}
