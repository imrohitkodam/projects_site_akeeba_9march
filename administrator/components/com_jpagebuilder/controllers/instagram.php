<?php
/**
 * @package     JPageBuilder.Administrator
 *
 ...
 * @since 1.5
 */
class JpagebuilderControllerInstagram extends BaseController {
	public function accessToken() {
		...
		$redirectUrl = Uri::root () . 'administrator/index.php?option=com_jpagebuilder&task=instagram.accessToken';
		...
		if ($input->get ( 'code', '' ) === '') {
			 ...
			 * token.
			 * A long lived token generally expires after 60 days.
			 */
		 ...
		 */
		$onClick = "
            window.opener.document.querySelector('.jpb-ig-token #access_token').value='" . $token . "';
            window.opener.document.querySelector('.jpb-ig-token #access_token').dispatchEvent(new Event('change'));
            window.opener.document.querySelector('.jpb-ig-token #ig_id').value='" . $igId . "';
            window.opener.document.querySelector('.jpb-ig-token #ig_id').dispatchEvent(new Event('change'));
            window.close();
        ";

		echo '<div class="wrapper" style="width: 100%;text-align: center;margin-top: 100px;word-break: break-word;">' . '<div><strong>Access Token: </strong>' . $token . '</div>' . '<div style="margin-top: 30px;"><strong>Instagram Id: </strong>' . $igId . '</div>' . '<button type="button" onclick="' . $onClick . '" class="btn btn-primary" style="cursor: pointer; margin-top: 50px;display: inline-block;font-weight: 400;color: #212529;text-align: center;vertical-align: middle;-webkit-user-select: none;-moz-user-select: none;-ms-user-select: none;user-select: none;background-color: transparent;border: 1px solid transparent;padding: 0.375rem 0.75rem;font-size: 1rem;line-height: 1.5;border-radius: 0.25rem;transition: color 0.15s ease-in-out, background-color 0.15s ease-in-out,border-color 0.15s ease-in-out, box-shadow 0.15s ease-in-out;background-color: #00d1b2;border-color: transparent;color: #fff;">Insert</button>' . '</div>';
