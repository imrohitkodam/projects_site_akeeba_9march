...
class JFormFieldCategoryEdit extends ListField {
	...
	protected function getOptions() {
		...
		$query->select ( 'a.id AS value, a.title AS text, a.level, a.published, a.lft, a.language' )->from ( '#__categories AS a' );
		...
		if ($oldCat != 0 && ($this->element ['parent'] == true || $jinput->get ( 'option' ) == 'com_categories')) {
			...
			$query->join ( 'LEFT', $db->quoteName ( '#__categories' ) . ' AS p ON p.id = ' . ( int ) $oldCat )->where ( 'NOT(a.lft >= p.lft AND a.rgt <= p.rgt)' );

			$rowQuery = method_exists ( $db, 'createQuery' ) ? $db->createQuery () : $db->getQuery ( true );
			$rowQuery->select ( 'a.id AS value, a.title AS text, a.level, a.parent_id' )->from ( '#__categories AS a' )->where ( 'a.id = ' . ( int ) $oldCat );
	...
	protected function getInput() {
		...
		if ($this->allowAdd) {
			...
			$attr .= ' data-custom_group_text="' . $customGroupText . '" ' . 'data-no_results_text="' . Text::_ ( 'JGLOBAL_ADD_CUSTOM_CATEGORY' ) . '" ' . 'data-placeholder="' . Text::_ ( 'JGLOBAL_TYPE_OR_SELECT_CATEGORY' ) . '" ';
