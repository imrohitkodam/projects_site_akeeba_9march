...
class JpagebuilderModelPages extends ListModel {
	...
	protected function getListQuery() {
		...
		$query->from('#__jpagebuilder as a');
		...
		if (is_numeric ( $categoryId )) {
			$db = Factory::getContainer()->get('DatabaseDriver');
			$cat_tbl = new Joomla\CMS\Table\Category($db);
	...
	public function getItems() {
		...
		if (is_array ( $items ) && count ( $items )) {
			foreach ( $items as $key => &$item ) {
				$item->preview = JpagebuilderHelperRoute::getPageRoute ( $item->id, $item->language );
				$item->frontend_edit = JpagebuilderHelperRoute::getFormRoute ( $item->id, $item->language );
			}
		}
		return $items;
	}
