...
class Quick2cartControllerDelaysreport extends quick2cartController
{
	...
	public function  csvexport()
	{
		...
		// 'Product Name';
		$headColumn[1] = Text::_('COM_QUICK2CART_DELAYSREPORT_ORDER_ID');
		$headColumn[2] = Text::_('COM_QUICK2CART_DELAYSREPORT_DAYS');
		$headColumn[3] = Text::_('COM_QUICK2CART_DELAYSREPORT_BUYER');
		$headColumn[4] = Text::_('COM_QUICK2CART_DELAYSREPORT_STATUS');
		...
		if (!empty($CSVData))
		{
			...
			foreach ($CSVData as $data)
			{
				...
				if ($data['status'] == 'C')
				{
					$status = Text::_('ORDER_CONFIRMED');
				}
				elseif ($data['status'] == 'S')
				{
					$status = Text::_('ORDER_SHIPPED');
				}
				else
				{
					$status = Text::_('ORDER_CANCELLED');
				}
		...
		echo $csvData . "\n";
		jexit();
		$link = Uri::base() . substr(Route::_('index.php?option=com_quick2cart&view=delaysreport', false), strlen(Uri::base(true)) + 1);
