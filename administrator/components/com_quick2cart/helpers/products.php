...
class Quick2cartBackendProductsHelper
{
	...
	public function getProductLink($item_id, $linkType = 'detailsLink')
	{
		...
		switch ($res["parent"])
		{
			case "com_content":
				...
				{
					$link = Uri::base() . 'index.php?option=com_content&task=article.edit&id=' . $res["product_id"];
				}
				break;
			...
			case "com_quick2cart":
				if ($linkType == 'detailsLink')
				{
					...
					$link           = Route::_($q2cLink, false);
				}
				elseif ($linkType == 'editLink')
				{
					$link = Uri::base() . 'index.php?option=com_quick2cart&view=product&layout=new&item_id=' . $res["product_id"];
				}
				break;
			...
			case "com_zoo":
				if ($linkType == 'detailsLink')
				{
					...
					$link = Route::_("index.php?option=com_zoo&task=item&item_id=" . $res["product_id"] . "&Itemid=" . $Itemid, false);
				}
				elseif ($linkType == 'editLink')
				{
					if (File::exists(JPATH_ADMINISTRATOR . '/components/com_zoo/config.php'))
					{
						...
						$link = Uri::base() . 'index.php?option=com_zoo&controller=item&changeapp=' . $appId . '&task=edit&cid[]=' . $res["product_id"];
					}
				}
				break;
			...
			case "com_k2":
				...
				if ($linkType == 'detailsLink')
				{
					...
					$link = Route::_($k2link, false);
				}
				elseif ($linkType == 'editLink')
				{
					$link = Uri::base() . 'index.php?option=com_k2&view=item&cid=' . $res["product_id"];
				}
				break;
			...
			case "com_flexicontent":
				if ($linkType == 'detailsLink')
				{
					$link = Uri::base() . 'index.php?option=com_flexicontent&task=items.edit&cid[]=' . $res["product_id"];
				}
				elseif ($linkType == 'editLink')
				{
					$link = Uri::base() . 'index.php?option=com_flexicontent&task=items.edit&cid[]=' . $res["product_id"];
	...
	public function getProductParentName($item_id)
	{
		...
		switch ($res["parent"])
		{
			default:
			case 'com_quick2cart':
				$parent = Text::_('COM_QUICK2CART_NATIVE');
				break;

			case 'com_content':
				$parent = Text::_('COM_QUICK2CART_CONTENT_ARTICLES');
				break;

			case 'com_flexicontent':
				$parent = Text::_('COM_QUICK2CART_FLEXICONTENT');
				break;

			case 'com_k2':
				$parent = Text::_('COM_QUICK2CART_K2');
				break;

			case 'com_zoo':
				$parent = Text::_('COM_QUICK2CART_ZOO');
				break;

			case 'com_cobalt':
			$parent = Text::_('COM_QUICK2CART_COBALT');
			break;
		}
