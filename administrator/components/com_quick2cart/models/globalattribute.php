 ...
 * @since  2.5
 */
class Quick2cartModelglobalAttribute extends AdminModel
{
	/**
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_quick2cart.edit.attribute.data', array());
	...
	public function save($data)
	{
		...
		$options          = $input->post->get('options', array(), 'array');
		...
		for ($i = 0;$i < count($ordering);$i++)
		{
			for ($j = $i + 1;$j < count($ordering);$j++)
			{
				...
				{
					$this->setError(Text::_('COM_QUICK2CART_DUPLICATE_OPTION_ORDERING_ERROR'));
				...
				if ($name[$i] == $name[$j])
				{
					$this->setError(Text::_('COM_QUICK2CART_DUPLICATE_OPTION_NAME_ERROR'));
	...
	/**
	 * Function to validate global attributes and related options
	 *
	 * @param   ARRAY  $data  data to be saved
	 *
	 * @return true/false
	...
	public function validate($form, $data, $group = null)
	{
		...
		$query->select('count(*)');
		$query->from('#__kart_global_attribute');
		$query->where('attribute_name' . ' = ' . $db->q($data['attribute_name']));
		...
		if ($count)
		{
			$this->setError(Text::_('COM_QUICK2CARET_MSG_ERROR_SAVE_GLOBAL_ATTR'));
