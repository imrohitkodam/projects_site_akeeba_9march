 ...
 * @since       2.2
 */
class Quick2cartTablePayout extends Table
{
	/**
	 * Constructor
	 *
	...
	{
		parent::__construct('#__kart_payouts', 'id', $_db);
	}
	...
	public function bind ($array, $ignore = '')
	{
		...
		if (! Factory::getUser()->authorise('core.admin', 'com_quick2cart.region.' . $array['id']))
		{
			$actions = Access::getActionsFromFile(JPATH_ADMINISTRATOR . '/components/com_quick2cart/access.xml', "/access/section[@name='region']/");
			$default_actions = Access::getAssetRules('com_quick2cart.region.' . $array['id'])->getData();
			$array_jaccess = array();
	...
	public function publish ($pks = null, $state = 1, $userId = 0)
	{
		...
		$this->_db->setQuery(
				'UPDATE `' . $this->_tbl . '`' . ' SET `state` = ' . (int) $state . ' WHERE (' . $where . ')' . $checkin
			);
	...
	protected function _getAssetName ()
	{
		...
		return 'com_quick2cart.payout.' . (int) $this->$k;
	}
