			...
			if (selectedOptionvalue != 0)
			{
				...
				techjoomla.jQuery.ajax({
					...
					success: function(response)
					{
						...
						{
							techjoomla.jquery(wrapper).append(`<div class="control-group row" id="qtcoptionheading">
																	<div class="controls">
																		<span class="qtc-attributeset-name span3">
						...
						techjoomla.jquery(wrapper).append(`<div class="control-group row">
							<div id="qtcoptionclone" class="controls qtcattributeclone">
								<span class="span3">
									<input type="text" class="input-medium disabled center" disabled="disabled" id="qtcattribute${selectedOptionvalue}" name="attributes[${optionListCount}][attribute_name]" placeholder="attribute Name" value="${selectedOptiontext}">
								</span>
								<span class="span3">
									<input type="text" class="input-small" name="attributes[${optionListCount}][attribute_option]" placeholder="Order" value="<?php echo $key+1;?>">
								</span>
								<span class="span3">
									<a class="btn btn-small btn-danger span3" onclick="removeclone(${selectedOptionvalue})"><?php echo Text::_('COM_QUICK2CART_REMOVE_OPTION');?></a>
								</span>
								<span class="center span3">
									<input type="hidden" class="input-small" name="attributes[${optionListCount}][id]" placeholder="attribute id" value="${selectedOptionvalue}">
								</span>
							</div>
						</div>`);
						optionListCount++;
		...
		<div class="row-fluid">
			...
			<?php endif;?>
				<div class="span12 form-horizontal">
					...
					<?php else:?>
						<div class="control-group">
							...
							<div class="controls">
								<?php echo HTMLHelper::_('select.genericlist', $this->attributeList, "attributelist", 'class="ad-status inputbox input-large" size="1" name="attributeList"', "value", "text", $this->state->get('filter.campaignslist'));?>
								<div class="btn btn btn-success" onclick="add_attribute_to_list()"><?php echo Text::_('COM_QUICK2CART_ADD_OPTION');?></div>
							</div>
						</div>
					<?php endif;?>
				</div><br><br>
				...
				if (!empty($this->attributeLists)):?>
					<div class="control-group row" id="qtcoptionheading">
						<div class="controls">
							<span class="qtc-attributeset-name span3">
								<b><?php echo Text::_('COM_QUICK2CART_GLOBALATTRIBUTES_ATTRIBUTE_NAME');?></b>
							</span>
					...
					foreach($this->attributeLists as $key => $attributeDetail):?>
						<div class="control-group row">
							<div id="qtcoptionclone" class="controls qtcattributeclone">
								...
								<span class="span3">
									<a class="btn btn-small btn-danger span3" onclick="removeclone(<?php echo $attributeDetail['id']?>)"><?php echo Text::_('COM_QUICK2CART_REMOVE_OPTION');?></a>
								</span>
								<span class="center span3">
