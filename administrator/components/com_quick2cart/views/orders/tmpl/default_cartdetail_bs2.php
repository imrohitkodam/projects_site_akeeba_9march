...
{?>
	<h4><?php echo Text::_("COM_QUICK2CART_CART_DETAILS")?></h4>
	<div>
		<?php
		...
		<div class="table table-responsive qtc-table">
			<form action="" name="orderItemForm" id="orderItemForm" class=" form-validate " method="post">
				<table width="100%" class="table table-condensed table-bordered adminlist">
					<tr>
						<th class="cartitem_num" width="5%"  style="<?php echo ($orders_email)?'text-align: left;' :'';  ?>" ><?php echo Text::_('QTC_NO'); ?></th>
						<th class="cartitem_name"  style="<?php echo ($orders_email)?'text-align: left;' :'';  ?>" ><?php echo  Text::_('QTC_PRODUCT_NAM'); ?></th>
						<?php
						if ($showoptioncol == 1)
						{
						?>
							<th class="cartitem_opt"  style="<?php echo ($orders_email)?'text-align: left;' :'';  ?>" ><?php echo Text::_('QTC_PRODUCT_OPTS'); ?></th>
						<?php
						}
						?>
						<th class="cartitem_qty q2c_width_10"  style="<?php echo ($orders_email)?'text-align: left;' :'';  ?>" ><?php echo Text::_('QTC_PRODUCT_QTY'); ?></th>
						<th class="cartitem_price q2c_width_25" width="20%"
							<?php echo $price_col_style;  ?>><?php echo Text::_('QTC_PRODUCT_PRICE'); ?>
						</th>
					</tr>
					<?php
							...
							if (! in_array($order->store_id, $store_array))
							{
								...
								<tr class="<?php echo $qtc_store_row_classes;?>">
									...
									<div class="invoice-pdf qtcHandPointer" style='float: right; text-align: right'>
									    <?php if (isset($layout) && $layout == 'order')
										{ ?>
											<!--Invoice layout and PDF -->
											<a href="<?php echo Route::_('index.php?option=com_quick2cart&view=orders&layout=invoice&orderid=' . $orderid . '&tmpl=component&store_id=' . $order->store_id . $streLinkPrarm); ?>"
											target="_blank" >
											...
											<a href="<?php echo Route::_('index.php?option=com_quick2cart&task=orders.generateInvoicePDF&orderid=' . $orderid . '&tmpl=component&store_id=' . $order->store_id . $streLinkPrarm); ?>" >
											...
											<a onclick="qtcSendInvoiceEmail('<?php echo Route::_('index.php?option=com_quick2cart&task=orders.resendInvoice&orderid=' . $orderid . '&tmpl=component&store_id=' . $order->store_id . $streLinkPrarm); ?>')" >
												<img title="<?php echo Text::_('COM_QUICK2CART_INVOICE_EMAIL_ICON_TITLE'); ?>" src="<?php echo Uri::root(); ?>components/com_quick2cart/assets/images/email_16.png"/>
											</a>
										<?php } ?>
									</div>
									</td>
						...
						<tr class="row0">
							...
							<td class="cartitem_name">
								...
								{ ?>
 								...
 								<span>
									<strong>
 								<?php $prodprice = (float) ($order->product_item_price + $order->product_attributes_price);
								...
								<?php $orderItemIds[] = $order->order_item_id;
								...
								if (!empty($this->orderinfo->status) && $this->orderinfo->status == 'C')
								{
									...
									if (!empty($medisFiles))
									{
										...
										<a href="<?php echo $downloadLink ;?>" class="no-print">
											...
											<?php echo Text::_('QTC_ORDER_PG_DOWN_NOW'); ?>
										</a>
										<?php
									}
								}
								...
								if (!empty($order->prodAttributeDetails))
								{
									...
									foreach ($order->prodAttributeDetails as $key=>$attribute)
									{
										...
										<div class="control-group">
											<label class="control-label "><?php echo $attribute->itemattribute_name; ?></label>
											...
											else
											{
												...
												$layout = new FileLayout('productpage.attribute_option_display',  null, array('client' => 0, 'component' => 'com_quick2cart'));
								...
								<div class="row-fluid">
									<div class="span11">
										<strong class="pull-right">
											<?php echo $this->comquick2cartHelper->getFromattedPrice(number_format($productPrice,2),$order_currency); ?>
										</strong>
									</div>
									...
									{ ?>
										<div class="span11">
											<b> + </b>
											<?php echo " " . Text::_("COM_QUICK2CART_ITEM_TAX") . " :"; ?>
											<input type="text" name="itemTaxShipDetail[<?php echo $order->order_item_id ?>][tax]" class="pull-right input input-mini" id="" value="<?php echo $order->item_tax?>">
										</div>
										<div class="span11">
											<b> +</b>
											<?php echo " " . Text::_("COM_QUICK2CART_ITEM_SHIP") . " :"; ?>
											<input type="text" name="itemTaxShipDetail[<?php echo $order->order_item_id ?>][ship]" class="pull-right input input-mini" id="" value="<?php echo $order->item_shipcharges?>">
										</div>
										<span class="span11">
											<b><?php echo Text::_("COM_QUICK2CART_ITEM_TAX_SHIP_PRICE") ?> :</b>
											<div class="pull-right">
												<?php echo $this->comquick2cartHelper->getFromattedPrice(number_format($productPrice + $order->item_tax + $order->item_shipcharges,2),$order_currency); ?>
											</div>
										</span>
									<?php
									}
					...
					<!--  sub total -->
					<?php $col = ($showoptioncol == 1) ? 4 : 3;?>
					<tr>
						<td  colspan="<?php echo $col;?>" class="cartitem_tprice_label rightalign">
					...
					if (!empty($disAmt))
					{
						...
						<tr style="<?php echo ($orders_email) ? $emailStyle_tr : '';?>">
							<td colspan="<?php echo $col;?>" class="cartitem_tprice_label hidden-xs hidden-xs rightalign"  style="<?php echo ($orders_email) ? $emailStyle_td . $emailStyle_priceNdTotPrice: '';?>">
								...
								if (!empty($discount_detail))
								{
									...
									if (!empty($dis_detail->coupon_code))
									{
										...
										(
											<small>
												<strong><?php echo Text::_('QTC_DISCOUNT_CODE') . " : " . $dis_detail->coupon_code . " "; ?> </strong>
											</small>
											<?php 
							...
							<td class="cartitem_tprice  "  data-title="<?php echo sprintf(Text::_('QTC_PRODUCT_DISCOUNT'), $coupon_code); ?>" style="<?php echo ($orders_email) ? $emailStyle_td . $emailStyle_priceNdTotPrice: '';?>">
					...
					if ($itemLevalTaxship == 0)
					{
						...
						<tr style="<?php echo ($orders_email) ? $emailStyle_tr : '';?>">
							...
							<td class="cartitem_tprice" <?php echo $price_col_style;?>>
								...
								?>
									<span id="tax_amt">
										<?php echo $this->comquick2cartHelper->getFromattedPrice($orderTaxAmount , $order_currency);?>
									</span>
									<?php
								}
								?>
					...
					{?>
						<tr>
							...
							<td class="cartitem_tprice" <?php echo $price_col_style;?>>
								...
								?>
									<span id="tax_amt">
										<?php echo $this->comquick2cartHelper->getFromattedPrice($orderShipAmount , $order_currency);?>
									</span>
									<?php
								}
								?>
					...
					<!--  final order  total -->
					<tr>
						<td colspan="<?php echo $col;?>" class="cartitem_tprice_label rightalign"><strong><?php echo Text::_('QTC_ORDER_TOTAL');?></strong></td>
						<td class="cartitem_tprice" <?php echo $price_col_style;?>>
							<strong>
							...
							<button type="button" class="btn btn-success pull-right no-print" onClick='updateOrderItemAttribute("<?php echo $this->orderinfo->id ;?>", "<?php echo Text::_('COM_QUICK2CART_ORDER_UPDATED', true); ?>")' title="<?php echo Text::_('COM_QUICK2CART_UPDATE_ORDER_CART_DESC', true); ?>"><?php echo Text::_('COM_QUICK2CART_UPDATE_ORDER_CART', true); ?></button>
						</td>
					</tr>
				</table>
			</form>
