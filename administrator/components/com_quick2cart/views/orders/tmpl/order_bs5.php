...
if (!empty($this->orderinfo[0]->id))
{
	...
	<div class="<?php echo Q2C_WRAPPER_CLASS; ?>" id="qtcMainOrderWrapper">
		...
		{ ?>
			<div class="row-fluid">
				<?php
					$view = $this->comquick2cartHelper->getViewpath('orders', 'default_basicdetails_bs5', "ADMINISTRATOR", "ADMINISTRATOR");
					ob_start();
					include($view);
		...
		<div class="row">
			<?php
			$view = $this->comquick2cartHelper->getViewpath('orders', 'default_cartdetail_bs5', "ADMINISTRATOR", "ADMINISTRATOR");
			ob_start();
			include($view);
			$html = ob_get_contents();
			ob_end_clean();
		...
		<div class="row no-print">
			<?php
			$view = $this->comquick2cartHelper->getViewpath('orders', 'statushistrory_bs5', "ADMINISTRATOR", "ADMINISTRATOR");
			ob_start();
			include($view);
			$html = ob_get_contents();
			ob_end_clean();
		...
		<div class="row mt-3">
			<?php
			$view = $this->comquick2cartHelper->getViewpath('orders', 'default_billing_bs5', "ADMINISTRATOR", "SITE");
			ob_start();
			include($view);
			$html = ob_get_contents();
			ob_end_clean();
