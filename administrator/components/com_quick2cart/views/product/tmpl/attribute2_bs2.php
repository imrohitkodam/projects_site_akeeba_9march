				...
				<div class="span3 " >
					...
					<label class="checkbox  <?php echo ($stockableAttrId || $qtc_stck_att  || $productType != 2) ? 'qtc_hideEle' : ''?>" title="<?php echo Text::_('COM_QUICK2CART_IS_STOCKABLE_UNIT_DESC');?>">
						<input
							...
							id="att_detail[<?php echo $i; ?>][is_stock_keeping]"
			...
			<div class="span12">
				<div class="span4">
					<input
						...
						class="input-medium bill inputbox qtc_attrib qtcAttrNameClass"
						type="text"
				...
				<div class="span4">
					...
					<div style="<?php echo $hideFieldsForStockableAttribute ?>">
						<?php echo HTMLHelper::_('select.genericlist', $fields, "att_detail[$i][fieldType]", 'class="no_chzn qtcfieldType span6" onChange="qtc_fieldTypeChange('.$fnparam.')"', "value", "text",$default);?>
					</div>
				</div>
				<!--- IS COMPULSORY  -->
				<div class="span3 ">
					<label class="checkbox " style="<?php echo $hideFieldsForStockableAttribute ?>">
						...
						<input
							type="checkbox"
							class="checkboxdiv"
							id="att_detail[<?php echo $i; ?>][iscompulsary_attr]"
					...
					for ($k = 0; $k <=$att_op_count ; $k++)
					{
						...
						<tr class="form-inline clonedInput" id="attri_opts<?php echo $k; ?>" >
							...
							<td >
								...
								echo HTMLHelper::_('select.genericlist', $option_status, "att_detail[$i][attri_opt][$k][state]", 'class="chzn-done input-medium"', "value", "text", $defOptionStatus);
								?>
							</td>
							<td class="<?php echo $stockRelatedFildClass; ?> qtcStockSkufields">
								<!-- For stockable attribute - sku and stock field -->
								<input type="number" class="input-mini qtcAttriOptionStock" name="att_detail[<?php echo $i; ?>][attri_opt][<?php echo $k; ?>][stock]" Onkeyup="" onchange=""	 id=""   placeholder="<?php echo Text::_('QTC_ADDATTRI_STOCK')?>" value="<?php echo isset($childProdDetail->stock) ? $childProdDetail->stock : '' ?>">
							</td>
							<td class="<?php echo $stockRelatedFildClass; ?> qtcStockSkufields">
							...
							<td >
								...
								echo HTMLHelper::_('select.genericlist', $addpre_select, "att_detail[$i][attri_opt][$k][prefix]", 'class="chzn-done input-mini" data-chosen="qtc"', "value", "text", $addpre_val);
							?>
							</td>
							<td>
								<?php
							...
							{ ?>
								<button type="button"  class="btnAdd btn btn-mini btn-primary" id="<?php echo "qtc_container". $i;?>" title="<?php echo Text::_('QTC_ADD_MORE_OPTION_TITLE')?>" onclick="addopt(this.id);">
							...
							{
								?>
								<button type="button" class="btn btn-mini btn-danger qtcRemoveOption" id="btnRemove<?php echo $k; ?>" title="<?php echo Text::_('QTC_REMOVE_MORE_OPTION_TITLE');?>" onclick="techjoomla.jQuery(this).closest('tr').remove();" >
