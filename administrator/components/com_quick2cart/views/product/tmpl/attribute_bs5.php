...
<script type="text/javascript">
	...
	var qtcJsData = {
			...
			qtcLoadArributefunction_URL:"index.php?option=com_quick2cart&task=product.loadGlobalAttriOptions&tmpl=component",
			urfnName : function() {
			   return '';
			}
		};
	...
	*/
	...
	function addClone(rId,rClass)
	{
		...
		var removeButton="<div class='com_qtc_remove_button float-start col-md-1'>";
		removeButton+="<button class='btn btn-mini btn-danger' type='button' id='remove"+num+"'";
		...
		techjoomla.jQuery(newElem).find('.com_qtc_repeating_block').find('.qtc_attrib').each(function()
		{
			...
			kid.addClass('required');
			kid.attr('id','atrri_name_id'+num);
			techjoomla.jQuery(this).attr({'value':''});
		});

		techjoomla.jQuery(newElem).find('.qtcAttriTabLabel').attr('for', 'atrri_name_id'+num);
		...
		techjoomla.jQuery('#'+newelementid+ ' table tr').each(function()
		{
			...
			{ // avoid first tr (which is used for giving heading to column)
				...
				var addOptionBtn=" <button type='button' id='"+newelementid+"' class='btnAdd btn btn-sm btn-primary'  onclick='addopt(this.id);'><i class='" + iconplus +" icon22-white22 '></i></button> ";
...
<div class="row-fluid">
	<div class="col-md-12">
		...
		<div id=<?php echo $attribute_container_id ?> class="qtc_container qtc_att_hide row af-mt-5">
			<div class="col-md-11">
				<div class="com_qtc_repeating_block well well-small col-md-10">
					<?php
					...
					$att_list_path = $comquick2cartHelper->getViewpath('product','attribute2_bs5',"ADMINISTRATOR","ADMINISTRATOR");
					ob_start();
		...
		for($i=1;$i<=$count;$i++) // for each attribute
		{
			...
			<div id=<?php echo $attribute_container_id ?> class="qtc_container row af-mt-5">
				...
				<div class="col-md-1">
					<div class='com_qtc_remove_button float-start'>
						<button class='btn btn-mini btn-danger' type='button' id='remove<?php echo $i;?>'
