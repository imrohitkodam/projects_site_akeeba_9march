...
<script type="text/javascript">
	...
	function addCloneMedia(rId,rClass)
	{
		...
		if (mdiaCount <= (maxAllowedMedia + 1))
		{
			...
			newElem . prepend('<hr class="ms-1 me-1">');
...
<div>
	...
	<div class="row-fluid">
		...
		$mediaDetail            = array();
		?>
		<div id=<?php echo $attribute_container_id ?> class="qtc_mediaContainer qtc_media_hide row " >
			<div class="com_qtc_media_repeat_block well well-small col-md-10">
				<?php
				$comquick2cartHelper = new comquick2cartHelper;
				$att_list_path=$comquick2cartHelper->getViewpath('product','media_bs5',"ADMINISTRATOR","ADMINISTRATOR");
				ob_start();
		...
		for($m=1; $m<=$mediaCount+1  && $m < $eProdMaxFileLimit+1; $m++) // for each attribute
		{
			$attribute_container_id = "qtc_mediaContainer".$m;
		?>
			<div id=<?php echo $attribute_container_id ?> class="qtc_mediaContainer row">
				<?php
					if ($m != 1)
					{
						echo "<hr>";
				...
				<div class='col-md-1'>
					<button
						...
						type='button'
						id='remove<?php echo $m;?>'
						onclick="removeClone('qtc_mediaContainer<?php echo $m;?>','qtc_mediaContainer');"
						title="<?php echo Text::_('COM_QUICK2CRT_REMOVE_TOOLTIP');?>">
