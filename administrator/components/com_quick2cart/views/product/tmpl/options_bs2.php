...
<div class ="form-horizontal">
	...
	<div class='qtc_title_textbox control-group' >
		<label for="item_name" class="control-label"><?php echo HTMLHelper::tooltip(Text::_('QTC_PROD_TITLE_DES'), Text::_('QTC_PROD_TITLE'), '', Text::_('QTC_PROD_TITLE') . ' *');?></label>
		<?php
	...
	<div class='control-group'>
		<label for="item_alias" class="control-label"><?php echo HTMLHelper::tooltip(Text::_('QTC_PROD_ALIAS_DES'), Text::_('QTC_PROD_ALIAS'), '', Text::_('QTC_PROD_ALIAS'));?></label>
		<div class="controls">
	...
	<div class="control-group" style="<?php echo $prodTypeStyle ?>">
		<label for="qtc_product_type" class="control-label"><?php echo HTMLHelper::tooltip(Text::_('QTC_PROD_SEL_TYPE_TOOLTIP'), Text::_('QTC_PROD_SEL_TYPE'), '', Text::_('QTC_PROD_SEL_TYPE'). ' *');?></label>
		<div class="controls">
			<?php
			if ($productType != 2)
			{
				echo HTMLHelper::_('select.genericlist',$this->product_types,'qtc_product_type','class="required"','value','text',$productType,'qtc_product_type');
			}
			else
			{
			?>
	...
	<div class="control-group">
		<label for="prod_cat" class="control-label"><?php echo HTMLHelper::tooltip(Text::_('QTC_PROD_SEL_CAT_TOOLTIP'), Text::_('QTC_PROD_SEL_CAT'), '', Text::_('QTC_PROD_SEL_CAT'). ' *');?></label>
		<div class="controls">
			<?php
	...
	<div class="control-group" style="<?php echo $StoreIds_style;?>">
		...
		<div class="controls">
			...
			$defaultStore = (!empty($this->itemDetail) ? stripslashes($this->itemDetail['store_id']) : $storeList[0]->id);
				...
				echo $this->dropdown = HTMLHelper::_('select.genericlist',$options,'store_id','class=" qtc_putmargintop10px"   onchange="getTaxprofile();" ','value','text',$defaultStore,'current_store_id');
			?>
		</div>
	</div>
	...
	<div class="control-group">
		<label for="qtc_product_tag" class="control-label">
		...
		<div class="controls">
			...
			echo HTMLHelper::_('select.genericlist',$options,'jform[tags][]','multiple class="chosen-select"','value','text',$defaultTags,'jform_tags');
		?>
		</div>
	</div>
	<!-- publish and unpublish-->
	<div class='control-group' >
		<label for="state" class="control-label"><?php echo HTMLHelper::tooltip(Text::_('QTC_PROD_STATUS'), Text::_('QTC_PROD_STATUS_DES'), '', Text::_('QTC_PROD_STATUS'));?></label>
		<div class="controls">
					...
					<div>
						<div class="input-append curr_margin">
							<label for="disc_price_<?php echo trim($value);?>" class="control-label">
								<?php echo HTMLHelper::tooltip(Text::_('COM_QUICK2CART_ITEM_DIS_PRICE_DESC'), Text::_('QTC_ITEM_DIS_PRICE'), '', Text::_('QTC_ITEM_DIS_PRICE') . ' ' . Text::_('COM_QUICK2CART_IN') . ' ' . trim($currsymbol));?>
							</label>
							<input
									Onkeyup="checkforalpha(this,'46', '<?php echo addslashes($entered_numerics); ?>')"
	...
	<div class="control-group imagediv" id="imagediv">
		<label for="avatar" class="control-label"><?php echo HTMLHelper::tooltip(Text::_('QTC_PROD_IMG_TOOLTIP'), Text::_('QTC_PROD_IMG'), '', Text::_('QTC_PROD_IMG'));?></label>
		<div class="controls">
			<?php
			...
			<div class="row-fluid">
				...
				<div class="filediv span12" id="filediv">
					<span>
						<input type="file" name="prod_img" id="avatar" placeholder="<?php echo  Text::_('COM_QUICK2CART_IMAGE_MSG');?>" accept="image/*, video/*">
					</span>
				</div>
	...
	<div class="control-group" style="<?php echo $qtc_min_max_style;?>">
		...
		<div class="controls">
			<input Onkeyup="checkforalpha(this,'', '<?php echo addslashes($entered_numerics); ?>')"  Onchange="checkSlabValue();" type="text" name="item_slab" id="item_slab"  value="<?php echo isset($minmaxstock) ? $minmaxstock->slab: 1  ?>" class="inputbox validate-integer"  >
		</div>
	</div>
	<div class="control-group" style="<?php echo $qtc_min_max_style;?>">
