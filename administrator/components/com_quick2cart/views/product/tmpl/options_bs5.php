...
Text::script('COM_QUICK2CART_SLAB_MAX_QTY', true);
...
$edit_task    = (!empty($this->item_id)) ? 1 : 0;
...
<script type="text/javascript">
	jQuery( document ).ready(function() {
		...
		if (store_id == null)
		{
			store_id = '"<?php echo $storeList[0]->id?>"';
		}
	...
	function addmoreImg(rId,rClass)
	{
		...
		if (remaing_imgs > 0)
		{
			...
			removeButton+="<button class='btn btn-danger btn-mini' type='button' id='remove"+num+"' onclick=\"removeClone('filediv"+num+"','jgive_container');\" title=\"<?php echo Text::_('COM_Q2C_REMOVE_TOOLTIP');?>\" >";
			...
			newElem.find(':file').attr('class','form-control');
	...
	function checkForSku(sku)
	{
		...
		if (skuval)
		{
			...
			if (skuval != oldSku)
			{
				var actUrl = Joomla.getOptions('system.paths').base + '/index.php?option=com_quick2cart&task=product.checkSku&sku='+sku;
...
<div class ="form-horizontal">
	...
	<div class='qtc_title_textbox form-group row' >
		<label for="item_name" class="form-label col-md-3">
			<?php echo HTMLHelper::tooltip('', Text::_('QTC_PROD_TITLE_DES'), '', Text::_('QTC_PROD_TITLE'). ' *');?>
		</label>
		<div class="col-md-9">
			<input
				type="text"
	...
	<div class='form-group row'>
		<label for="item_alias" class="form-label col-md-3">
			<?php echo HTMLHelper::tooltip('', Text::_('QTC_PROD_ALIAS_DES'), '', Text::_('QTC_PROD_ALIAS'));?>
		</label>
		<div class="col-md-9">
			<input
				type="text"
	...
	?>
	<div class="form-group row" style="<?php echo $prodTypeStyle ?>">
		<label for="qtc_product_type" class="form-label col-md-3">
			<?php echo HTMLHelper::tooltip('', Text::_('QTC_PROD_SEL_TYPE_TOOLTIP'), '', Text::_('QTC_PROD_SEL_TYPE'). ' *');?>
		</label>
		<div class="col-md-9">
			<?php
			if ($productType != 2)
			...
			?>
				...
				<span class="badge af-bg-success"><?php echo $this->product_types[$productType]->text; ?></span>
				<?php
			}
	...
	<div class="form-group row">
		<label for="prod_cat" class="form-label col-md-3">
			<?php echo HTMLHelper::tooltip('', Text::_('QTC_PROD_SEL_CAT_TOOLTIP'), '', Text::_('QTC_PROD_SEL_CAT'). ' *');?>
		</label>
		<div class="col-md-9">
			<?php
	...
	$StoreIds_style = (count($storeList)) ? "" : "display:none";
	?>
	<div class="form-group row" style="<?php echo $StoreIds_style;?>">
		<label for="qtc_store" class="form-label col-md-3">
			<?php echo HTMLHelper::tooltip('', Text::_('QTC_PROD_SELECT_STORE_DES'), '', Text::_('QTC_PROD_SELECT_STORE'));?>
		</label>
		<div class="col-md-9">
			<?php
			 $defaultStore = (!empty($this->itemDetail) ? stripslashes($this->itemDetail['store_id']) : $storeList[0]->id);
				...
				echo HTMLHelper::_('select.genericlist',$options,'store_id','class="form-select" onchange="getTaxprofile();" ','value','text',$defaultStore,'current_store_id');
			?>
		</div>
	</div>
	...
	<!-- sku -->
	<div class="form-group row">
		<label for="qtc_sku" class="form-label col-md-3">
			<?php echo HTMLHelper::tooltip('', Text::_('QTC_PROD_SKU_TOOLTIP'), '', Text::_('QTC_PROD_SKU'). ' *');?>
		</label>
		<div class="col-md-9">
			<?php
	...
	<div class="form-group row">
		<label for="qtc_product_tag" class="form-label col-md-3">
			<?php echo HTMLHelper::tooltip('', Text::_('QTC_PROD_TAG_TOOLTIP'), '', Text::_('QTC_PROD_TAG'));?>
		</label>
		<div class="col-md-9">
			<?php
			$defaultTags = 0;
	...
	<div class='form-group row' >
		<label for="state" class="form-label col-md-3">
			<?php echo HTMLHelper::tooltip('', Text::_('QTC_PROD_STATUS'), '', Text::_('QTC_PROD_STATUS'));?>
		</label>
		<div class="col-md-9">
			<?php
			$isPublished   = " checked ";
	...
	<div class='form-group row qtc_currencey_textbox'>
		<label for="price_<?php echo !empty($curr[0]) ? $curr[0] : '' ;?>" class="form-label col-md-3">
			<?php echo HTMLHelper::tooltip('', Text::_('COM_QUICK2CART_ITEM_PRICE_DESC'), '', Text::_('QTC_ITEM_PRICE'). ' *');?>
		</label>
		<div class="col-md-9">
			<?php
			$currdata = array();
				...
				<?php if ($multiCurrencies) : ?>
					<div>
						<div class="input-group curr_margin">
							<label for="price_<?php echo trim($value);?>" class="form-label col-md-3">
								<?php echo HTMLHelper::tooltip('', Text::_('COM_QUICK2CART_ITEM_PRICE_DESC'), '', Text::_('QTC_ITEM_PRICE') . ' ' . Text::_('COM_QUICK2CART_IN') . ' ' . trim($currtext));?> &nbsp;
							</label>
							<input
								Onkeyup="checkforalpha(this,'46', '<?php echo addslashes($entered_numerics); ?>')"
								class="form-control span1 currtext required qtc_requiredoption"
								style="align:right;"
	...
	<!-- DISCOUNT PRICE -->
	<div class='form-group row qtc_currencey_textbox' style="<?php echo (($params->get('usedisc') == '0') ? 'display:none;' : ''); ?>" >
		<label for="disc_price_<?php echo !empty($curr[0]) ? $curr[0] : '' ;?>" class="form-label col-md-3">
			<?php echo HTMLHelper::tooltip('', Text::_('COM_QUICK2CART_ITEM_DIS_PRICE_DESC'), '', Text::_('QTC_ITEM_DIS_PRICE'));?>
		</label>
		<div class="col-md-9">
			<?php
			$currdata = array();
			...
			foreach ($curr as $key=>$value)
			{
				...
				<?php if ($multiCurrencies) : ?>
					<div>
						<div class="input-group curr_margin">
							<label for="disc_price_<?php echo trim($value);?>" class="form-label col-md-3">
								<?php echo HTMLHelper::tooltip('', Text::_('COM_QUICK2CART_ITEM_DIS_PRICE_DESC'), '', Text::_('QTC_ITEM_DIS_PRICE') . ' ' . Text::_('COM_QUICK2CART_IN') . ' ' . trim($currsymbol));?>
							</label>
							<input
									Onkeyup="checkforalpha(this,'46', '<?php echo addslashes($entered_numerics); ?>')"
									onchange="validateDiscountPrice('<?php echo trim($value);?>')"
									class="form-control span1 currtext"
									style="align:right;"
	...
	<!--  PROD description -->
	<div class="form-group row">
		<label for="description" class="form-label col-md-3">
			<?php echo HTMLHelper::tooltip('', Text::_('QTC_PROD_DES_TOOLTIP'), '',Text::_('QTC_PROD_DES'));?>
		</label>
		<div class="col-md-9">
			<?php
			...
			{
				?>
				<textarea rows="10" name="description[data]" id="description" class="form-control inputbox" ><?php if (!empty($this->itemDetail['description'])){  echo trim($this->itemDetail['description']); } ?></textarea>
			<?php
			}
			else
			{
	...
	<div class="form-group row imagediv" id="imagediv">
		<label for="avatar" class="form-label col-md-3">
			<?php echo HTMLHelper::tooltip('', Text::_('QTC_PROD_IMG_TOOLTIP'), '', Text::_('QTC_PROD_IMG'));?>
		</label>
		<div class="col-md-9">
			<?php
			...
			<div class="row-fluid">
				...
				<div class="filediv span12" id="filediv">
					<span>
						<input type="file" name="prod_img" id="avatar" class="form-control" placeholder="<?php echo  Text::_('COM_QUICK2CART_IMAGE_MSG');?>" accept="image/*, video/*">
					</span>
				</div>
				...
				<div class="span6">
					<span class="addmore"  id="addmoreid"  id="addmoreid" >
						<button onclick="addmoreImg('filediv','filediv');" type="button" class="btn btn-mini btn-primary" title="<?php echo Text::_('COM_Q2C_IMAGE_ADD_MORE');?>">
				...
				<div class="text-warning">
					<p><?php echo Text::sprintf('COM_QUICK2CART_ALLOWED_IMG_FORMATS', 'gif, jpeg, jpg, png, pjpeg', $this->params->get('max_size', '1024'));?></p>
					<?php
	...
	<div class="form-group row">
		...
		<div class="col-md-9">
			<input type="text" name="youtube_link" id="youtube_link" class="form-control inputbox "
	...
	{
		$qtc_stock_style = ($params->get('usestock')==1)?"":"display:none";
		?>
		<div class="form-group row" style="<?php echo $qtc_stock_style;?>">
			<label for="stock" class="form-label col-md-3">
				<?php echo HTMLHelper::tooltip(Text::_(''), Text::_('COM_QUICK2CART_ITEM_STOCK_DESC'), '', Text::_('PLG_QTC_ITEM_STOCK'));?>
			</label>
			<div class="col-md-9">
				<input
				...
				class="form-control input-mini inputbox validate-integer" />
				<?php
				if ($productType == 2)
				{ ?>
	...
	$qtc_min_max_style = ($qtc_min_max_status==1) ? "" : "display:none";
	?>
	<div class="form-group row" style="<?php echo $qtc_min_max_style;?>">
		<label class="form-label col-md-3" for="item_slab">
			<?php echo HTMLHelper::tooltip(Text::_(''), Text::_('COM_QUICK2CART_ITEM_SLAB_DESC'), '', Text::_('COM_QUICK2CART_ITEM_SLAB'));?>
		</label>
		<div class="col-md-9">
			<input
	...
	<div class="form-group row" style="<?php echo $qtc_min_max_style;?>">
		<label for="min_item" class="form-label col-md-3">
			<?php echo HTMLHelper::tooltip(Text::_(''), Text::_('COM_QUICK2CART_ITEM_MIN_QTY_DESC'), '', Text::_('QTC_ITEM_MIN_QTY'));?>
		</label>
		<div class="col-md-9">
	...
	<div class="form-group row" style="<?php echo $qtc_min_max_style;?>">
		<label for="max_item" class="form-label col-md-3">
			<?php echo HTMLHelper::tooltip(Text::_(''), Text::_('COM_QUICK2CART_ITEM_MAX_QTY_DESC'), '', Text::_('QTC_ITEM_MAX_QTY'));?>
		</label>
		<div class="col-md-9">
			<input onChange="checkSlabValueField(this,'', '<?php echo addslashes($entered_numerics); ?>')"
	...
	<div class="form-group row">
		<label class="form-label col-md-3" for="metadesc">
			<?php echo HTMLHelper::tooltip(Text::_(''), Text::_('COM_QUICK2CART_META_DESC_TOOLTIP'), '', Text::_('COM_QUICK2CART_META_DESC'));?>
		</label>
		<div class="col-md-9">
			<div>
	...
	<div class="form-group row">
		<label class="form-label col-md-3" for="metakey">
			<?php echo HTMLHelper::tooltip(Text::_(''), Text::_('COM_QUICK2CART_META_KEYWORDS_TOOLTIP'), '', Text::_('COM_QUICK2CART_META_KEYWORDS'));?>
		</label>
		<div class="col-md-9">
			<div class="">
