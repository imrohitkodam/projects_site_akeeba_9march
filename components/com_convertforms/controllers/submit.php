...
class ConvertFormsControllerSubmit extends FormController
{
    ...
    {  
        ...
        try
        {
            ...
            if ($this->params->get('csrf_check', false) && !Session::checkToken('request'))
            {
                \ConvertForms\Spam\Helper::error(Text::_('JINVALID_TOKEN'), 'csrf');
            }
        ...
        catch (\ConvertForms\Spam\Exception $e)
        {
            $this->triggerError($e->getMessage(), 'spam');
     ...
     * @return void
     */
    protected function triggerError($message, $type = null)
    {
        $type = $type ?: ucfirst($this->input->get('task'));
        Helper::triggerError($message, $type, $this->form_id, $this->form_data);
    }
