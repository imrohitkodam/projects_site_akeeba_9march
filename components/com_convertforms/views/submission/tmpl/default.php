...
$print_view = Factory::getApplication()->input->get('print') == 1;
...
<div class="convertforms-submissions item <?php echo $print_view ? 'print' : '' ?>">
	<h1><?php echo Text::_('COM_CONVERTFORMS_SUBMISSION') ?> #<?php echo $this->submission->id ?></h1>
	...
	<div class="submission_section submission_info">
		...
		<table>
			...
			<tr class="cfs-state">
				...
				<td>
					<span class="badge <?php echo ($this->submission->state == '1' ? 'badge-success bg-success' : 'badge-danger bg-danger') ?>">
						<?php echo Text::_(($this->submission->state == '1' ? 'COM_CONVERTFORMS_SUBMISSION_CONFIRMED' : 'COM_CONVERTFORMS_SUBMISSION_UNCONFIRMED')) ?>
					</span>
					<?php 
			...
			<tr class="cfs-date-modified">
				<th><?php echo Text::_('COM_CONVERTFORMS_MODIFIED') ?></th>
				<td><?php echo $this->submission->modified ?></td>
			</tr>
			<tr class="cfs-form">
				<th><?php echo Text::_('COM_CONVERTFORMS_FORM_NAME') ?></th>
			...
			<?php
				...
				Factory::getApplication()->triggerEvent('onConvertFormsFrontSubmissionViewInfo', array($this->submission));
			?>
		</table>
	</div>
	<div class="submission_section submission_info">
		...
		<?php } else { ?>
			<p><?php echo Text::_('COM_CONVERTFORMS_NO_SUBMITTED_DATA') ?></p>
		<?php } ?>
	</div>
	
	<?php if (!$print_view) { ?>
	<a href="<?php echo $this->submissions_link ?>">
		<?php echo Text::_('COM_CONVERTFORMS_SUBMISSIONS_LIST') ?>
	</a>
	<?php } ?>
</div>