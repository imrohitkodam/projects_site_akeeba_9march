...
function savedata(likecontainerid,success_msg){
		...
		if(jLike.jQuery("#annotation").val() == ''){
			...
			jLike.jQuery("#annotationform").prepend('<div id="alert_message" class="catch-error alert alert-error alert-dismissable"><a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a><span>'+Joomla.JText._('COM_JLIKE_SAVE_NOTE_ERROR_MSG')+'</span></div>');
			return false;
		}
		...
		jLike.jQuery.ajax({
			...
			success : function() {
				jLike.jQuery("#alert_message").remove();
				jLike.jQuery("#annotationform").prepend('<div id="alert_message" class="catch-error alert alert-success alert-dismissable"><a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a><span>' + Joomla.JText._('COM_JLIKE_SAVE_SUCCESS_MSG') + '</span></div>');
			}
	});
}
