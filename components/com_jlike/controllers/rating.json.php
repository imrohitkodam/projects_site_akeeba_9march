...
class JlikeControllerRating extends FormController
{
	...
	public function save($key = null, $urlVar = null)
	{
		...
		if ($table->id)
		{
			echo new JResponseJson(null, Text::_('COM_JLIKE_RATING_ALREADY_SUBMITTED'), true);
			$app->close();
		}
		...
		{
			echo new JResponseJson(null, Text::_('COM_JLIKE_RATING_MANDATORY_FIELD'), true);
			$app->close();
		}
		...
		if (!$recordId)
		{
			...
			for ($i = 0, $n = count($errors); $i < $n && $i < 3; $i++)
			{
				...
				{
					echo new JResponseJson(null, Text::_($errors[$i]->getMessage()), true);
					$app->close();
				}
				else
				{
					echo new JResponseJson(null, Text::_($errors[$i]), true);
					$app->close();
				}
			}
		}
		...
		echo new JResponseJson($data, Text::_('COM_JLIKE_RATING_SUCCESS_MESSAGE'));
		$app->close();
	}
	...
	public function getLoggedInUserRating()
	{
		...
		if ($rating->id)
		{
			echo new JResponseJson($rating, Text::_('COM_JLIKE_RATING_GET_RATING'));
			$app->close();
		}
		else
		{
			echo new JResponseJson(null, Text::_('COM_JLIKE_RATINGS_NO_CUSTOMER_REVIEWS'), true);
			$app->close();
		}
	}
}
