 ...
 * @since  1.6
 */
class JlikeTablecontent extends Table
{
	/**
	 * Constructor
	 *
	...
	public function __construct(&$db)
	{
		if (JVERSION < 4.0)
		{
			JObserverMapper::addObserverClassToClass('JTableObserverContenthistory', 'JlikeTablecontent', array('typeAlias' => 'com_jlike.content'));
		}
		else
		{
	...
	public function bind($array, $ignore = '')
	{
		...
		if (!Factory::getUser()->authorise('core.admin', 'com_jlike.content.' . $array['id']))
		{
			$actions         = Access::getActionsFromFile(
				JPATH_ADMINISTRATOR . '/components/com_jlike/access.xml',
				"/access/section[@name='content']/"
			);
			$default_actions = Access::getAssetRules('com_jlike.content.' . $array['id'])->getData();
			$array_jaccess   = array();
	...
	protected function _getAssetName()
	{
		...
		return 'com_jlike.content.' . (int) $this->$k;
	}
