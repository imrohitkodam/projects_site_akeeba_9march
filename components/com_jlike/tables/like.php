 ...
 * @since  1.6
 */
class JlikeTablelike extends Table
{
	/**
	 * Constructor
	 *
	...
	public function __construct(&$db)
	{
		if (JVERSION < 4.0)
		{
			JObserverMapper::addObserverClassToClass('JTableObserverContenthistory', 'JlikeTablelike', array('typeAlias' => 'com_jlike.like'));
		}
		else
		{
	...
	public function bind($array, $ignore = '')
	{
		...
		if (!Factory::getUser()->authorise('core.admin', 'com_jlike.like.' . $array['id']))
		{
			$actions         = Access::getActionsFromFile(
				JPATH_ADMINISTRATOR . '/components/com_jlike/access.xml',
				"/access/section[@name='like']/"
			);
			$default_actions = Access::getAssetRules('com_jlike.like.' . $array['id'])->getData();
			$array_jaccess   = array();
	...
	protected function _getAssetName()
	{
		...
		return 'com_jlike.like.' . (int) $this->$k;
	}
