...
<script type="text/javascript">
					...
					for(index = 0; index < data.length; index++)
					{
						var html='<div id="jlcomment'+data[index]['annotation_id']+'" style="'+style+'"  class="media row-fluid  jlike_commentingArea  jlike_renderedComment">';
							...
							<?php if (!empty($comment->user_profile_url)) : ?>
								html+=	'<a target="_blank" class="pull-left" href="'+data[index]['user_profile_url']+'">';
							<?php else:	?>
							...
							if(user_id==data[index]['user_id']){
								...
								html+=					'<a id="" target="#showEditDeleteButton'+data[index]['annotation_id']+'" onclick="EditComment(this)"><?php echo Text::_('COM_JLIKE_EDIT'); ?></a>';
								html+=				'</li>';
								html+=				'<li id="DeleteButton'+data[index]['annotation_id']+'" tabindex="-1">';
								html+=					'<a href="javascript:void(0)" id="" target="#DeleteButton'+data[index]['annotation_id']+'" onclick="DeleteComment(this,\''+likecontainerid+'\')"><?php echo Text::_('COM_JLIKE_DELETE'); ?></a>';
								html+=				'</li>';
								html+=			'</ul>';
								html+=		'</div>';
							 }
							...
							html+=			'</div>';
								if(user_id==data[index]['user_id'])
								{
									...
									html+=	'<div class="jlike_textarea taggable jlike-mention_'+likecontainerid+'" id="CommentText'+data[index]['annotation_id']+'" contenteditable="true" required="required"  onkeyup="characterLimit(id, <?php echo $maxlength; ?>)">'+data[index]['annotation']+'</div>';
								...
								{
									html+=	'<div class="jlike_display_none jlike_textarea jlike-mention_'+likecontainerid+' taggable" id="CommentText'+data[index]['annotation_id']+'" contenteditable="true" required="required" onkeyup="characterLimit(id, <?php echo $maxlength; ?>)">'+data[index]['annotation']+'</div>';
								}
							...
							{
								html+=	'<a id="parentid'+data[index]['annotation_id']+'" class="jlike_pointer" onclick="jlike_reply(this,'+(margin_left+8)+','+(width-8)+','+(commentbtn_margin-5)+','+(threadlevel+1)+',\''+likecontainerid+'\')" ><?php echo Text::_('COM_JLIKE_REPLY_BTN') ?></a>';
							}
		...
		*/
		function addCommentArea(likecontainerid, parent_id,margin_left,width,commentbtn_margin,threadlevel)
		{
			...
			if(!parseInt(user_id))
			{
				/*var msg_error_html = "<div class='alert alert-danger'><?php echo Text::_('COM_JLIKE_LOGIN_TO_COMMENT'); ?></div>";
			...
			user_name='<?php echo ucfirst($userInfo->name); ?>';
			...
			var html ='<div id="jlcomment'+textAreaId+'" style=" margin-left:'+margin_left+'%;" class="media jlike_add_comment jlike_commentingArea jlike_commentBox clone_othere_reply_box jlike_no_radius" >';
				...
				html+=					'<a id="" target="#showEditDeleteButton'+textAreaId+'" onclick="EditComment(this)"><?php echo Text::_('COM_JLIKE_EDIT'); ?></a>';
				html+=				'</li>';
				html+=				'<li id="DeleteButton'+textAreaId+'" tabindex="-1">';
				html+=					'<a href="javascript:void(0)" target="#DeleteButton'+textAreaId+'" onclick="DeleteComment(this,\''+likecontainerid+'\')"><?php echo Text::_('COM_JLIKE_DELETE'); ?></a>';
				html+=				'</li>';
				html+=			'</ul>';
				html+=		'</div>';
				html+='		</span>';
				...
				// SMILEY BUTTON
				html+=		'<div class="jlike_textarea jlike-mention_'+likecontainerid+'" taggable tag_editor editor_0 jl_padding" tag_editor_for="0" id="CommentText'+textAreaId+'" contenteditable="true" required="required" onkeyup="characterLimit(id, <?php echo $maxlength; ?>)" ></div>';
				...
				html+=	'<button  type="button" class="btn btn-success pull-right me-2" onclick="SaveNewComment(this,\''+likecontainerid+'\')"><?php echo Text::_('COM_JLIKE_COMMENT') ?></button>';
				html+=	"<button  type='button' class='btn pull-right jlike_cancel_btn btn-secondary' onclick='CancelNewComment(this)'><?php echo Text::_('COM_JLIKE_CACEL') ?></button> ";
				html+= '</div>';
				//COMMENT & CANCEL BUTTON
				...
				if((allow_threaded_comment) && ((threadlevel)<max_thread_level_limit))
				{
					html+= '<a id="parentid0" class="jlike_pointer jlike_display_none" onclick="jlikeReplyCallFromAddcomment(this,'+margin_left+','+width+','+commentbtn_margin+','+(threadlevel+1)+',\''+likecontainerid+'\')" ><?php echo Text::_('COM_JLIKE_REPLY_BTN') ?> </a>';
				}
				...
				html+='<span id="like_dislike_button'+textAreaId+'" class="jlike_display_none">';
					...
					<?php if($params->get('show_comment_dislike_button')){ ?>
						...
						//@S Dislike button
						html+=	'<a id="dislike_annotationid'+textAreaId+'" class="jlike_dislike_btn jlike_pointer" onclick="increaseDislikeCount(this,\''+likecontainerid+'\')" >';
		...
		function ViewMore(data,callFromAscDesc, likecontainerid)
		{
			...
			if(callFromAscDesc)
			{
				...
				if (data.length >= <?php echo $comment_limit; ?>)
				{
					var show = 1;
			...
			var html='<div id="jlcomment'+data[index]['annotation_id']+'" class="media row-fluid jlike_commentingArea  jlike_renderedComment jlike_comment_border_shadow jlike_commment_padding">';
				...
				if(user_id==data[index]['user_id']){
					...
					html+=				'</li>';
					html+=				'<li id="DeleteButton'+data[index]['annotation_id']+'" tabindex="-1">';
					html+=					'<a href="javascript:void(0)" target="#DeleteButton'+data[index]['annotation_id']+'" onclick="DeleteComment(this,\''+likecontainerid+'\')"><?php echo Text::_('COM_JLIKE_DELETE'); ?></a>';
					html+=				'</li>';
					html+=			'</ul>';
					html+=		'</div>';
				 }
				...
				html+=			'</div>';
					...
					{
						html+=	'<div id="CommentText'+data[index]['annotation_id']+'" class="jlike_display_none jlike_textarea taggable jlike-mention_'+likecontainerid+'" contenteditable="true" required="required" onkeyup="characterLimit(id, <?php echo $maxlength; ?>)"> '+data[index]['annotation']+'</div>';
						html+=	'<div id="displaytagsfor_CommentText'+data[index]['annotation_id']+'" class="displayme_CommentText"></div>';
				...
				{
				html+=	'<a id="parentid'+data[index]['annotation_id']+'" class="jlike_pointer" onclick="jlike_reply(this,8,92,1,1, \''+likecontainerid+'\')" ><?php echo Text::_('COM_JLIKE_REPLY_BTN') ?></a>';
				}
				...
				html+=					'<a id="dislike_annotationid'+data[index]['annotation_id']+'" class="jlike_dislike_btn jlike_pointer" onclick="increaseDislikeCount(this,\''+likecontainerid+'\')" >';
				html+=						'<span id="dislike_undislike'+data[index]['annotation_id']+'">';
											if(data[index]['userLikeDislike']==2){
				html+=							'<?php echo Text::_('COM_JLIKE_UNDISLIKE_BTN');?>';
											}
	...
	//		init_mention(className, jLikeVal[likecontainerid]['userslist']);
		...
		*/
		function SaveNewComment(selector,likecontainerid)
		{
			...
			{
				alert('<?php echo Text::_('COM_JLIKE_COMMENT_BLANK'); ?>');
				return false;
			}
				// Normal comment
				var response;
	...
	function strip_tags(html)
	{
		...
		*/
		function DeleteComment(selector, likecontainerid)
		{
			...
			jQuery.ajax({
				url:'<?php echo Uri::root();?>index.php?option=com_jlike&task=DeleteComment&annotation_id='+elementId+'&tmpl=component&format=row',
				type:'GET',
				dataType:'json',
				data:
				{
		...
		{
			var content = strip_tags(jLike.jQuery("#"+fieldId).text());
