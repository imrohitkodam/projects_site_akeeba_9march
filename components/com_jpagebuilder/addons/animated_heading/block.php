...
class JpagebuilderAddonAnimated_heading extends JpagebuilderAddons {
	/**
	...
	public function render() {
		...
		$output .= '<div class="jpb-addon jpb-addon-animated-heading' . $class . '">';
		$output .= '<' . $heading_selector . ' class="jpb-addon-title ' . ($heading_style !== 'highlighted' ? $animated_text_class : '') . '">';
	...
	public function css() {
		...
		$heading = $cssHelper->generateStyle ( '.jpb-addon-title', $settings, $headingProps, $headingUnit, $modifier );

		$headingTypography = $cssHelper->typography ( '.jpb-addon-title', $settings, 'heading_typography', [ 
		...
		$alignment = $cssHelper->generateStyle ( '.jpb-addon.jpb-addon-animated-heading .jpb-addon-title', $settings, [ 
				'alignment' => 'justify-content'
		], false );
		$transformCss = $cssHelper->generateTransformStyle ( '.jpb-addon-animated-heading', $settings, 'transform' );
	...
	public static function getFrontendEditor() {
        ...
        <style type="text/css">
            <# if(data.highlighted_shape) { #>
                #jpb-addon-{{ data.id }} .animated-heading-highlighted-wrap svg path {
		...
		// Global
		$output .= $lodash->alignment ( 'text-align', '.jpb-addon-animated-heading', 'data.alignment' );
		...
		$output .= $lodash->spacing ( 'padding', '.jpb-addon-title', 'data.heading_padding' );
		$output .= $lodash->spacing ( 'margin', '.jpb-addon-title', 'data.heading_margin' );
		$output .= $lodash->color ( 'color', '.jpb-addon-title', 'data.heading_color' );
		$output .= $lodash->typography ( '.jpb-addon-title', 'data.heading_typography', $headingTypographyFallbacks );
		...
		$output .= $lodash->generateTransformCss ( '.jpb-addon-animated-heading', 'data.transform' );
        ...
        <div class="jpb-addon jpb-addon-animated-heading {{data.class}}">
        <{{data.heading_selector}} class="jpb-addon-title {{animated_text_class}}">
        <# if(data.heading_before_part) { #>
            <span class="animated-heading-before-part jp-inline-editable-element" data-id={{data.id}} data-fieldName="heading_before_part" contenteditable="true">{{{data.heading_before_part}}}</span>
        <# }
        if(data.heading_style == "highlighted") {
            if(data.highlighted_text) {
            #>
                <span class="animated-heading-highlighted-wrap">
                    <span class="animated-heading-highlighted-text jp-inline-editable-element shape-{{data.highlighted_shape}}" data-id={{data.id}} data-fieldName="highlighted_text" contenteditable="true">
        ...
        #>
            <span class="animated-heading-after-part jp-inline-editable-element" data-id={{data.id}} data-fieldName="heading_after_part" contenteditable="true">{{{data.heading_after_part}}}</span>
        <# } #>
