...
class JpagebuilderAddonAnimated_number extends JpagebuilderAddons {
	/**
	...
	public function render() {
		...
		$output = '<div class="jpb-addon jpb-addon-animated-number ' . $class . ' ' . $number_position . '">';
		$output .= '<div class="jpb-addon-content">';
		$output .= '<div class="jpb-animated-number" data-format="' . $format . '" data-digit="' . $number . '" data-duration="' . $duration . '">0</div>';

		if ($counter_title) {
			$output .= '<div class="jpb-animated-number-title">' . $counter_title . '</div>';
		}
	...
	public function css() {
		...
		$alignmentStyle = $cssHelper->generateStyle ( '.jpb-addon.jpb-addon-animated-number', $settings, [ 
				'alignment' => 'text-align'
		], false );
		$numberStyle = $cssHelper->generateStyle ( '.jpb-animated-number', $settings, [ 
				'color' => 'color'
		], false );
		...
		$numberTypography = $cssHelper->typography ( '.jpb-animated-number', $settings, 'number_typography', $numberTypographyFallbacks );
		$titleStyle = $cssHelper->generateStyle ( '.jpb-animated-number-title', $settings, [ 
				'title_color' => 'color',
				'title_margin' => 'margin'
		], [ 
				'title_color' => false,
		...
		$numberTitleTypography = $cssHelper->typography ( '.jpb-animated-number-title', $settings, 'title_typography', $numberTitleTypographyFallbacks );
		...
		if ($number_before_after_text_position === 'right') {
			if ($number_before_after_text) {
				$css .= $addon_id . ' .jpb-animated-number::after {';
			...
			if ($number_before_after_text) {
				$css .= $addon_id . ' .jpb-animated-number::before {';
				$css .= 'content:"' . $number_before_after_text . '";';
				$css .= 'display: inline-block;';
				$css .= '}';
			}
	...
	public static function getFrontendEditor() {
		...
		<style type="text/css">
			<# if(data.number_before_after_text_position == "right") { #>
				#{{ addonId }} .jpb-animated-number::after{
					content:"{{data.number_before_after_text}}";
				}
			<# } else { #>
				#{{ addonId }} .jpb-animated-number::before{
					content:"{{data.number_before_after_text}}";
				}
			<# } #>
			';
		...
		// global
		$output .= $lodash->alignment ( 'text-align', '.jpb-addon-animated-number', 'data.alignment' );
		...
		$output .= $lodash->color ( 'color', '.jpb-animated-number', 'data.color' );
		$output .= $lodash->typography ( '.jpb-animated-number', 'data.number_typography', $numberTypographyFallbacks );
		...
		$output .= $lodash->spacing ( 'margin', '.jpb-animated-number-title', 'data.title_margin' );
		$output .= $lodash->color ( 'color', '.jpb-animated-number-title', 'data.title_color' );
		$output .= $lodash->typography ( '.jpb-animated-number-title', 'data.title_typography', $titleTypographyFallbacks );
		$output .= $lodash->generateTransformCss ( '.jpb-addon-content', 'data.transform' );
		$output .= '
		</style>
		<div class="jpb-addon jpb-addon-animated-number {{ data.class }} {{number_position}}">
			<div class="jpb-addon-content">
				<div class="jpb-animated-number jp-inline-editable-element" data-id={{data.id}} data-fieldName="number" contenteditable="true" data-format="{{data.use_number_format}}"  data-digit="{{ data.number }}" data-duration="{{ data.duration }}">0</div>
				<# if(data.counter_title){ #>
					<div class="jpb-animated-number-title jp-inline-editable-element" data-id={{data.id}} data-fieldName="counter_title" contenteditable="true"> {{{data.counter_title}}} </div>
				<# } #>
			</div>
		</div>';
