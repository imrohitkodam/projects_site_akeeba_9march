...
class JpagebuilderAddonButton extends JpagebuilderAddons {
	/**
	...
	public function render() {
		...
		$class .= (isset ( $settings->type ) && $settings->type) ? ' jpb-btn-' . $settings->type : '';
		$class .= (isset ( $settings->size ) && $settings->size) ? ' jpb-btn-' . $settings->size : '';
		$class .= (isset ( $settings->block ) && $settings->block) ? ' ' . $settings->block : '';
		$class .= (isset ( $settings->shape ) && $settings->shape) ? ' jpb-btn-' . $settings->shape : ' jpb-btn-rounded';
		$class .= (isset ( $settings->appearance ) && $settings->appearance) ? ' jpb-btn-' . $settings->appearance : '';

		// Need to create a function
		$attribs = '';

		list ( $link, $new_tab ) = JpagebuilderAddonHelper::parseLink ( $settings, 'url', [ 
				'url' => 'link',
				'new_tab' => 'target'
		] );
		...
		$output = '<div class="jpb-button-wrapper">';
		$output .= '<a ' . $hrefTag . ' ' . $new_tab . ' ' . $attribs . ' ' . $ariaLabel . ' class="jpb-btn ' . $class . '">' . $text . '</a>';
		$output .= '</div>';
	...
	public function css() {
		...
		$alignmentStyle = $cssHelper->generateStyle ( '.jpb-button-wrapper', $settings, [ 
		...
		$transformCss = $cssHelper->generateTransformStyle ( '.jpb-btn', $settings, 'transform' );
	...
	public static function getFrontendEditor() {
		...
		$output .= $lodash->generateTransformCss ( '.jpb-btn', 'data.transform' );
		$output .= '
		</style>
		...
		#>
		<a  {{href}} {{target}} {{rel}} id="btn-{{ data.id }}" aria-label="{{ariaLabel}}" class="jpb-btn {{ classList }}" data-id={{ data.id }}><# if(data.icon_position == "left" && !_.isEmpty(data.icon)) { #><i class="{{ icon_name }}" aria-hidden="true" ></i> <# } #><span class="jp-editable-content" data-id={{ data.id }} data-fieldName="text" data-placeholder="Add text...">{{{ data.text }}}</span><# if(data.icon_position == "right" && !_.isEmpty(data.icon)) { #> <i class="{{ icon_name }}" aria-hidden="true" ></i><# } #></a>';
