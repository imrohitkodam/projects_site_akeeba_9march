...
class JpagebuilderAddonButton_group extends JpagebuilderAddons {
	/**
	...
	public function render() {
		...
		$output = '<div class="jpb-addon jpb-addon-button-group' . $class . '">';
		$output .= '<div class="jpb-addon-content">';

		if (isset ( $this->addon->settings->jp_button_group_item ) && count ( ( array ) $this->addon->settings->jp_button_group_item )) {
			foreach ( $this->addon->settings->jp_button_group_item as $key => $value ) {
				if ($value->title || $value->icon) {
					...
					$class = (isset ( $value->type ) && $value->type) ? ' jpb-btn-' . $value->type : '';
					$class .= (isset ( $value->size ) && $value->size) ? ' jpb-btn-' . $value->size : '';
					$class .= (isset ( $value->block ) && $value->block) ? ' ' . $value->block : '';
					$class .= (isset ( $value->shape ) && $value->shape) ? ' jpb-btn-' . $value->shape : ' jpb-btn-rounded';
					$class .= (isset ( $value->appearance ) && $value->appearance) ? ' jpb-btn-' . $value->appearance : '';
					...
					$output .= '<a ' . $hrefTag . ' ' . $target . $attribs . ' class="jpb-btn ' . $class . '">' . $text . '</a>';
				}
			}
		}
	...
	public function css() {
		...
		$css = '';
		$groupStyle = $cssHelper->generateStyle ( '.jpb-addon-content', $settings, [ 
				'margin' => 'margin: -%s'
		] );
		$groupStyle .= $cssHelper->generateStyle ( '.jpb-addon-content .jpb-btn', $settings, [ 
				'margin' => 'margin'
		] );
		$alignmentStyle = $cssHelper->generateStyle ( '.jpb-addon.jpb-addon-button-group', $settings, [ 
		...
		// Buttons style
		if (isset ( $settings->jp_button_group_item ) && count ( ( array ) $settings->jp_button_group_item )) {
			foreach ( $settings->jp_button_group_item as $key => $value ) {
				if ($value->title) {
	...
	public static function getFrontendEditor() {
		...
		// global
		$output .= $lodash->alignment ( 'text-align', '.jpb-addon-button-group .jpb-addon-content', 'data.alignment' );

		$output .= $lodash->unit ( 'margin', '.jpb-addon-button-group .jpb-addon-content', 'data.margin', 'px', true, '-' );
		$output .= $lodash->unit ( 'margin', '.jpb-addon-button-group .jpb-addon-content .jpb-btn', 'data.margin', 'px' );

		$output .= ' <# _.each(data.jp_button_group_item, function(button, key) { #>';
		...
		$output .= $lodash->typography ( '#btn-{{ addonId }}{{ key }}.jpb-btn-{{ button.type }}', 'button.typography', $buttonTypographyFallbacks );

		// Custom
		$output .= '<# if (button.type == "custom") { #>';
		$output .= $lodash->color ( 'color', '#btn-{{ addonId }}{{ key }}.jpb-btn-custom', 'button.color' );
		$output .= $lodash->color ( 'color', '#btn-{{ addonId }}{{ key }}.jpb-btn-custom:hover', 'button.color_hover' );
		$output .= '<# if (button.appearance != "outline") { #>';
		$output .= $lodash->color ( 'background-color', '#btn-{{ addonId }}{{ key }}.jpb-btn-custom:hover', 'button.background_color_hover' );
		$output .= '<# } #>';
		$output .= '<#  if(_.isObject(button.button_padding)) { ' . $lodash->generateMissingBreakPoints ( 'button.button_padding' ) . ' #>';
		$output .= $lodash->spacing ( 'padding', '#btn-{{ addonId }}{{ key }}.jpb-btn-custom', 'button.button_padding' );
		$output .= '<# } else { #>';
		$output .= $lodash->spacing ( 'padding', '#btn-{{ addonId }}{{ key }}.jpb-btn-custom', 'button.button_padding' );
		$output .= '<# } #>';
		$output .= $lodash->unit ( 'font-size', '#btn-{{ addonId }}{{ key }}.jpb-btn-custom', 'button.fontsize', 'px' );

		// Outline
		$output .= '<# if (button.appearance == "outline") { #>';
		$output .= $lodash->border ( 'border-color', '#btn-{{ addonId }}{{ key }}.jpb-btn-custom', 'button.background_color' );
		$output .= $lodash->border ( 'border-color', '#btn-{{ addonId }}{{ key }}.jpb-btn-custom:hover', 'button.background_color_hover' );
		$output .= '#jpb-addon-{{ addonId }} #btn-{{ addonId }}{{ key }}.jpb-btn-custom { background-color: transparent; }';

		// 3D
		$output .= '<# } else if (button.appearance == "3d") { #>';
		$output .= $lodash->border ( 'border-bottom-color', '#btn-{{ addonId }}{{ key }}.jpb-btn-custom', 'button.background_color_hover' );
		$output .= $lodash->color ( 'background-color', '#btn-{{ addonId }}{{ key }}.jpb-btn-custom', 'button.background_color ' );

		// Gradient
		$output .= '<# } else if (button.appearance == "gradient" && _.isObject(button.background_gradient) ) { #>';
		$output .= '#jpb-addon-{{ addonId }} #btn-{{ addonId }}{{ key }}.jpb-btn-custom { border: none; } ';
		$output .= $lodash->color ( 'background-color', '#btn-{{ addonId }}{{ key }}.jpb-btn-custom', 'button.background_gradient' );
		$output .= $lodash->color ( 'background-color', '#btn-{{ addonId }}{{ key }}.jpb-btn-custom:hover', 'button.background_gradient_hover' );
		$output .= '<# } else { #>';
		$output .= $lodash->color ( 'background-color', '#btn-{{ addonId }}{{ key }}.jpb-btn-custom', 'button.background_color' );
		$output .= '<# } #>';
		$output .= '<# } #>';
		...
		// Normal
		$output .= $lodash->color ( 'color', '#btn-{{ addonId }}{{ key }}.jpb-btn-link', 'button.link_button_color' );
		$output .= $lodash->border ( 'border-color', '#btn-{{ addonId }}{{ key }}.jpb-btn-link', 'button.link_border_color' );
		$output .= $lodash->unit ( 'border-bottom-width', '#btn-{{ addonId }}{{ key }}.jpb-btn-link', 'button.link_button_border_width', 'px', false );
		$output .= $lodash->unit ( 'padding-bottom', '#btn-{{ addonId }}{{ key }}.jpb-btn-link', 'button.link_button_padding_bottom', 'px', false );
		$output .= '#jpb-addon-{{ addonId }} #btn-{{ addonId }}{{ key }}.jpb-btn-link { text-decoration: none; }';
		$output .= '#jpb-addon-{{ addonId }} #btn-{{ addonId }}{{ key }}.jpb-btn-link { border-radius: 0; }';

		// Hover
		$output .= '<# if (button.link_button_status == "hover") { #>';
		$output .= $lodash->color ( 'color', '#btn-{{ addonId }}{{ key }}.jpb-btn-link:hover, #btn-{{ addonId }}{{ key }}.jpb-btn-link:focus', 'button.link_button_hover_color' );
		$output .= $lodash->border ( 'border-color', '#btn-{{ addonId }}{{ key }}.jpb-btn-link:hover, #btn-{{ addonId }}{{ key }}.jpb-btn-link:focus', 'button.link_button_border_hover_color' );
		$output .= '<# } #>';
		$output .= '<# } #>';
		...
		<div class="jpb-addon jpb-addon-button-group {{ data.class }}">
			<div class="jpb-addon-content">
				<#
				 _.each(data.jp_button_group_item, function(button, key){
					var classList = button.class;
					classList += " jpb-btn-"+button.type;
					classList += " jpb-btn-"+button?.size;
					classList += " jpb-btn-"+button?.shape;
					if(!_.isEmpty(button.appearance)){
						classList += " jpb-btn-"+button.appearance;
					}
					...
					#>
					<a {{href}} {{target}} {{rel}} id="btn-{{ addonId }}{{ key }}"  class="jpb-btn {{ classList }}"><# if(button.icon_position == "left" && !_.isEmpty(button.icon)) { #><i class="{{ icon_name }}"></i> <# } #><span class="jp-editable-content" data-id={{ data.id }} data-button-group-index={{ key }} data-fieldName="text" data-placeholder="Add text...">{{{ button.title }}}</span><# if(button.icon_position == "right" && !_.isEmpty(button.icon)) { #> <i class="{{ icon_name }}"></i><# } #></a>
				<# }); #>
			</div>
		</div>
		<# if(!data.jp_button_group_item.length){ #>
			<div class="jpb-empty-addon">
