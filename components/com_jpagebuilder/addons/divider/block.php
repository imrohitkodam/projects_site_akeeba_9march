...
class JpagebuilderAddonDivider extends JpagebuilderAddons {
	public function render() {
		...
		// output start
		$output = '';
		$output .= '<div class="jpb-addon-divider-wrap divider-position">';
		$output .= '<div class="jpb-divider jpb-divider-' . $divider_type . ' ' . $class . '" role="none"></div>';
		$output .= '</div>';
		return $output;
	}
	public function css() {
		...
		// Alignment Style
		$settings->divider_position = JpagebuilderCSSHelper::parseAlignment ( $settings, 'divider_position' );
		$css .= $cssHelper->generateStyle ( '.divider-position', $settings, [ 
				'divider_position' => 'text-align'
		], false );

		$dividerStyle = $cssHelper->generateStyle ( '.jpb-divider', $settings, $dividerProps, $units );
		$css .= $dividerStyle;

		$transformCss = $cssHelper->generateTransformStyle ( '.jpb-divider', $settings, 'transform' );
		$css .= $transformCss;

		if ($innerStyle) {
			$css .= $addon_id . ' .jpb-divider {';
			$css .= $innerStyle;
			$css .= '}';
		}
	...
	public static function getFrontendEditor() {
		...
		$output .= $lodash->unit ( 'margin-top', '.jpb-divider', 'data.margin_top', 'px' );
		$output .= $lodash->unit ( 'margin-bottom', '.jpb-divider', 'data.margin_bottom', 'px' );
		$output .= $lodash->unit ( 'width', '.jpb-divider', 'data.container_div_width', 'px' );
		$output .= $lodash->alignment ( 'text-align', '.divider-position', 'data.divider_position' );

		$output .= '<# if(data.divider_type == "border") { #>';
		$output .= $lodash->unit ( 'border-radius', '.jpb-divider', 'data.border_radius', 'px' );
		$output .= '<# if(!data.divider_vertical) { #>';
		$output .= $lodash->unit ( 'border-bottom-width', '.jpb-divider', 'data.border_width', 'px' );
		$output .= $lodash->unit ( 'border-bottom-style', '.jpb-divider', 'data.border_style', '', false );
		$output .= $lodash->unit ( 'border-bottom-color', '.jpb-divider', 'data.border_color', '', false );
		$output .= '<# } else { #>';
		$output .= $lodash->unit ( 'height', '.jpb-divider', 'data.divider_height_vertical', 'px' );
		$output .= $lodash->unit ( 'width', '.jpb-divider', 'data.border_width', 'px' );
		$output .= $lodash->unit ( 'border-left-width', '.jpb-divider', 'data.border_width', 'px' );
		$output .= $lodash->unit ( 'border-left-style', '.jpb-divider', 'data.border_style', '', false );
		$output .= $lodash->unit ( 'border-left-color', '.jpb-divider', 'data.border_color', '', false );
		$output .= '<# } #>';
		$output .= '<# } else { #>';
		$output .= '<#
				let media = {};
		...
		$output .= '.jpb-divider {background-image: url({{mediaSrc}});background-position: 50% 50%;}';
		$output .= $lodash->unit ( 'background-repeat', '.jpb-divider', 'data.background_repeat', '', false );
		$output .= $lodash->unit ( 'height', '.jpb-divider', 'data.divider_height', 'px' );

		$output .= '<# } #>';
		$output .= $lodash->generateTransformCss ( '.jpb-divider', 'data.transform' );
		$output .= '
		</style>
		
		<# let dividerPosition = (!_.isEmpty(data.divider_type) && data.divider_type) ? "divider-position" : "";#>
		<div class="jpb-addon-divider-wrap {{dividerPosition}}">
			<div class="jpb-divider jpb-divider-{{ data.divider_type }} {{ data.class }}" role="none"></div>
		</div>';
