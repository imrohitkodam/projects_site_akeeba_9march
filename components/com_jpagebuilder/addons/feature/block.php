...
class JpagebuilderAddonFeature extends JpagebuilderAddons {
	/**
	...
	public function render() {
		...
		// Options
		list ( $link, $target ) = JpagebuilderAddonHelper::parseLink ( $settings, 'title_url', [ 
				'url' => 'title_url',
		...
		list ( $btn_link, $btn_link_target ) = JpagebuilderAddonHelper::parseLink ( $settings, 'btn_url', [ 
				'url' => 'btn_url',
				'new_tab' => 'btn_target'
		] );
		...
		$visually_hidden_text = $btn_aria_label ? '<span class="jpb-visually-hidden">' . $btn_aria_label . '' : '';
		...
		if (($icon_image_position === 'left') || ($icon_image_position === 'right')) {
			$alignment = 'jpb-text-' . $icon_image_position;
		}
		...
		if ($feature_type === 'icon') {
			if ($icon_name) {
				$media .= '<div class="jpb-icon">';

				if (($link && $url_appear == 'icon') || ($link && $url_appear == 'both')) {
					$media .= '<a href="' . $link . '" ' . $target . '>';
				}

				$media .= '<span class="jpb-icon-container" role="img" aria-label="' . strip_tags ( $title ) . '">';
			...
			if ($feature_image_src) {
				$media .= '<span class="jpb-img-container">';

				if (($link && $url_appear == 'icon') || ($link && $url_appear == 'both')) {
					$media .= '<a href="' . $link . '" ' . $target . '>';
				}

				$media .= '<img class="jpb-img-responsive' . ($placeholder ? ' jpb-element-lazy' : '') . '" style="display: inline-block" src="' . ($placeholder ? $placeholder : $feature_image_src) . '" alt="' . strip_tags ( $feature_image_alt ) . '" ' . ($placeholder ? 'data-large="' . $feature_image_src . '"' : '') . ' ' . ($feature_image_width ? 'width="' . $feature_image_width . '"' : '') . ' ' . ($feature_image_height ? 'height="' . $feature_image_height . '"' : '') . ' loading="lazy">';
		...
		if ($title) {
			...
			if (\in_array ( $icon_image_position, [ 
					'left',
					'right'
			] )) {
				$heading_class = ' jpb-media-heading';
			}

			$feature_title .= '<' . $heading_selector . ' class="jpb-addon-title jpb-feature-box-title' . $heading_class . '">';
		...
		$feature_text = '';
		if (! empty ( $text )) {
			$feature_text = '<div class="jpb-addon-text">';
			$feature_text .= $text;
			$feature_text .= '</div>';
		}

		// Output
		$output = '<div class="jpb-addon jpb-addon-feature ' . $alignment . ' ' . $class . '">';
		$output .= '<div class="jpb-addon-content">';

		if ($icon_image_position === 'before') {
			$output .= ($media) ? $media : '';
			$output .= '<div class="jpb-media-content">';
			$output .= ($title) ? $feature_title : '';
			$output .= $feature_text;

			if ($btn_text) {
				$output .= '<a ' . $attribs . ' class="jpb-btn ' . $btn_class . '">' . $btn_text . $visually_hidden_text . '</a>';
			}
			...
			$output .= '<div class="jpb-media">';
			$output .= '<div class="pull-' . $icon_image_position . '">';
			$output .= $media ?? '';
			$output .= '</div>';
			$output .= '<div class="jpb-media-body">';
			$output .= '<div class="jpb-media-content">';
			$output .= ($title) ? $feature_title : '';
			$output .= $feature_text;
	...
	public function css() {
		...
		$css .= $cssHelper->generateStyle ( '.jpb-addon-text', $settings, [ 
				'addon_color' => 'color'
		], false );
		...
		$textModifiers = [ 
				'text_padding' => 'spacing'
		];

		$textStyle = $cssHelper->generateStyle ( '.jpb-media-content', $settings, $textProps, $textUnits, $textModifiers );
		$css .= $textStyle;
		...
		$contentTypography = $cssHelper->typography ( '.jpb-addon-text', $settings, 'content_typography', $contentFallbacks );
		...
		$textTitleTypography = $cssHelper->typography ( '.jpb-feature-box-title', $settings, 'title_typography', $titleFallbacks );
		$css .= $textTitleTypography;

		if ($feature_type === 'icon' || $feature_type === 'both') {
			if (! empty ( $icon_name )) {
				$settings->icon_boxshadow = JpagebuilderCSSHelper::parseBoxShadow ( $settings, 'icon_boxshadow' );
				...
				$modifiers = [ 
						'icon_padding' => 'spacing'
				];
				$iconStyle = $cssHelper->generateStyle ( '.jpb-icon .jpb-icon-container', $settings, $props, $units, $modifiers, null, false, 'display:inline-block;text-align:center;' );
				$iconMarginStyle = $cssHelper->generateStyle ( '.jpb-icon', $settings, [ 
						'icon_margin_top' => 'margin-top',
						'icon_margin_bottom' => 'margin-bottom'
				] );
				$iconSizeStyle = $cssHelper->generateStyle ( '.jpb-icon .jpb-icon-container > i', $settings, [ 
						'icon_size' => [ 
								'font-size',
								'width',
								'height',
		...
		$textAlignment = $cssHelper->generateStyle ( '.jpb-addon-content', $settings, [ 
		...
		if ($feature_image_src && ($feature_type === 'image' || $feature_type === 'both')) {
			...
			if (\in_array ( $title_position, [ 
					'left',
					'right'
			] )) {
				$imageMarginSelector = '.jpb-media .pull-left, .jpb-media .pull-right';
				$css .= $cssHelper->generateStyle ( $imageMarginSelector, $settings, [ 
						'feature_image_width' => 'width'
				], [ 
						'feature_image_width' => '%'
				] );
			} elseif (\in_array ( $title_position, [ 
					'before',
					'after'
			] )) {
				$imageMarginSelector = '.jpb-img-container';
				$isContainer = true;
			}

			if (! empty ( $imageMarginSelector )) {
				$featureImageStyle = $cssHelper->generateStyle ( $imageMarginSelector, $settings, $imageProps, $imageUnits, [ 
						'feature_image_margin' => 'spacing'
				], null, false, $isContainer ? 'display: block;' : '' );
			}

			$device = JpagebuilderBase::$defaultDevice;

			if (! empty ( $settings->feature_image_width->$device ) && $settings->feature_image_width->$device === '100') {
				$mediaBodyStyle = $cssHelper->generateStyle ( '.jpb-media .jpb-media-body', $settings, [ 
						'feature_image_width' => 'width'
				], '%' );
			}
		...
		$buttonMarginStyle = $cssHelper->generateStyle ( '.jpb-media-content .jpb-btn', $settings, [ 
		...
		$settings->addon_hover_boxshadow = JpagebuilderCSSHelper::parseBoxShadow ( $settings, 'addon_hover_boxshadow' );
		...
		$addonTransitionStyle = $cssHelper->generateStyle ( ':self', $settings, [ 
				'addon_mock_transition' => 'transition'
		], false );
		$addonHoverStyle = $cssHelper->generateStyle ( '&:hover', $settings, $hoverProps, false );

		if (empty ( $settings->text )) {
			$css .= $addon_id . ' .jpb-addon-title.jpb-feature-box-title {display: block;}';
		}
		$css .= $cssHelper->generateStyle ( '.jpb-feature-box-title, .jpb-feature-box-title a', $settings, [ 
				'title_text_color' => 'color'
		], false );
		$css .= $cssHelper->generateStyle ( '&:hover .jpb-feature-box-title, &:hover .jpb-feature-box-title a', $settings, [ 
				'title_hover_color' => 'color'
		], false );
		$css .= $cssHelper->generateStyle ( '&:hover .jpb-addon-text', $settings, [ 
				'addon_hover_color' => 'color'
		], false );
		$css .= $cssHelper->generateStyle ( '&:hover .jpb-icon-container', $settings, [ 
				'icon_hover_color' => 'color',
				'icon_hover_bg' => 'background-color'
		], false );
	...
	public static function getFrontendEditor() {
		...
		$output = '
		<#
			let classList = " jpb-btn-" + data.btn_type;
			...
			if(isObject || isString) {
				...
				if(urlObj.type === "url") {	
					...
					url = `index.php?option=com_jpagebuilder&view=page&id=${urlObj.page}`
				}
			...
			if (data.feature_type == "icon") {
				if (data.icon_name) {
					media += \'<div class="jpb-icon">\';
						...
						media  += \'<span class="jpb-icon-container">\';
				...
				if (feature_image.src) {
					...
					if (feature_image.src.indexOf("http://") != -1 || feature_image.src.indexOf("https://") != -1) {
						media  += \'<img class="jpb-img-responsive" style="display: inline-block" src="\'+feature_image.src+\'" alt="\'+data.title+\'">\';
					} else {
						media  += \'<img class="jpb-img-responsive" style="display: inline-block" src="\'+pagebuilder_base+feature_image.src+\'" alt="\'+data.title+\'">\';
					}
			...
			if (data.title) {
				...
				if (((isObject || isString) && data.url_appear == "title") || ((isObject || isString) && data.url_appear == "both" )) {
					feature_title += \'<a href="\'+ url +\'" target="\' + target + \'" rel="\' + rel + \'" class="jp-inline-editable-element" data-id="\'+data.id+\'" data-fieldName="title" contenteditable="true">\';
				}

				if (_.isEmpty(data.title_url)) {
					feature_title += \'<span class="jp-inline-editable-element" data-id="\'+data.id+\'" data-fieldName="title" contenteditable="true">\';
				}
			...
			let feature_text = "";
			if(data.text) {
				feature_text  = \'<div id="addon-text-\'+data.id+\'" class="jpb-addon-text jp-editable-content" data-id="\'+data.id+\'" data-fieldName="text">\';
		...
		$output .= $lodash->typography ( '.jpb-btn', 'data.btn_typography', $buttonTypographyFallbacks );
		$output .= '<# if (data.btn_size == "custom") { #>';
		$output .= $lodash->spacing ( 'padding', '.jpb-btn', 'data.button_padding' );
		$output .= '<# } #>';
		$output .= $lodash->spacing ( 'margin', '.jpb-btn', 'data.button_margin' );
		$output .= '<# if (data.btn_block == "jpb-btn-block") { #>';
		$output .= '#jpb-addon-{{ data.id }} .jpb-btn {display: block;}';
		$output .= '<# } #>';
		...
		$output .= $lodash->color ( 'color', '.jpb-btn', 'data.btn_color' );
		$output .= $lodash->color ( 'color', '.jpb-btn:hover', 'data.btn_color_hover' );

		$output .= '<# if (data.btn_appearance == "outline") { #>';
		$output .= '#jpb-addon-{{ data.id }} .jpb-btn {background-color: transparent;}';
		$output .= $lodash->unit ( 'border-color', '.jpb-btn', 'data.btn_background_color', '', false );
		$output .= $lodash->unit ( 'border-color', '.jpb-btn:hover', 'data.btn_background_color_hover', '', false );
		$output .= $lodash->unit ( 'background-color', '.jpb-btn:hover', 'data.btn_background_color_hover', '', false );
		$output .= '<# } else if (data.btn_appearance == "gradient") { #>';
		$output .= '#jpb-addon-{{ data.id }} .jpb-btn {border: none;}';
		$output .= $lodash->color ( 'background-color', '.jpb-btn', 'data.btn_background_gradient' );
		$output .= $lodash->color ( 'background-color', '.jpb-btn:hover', 'data.btn_background_gradient_hover' );
		$output .= '<# } else { #>';
		$output .= $lodash->color ( 'background-color', '.jpb-btn', 'data.btn_background_color' );
		$output .= $lodash->color ( 'background-color', '.jpb-btn:hover', 'data.btn_background_color_hover' );
		$output .= '<# } #>';
		$output .= '<# } #>';
		...
		$output .= $lodash->color ( 'color', '.jpb-btn', 'data.link_btn_color' );
		$output .= $lodash->unit ( 'border-color', '.jpb-btn', 'data.link_btn_border_color', '', false );
		$output .= $lodash->unit ( 'border-bottom-width', '.jpb-btn', 'data.link_btn_border_width', 'px' );
		$output .= $lodash->unit ( 'padding-bottom', '.jpb-btn', 'data.link_btn_padding_bottom', 'px' );
		$output .= $lodash->color ( 'color', '.jpb-btn:hover, .jpb-btn:focus', 'data.link_btn_hover_color' );
		$output .= $lodash->unit ( 'border-color', '.jpb-btn:hover, .jpb-btn:focus', 'data.link_btn_border_hover_color', '', false );
		$output .= '<# } #>';
		// button end

		// icon
		$output .= '#jpb-addon-{{ data.id }} .jpb-icon .jpb-icon-container {display: inline-block; text-align: center; transition: 300ms;}';
		$output .= $lodash->color ( 'color', '.jpb-icon .jpb-icon-container', 'data.icon_color' );
		$output .= $lodash->spacing ( 'padding', '.jpb-icon .jpb-icon-container', 'data.icon_padding' );
		$output .= $lodash->color ( 'background-color', '.jpb-icon .jpb-icon-container', 'data.icon_background' );
		$output .= '<# if (data.icon_border_color) { #>';
		$output .= '#jpb-addon-{{ data.id }} .jpb-icon .jpb-icon-container {border-style: solid;}';
		$output .= $lodash->unit ( 'border-color', '.jpb-icon .jpb-icon-container', 'data.icon_border_color', '', false );
		$output .= $lodash->unit ( 'border-width', '.jpb-icon .jpb-icon-container', 'data.icon_border_width', 'px' );
		$output .= '<# } #>';
		$output .= $lodash->unit ( 'border-radius', '.jpb-icon .jpb-icon-container', 'data.icon_border_radius', 'px' );
		$output .= $lodash->boxShadow ( '.jpb-icon .jpb-icon-container', 'data.icon_boxshadow' );

		$output .= $lodash->unit ( 'margin-top', '.jpb-icon', 'data.icon_margin_top', 'px' );
		$output .= $lodash->unit ( 'margin-bottom', '.jpb-icon', 'data.icon_margin_bottom', 'px' );

		$output .= $lodash->unit ( 'font-size', '.jpb-icon .jpb-icon-container > i', 'data.icon_size', 'px' );
		$output .= $lodash->unit ( 'width', '.jpb-icon .jpb-icon-container > i', 'data.icon_size', 'px' );
		$output .= $lodash->unit ( 'height', '.jpb-icon .jpb-icon-container > i', 'data.icon_size', 'px' );
		$output .= $lodash->unit ( 'line-height', '.jpb-icon .jpb-icon-container > i', 'data.icon_size', 'px' );

		$output .= $lodash->color ( 'color', '&:hover .jpb-icon .jpb-icon-container', 'data.icon_hover_color' );
		$output .= $lodash->color ( 'background-color', '&:hover .jpb-icon .jpb-icon-container', 'data.icon_hover_bg' );

		// image
		$output .= '#jpb-addon-{{ data.id }} .jpb-img-container {display: block;}';
		$output .= '<# if (!_.isEmpty(data.feature_image_margin) && (data.title_position == "left" || data.title_position == "right")) { #>';
		$output .= $lodash->spacing ( 'margin', '.jpb-media .pull-left, .jpb-media .pull-right', 'data.feature_image_margin' );
		$output .= '<# } #>';

		$output .= '<# if (!_.isEmpty(data.feature_image_margin) && (data.title_position == "after" || data.title_position == "before")) { #>';
		$output .= $lodash->spacing ( 'margin', '.jpb-img-container', 'data.feature_image_margin' );
		$output .= '<# } #>';
		$output .= '<# if (data.feature_type !== "icon") { #>';
		$output .= $lodash->unit ( 'width', '.jpb-media .pull-left, .jpb-media .pull-right', 'data.feature_image_width', '%' );
		$output .= '<# } #>';

		$output .= '#jpb-addon-{{ data.id }} {transition: 300ms;}';
		$output .= '#jpb-addon-{{ data.id }} .jpb-feature-box-title {transition: 300ms;}';
		...
		$output .= $lodash->typography ( '.jpb-feature-box-title', 'data.title_typography', $titleTypographyFallbacks );

		$output .= '<# if (!data.text) { #>';
		$output .= '#jpb-addon-{{ data.id }} .jpb-addon-title.jpb-feature-box-title {display: block;}';
		$output .= '<# } #>';

		$output .= $lodash->color ( 'color', '.jpb-feature-box-title, .jpb-feature-box-title a', 'data.title_text_color' );
		$output .= $lodash->color ( 'color', '.jpb-addon-text', 'data.addon_color' );
		...
		$output .= $lodash->typography ( '.jpb-addon-text', 'data.content_typography', $textTypographyFallbacks );
		$output .= $lodash->spacing ( 'padding', '.jpb-media-content', 'data.text_padding' );
		$output .= $lodash->color ( 'background-color', '.jpb-media-content', 'data.text_background' );
		$output .= $lodash->alignment ( 'text-align', '.jpb-addon-content', 'data.alignment' );

		// Addon Background Color
		$output .= $lodash->color ( 'background-color', ':self', 'data.background_color' );

		// Hover
		$output .= $lodash->color ( 'color', '&:hover .jp-editable-content', 'data.addon_hover_color' );
		$output .= $lodash->color ( 'background-color', '&:hover', 'data.addon_hover_bg' );
		$output .= $lodash->boxShadow ( '&:hover', 'data.addon_hover_boxshadow' );
		$output .= $lodash->color ( 'color', '&:hover .jpb-feature-box-title, &:hover .jpb-feature-box-title a', 'data.title_hover_color' );
		$output .= $lodash->generateTransformCss ( '', 'data.transform' );

		$output .= '
		</style>
		<div class="jpb-addon jpb-addon-feature {{ data.class }} {{ alignment }}">
			...
			const visuallyHiddenText = ariaLabel ? "<span class=\'jpb-visually-hidden\'>" + ariaLabel + "</span>" : "";
				...
				if(urlObjBtn.type === "page") {
					urlBtn = urlObjBtn.page ? `index.php?option=com_jpagebuilder&view=page&id=${urlObjBtn.page}` : "";
				}
			...
			#>
				<# if (icon_image_position == "before") { #>
                    ...
                    <div class="jpb-media-content">
						...
						#>
							<a href=\'{{ urlBtn }}\' target=\'{{ targetBtn }}\' rel=\'{{ relBtn }}\' id="btn-{{ data.id }}" aria-label="{{ ariaLabel }}" class="jpb-btn {{ classList }}"><# if(data.btn_icon_position == "left" && !_.isEmpty(data.btn_icon)) { #><i class="{{ icon_name }}"></i> <# } #>{{ data.btn_text }}<# if(data.btn_icon_position == "right" && !_.isEmpty(data.btn_icon)) { #> <i class="{{ icon_name }}"></i><# } #>{{{ visuallyHiddenText }}}</a>
						<# } #>
                    </div>
				<# } else if (icon_image_position == "after") { #>
					<# if(data.title){ #>
					...
					<# if(data.btn_text && _.trim(data.btn_text)){
						...
						let icon_name = icon_arr.length === 1 ? "fa "+data.btn_icon : data.btn_icon;
					#>
						<a href=\'{{ urlBtn }}\' target=\'{{ targetBtn }}\' rel=\'{{ relBtn }}\' class="jpb-btn {{ classList }}"><# if(data.btn_icon_position == "left" && !_.isEmpty(data.btn_icon)) { #><i class="{{ icon_name }}"></i> <# } #>{{ data.btn_text }}<# if(data.btn_icon_position == "right" && !_.isEmpty(data.btn_icon)) { #> <i class="{{ icon_name }}"></i><# } #>{{{ visuallyHiddenText }}}</a>
					<# } #>
                    </div>
				<# } else { #>
					<# if(media) { #>
