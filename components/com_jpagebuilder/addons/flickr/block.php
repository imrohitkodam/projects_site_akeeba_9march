...
class JpagebuilderAddonFlickr extends JpagebuilderAddons {
	/**
	...
	private function getImages() {
		$cache_path = JPATH_CACHE . '/com_jpagebuilder/addons/addon-' . $this->addon->id;
	...
	public function render() {
		...
		// Output
		$output = '<div class="jpb-addon jpb-addon-flickr ' . $class . '">';
		$output .= ($title) ? '<' . $heading_selector . ' class="jpb-addon-title">' . $title . '</' . $heading_selector . '>' : '';
		$output .= '<div class="jpb-addon-content">';
		$output .= '<ul class="jpb-flickr-gallery">';

		if ($api_code === '2fc5721d612333f915b8ab6c9def835f') {
			for($i = 0; $i < $count; $i ++) {
				$output .= '<li>';
				$output .= '<a target="_blank" rel="noopener noreferrer" href="' . str_replace ( '_m', '_b', $images [$i]->media->m ) . '" class="jpb-flickr-gallery-btn">';
				$output .= '<img class="jpb-img-responsive" src="' . str_replace ( '_m', '_q', $images [$i]->media->m ) . '" alt="' . $images [$i]->title . '" loading="lazy">';
				$output .= '</a>';
				$output .= '</li>';
			}
		} else {
			foreach ( $images as $image ) {
				...
				$output .= '<li>';
				$output .= '<a target="_blank" rel="noopener noreferrer" href="' . $image_url . '" class="jpb-flickr-gallery-btn">';
				$output .= '<img class="jpb-img-responsive" src="' . substr_replace ( $image_url, "_q", - 6, 2 ) . '" alt="' . $image->title . '" loading="lazy">';
				$output .= '</a>';
				$output .= '</li>';
			}
		}
	...
	public function css() {
		...
		if ($thumb_per_row) {
			$thumbStyle = $cssHelper->generateStyle ( '.jpb-flickr-gallery li', $settings, [ 
					'flickr_thumbnail_width' => 'height: auto; width: %s'
			], '%' );
			$css .= $thumbStyle;
		}
		$transformCss = $cssHelper->generateTransformStyle ( '.jpb-addon-flickr', $settings, 'transform' );
		$css .= $transformCss;
