...
class JpagebuilderAddonFlip_box extends JpagebuilderAddons {
	/**
	...
	public function render() {
		...
		$output = '';
		$output .= '<div class="jpb-addon jpb-addon-jpb-flibox ' . $class . ' ' . $flip_style . ' ' . $rotate . ' flipon-' . $flip_bhave . '">';
	...
	public function css() {
		...
		$frontProps = [ 
				...
				'front_textcolor' => 'color'
		];

		$frontStyle = $cssHelper->generateStyle ( '.jpb-flipbox-front, .threeD-flip-front', $settings, $frontProps, false );
		$frontBgStyle = $cssHelper->generateStyle ( '.threeD-flip-front:before, .jpb-flipbox-front.flip-box:before', $settings, [ 
				'front_bgcolor' => 'background-color'
		], false );
		...
		$backProps = [ 
				...
				'back_textcolor' => 'color'
		];

		$backStyle = $cssHelper->generateStyle ( '.jpb-flipbox-back, .threeD-flip-back', $settings, $backProps, false );
		$backBgStyle = $cssHelper->generateStyle ( '.threeD-flip-back:before, .jpb-flipbox-back.flip-box:before', $settings, [ 
				'back_bgcolor' => 'background-color'
		], false );
	...
	public static function getFrontendEditor() {
		...
		$output .= $lodash->color ( 'color', '.jpb-flipbox-front, .threeD-flip-front', 'data.front_textcolor' );
		$output .= $lodash->color ( 'color', '.jpb-flipbox-back, .threeD-flip-back', 'data.back_textcolor' );
		$output .= $lodash->color ( 'background-color', '.threeD-flip-front:before, .jpb-flipbox-front.flip-box:before', 'data.front_bgcolor' );
		$output .= $lodash->color ( 'background-color', '.threeD-flip-back:before, .jpb-flipbox-back.flip-box:before', 'data.back_bgcolor' );
		...
		$output .= '</style>
        <div class="jpb-addon jpb-addon-jpb-flibox {{ data.class }} {{ flip_style }} {{ data.rotate }} flipon-{{ data.flip_bhave }}">
            <# if (flip_style == "threeD-flipbox") { #>
                <div class="threeD-content-wrap">
                    <div class="threeD-item">
                        <div class="threeD-flip-front">
                            <div class="threeD-content-inner jp-inline-editable-element" data-id={{data.id}} data-fieldName="front_text" contenteditable="true">{{{ data.front_text }}}</div>
                        </div>
                        <div class="threeD-flip-back">
                            <div class="threeD-content-inner jp-inline-editable-element" data-id={{data.id}} data-fieldName="flip_text" contenteditable="true">{{{ data.flip_text }}}</div>
                        </div >
                    </div>
                </div>
            <# } else { #>
                <div class="jpb-flipbox-panel">
                    <div class="jpb-flipbox-front flip-box">
                        <div class="flip-box-inner jp-inline-editable-element" data-id={{data.id}} data-fieldName="front_text" contenteditable="true">{{{ data.front_text }}}</div>
                    </div>
                    <div class="jpb-flipbox-back flip-box">
                        <div class="flip-box-inner jp-inline-editable-element" data-id={{data.id}} data-fieldName="flip_text" contenteditable="true">{{{ data.flip_text }}}</div>
                    </div>
                </div>
            <# } #>
        </div>
