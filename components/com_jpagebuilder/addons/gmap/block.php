...
class JpagebuilderAddonGmap extends JpagebuilderAddons {
	public function render() {
		...
		if ($map) {
			$map = explode ( ',', $map );
			$output = '<div id="jpb-addon-map-' . $this->addon->id . '" class="jpb-addon jpb-addon-gmap ' . $class . '">';
			$output .= ($title) ? '<' . $heading_selector . ' class="jpb-addon-title">' . $title . '</' . $heading_selector . '>' : '';
			$output .= '<div class="jpb-addon-content">';
			$output .= '<div id="jpb-addon-gmap-' . $this->addon->id . '" class="jpb-addon-gmap-canvas" data-lat="' . trim ( $map [0] ) . '" data-lng="' . trim ( $map [1] ) . '" data-location=\'' . base64_encode ( $location_json ) . '\' data-maptype="' . $type . '" data-mapzoom="' . $zoom . '" data-mousescroll="' . $mousescroll . '" data-infowindow="' . base64_encode ( $infowindow ) . '" data-show-controll=\'' . $show_controllers . '\'></div>';
			$output .= '</div>';
			$output .= '</div>';
			return $output;
		}
	...
	public function scripts() {
		...
		Factory::getApplication ()->getDocument ()->getWebAssetManager ()->registerAndUseScript ( 'gmap-api', "https://maps.googleapis.com/maps/api/js?key=" . $gmap_api . '&libraries=marker', [ ], [ ] );

		return [ 
				'components/com_jpagebuilder/assets/js/gmap.js'
		];
	}
	public function css() {
		...
		$height = $cssHelper->generateStyle ( '.jpb-addon-gmap-canvas', $settings, [ 
				'height' => 'height'
		] );
		$css .= $height;
		...
		$transformCss = $cssHelper->generateTransformStyle ( '.jpb-addon-gmap-canvas', $settings, 'transform' );
		$css .= $transformCss;
	...
	public static function getFrontendEditor() {
		...
		$output .= $lodash->unit ( 'height', '.jpb-addon-gmap-canvas', 'data.height', 'px' );
		$output .= $lodash->generateTransformCss ( '.jpb-addon-gmap-canvas', 'data.transform' );
		$output .= '
		#jpb-addon-map-{{ data.id }} a>div>img{top:-15px!important;}
		</style>
		<div id="jpb-addon-map-{{ data.id }}" class="jpb-addon jpb-addon-gmap {{ data.class }}">
