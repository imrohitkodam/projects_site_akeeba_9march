...
class JpagebuilderAddonImage extends JpagebuilderAddons {
	/**
	...
	public function render() {
		...
		list ( $link, $target ) = JpagebuilderAddonHelper::parseLink ( $settings, 'link', [ 
				'url' => 'link',
				'new_tab' => 'target'
		] );
		...
		if ($image_src) {
			$output .= '<div class="jpb-addon jpb-addon-single-image ' . ' ' . $class . ' jpb-addon-image-shape">';
			...
			$output .= '<div class="jpb-addon-single-image-container">';
			...
			if ($open_lightbox) {
				$output .= '<a class="jpb-magnific-popup jpb-image-lightbox jpb-addon-image-overlay-icon" data-popup_type="image" data-mainclass="mfp-no-margins mfp-with-zoom" href="' . $image_src . '">+</a>';
			}
			...
			if ($is_image_shape_enabled && $image_shape) {
				...
				$image_shape_class = 'jpb-addon-image-shape-' . $image_shape_value;
				...
				if (in_array ( $image_shape, $default_shapes )) {
					$output .= '<img class="jpb-img-responsive' . ($placeholder ? ' jpb-element-lazy "' : '') . ' ' . $image_shape_class . '" src="' . ($placeholder ? $placeholder : $image_src) . '" ' . $image2x . ($placeholder ? 'data-large="' . $image_src . '"' : '') . ' alt="' . $alt_text . '" title="' . $final_image_title . '" ' . ($image_width ? 'width="' . $image_width . '"' : '') . ' ' . ($image_height ? 'height="' . $image_height . '"' : '') . ($placeholder ? 'loading="lazy"' : '') . ' ' . $dimension . '/>';
				} else {
					...
					if (preg_match ( $pattern, $decoded_shape, $matches )) {
						...
						$output .= '<img data-scale="' . $image_shape_scale . '" style="clip-path: url(#svg-shape-' . $this->addon->id . '); visibility: hidden;" class="jpb-img-responsive' . ($placeholder ? ' jpb-element-lazy "' : '') . ' " src="' . ($placeholder ? $placeholder : $image_src) . '" ' . $image2x . ($placeholder ? 'data-large="' . $image_src . '"' : '') . ' alt="' . $alt_text . '" title="' . $final_image_title . '" ' . ($image_width ? 'width="' . $image_width . '"' : '') . ' ' . ($image_height ? 'height="' . $image_height . '"' : '') . ($placeholder ? 'loading="lazy"' : '') . ' ' . $dimension . '/>';
						$output .= '<svg>
							<defs>
	...
	public function js() {
		...
		$js = 'jQuery(document).ready(function ($) {
			...
			$(document).on("click", "' . $addon_id . ' .jpb-image-lightbox", function (event) {
	...
	public function scripts() {
		return [ 
				...
				'components/com_jpagebuilder/assets/js/addons/image.js'
		];
	}
	...
	public function css() {
		...
		$settings->position = JpagebuilderCSSHelper::parseAlignment ( $settings, 'position' );
		...
		$object_fit = (isset ( $settings->object_fit ) && $settings->object_fit != '') ? $settings->object_fit : 'unset';
		$settings->object_fit = $object_fit;
		
		$imageStyle = $cssHelper->generateStyle ( 'img', $settings, [
				'image_width' => [
						'width',
						'max-width'
				],
		...
		$imagePositionStyle = $cssHelper->generateStyle ( ':self', $settings, [ 
				'position' => 'text-align'
		], false );
		...
		if ($isZoomOnScale && $zoomScale) {
			...
			$imageHoverStyle .= $cssHelper->generateStyle ( '.jpb-addon-single-image-container', $settings, [ 
					'container_overflow' => 'overflow',
					'container_display' => 'display'
			], false );
			$imageHoverStyle .= $cssHelper->generateStyle ( '.jpb-addon-single-image-container img', $settings, [ 
					'zoom_scale_transition' => 'transition'
			], false );
			$imageHoverStyle .= $cssHelper->generateTransformStyle ( '.jpb-addon-single-image-container:hover img', $settings, 'transform' );
		}
	...
	static function getImageShape($shape_name) {
		...
		$query->from ( $db->quoteName ( '#__jpagebuilder_image_shapes', 'a' ) );
	...
	public static function getFrontendEditor() {
		...
		<#
			...
			if(default_shapes.includes(image_shape) && is_image_shape_enabled && image_shape) {
				cssClass = "jpb-addon-image-shape-" + data.image_shape.replaceAll("_", "-");
			}
		...
		$output .= $lodash->color ( 'background-color', '.jpb-addon-image-overlay', 'data.overlay_color' );
		$output .= '<# if((data.border_radius + "").split(" ").length < 2) { #>';
		$output .= $lodash->unit ( 'border-radius', '.jpb-addon-image-overlay', 'data.border_radius', 'px' );
		$output .= '<# } else { #>';
		$output .= $lodash->unit ( 'border-radius', '.jpb-addon-image-overlay', 'data.border_radius', '' );
		$output .= '<# } #>';

		$output .= $lodash->generateTransformCss ( 'img', 'transform' );

		$output .= '.jpb-addon-single-image-container {  overflow: hidden;
			display: inline-block;}';
		$output .= '.jpb-addon-single-image-container img{transition: transform 0.5s ease;}';
		...
		<# if(data.image) {
			...
			#>
			<div class="jpb-addon jpb-addon-single-image {{ data.class }} jpb-addon-image-shape">
				<# if( !_.isEmpty( data.title ) && data.title_position != "bottom" ){ #><{{ data.heading_selector }} class="jpb-addon-title jp-inline-editable-element" data-id={{data.id}} data-fieldName="title" contenteditable="true">{{ data.title }}</{{ data.heading_selector }}><# } #>
				<div class="jpb-addon-content">
					<div class="jpb-addon-single-image-container">
						<# if(image_overlay && open_lightbox) { #>
							<div class="jpb-addon-image-overlay"></div>
						<# } #>
				
						<# if(open_lightbox) { #>
				
							<a class="jpb-magnific-popup jpb-image-lightbox jpb-addon-image-overlay-icon" data-popup_type="image" data-mainclass="mfp-no-margins mfp-with-zoom" data-clip-path={{clip_path_url ?? false}} data-shape-enabled={{data.is_image_shape_enabled === 0 ? false : true}} data-shape-css={{cssClass ?? false}} href=\'{{ media.src }}\'>+</a>
						<# } #>
						...
						<# if(!open_lightbox) {  #>
							...
							if(isUrlObject || isUrlString ) {
								...
								const clickUrl = (type === "url" && url) || (type ==="menu" && urlObj.menu) || ( (type === "page" && !!urlObj.page) && "index.php/component/jpagebuilder/index.php?option=com_jpagebuilder&view=page&id=" +urlObj.page) || "";
								#>
								  <# if(clickUrl) {#>
						  ...
						  <# if(is_image_shape_enabled && image_shape && !default_shapes.includes(image_shape)) { #>
							<# if(media.src.indexOf("http://") == -1 && media.src.indexOf("https://") == -1){ #>
								<img data-scale="{{{image_shape_scale}}}" style="clip-path: url(#svg-shape-{{{data.id}}}); visibility: hidden;" class="jpb-img-responsive" src=\'{{ pagebuilder_base + media.src }}\' alt="{{ alt_text }}" title="{{ final_image_title }}">
							<# } else { #>
								<img data-scale="{{{image_shape_scale}}}" style="clip-path: url(#svg-shape-{{{data.id}}}); visibility: hidden;" class="jpb-img-responsive" src=\'{{ media.src }}\' alt="{{ alt_text }}" title="{{ final_image_title }}">
							<# } #>
							...
							<# } else { #>
								<# if(media.src.indexOf("http://") == -1 && media.src.indexOf("https://") == -1){ #>
									<img class="jpb-img-responsive {{image_shape_class}}" src=\'{{ pagebuilder_base + media.src }}\' alt="{{ alt_text }}" title="{{ final_image_title }}">
								<# } else { #>
									<img class="jpb-img-responsive {{image_shape_class}}" src=\'{{ media.src }}\' alt="{{ alt_text }}" title="{{ final_image_title }}">
								<# } #>
							<# } #>
					...
					<# if( !_.isEmpty( data.title ) && data.title_position == "bottom" ){ #><{{ data.heading_selector }} class="jpb-addon-title" style="display: block;">{{ data.title }}</{{ data.heading_selector }}><# } #>
				</div>
			</div>
		<# } else { #>
