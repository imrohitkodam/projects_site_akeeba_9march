...
class JpagebuilderAddonImage_carousel extends JpagebuilderAddons {
	/**
	...
	public function render() {
		...
		if (isset ( $settings->jp_image_carousel_item ) && is_array ( $settings->jp_image_carousel_item )) {
			foreach ( $settings->jp_image_carousel_item as $item_key => $carousel_item ) {
				...
				if (! empty ( $carousel_item->image_carousel_img_link )) {
					list ( $link, $target ) = JpagebuilderAddonHelper::parseLink ( $carousel_item, 'image_carousel_img_link', [ 
							'url' => 'image_carousel_img_link',
				...
				if ($carousel_overlay) {
					$output .= '<div class="jpb-carousel-extended-item-overlay"></div>';
				}

				if ($image_carousel_layout != '') {
					$output .= '<div class="jpb-carousel-extended-content-wrap">';

					if (isset ( $carousel_item->item_title )) {
						$output .= '<div class="jpb-carousel-extended-heading">';
						$output .= $carousel_item->item_title;
						$output .= '</div>';
					}

					if (isset ( $carousel_item->item_subtitle )) {
						$output .= '<div class="jpb-carousel-extended-subheading">';
						$output .= $carousel_item->item_subtitle;
						$output .= '</div>';
					}

					if (isset ( $carousel_item->item_description )) {
						$output .= '<div class="jpb-carousel-extended-description">';
	...
	public function css() {
		...
		if ($carousel_arrow) {
			$carouselArrowPosition = $cssHelper->generateStyle ( '.jpb-carousel-extended-nav-control .nav-control', $settings, [ 
		...
		if ($carousel_bullet) {
			...
			$bulletPositionStyle = $cssHelper->generateStyle ( '.jpb-carousel-extended-dots', $settings, [ 
			...
			$bulletStyle = $cssHelper->generateStyle ( '.jpb-carousel-extended-dots ul li', $settings, $bulletStyleProps, $bulletStyleUnits );
			// Bullet hover style
			...
			$bulletHoverStyle = $cssHelper->generateStyle ( '.jpb-carousel-extended-dots ul li:hover span, .jpb-carousel-extended-dots ul li.active span', $settings, $bulletHoverStyleProps, $bulletHoverStyleUnits );
		...
		if ($carousel_overlay) {
			...
			$overlayStyle = $cssHelper->generateStyle ( '.jpb-carousel-extended-item-overlay', $settings, [ 
		...
		if ($item_content_verti_align || $item_content_hori_align) {
			$css .= '#jpb-addon-' . $this->addon->id . ' .jpb-carousel-extended-content-wrap {';
		...
		$titleTypographyStyle = $cssHelper->typography ( '.jpb-carousel-extended-heading', $settings, 'content_title_typography', $titleTypographyFallbacks );
		$subtileTypographyStyle = $cssHelper->typography ( '.jpb-carousel-extended-subheading', $settings, 'content_subtitle_typography', $subtileTypographyFallbacks );
		$descriptionTypographyStyle = $cssHelper->typography ( '.jpb-carousel-extended-description', $settings, 'description_typography', $descriptionTypographyFallbacks );

		$titleStyle = $cssHelper->generateStyle ( '.jpb-carousel-extended-heading', $settings, [ 
				'content_title_text_color' => 'color',
				'content_title_margin' => 'margin'
		], [ 
				'content_title_text_color' => false,
				'content_title_margin' => false
		], [ 
				'content_title_margin' => 'spacing'
		] );
		$subtileStyle = $cssHelper->generateStyle ( '.jpb-carousel-extended-subheading', $settings, [ 
				'content_subtitle_text_color' => 'color'
		], [ 
				'content_subtitle_text_color' => false
		] );
		$descriptionStyle = $cssHelper->generateStyle ( '.jpb-carousel-extended-description', $settings, [ 
				'description_text_color' => 'color',
				'description_margin' => 'margin'
		], [ 
				'description_text_color' => false,
				'description_margin' => false
		], [ 
				'description_margin' => 'spcacing'
		] );
		...
		if ($image_carousel_layout == 'layout3' || $image_carousel_layout == 'layout4') {
			$css .= '#jpb-addon-' . $this->addon->id . ' .jpb-carousel-extended-center .jpb-carousel-extended-item .jpb-addon-wrapper {';
		...
		$transformCss = $cssHelper->generateTransformStyle ( '.jpb-carousel-extended', $settings, 'transform' );
	...
	public static function getFrontendEditor() {
        ...
        <style  type="text/css">
            ...
            <# if(data.image_carousel_layout == "layout3" || data.image_carousel_layout == "layout4"){ #>
                #jpb-addon-{{ data.id }} .jpb-carousel-extended-center .jpb-carousel-extended-item .jpb-addon-wrapper {
            ...
            <# if(data.item_content_verti_align || data.item_content_hori_align) { #>
                #jpb-addon-{{ data.id }} .jpb-carousel-extended-content-wrap {
		...
		$output .= $lodash->typography ( '.jpb-carousel-extended-heading', 'data.content_title_typography', $titleTypographyFallbacks );
		$output .= $lodash->color ( 'color', '.jpb-carousel-extended-heading', 'data.content_title_text_color' );
		$output .= $lodash->spacing ( 'margin', '.jpb-carousel-extended-heading', 'data.content_title_margin' );
		...
		$output .= $lodash->typography ( '.jpb-carousel-extended-subheading', 'data.content_subtitle_typography', $subtitleTypographyFallbacks );
		$output .= $lodash->color ( 'color', '.jpb-carousel-extended-subheading', 'data.content_subtitle_text_color' );
		...
		$output .= $lodash->typography ( '.jpb-carousel-extended-description', 'data.description_typography', $descriptionTypographyFallbacks );
		$output .= $lodash->spacing ( 'margin', '.jpb-carousel-extended-description', 'data.description_margin' );
		$output .= $lodash->color ( 'color', '.jpb-carousel-extended-description', 'data.description_text_color' );

		$output .= $lodash->generateTransformCss ( '.jpb-carousel-extended', 'data.transform' );
        ...
        #>
            <div class="jpb-addon jpb-carousel-extended {{data.class}} jpb-image-carousel-{{data.image_carousel_layout}}" 
            ...
            {{{data.image_carousel_layout == "layout1" && data.carousel_fade ?  `data-fade="${data.carousel_fade}"` : ""}}}>
                <# if(_.isArray(data.jp_image_carousel_item)){
                    _.each(data.jp_image_carousel_item, function(carousel_item){
                ...
                #>
                        <div class="jpb-carousel-extended-item">
                            ...
                            if(isUrlObj || isUrlString){
                                ...
                                const _url= (type === "url" && url) || (type === "menu" && urlObj.menu) || ((type === "page" && !!urlObj.page) && "index.php?option=com_jpagebuilder&view=page&id=" + urlObj.page) || "";
                            #>
                                <# if(_url){ #>
                            ...
                            #>
                                <div class="jpb-carousel-extended-item-overlay"></div>
                            <# }
                            if(data.image_carousel_layout !== ""){
                            #>
                                <div class="jpb-carousel-extended-content-wrap">
                                
                                    <# if(carousel_item.item_title){ #>
                                        <div class="jpb-carousel-extended-heading">
                                            {{carousel_item.item_title}}
                                        </div>
                                    <# } 
                                    if(carousel_item.item_subtitle) { 
                                    #>
                                        <div class="jpb-carousel-extended-subheading">
                                            {{carousel_item.item_subtitle}}
                                        </div>
                                    <# } 
                                    if(carousel_item.item_description) {
                                    #>
                                        <div class="jpb-carousel-extended-description">
