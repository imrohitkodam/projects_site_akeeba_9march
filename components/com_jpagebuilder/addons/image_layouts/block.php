...
class JpagebuilderAddonImage_layouts extends JpagebuilderAddons {
	/**
	...
	public function render() {
		...
		list ( $link, $target ) = JpagebuilderAddonHelper::parseLink ( $settings, 'click_url', [ 
				'url' => 'click_url',
				'new_tab' => 'url_in_new_window'
		] );
		...
		$content_text_align = (isset ( $settings->content_text_align ) && $settings->content_text_align) ? ' jpb-text-alignment' : '';
		...
		if ($image_desktop_order && $image_preset_thumb !== 'poster') {
			$order_class .= ' jpb-order-' . $image_desktop_order;
		}

		if ($image_tab_order_land && $image_preset_thumb !== 'poster') {
			$order_class .= ' jpb-order-lg-' . $image_tab_order_land;
		}

		if ($image_tab_order && $image_preset_thumb !== 'poster') {
			$order_class .= ' jpb-order-md-' . $image_tab_order;
		}

		if ($image_mob_order_land && $image_preset_thumb !== 'poster') {
			$order_class .= ' jpb-order-sm-' . $image_mob_order_land;
		}

		if ($image_mob_order && $image_preset_thumb !== 'poster') {
			$order_class .= ' jpb-order-xs-' . $image_mob_order;
		}
		...
		if ($content_desktop_order && $image_preset_thumb !== 'poster') {
			$cont_order_class .= ' jpb-order-' . $content_desktop_order;
		}

		if ($content_tab_order_land && $image_preset_thumb !== 'poster') {
			$cont_order_class .= ' jpb-order-lg-' . $content_tab_order_land;
		}

		if ($content_tab_order && $image_preset_thumb !== 'poster') {
			$cont_order_class .= ' jpb-order-md-' . $content_tab_order;
		}

		if ($content_mob_order_land && $image_preset_thumb !== 'poster') {
			$cont_order_class .= ' jpb-order-sm-' . $content_mob_order_land;
		}

		if ($content_mob_order && $image_preset_thumb !== 'poster') {
			$cont_order_class .= ' jpb-order-xs-' . $content_mob_order;
		}
		...
		$btn_class = '';
		$btn_class .= (isset ( $settings->button_type ) && $settings->button_type) ? ' jpb-btn-' . $settings->button_type : '';
		$btn_class .= (isset ( $settings->btn_type ) && $settings->btn_type) ? ' jpb-btn-' . $settings->btn_type : '';
		$btn_class .= (isset ( $settings->button_size ) && $settings->button_size) ? ' jpb-btn-' . $settings->button_size : '';
		$btn_class .= (isset ( $settings->btn_size ) && $settings->btn_size) ? ' jpb-btn-' . $settings->btn_size : '';
		$btn_class .= (isset ( $settings->button_shape ) && $settings->button_shape) ? ' jpb-btn-' . $settings->button_shape : ' jpb-btn-rounded';
		$btn_class .= (isset ( $settings->btn_shape ) && $settings->btn_shape) ? ' jpb-btn-' . $settings->btn_shape : ' jpb-btn-rounded';
		$btn_class .= (isset ( $settings->button_appearance ) && $settings->button_appearance) ? ' jpb-btn-' . $settings->button_appearance : '';
		...
		list ( $buttonLink, $buttonTarget ) = JpagebuilderAddonHelper::parseLink ( $settings, 'button_url', [ 
				'url' => 'btn_url',
				'new_tab' => 'btn_target'
		] );
		...
		// Output start
		$output = '';
		$output .= '<div class="jpb-addon-image-layouts' . $class . '">';
		$output .= '<div class="jpb-addon-content">';

		if ($image_preset_thumb === 'inline') {
			$output .= '<div class="jpb-image-layouts-inline">';
			$output .= '<div class="jpb-image-layouts-inline-img">';

			if ($link) {
				$output .= '<a href="' . $link . '" ' . $target . '>';
			}

			$output .= '<img class="jpb-img-responsive' . $image_strech . '' . ($placeholder ? ' jpb-element-lazy' : '') . '" src="' . ($placeholder ? $placeholder : $image_link) . '" alt="' . $alt_text . '" ' . ($placeholder ? 'data-large="' . $image_link . '"' : '') . ' ' . ($image_width ? 'width="' . $image_width . '"' : '') . ' ' . ($image_height ? 'height="' . $image_height . '"' : '') . ' loading="lazy">';
			...
			if ($open_in_lightbox) {
				if ($image) {
					$output .= '<a class="jpb-magnific-popup jpb-addon-image-overlay-icon" data-popup_type="image" data-mainclass="mfp-no-margins mfp-with-zoom" href="' . $image_link . '">+</a>';
				}
				if ($image_overlay_color) {
					$output .= '<div class="jpb-addon-image-overlay">';
					$output .= '</div>';
				}
			}
			$output .= '</div>'; // .jpb-image-layouts-inline-img

			if ($caption && $caption_postion !== 'no-caption') {
				$output .= '<div class="jpb-addon-image-layout-caption ' . $caption_postion . '">';
				$output .= $caption;
				$output .= '</div>';
			}
			$output .= '</div>';
		} else {
			$output .= '<div class="jpb-addon-image-layout-wrap' . $image_preset_class . '">';

			if ($image_preset_thumb === 'card' || $image_preset_thumb === 'overlap' || $image_preset_thumb === 'collage') {
				$output .= '<div class="jpb-row">';
				$output .= '<div class="jpb-col-sm-' . ($image_container_column ? $image_container_column : 6) . '' . $order_class . '">';
			}

			$output .= '<div class="jpb-addon-image-layout-image' . $image_strech . '' . (($image_preset_thumb !== 'card' && $image_preset_thumb !== 'overlap' && $image_preset_thumb !== 'collage') ? $order_class : '') . '">';
			...
			if ($popup_video_on_image && $image_preset_thumb == 'card' && $popup_video_src) {
				$output .= '<a class="jpb-magnific-popup jpb-addon-image-overlay-icon" data-popup_type="iframe" data-mainclass="mfp-no-margins mfp-with-zoom" href="' . $popup_video_src . '">';
				$output .= '</a>';
				$output .= '<div class="jpb-addon-image-layouts-card-text-caption">';
			...
			if ($title) {
				...
				$output .= '<' . $heading_selector . ' class="jpb-image-layout-title">';
			...
			if ($text_content) {
				$output .= '<div class="jpb-addon-image-layout-text">';
				$output .= $text_content;
				$output .= '</div>';
			}

			if ($btn_text) {
				$output .= '<a href="' . $buttonLink . '" ' . $buttonTarget . ' ' . $attribs . ' class="jpb-btn ' . $btn_class . '">' . $btn_text . '</a>';
			}

			$output .= '</div>'; // .jpb-addon-image-layout-content
	...
	public function css() {
		...
		$imageBorderRadiusStyle = $cssHelper->generateStyle ( '.jpb-addon-image-layout-image .jpb-img-responsive, .jpb-image-layouts-inline-img .jpb-img-responsive, .jpb-addon-image-overlay', $settings, [ 
				'image_border_radius' => 'border-radius'
		] );
		$imageOverlayStyle = $cssHelper->generateStyle ( '.jpb-addon-image-overlay', $settings, [ 
				'image_overlay_color' => 'background-color'
		], false );
		$lightBoxStyle = $cssHelper->generateStyle ( '.jpb-addon-image-overlay-icon', $settings, [ 
				'lightbox_icon_bg' => 'background'
		], false );
		$captionStyle = $cssHelper->generateStyle ( '.jpb-addon-image-layout-caption', $settings, [ 
				'caption_text_color' => 'color',
				'caption_background' => 'background-color',
				'caption_padding' => 'padding'
		], false, [ 
				'caption_padding' => 'spacing'
		] );
		$captionFontStyle = $cssHelper->typography ( '.jpb-addon-image-layout-caption', $settings, 'caption_typography', [ 
		...
		if ($image_preset_thumb !== 'overlap') {
			$titleProps = array_merge ( $titleProps, [ 
					'title_margin' => 'margin',
			...
			$titleProps = array_merge ( $titleProps, [ 
					'title_background' => [ 
							'background',
							'box-shadow: 12px 0 0 %1$s, -12px 0 0 %1$s'
					]
			] );
		}

		$titleStyle = $cssHelper->generateStyle ( '.jpb-image-layout-title', $settings, $titleProps, false, [ 
				'title_margin' => 'spacing',
				'title_padding' => 'spacing'
		] );
		$titleFontStyle = $cssHelper->typography ( '.jpb-image-layout-title', $settings, 'title_typography', [ 
		...
		$textStyle = $cssHelper->generateStyle ( '.jpb-addon-image-layout-text', $settings, [ 
				'text_content_color' => 'color',
				'text_content_margin' => 'margin',
				'text_content_padding' => 'padding'
		], false, [ 
				'text_content_padding' => 'spacing',
				'text_content_margin' => 'spacing'
		] );
		$textFontStyle = $cssHelper->typography ( '.jpb-addon-image-layout-text', $settings, 'text_content_typography', [ 
		...
		$settings->wrapper_background_gradient = JpagebuilderCSSHelper::parseColor ( $settings, 'wrapper_gradient' );
		$settings->wrapper_box_shadow = JpagebuilderCSSHelper::parseBoxShadow ( $settings, 'wrapper_box_shadow' );

		$wrapperProps = [ 
				'wrapper_border_radius' => 'border-radius'
		];
		$wrapperUnits = [ 
		...
		$modifiers = [ 
				'wrapper_margin' => 'spacing',
				'wrapper_padding' => 'spacing'
		];

		// Back Drop Filter
		if (isset ( $settings->wrapper_backdrop_filter ) && $settings->wrapper_backdrop_filter) {
			$settings->wrapper_backdrop_filter = $settings->wrapper_backdrop_filter_webkit = JpagebuilderCSSHelper::parseBackDropFilter ( $settings, 'wrapper_backdrop_filter', 'wrapper_backdrop_filter_value' );

			$wrapperProps = array_merge ( $wrapperProps, [ 
					'wrapper_backdrop_filter' => 'backdrop-filter',
					'wrapper_backdrop_filter_webkit' => '-webkit-backdrop-filter'
			] );
			$wrapperUnits = array_merge ( $wrapperUnits, [ 
					'wrapper_backdrop_filter' => false,
					'wrapper_backdrop_filter_webkit' => false
			] );
		}

		if ($wrapper_color_type === 'gradient') {
			$wrapperProps = array_merge ( $wrapperProps, [ 
					'wrapper_background_gradient' => 'background'
			] );
		} else {
			$wrapperProps = array_merge ( $wrapperProps, [ 
		...
		if ($image_preset_thumb === 'poster') {
			$wrapperProps = array_merge ( $wrapperProps, [ 
					'wrapper_margin' => 'margin'
			] );
		}

		$wrapperProps = array_merge ( $wrapperProps, [ 
				'wrapper_padding' => 'padding',
				'wrapper_border' => 'border',
				'wrapper_box_shadow' => 'box-shadow'
		] );
		$wrapperStyle = $cssHelper->generateStyle ( '.jpb-addon-image-layout-content', $settings, $wrapperProps, $wrapperUnits, $modifiers );

		$textAlignStyle = $cssHelper->generateStyle ( '.jpb-text-alignment', $settings, [ 
				'content_text_align' => 'text-align'
		], false );
		$transformCss = $cssHelper->generateTransformStyle ( '.jpb-addon-image-layouts', $settings, 'transform' );
		...
		$buttonMarginStyle = $cssHelper->generateStyle ( '.jpb-addon-image-layout-content .jpb-btn', $settings, [ 
				'button_margin_top' => 'margin-top'
		] );
	...
	public static function getFrontendEditor() {
		...
		// button margin
		$output .= $lodash->unit ( 'margin-top', '#btn-{{ data.id }}.jpb-btn-{{ data.type }}', 'data.button_margin_top', 'px' );

		// Image Border Radius
		$output .= $lodash->unit ( 'border-radius', '.jpb-addon-image-layout-image .jpb-img-responsive', 'data.image_border_radius', 'px', false );
		$output .= $lodash->unit ( 'border-radius', '.jpb-image-layouts-inline-img .jpb-img-responsive', 'data.image_border_radius', 'px', false );
		$output .= $lodash->unit ( 'border-radius', '.jpb-addon-image-overlay', 'data.image_border_radius', 'px', false );

		// Overlay
		$output .= $lodash->color ( 'background-color', '.jpb-addon-image-overlay', 'data.image_overlay_color' );
		$output .= $lodash->color ( 'background-color', '.jpb-addon-image-overlay-icon', 'data.lightbox_icon_bg' );

		// Backdrop Filter
		$output .= '<# if(data.wrapper_backdrop_filter) { ';
		$output .= 'let unit = (data.wrapper_backdrop_filter == "blur") ? "px" : "%";  #>';
		$output .= $lodash->backdrop_filter ( '{{data.wrapper_backdrop_filter}}', '.jpb-addon-image-layout-content', 'data.wrapper_backdrop_filter_value', '{{unit}}' );
		$output .= '<# } #>';

		// Caption
		$output .= $lodash->color ( 'color', '.jpb-addon-image-layout-caption', 'data.caption_text_color' );
		$output .= $lodash->color ( 'background-color', '.jpb-addon-image-layout-caption', 'data.caption_background' );
		...
		$output .= $lodash->typography ( '.jpb-addon-image-layout-caption', 'data.caption_typography', $captionTypographyFallbacks );
		...
		$output .= $lodash->typography ( '#btn-{{ data.id }}.jpb-btn-{{ data.button_type }}', 'data.button_typography', $btnTypographyFallbacks );
		...
		$output .= $lodash->typography ( '.jpb-image-layout-title', 'data.title_typography', $titleTypographyFallbacks );
		$output .= $lodash->color ( 'color', '.jpb-image-layout-title', 'data.title_text_color' );
		...
		$output .= $lodash->typography ( '.jpb-addon-image-layout-text', 'data.text_content_typography', $textContentTypographyFallbacks );
		$output .= $lodash->color ( 'color', '.jpb-addon-image-layout-text', 'data.text_content_color' );
		$output .= $lodash->spacing ( 'padding', '.jpb-addon-image-layout-text', 'data.text_content_padding' );
		$output .= $lodash->spacing ( 'margin', '.jpb-addon-image-layout-text', 'data.text_content_margin' );

		$output .= $lodash->spacing ( 'margin', '.jpb-addon-image-layout-content .jpb-btn', 'data.button_margin' );

		// alignment
		$output .= $lodash->alignment ( 'text-align', '.jpb-text-alignment', 'data.content_text_align' );

		// Not Overlap
		$output .= '<# if (data.image_preset_thumb !== "overlap") { #>';
		$output .= $lodash->spacing ( 'margin', '.jpb-image-layout-title', 'data.title_margin' );
		$output .= $lodash->spacing ( 'padding', '.jpb-image-layout-title', 'data.title_padding' );
		$output .= '<# } #>';

		// Overlap
		$output .= '<# if (data.image_preset_thumb == "overlap" && data?.title_background) { #>';
		$output .= $lodash->color ( 'background-color', '.jpb-image-layout-title', 'data.title_background' );
		$output .= '#jpb-addon-{{ data.id }} .jpb-image-layout-title { box-shadow: 12px 0 0 {{data.title_background}}, -12px 0 0 {{data.title_background}}; }';
		$output .= '<# } #>';

		// poster
		$output .= '<# if (data.image_preset_thumb == "poster") { #>';
		$output .= $lodash->spacing ( 'margin', '.jpb-addon-image-layout-content', 'data.wrapper_margin' );
		$output .= '<# } #>';

		$output .= '<# if (data.wrapper_box_shadow !== "0 0 0 0 #fff" && data.wrapper_box_shadow?.enabled !== undefined) { #>';
		$output .= $lodash->boxShadow ( '.jpb-addon-image-layout-content', 'data.wrapper_box_shadow' );
		$output .= '<# } #>';
		$output .= '<# if (data.wrapper_color_type == "gradient") { #>';
		$output .= $lodash->color ( 'background-color', '.jpb-addon-image-layout-content', 'data.wrapper_gradient' );
		$output .= '<# } else { #>';
		$output .= '<# if(!_.isEmpty(data.wrapper_background)) { #>';
		$output .= '#jpb-addon-{{ data.id }} .jpb-addon-image-layout-content {background: none;}';
		$output .= '<# } #>';
		$output .= $lodash->color ( 'background-color', '.jpb-addon-image-layout-content', 'data.wrapper_background' );
		$output .= '<# } #>';

		$output .= $lodash->border ( 'border-width', '.jpb-addon-image-layout-content', 'data.wrapper_border' );
		$output .= $lodash->border ( 'border-color', '.jpb-addon-image-layout-content', 'data.wrapper_border_color' );
		$output .= $lodash->spacing ( 'padding', '.jpb-addon-image-layout-content', 'data.wrapper_padding' );
		$output .= $lodash->unit ( 'border-radius', '.jpb-addon-image-layout-content', 'data.wrapper_border_radius', 'px', false );
		...
		$output .= '<# } #>';
		$output .= '<# } #>';
		$output .= $lodash->generateTransformCss ( '.jpb-addon-image-layouts', 'data.transform' );

		$output .= '
			#jpb-addon-{{ data.id }} .jpb-addon-image-layout-content {
		...
		let content_text_align = (!_.isEmpty(data.content_text_align) && data.content_text_align) ?" jpb-text-alignment" : "";
		...
		if(isUrlObject_click || isUrlString_click){
			...
			clickUrl = (type === "url" && url) || ( type === "menu" && urlObj.menu) || ( (type === "page" && !!urlObj.page ) && "index.php/component/jpagebuilder/index.php?option=com_jpagebuilder&view=page&id=" + urlObj.page) || "";
		}
		...
		<div class="jpb-addon-image-layouts {{data.class}}">
		...
		#>
			...
			<div class="jpb-image-layouts-inline">
			<div class="jpb-image-layouts-inline-img">
			...
			<# if(img.src.indexOf("http://") == -1 && img.src.indexOf("https://") == -1){ #>
				<img class="jpb-img-responsive{{image_strech}}" src=\'{{ pagebuilder_base + img.src }}\' alt="{{data.image_alt_text}}">
			<# } else { #>
				<img class="jpb-img-responsive{{image_strech}}" src=\'{{ img.src }}\' alt="{{data.image_alt_text}}">
			<# } #>
			...
			if(data.open_in_lightbox){
				if(img.src){
			#>
					<a class="jpb-magnific-popup jpb-addon-image-overlay-icon" data-popup_type="image" data-mainclass="mfp-no-margins mfp-with-zoom" href="{{img.src}}">+</a>
				<# } 
				if(data.image_overlay_color) {
				#>
					<div class="jpb-addon-image-overlay">
					</div>
				<# } 
			} #>
			</div>
			...
			<# if(data.caption && data.caption_postion !== "no-caption"){ #>
				<div class="jpb-addon-image-layout-caption jp-inline-editable-element {{data.caption_postion}}" data-id={{data.id}} data-fieldName="caption" contenteditable="true">
				{{{data.caption}}}
				</div>
			<# } #>
			</div>
		<# } else { #>
			<div class="jpb-addon-image-layout-wrap{{image_preset_class}}">
			<# if(image_preset_thumb === "card" || image_preset_thumb === "overlap" || image_preset_thumb === "collage"){ #>
				<div class="jpb-row">
				<div class="jpb-col-sm-{{(data.image_container_column ? data.image_container_column : 6)}}{{order_class}}">
			<# } #>
			
			<div class="jpb-addon-image-layout-image{{image_strech}}
			<# if(image_preset_thumb !== "card" && image_preset_thumb !== "overlap" && image_preset_thumb !== "collage"){ #>
				{{order_class}}
			<# } #>
			">
			...
			#>
				<a class="jpb-magnific-popup jpb-addon-image-overlay-icon" data-popup_type="iframe" data-mainclass="mfp-no-margins mfp-with-zoom" href="{{data.popup_video_src}}"></a>
				<div class="jpb-addon-image-layouts-card-text-caption">
			...
			#>
				...
				<div class="jpb-col-sm-{{(data.image_container_column ? (data.image_container_column == 12 ? 12 : 12-data.image_container_column) : 6)}}{{cont_order_class}}{{collage_content_vertical}}">
			<# } #>
			<# 
			...
			#>
			<div class="jpb-addon-image-layout-content{{content_text_align}}{{collage_content_right}}{{collage_content_sm_right}} <# if(image_preset_thumb !== "card" && image_preset_thumb !== "overlap" && image_preset_thumb !== "collage") { #>{{cont_order_class}}<# } #>
			">
			<# if(data.title){
			...
			#>
				...
				<# } #>
					<{{heading_selector}} class="jpb-image-layout-title jp-inline-editable-element" data-id={{data.id}} data-fieldName="title" contenteditable="true">
			...
			#>
				<div class="jpb-addon-image-layout-text jp-editable-content" data-id={{data.id}} data-fieldName="text_content">
					{{{data.text_content}}}
				</div>
			<# }
			...
			if(_.trim(buttonText)){
				...
				if(isUrlString_url || isUrlObject_url ){
					...
					buttonUrl = (type === "url" && url) || ( type === "menu" && urlObj.menu) || ( (type === "page" && !!urlObj.page ) && "index.php/component/jpagebuilder/index.php?option=com_jpagebuilder&view=page&id=" + urlObj.page) || "";
				}

			#>
				<a href=\'{{ buttonUrl }}\' id="btn-{{ data.id }}" target="{{buttonTarget}}" rel=\'{{buttonRel}}\' class="jpb-btn {{ classList }}"><# if( (data.btn_icon_position == "left" || data.button_icon_position == "left" ) && !_.isEmpty(data.btn_icon || data.button_icon)) { #><i class="{{ finial_icon_name }}"></i> <# } #>{{ buttonText }}<# if( (data.btn_icon_position == "right" || data.button_icon_position == "right") && !_.isEmpty(data.btn_icon || data.button_icon)) { #> <i class="{{ finial_icon_name }}"></i><# } #></a>
			<# } #>
			</div>
