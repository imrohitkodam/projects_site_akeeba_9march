...
class JpagebuilderAddonImage_overlay extends JpagebuilderAddons {
	/**
	...
	public function render() {
		...
		list ( $titleLink, $titleTarget ) = JpagebuilderAddonHelper::parseLink ( $settings, 'title_link', [ 
				'url' => 'title_link',
				'new_tab' => 'title_link_new_window'
		] );
		...
		// Button options
		$btn_class = '';
		$btn_class .= (isset ( $settings->type ) && $settings->type) ? ' jpb-btn-' . $settings->type : '';
		$btn_class .= (isset ( $settings->size ) && $settings->size) ? ' jpb-btn-' . $settings->size : '';
		$btn_class .= (isset ( $settings->block ) && $settings->block) ? ' ' . $settings->block : '';
		$btn_class .= (isset ( $settings->shape ) && $settings->shape) ? ' jpb-btn-' . $settings->shape : ' jpb-btn-rounded';
		$btn_class .= (isset ( $settings->appearance ) && $settings->appearance) ? ' jpb-btn-' . $settings->appearance : '';
		...
		list ( $btn_url, $btn_target ) = JpagebuilderAddonHelper::parseLink ( $settings, 'url', [ 
				'url' => 'url',
				'new_tab' => 'target'
		] );
		...
		$output .= '<div class="jpb-addon jpb-addon-overlay-image ' . $class . ' image-effect-' . $background_image_animation . ' ' . ($show_content_on_hover ? 'overlay-show-content-on-hover' : '') . '">';
		$output .= '<div class="jpb-addon-overlay-image-content title-subtitle-' . $title_subtitle_position . '">';
		...
		if (($title || $sub_title) && $title_subtitle_position) {
			...
			if ($sub_title) {
				$output .= '<' . $subtitle_heading_selector . ' class="jpb-addon-subtitle">';
				if ($sub_title_icon) {
			...
			if ($text) {
				...
				$output .= '<a' . $attribs . ' class="jpb-btn ' . $btn_class . '">' . $text . '</a>';
				$output .= '</div>';
			}
			$output .= '</div>';
		}
		...
		$output .= '<div class="overlay-background-image" style="background-image:url(' . preg_replace('/\s/', '%20', $image_link) . ');"></div>';
	...
	public function css() {
		...
		if ($global_border_radius) {
			...
			$selfStyle = $cssHelper->generateStyle ( ':self', $settings, [ 
					'dummy_overflow' => 'overflow'
			], false );
			$css .= $selfStyle;
		}

		$titleStyle = $cssHelper->generateStyle ( '.jpb-addon-title a', $settings, [ 
				'title_text_color' => 'color'
		], false );
		$titleStyle .= $cssHelper->generateStyle ( '.jpb-addon-title', $settings, [ 
				'title_margin' => 'margin'
		], false, [ 
				'title_margin' => 'spacing'
		] );
		$subtitleFontStyle = $cssHelper->typography ( '.jpb-addon-subtitle', $settings, 'sub_title_typography', [ 
		...
		$subtitleStyle = $cssHelper->generateStyle ( '.jpb-addon-subtitle', $settings, [ 
				'sub_title_margin' => 'margin',
				'sub_title_text_color' => 'color'
		], false, [ 
				'sub_title_margin' => 'spacing'
		] );
		$imageStyle = $cssHelper->generateStyle ( '.jpb-addon-overlay-image-content', $settings, [ 
				'image_height' => 'height'
		] );
		$iconStyle = $cssHelper->generateStyle ( '.jpb-addon-image-overlay-icon', $settings, [ 
		...
		if ($overlay_type !== 'none') {
			...
			$props = [ 
					'overlay_transparent' => 'background'
			];
		}
		...
		if ($overlay_hover_type !== 'none') {
			if ($overlay_hover_type === 'color') {
				$overlayHoverStyle .= $cssHelper->generateStyle ( '.jpb-addon-overlay-image-content:hover .overlay-background-style', $settings, [ 
						'overlay_hover_color' => 'background'
				], false );
			} elseif ($overlay_hover_type === 'gradient') {
				$settings->dummy_overlay_opacity = '0.8';
				$overlayHoverStyle .= $cssHelper->generateStyle ( '.jpb-addon-overlay-image-content:hover .overlay-background-style', $settings, [ 
						'dummy_overlay_opacity' => 'opacity'
				] );

				$settings->overlay_hover_gradient = JpagebuilderCSSHelper::parseColor ( $settings, 'overlay_hover_gradient' );
				$overlayHoverStyle .= $cssHelper->generateStyle ( '.overlay-background-style::after', $settings, [ 
						'overlay_hover_gradient' => 'background'
				], false );

				$settings->dummy_hover_after_opacity = '1';
				$overlayHoverStyle .= $cssHelper->generateStyle ( '.jpb-addon-overlay-image-content:hover .overlay-background-style::after', $settings, [ 
						'dummy_hover_after_opacity' => 'opacity'
				], false );
			}
		} else {
			$settings->overlay_hover_transparent = 'transparent';
			$overlayHoverStyle = $cssHelper->generateStyle ( '.jpb-addon-overlay-image-content:hover .overlay-background-style', $settings, [ 
					'overlay_hover_transparent' => 'background'
			], false );
		}

		$buttonStyle = $cssHelper->generateStyle ( '.overlay-image-button-wrap', $settings, [ 
				'button_margin' => 'margin'
		], false, [ 
				'button_margin' => 'spacing'
		] );
		$contentStyle = $cssHelper->generateStyle ( '.jpb-addon-overlay-image-content', $settings, [ 
				'content_padding' => 'padding'
		], false, [ 
				'content_padding' => 'spacing'
		] );
		$transformCss = $cssHelper->generateTransformStyle ( '.jpb-addon-overlay-image', $settings, 'transform' );
	...
	public static function getFrontendEditor() {
		...
		<#
			...
			var classList = "";
			classList += " jpb-btn-"+data.type;
			classList += " jpb-btn-"+data.size;
			classList += " jpb-btn-"+data.shape;
			if(!_.isEmpty(data.appearance)){
				classList += " jpb-btn-"+data.appearance;
			}
	...
	<style type="text/css">';
		...
		$output .= $lodash->color ( 'color', '.jpb-addon-title a', 'data.title_color' );
		...
		// Subtitle
		$output .= $lodash->typography ( '.jpb-addon-subtitle', 'data.sub_title_typography', $subtitleTypographyFallbacks );
		$output .= $lodash->color ( 'color', '.jpb-addon-subtitle', 'data.sub_title_text_color' );
		$output .= $lodash->spacing ( 'margin', '.jpb-addon-subtitle', 'data.sub_title_margin' );
		$output .= $lodash->unit ( 'font-size', '.jpb-addon-subtitle', 'data.sub_title_fontsize', 'px' );

		$output .= $lodash->unit ( 'background', '.jpb-addon-image-overlay-icon', 'data.lightbox_icon_bg', '', false );
		$output .= $lodash->spacing ( 'padding', '.jpb-addon-overlay-image-content', 'data.content_padding' );
		$output .= $lodash->spacing ( 'margin', '.overlay-image-button-wrap', 'data.button_margin' );
		$output .= $lodash->unit ( 'height', '.jpb-addon-overlay-image-content', 'data.image_height', 'px' );
		...
		$output .= $lodash->color ( 'color', '#btn-{{ data.id }}.jpb-btn-custom', 'data.color' );
		$output .= $lodash->color ( 'color', '#btn-{{ data.id }}.jpb-btn-custom:hover', 'data.color_hover' );
		$output .= $lodash->color ( 'background-color', '#btn-{{ data.id }}.jpb-btn-custom:hover', 'data.background_color_hover' );
		$output .= $lodash->spacing ( 'padding', '#btn-{{ data.id }}.jpb-btn-custom', 'data.button_padding' );

		$output .= '<# if (data.appearance == "outline") { #>';
		$output .= $lodash->border ( 'border-color', '#btn-{{ data.id }}.jpb-btn-custom', 'data.background_color' );
		$output .= $lodash->border ( 'border-color', '#btn-{{ data.id }}.jpb-btn-custom:hover', 'data.background_color_hover' );
		$output .= '#jpb-addon-{{ data.id }} #btn-{{ data.id }}.jpb-btn-custom { background-color: transparent; }';
		$output .= '<# } else if (data.appearance == "3d") { #>';
		$output .= $lodash->border ( 'border-bottom-color', '#btn-{{ data.id }}.jpb-btn-custom', 'data.background_color_hover' );
		$output .= $lodash->color ( 'background-color', '#btn-{{ data.id }}.jpb-btn-custom', 'data.background_color' );
		$output .= '<# } else if (data.appearance == "gradient") { #>';
		$output .= '#jpb-addon-{{ data.id }} #btn-{{ data.id }}.jpb-btn-custom { border: none; }';
		$output .= $lodash->color ( 'background-color', '#btn-{{ data.id }}.jpb-btn-custom', 'data.background_gradient' );
		$output .= $lodash->color ( 'background-color', '#btn-{{ data.id }}.jpb-btn-custom:hover', 'data.background_gradient_hover' );
		$output .= '<# } else { #>';
		$output .= $lodash->color ( 'background-color', '#btn-{{ data.id }}.jpb-btn-custom', 'data.background_color' );
		$output .= '<# } #>';
		...
		$output .= $lodash->color ( 'background-color', '.jpb-addon-overlay-image-content .overlay-background-style', 'data.overlay_color' );
		$output .= '<# } else if (data.overlay_type=="gradient") { #>';
		$output .= $lodash->color ( 'background-color', '.jpb-addon-overlay-image-content .overlay-background-style', 'data.overlay_gradient' );
		$output .= '<# } #>';
		$output .= '<# } else { #>';
		$output .= '#jpb-addon-{{ data.id }} .overlay-background-style { background: transparent; }';
		$output .= '<# } #>';

		$output .= '<# if (data.overlay_hover_type !== "none") { #>';
		$output .= '<# if (data.overlay_hover_type=="color") { #>';
		$output .= $lodash->color ( 'background-color', '.jpb-addon-overlay-image-content:hover .overlay-background-style', 'data.overlay_hover_color' );
		$output .= '#jpb-addon-{{ data.id }} .jpb-addon-overlay-image-content:hover .overlay-background-style { background-image: none; }';
		$output .= '<# } else if (data.overlay_hover_type == "gradient") { #>';
		$output .= $lodash->color ( 'background-color', '.jpb-addon-overlay-image-content:hover .overlay-background-style', 'data.overlay_hover_gradient' );
		$output .= '#jpb-addon-{{ data.id }} .jpb-addon-overlay-image-content:hover .overlay-background-style { opacity:.8; }';
		$output .= '#jpb-addon-{{ data.id }} .jpb-addon-overlay-image-content:hover .overlay-background-style::after { opacity:1; }';
		$output .= '.jpb-addon-overlay-image-content:hover .overlay-background-style { background-image: none; }';
		$output .= '<# } #>';
		$output .= '<# } else { #>';
		$output .= '#jpb-addon-{{ data.id }} .jpb-addon-overlay-image-content:hover  .overlay-background-style { background: transparent; }';
		$output .= '<# } #>';
		$output .= $lodash->generateTransformCss ( '.jpb-addon-overlay-image', 'data.transform' );

		$output .= '
		<# if(image_link){ #>
			#jpb-addon-{{ data.id }} .overlay-background-image {
		...
		<div class="jpb-addon jpb-addon-overlay-image {{data.class}} image-effect-{{data.background_image_animation}} {{data.show_content_on_hover ? "overlay-show-content-on-hover" : ""}}">
		<div class="jpb-addon-overlay-image-content title-subtitle-{{data.title_subtitle_position}}">
		<#
		...
		#>
			<div class="overlay-image-title">
				<{{data.heading_selector}} class="jpb-addon-title">
					<# 
					...
					if(isUrlObject_title || isUrlString_title ){
						...
						const title_url = (type === "url" && url) || (type === "menu" && urlObj.menu) || (type === "page"  && "index.php/component/jpagebuilder/index.php?option=com_jpagebuilder&view=page&id=" + urlObj.page);
						#>
				...
				<{{data.subtitle_heading_selector}} class="jpb-addon-subtitle">
					<#
					...
					<# } #>
					<span class="jp-inline-editable-element" data-id={{data.id}} data-fieldName="sub_title" contenteditable="true">{{{data.sub_title}}}</span>
				...
				<div class="overlay-image-button-wrap">
					<# 
					 ...
					 if(isUrlObject_button || isUrlString_button){
						...
						const {url, new_tab, nofollow, noopener, noreferrer, type} = urlObj;
						button_url = ( type === "url" && url) || (type === "menu" && urlObj.menu) || (type === "page" && "index.php/component/jpagebuilder/index.php?option=com_jpagebuilder&view=page&id="+ urlObj.page); 
					...
					#>
					<a href=\'{{ button_url }}\' id="btn-{{ data.id }}" target="{{ button_target }}" rel="{{button_rel}}" class="jpb-btn {{ classList }}"><# if(data.button_icon_position == "left" && !_.isEmpty(data.button_icon)) {
		...
		<# if(data.image_in_lightbox && data.title_subtitle_position !== "center-center" && image_link){ #>
			<a class="jpb-magnific-popup jpb-addon-image-overlay-icon" data-popup_type="image" data-mainclass="mfp-no-margins mfp-with-zoom" href=\'{{image_link}}\'>+</a>
		<# } #>
		</div>
