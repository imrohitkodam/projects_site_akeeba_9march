...
define ( 'JPAGEBUILDER_FACEBOOK_URI', 'https://graph.facebook.com/v7.0' );
...
if (version_compare ( $JoomlaVersion, '4.0.0', '>=' )) {
	JLoader::registerAlias ( 'JHtmlString', 'Joomla\CMS\HTML\Helpers\StringHelper' );
}
class JpagebuilderAddonInstagram_gallery extends JpagebuilderAddons {
	...
	public function render() {
		...
		$output = '';
		$output .= '<div class="jpb-addon jpb-addon-instagram-gallery ' . $class . ' layout-' . $layout_type . '">';

		if ($title) {
			$output .= '<div class="jpb-addon-instagram-text-wrap">';
			$output .= '<' . $heading_selector . ' class="jpb-addon-title">' . $title . '</' . $heading_selector . '>';
			$output .= '</div>'; // .jpb-addon-instagram-text-wrap
		}
		...
		if (empty ( $items )) {
			$output .= '<p class="alert alert-warning">' . Text::_ ( 'COM_JPAGEBUILDER_ADDON_INSTAGRAM_ERORR' ) . '</p>';
		} else {
			$output .= '<ul class="jpb-instagram-images">';
			foreach ( $items as $item ) {
				$image_url = isset ( $item->media_url ) ? $item->media_url : Uri::root () . 'components/com_jpagebuilder/addons/instagram_gallery/assets/images/instagram-logo.png';
				...
				if ($layout_type === 'classic') {
					$output .= '<li class="jpb-instagram-image">';
					$output .= '<div class="jpb-instagram-classic-content-wrap">';
					if (in_array ( 'author', $show_stats ) && isset ( $media->name )) {
						...
						$output .= '<a class="instagram-redirect-link" href="https://www.instagram.com/' . $item->username . '/" target="_blank" rel="noopener noreferrer"><i class="fa fa-instagram" aria-hidden="true" title="' . Text::_ ( 'COM_JPAGEBUILDER_ADDON_INSTAGRAM_REDIRECT' ) . '"></i><span class="jpb-sr-only">' . Text::_ ( 'COM_JPAGEBUILDER_ADDON_INSTAGRAM_REDIRECT' ) . '</span></a>';
						$output .= '</div>'; // .addon-instagram-item-author-wrap
					}

					$output .= (! empty ( $image_url )) ? '<a class="jpb-instagram-gallery-btn" href="' . $image_url . '">' : '';
					$output .= '<div class="addon-instagram-image-wrap">';
					$output .= '<img class="instagram-image jpb-img-responsive" src="' . $image_url . '" alt="" loading="lazy">';
					...
					$output .= '</div>'; // .jpb-instagram-classic-content-wrap
					$output .= '</li>'; // .jpb-instagram-image
				} else {
					$output .= '<li class="jpb-instagram-image">';
					$output .= (! empty ( $image_url )) ? '<a class="jpb-instagram-gallery-btn" href="' . $image_url . '">' : '';
			...
			$output .= '</ul>'; // .jpb-instagram-images
		}

		$output .= '</div>'; // .jpb-addon-instagram-gallery
		return $output;
	}
	...
	public function css() {
		...
		if ($thumb_per_row) {
			$widthObject = JpagebuilderAddonHelper::initDeviceObject ();
			$widthObject->xl = null;
			$widthObject->lg = null;
			$widthObject->md = '50';
			$widthObject->sm = '100';
			...
			$itemStyle = $cssHelper->generateStyle ( '.jpb-instagram-images .jpb-instagram-image', $settings, [ 
					'dummy_item_plain_width' => 'width',
					'dummy_item_height' => 'height'
			], [ 
					'dummy_item_plain_width' => '%',
					'dummy_item_height' => false
			] );

			if ($layout_type === 'classic') {
				$itemStyle .= $cssHelper->generateStyle ( '.jpb-instagram-images .jpb-instagram-image', $settings, [ 
						'dummy_item_plain_width' => 'flex: 0 0 %s'
				], '%' );
			}

			$itemStyle .= $cssHelper->generateStyle ( '.jpb-instagram-images .jpb-instagram-image', $settings, [ 
					'dummy_item_object_width' => [ 
							'width',
							'flex: 0 0 %s'
					]
			], '%' );
			$transformCss = $cssHelper->generateTransformStyle ( '.addon-instagram-item-wrap', $settings, 'transform' );
			$itemStyle .= $transformCss;
	...
	public function js() {
		$addon_id = '#jpb-addon-' . $this->addon->id;
		$js = 'jQuery(function($){
				$("' . $addon_id . ' .jpb-instagram-gallery-btn").magnificPopup({
					type: "image",
					gallery: {
						enabled: true
					  },
	...
	private function getMedias() {
		...
		if ($item_resource === 'userid') {
			...
			$url = JPAGEBUILDER_FACEBOOK_URI . '/' . $user_id . '?fields=media.limit(' . $limit . '){' . implode ( ',', $fields ) . '},profile_picture_url,name&access_token=' . $access_token;
		} elseif ($item_resource === 'hashtag') {
			$hashtagUrl = JPAGEBUILDER_FACEBOOK_URI . '/ig_hashtag_search?user_id=' . $user_id . '&q=' . $hashtag . '&access_token=' . $access_token;
			$igHashtagId = '';
			...
			if ($hashtag_type === 'recent') {
				$url = JPAGEBUILDER_FACEBOOK_URI . '/' . $igHashtagId . '/recent_media?user_id=' . $user_id . '&fields=' . implode ( ',', $fields ) . '&limit=' . $limit . '&media_type=IMAGE&access_token=' . $access_token;
			} else {
				$url = JPAGEBUILDER_FACEBOOK_URI . '/' . $igHashtagId . '/top_media?user_id=' . $user_id . '&fields=' . implode ( ',', $fields ) . '&limit=' . $limit . '&media_type=IMAGE&access_token=' . $access_token;
			}
		}
