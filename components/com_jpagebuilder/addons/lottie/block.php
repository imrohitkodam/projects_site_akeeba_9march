...
class JpagebuilderAddonLottie extends JpagebuilderAddons {
	public function render() {
		...
		if (is_object ( $src_file )) {
			$file_src = $src_file->src;
		}
		...
		$output = '<div class="jpb-addon-lottie">';
		$output .= '<div class="jpb-addon-content">';
		$output .= '<div id="jpb-addon-lottie-player-' . $this->addon->id . '" class="lottie-player" data-autoplay="' . $autoplay . '" data-loop="' . $loop . '" data-mode="' . $mode . '" data-renderer="' . $renderer . '" data-hover_out="' . $hover_out . '" src="' . $file_src . '"  background="transparent" data-direction="' . $direction . '" data-speed="' . $speed . '" data-frame_start="' . $start . '" data-frame_end="' . $end . '" data-viewport_top="' . $viewport_top . '" data-viewport_bottom="' . $viewport_bottom . '"></div>';
		$output .= '</div>';
		$output .= '</div>';
	...
	public function css() {
		$settings = $this->addon->settings;
		$addon_id = "#jpb-addon-lottie-player-" . $this->addon->id;
		$cssHelper = new JpagebuilderCSSHelper ( $addon_id );
		$css = '';
		$style = $cssHelper->generateStyle ( ':self', $settings, [ 
				'width' => 'width',
				'height' => 'height'
		], 'px' );
	...
	public function scripts() {
		return [ 
				'components/com_jpagebuilder/assets/js/lottie.min.js',
				'components/com_jpagebuilder/assets/js/lottie-addon.js'
		];
	}
	public static function getFrontendEditor() {
		$lodash = new JpagebuilderLodashlib ( '#jpb-addon-lottie-{{ data.id }}' );
		$output = '
		<#
			...
			let file_src = src_file;

			if(_.isObject(src_file)) {
				file_src = src_file.src;
			}
		#>';
		$output .= '<style type="text/css">';
		$output .= $lodash->unit ( 'width', "#jpb-addon-lottie-player-{{ data.id }}", 'data?.width', "{{ data?.width?.unit }}" );
		$output .= $lodash->unit ( 'height', "#jpb-addon-lottie-player-{{ data.id }}", 'data?.height', "{{data?.height?.unit}}" );
		$output .= $lodash->generateTransformCss ( '', 'data.transform' );
		$output .= '</style>';
		$output .= '
		<div class="jpb-addon-lottie-wrapper">
			<div class="jpb-addon-content">
				<div id="jpb-addon-lottie-{{ data.id }}" class="jpb-addon-lottie {{data.class}}">
					<# if(file_src) {#>
						<div 
						class="lottie-player"
						id="jpb-addon-lottie-player-{{ data.id }}"  
						src=\'{{file_src}}\' 
