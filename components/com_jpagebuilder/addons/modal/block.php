...
class JpagebuilderAddonModal extends JpagebuilderAddons {
	/**
	...
	public function render() {
		...
		$modal_unique_id = 'jpb-modal-' . $this->addon->id;
		...
		if ($modal_selector === 'image') {
			if ($selector_image_src) {
				...
				$output .= '<a class="jpb-modal-selector jpb-magnific-popup" ' . $attribs . ' href="' . $url . '" id="' . $modal_unique_id . '-selector">
				<img ' . ($placeholder ? 'class="jpb-element-lazy"' : '') . ' src="' . ($placeholder ? $placeholder : $image_link) . '" alt="' . $selector_image_alt_text . '" ' . ($placeholder ? 'data-large="' . $image_link . '"' : '') . ' ' . ($selector_image_width ? 'width="' . $selector_image_width . '"' : '') . ' ' . ($selector_image_height ? 'height="' . $selector_image_height . '"' : '') . ' loading="lazy" />';
				$output .= ($selector_text) ? '<span class="text">' . $selector_text . '</span>' : '';
				if ($show_ripple_effect) {
					$output .= '<span class="jpb-ripple-effect"></span>';
				}
				$output .= '</a>';
			}
			...
			if ($selector_icon_name) {
				...
				$output .= '<a class="jpb-modal-selector jpb-magnific-popup" href="' . $url . '" ' . $attribs . ' id="' . $modal_unique_id . '-selector">';
				$output .= '<span class="jpb-modal-icon-wrap">';
			...
			$output .= '<a class="jpb-btn ' . $button_class . ' jpb-magnific-popup jpb-modal-selector" ' . $attribs . ' href="' . $url . '" id="' . $modal_unique_id . '-selector">' . $button_text . '</a>';
		}
	...
	public function css() {
		...
		if ($modal_selector == 'icon' || $modal_selector == 'image') {
			if ($selector_icon_name || $selector_image) {
				...
				if ($selector_text_style) {
					$css .= $addon_id . ' .jpb-modal-selector span.text {';
		...
		if ($modal_selector == 'icon') {
			if ($selector_icon_name) {
				...
				$settings->selector_icon_color = JpagebuilderCssHelper::parseColor ( $settings, 'selector_icon_color' );
				...
				$selectorUnits = [ 
						'selector_icon_background' => false,
						'selector_icon_border_color' => false,
						'selector_icon_color' => false
				];

				$iconSelectorStyle = $cssHelper->generateStyle ( '.jpb-modal-selector span', $settings, $selectorStyleProps, $selectorUnits, null, null, false, 'display:inline-block;line-height:1;' );
				...
				$iconStyle = $cssHelper->generateStyle ( '.jpb-modal-selector span > i', $settings, $iconStyleProps );

				$css .= $iconStyle;
				$css .= $iconSelectorStyle;
			}
		} else {
			$selectorStyle = $cssHelper->generateStyle ( '.jpb-modal-selector', $settings, [ 
					'selector_margin_top' => 'margin-top',
					'selector_margin_bottom' => 'margin-bottom'
			] );
			$css .= $selectorStyle;
		...
		if ($modal_content_type != 'video' && $modal_size) {
			if ($modal_content_type == 'image') {
				$cssHelper->setID ( '#jpb-modal-' . $this->addon->id . '.popup-image-block img' );

				$popupImage = $cssHelper->generateStyle ( ':self', $settings, [ 
						'modal_popup_width' => 'max-width',
						'modal_popup_height' => 'height'
				] );
				$css .= $popupImage;

				$cssHelper->setID ( $addon_id );
			} else {
				$cssHelper->setID ( '#jpb-modal-' . $this->addon->id . '.white-popup-block' );

				$popupImage = $cssHelper->generateStyle ( ':self', $settings, [ 
						'modal_popup_width' => 'max-width',
						'modal_popup_height' => 'height',
						'modal_popup_overflow' => 'overflow-y'
				], [ 
						'modal_popup_overflow' => false
				] );
				$css .= $popupImage;
		...
		$css .= $css_path->render ( array (
				'addon_id' => $addon_id,
				'options' => $settings,
				'id' => 'jpb-modal-' . $this->addon->id . '-selector'
		) );

		$transformCss = $cssHelper->generateTransformStyle ( '.jpb-modal-selector', $settings, 'transform' );
	...
	public static function getFrontendEditor() {
		...
		let modalUniqueId = "jpb-modal-"+ data.id
		let modalUrl = "#" + modalUniqueId
		let attribs = \'data-popup_type="inline" data-mainclass="mfp-no-margins mfp-with-zoom"\'

		let buttonClass = ( data.button_type )? "jpb-btn-" + data.button_type : "jpb-btn-default"
			buttonClass += ( data.button_size )? " jpb-btn-" + data.button_size : ""
			buttonClass += ( data.button_shape )? " jpb-btn-" + data.button_shape : " jpb-btn-rounded"
			buttonClass += ( data.button_appearance )? " jpb-btn-" + data.button_appearance : ""
		...
		$output .= $lodash->typography ( '#jpb-modal-{{ data.id }}-selector.jpb-btn-{{ data.button_type }}', 'data.button_typography', $buttonTypographyFallbacks );

		$output .= '<# if(data.button_type == "custom") { #>';
		$output .= $lodash->color ( 'color', '#jpb-modal-{{ data.id }}-selector.jpb-btn-custom', 'data.button_color' );
		$output .= $lodash->color ( 'background-color', '#jpb-modal-{{ data.id }}-selector.jpb-btn-custom:hover', 'data.button_background_color_hover' );
		$output .= $lodash->color ( 'color', '#jpb-modal-{{ data.id }}-selector.jpb-btn-custom:hover', 'data.button_color_hover' );
		$output .= $lodash->spacing ( 'padding', '#jpb-modal-{{ data.id }}-selector.jpb-btn-custom', 'data.button_padding' );

		$output .= '<# if (data.button_appearance == "outline") { #>';
		$output .= $lodash->border ( 'border-color', '#jpb-modal-{{ data.id }}-selector.jpb-btn-custom', 'data.button_background_color' );
		$output .= $lodash->border ( 'border-color', '#jpb-modal-{{ data.id }}-selector.jpb-btn-custom:hover', 'data.button_background_color_hover' );
		$output .= '<# } else if(data.button_appearance == "3d") { #>';
		$output .= $lodash->border ( 'border-bottom-color', '#jpb-modal-{{ data.id }}-selector.jpb-btn-custom', 'data.button_background_color_hover' );
		$output .= $lodash->color ( 'background-color', '#jpb-modal-{{ data.id }}-selector.jpb-btn-custom', 'data.button_background_color' );
		$output .= '<# } else if(data.button_appearance == "gradient") { #>';
		$output .= '#jpb-addon-{{ data.id }} #jpb-modal-{{ data.id }}-selector.jpb-btn-custom { border: none; }';
		$output .= $lodash->color ( 'background-color', '#jpb-modal-{{ data.id }}-selector.jpb-btn-custom', 'data.button_background_gradient' );
		$output .= $lodash->color ( 'background-color', '#jpb-modal-{{ data.id }}-selector.jpb-btn-custom:hover', 'data.button_background_gradient_hover' );
		$output .= '<# } else { #>';
		$output .= $lodash->color ( 'background-color', '#jpb-modal-{{ data.id }}-selector.jpb-btn-custom', 'data.button_background_color' );
		$output .= '<# } #>';
		$output .= '<# } #>';
		...
		$output .= $lodash->unit ( 'max-width', '#jpb-modal-{{ data.id }} .popup-image-block', 'data.modal_popup_width', 'px' );
		$output .= $lodash->unit ( 'height', '#jpb-modal-{{ data.id }} .popup-image-block', 'data.modal_popup_height', 'px' );
		$output .= '<# } else if (modalContentType == "text") { #>';
		$output .= $lodash->unit ( 'max-width', '#jpb-modal-{{ data.id }} .white-popup-block', 'data.modal_popup_width', 'px' );
		$output .= $lodash->unit ( 'height', '#jpb-modal-{{ data.id }} .white-popup-block', 'data.modal_popup_height', 'px' );
		$output .= '<# } #>';

		$output .= '<# if( data.modal_selector == "icon") { #>';

		$output .= $lodash->color ( 'color', '.jpb-modal-selector span', 'data.selector_icon_color' );
		$output .= $lodash->color ( 'background-color', '.jpb-modal-selector span', 'data.selector_icon_background' );
		$output .= $lodash->border ( 'border-color', '.jpb-modal-selector span', 'data.selector_icon_border_color' );
		$output .= '<# if(!_.isEmpty(data.selector_icon_border_color)) { #>';
		$output .= '#jpb-addon-{{ data.id }} .jpb-modal-selector span { border-style:solid;}';
		$output .= '<# } #>';
		$output .= '#jpb-addon-{{ data.id }} .jpb-modal-selector span { display:inline-block; line-height:1;}';

		$output .= $lodash->unit ( 'border-width', '.jpb-modal-selector span', 'data.selector_icon_border_width', 'px' );
		$output .= $lodash->unit ( 'border-radius', '.jpb-modal-selector span', 'data.selector_icon_border_radius', 'px' );
		$output .= $lodash->unit ( 'padding', '.jpb-modal-selector span', 'data.selector_icon_padding', 'px' );

		$output .= $lodash->unit ( 'font-size', '.jpb-modal-selector span > i', 'data.selector_icon_size', 'px' );
		$output .= $lodash->unit ( 'width', '.jpb-modal-selector span > i', 'data.selector_icon_size', 'px' );
		$output .= $lodash->unit ( 'height', '.jpb-modal-selector span > i', 'data.selector_icon_size', 'px' );
		$output .= $lodash->unit ( 'line-height', '.jpb-modal-selector span > i', 'data.selector_icon_size', 'px' );

		$output .= '<# } #>';

		$output .= ' <# if( (data.modal_selector == "icon" || data.modal_selector == "image") && (data.selector_icon_name || data.selector_image)) { #>';
		$output .= $lodash->unit ( 'font-size', 'jpb-modal-selector span.text', 'data.selector_text_size', 'px' );
		$output .= $lodash->unit ( 'font-weight', 'jpb-modal-selector span.text', 'data.selector_text_weight' );
		$output .= $lodash->color ( 'color', 'jpb-modal-selector span.text', 'data.selector_text_color' );
		$output .= $lodash->spacing ( 'margin', 'jpb-modal-selector span.text', 'data.selector_text_margin' );
		$output .= '<# } #>';

		$output .= '
		<# if( (data.modal_selector == "icon" || data.modal_selector == "image") && (data.selector_icon_name || data.selector_image)) { #>
			#jpb-addon-{{ data.id }} .jpb-modal-selector span.text {
		...
		$output .= $lodash->generateTransformCss ( '.jpb-modal-selector', 'data.transform' );
	    ...
	    <# } #>
			...
			#>
				<a class="jpb-modal-selector jpb-magnific-popup" {{{ attribs }}} href=\'{{ modalUrl }}\' id="{{ modalUniqueId }}-selector">
			...
			#>
				<a class="jpb-modal-selector jpb-magnific-popup" href=\'{{ modalUrl }}\' {{{ attribs }}} id="{{ modalUniqueId }}-selector">
					<span class="jpb-modal-icon-wrap">
						<i class="{{ select_icon_name }}"></i>
						<# if(data.show_ripple_effect) { #>
							<span class="jpb-ripple-effect"></span>
						<# } #>
					</span>
					<# if(data.selector_text){ #>
						<span class="text">{{ data.selector_text }}</span>
			...
			#>
				<a class="jpb-btn {{ buttonClass }} jpb-magnific-popup jpb-modal-selector" {{{ attribs }}} href=\'{{ modalUrl }}\' id="{{ modalUniqueId }}-selector"><# if( buttonIconPosition == "left" && data.button_icon ) { #> <i class="{{ btn_icon_name }}"></i><# } #> {{ data.button_text }} <# if( buttonIconPosition == "right" && data.button_icon ) { #> <i class="{{ btn_icon_name }}"></i><# } #></a>
			<# } #>
	    </div>';
