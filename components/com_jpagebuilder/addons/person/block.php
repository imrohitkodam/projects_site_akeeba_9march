...
class JpagebuilderAddonPerson extends JpagebuilderAddons {
	/**
	...
	public function render() {
		...
		if (strpos ( $image_src, "http://" ) !== false || strpos ( $image_src, "https://" ) !== false) {
			...
			if ($image_src) {
				...
				$image_src = JpagebuilderHelperSite::cleanPath ( $original_src );
			}
		}
		...
		if ($facebook || $twitter || $youtube || $linkedin || $pinterest || $flickr || $dribbble || $behance || $instagram) {
			$social_icons .= '<div class="jpb-person-social-icons">';
			$social_icons .= '<ul class="jpb-person-social">';

			if ($facebook)
				$social_icons .= '<li><a target="_blank" rel="noopener noreferrer" href="' . $facebook . '" aria-label="Facebook"><i class="fab fa-facebook-f" aria-hidden="true" title="Facebook"></i></a></li>';
			if ($twitter)
				$social_icons .= '<li><a target="_blank" rel="noopener noreferrer" href="' . $twitter . '" aria-label="Twitter"><i class="fab fa-twitter" aria-hidden="true" title="Twitter"></i></a></li>';
			if ($youtube)
				$social_icons .= '<li><a target="_blank" rel="noopener noreferrer" href="' . $youtube . '" aria-label="YouTube"><i class="fab fa-youtube" aria-hidden="true" title="YouTube"></i></a></li>';
			if ($linkedin)
				$social_icons .= '<li><a target="_blank" rel="noopener noreferrer" href="' . $linkedin . '" aria-label="LinkedIn"><i class="fab fa-linkedin-in" aria-hidden="true" title="LinkedIn"></i></a></li>';
			if ($pinterest)
				$social_icons .= '<li><a target="_blank" rel="noopener noreferrer" href="' . $pinterest . '" aria-label="Pinterest"><i class="fab fa-pinterest" aria-hidden="true" title="Pinterest"></i></a></li>';
			if ($flickr)
				$social_icons .= '<li><a target="_blank" rel="noopener noreferrer" href="' . $flickr . '" aria-label="Flickr"><i class="fab fa-flickr" aria-hidden="true" title="Flickr"></i></a></li>';
			if ($dribbble)
				$social_icons .= '<li><a target="_blank" rel="noopener noreferrer" href="' . $dribbble . '" aria-label="Dribble"><i class="fab fa-dribbble" aria-hidden="true" title="Dribble"></i></a></li>';
			if ($behance)
				$social_icons .= '<li><a target="_blank" rel="noopener noreferrer" href="' . $behance . '" aria-label="Behance"><i class="fab fa-behance" aria-hidden="true" title="Behance"></i></a></li>';
			if ($instagram)
				$social_icons .= '<li><a target="_blank" rel="noopener noreferrer" href="' . $instagram . '" aria-label="Instagram"><i class="fab fa-instagram" aria-hidden="true" title="Instagram"></i></a></li>';

			$social_icons .= '</ul>';
			$social_icons .= '</div>';
		}

		$output .= '<div class="jpb-addon jpb-addon-person ' . $class . ' ' . $content_position . '">';
		$output .= '<div class="jpb-addon-content">';

		if ($person_style_preset == 'layout4') {
			$output .= '<div class="jpb-row jpb-no-gutter">';
			$output .= '<div class="jpb-col-sm-5">';
		}

		if ($image_src) {
			$output .= '<div class="jpb-person-image ' . ($person_style_preset == 'layout4' ? 'person-layout-4' : '') . '">';
			$output .= '<img class="jpb-img-responsive' . ($placeholder ? ' jpb-element-lazy' : '') . '" style="display: inline-block;" src="' . ($placeholder ? $placeholder : $image_src) . '" alt="' . $alt_text . '" ' . ($placeholder ? 'data-large="' . $image_src . '"' : '') . ' ' . ($image_width ? 'width="' . $image_width . '"' : '') . ' ' . ($image_height ? 'height="' . $image_height . '"' : '') . ' loading="lazy">';
				...
				if ($person_style_preset !== 'layout4') {
					...
					if ($person_style_preset == 'layout1') {
						if ($social_position == 'before')
							$output .= $social_icons;
						if ($introtext)
							$output .= '<div class="jpb-person-introtext">' . $introtext . '</div>';
					...
					if ($person_style_preset == 'layout2' || $person_style_preset == 'layout3') {
						if ($name || $designation) {
							if ($name) {
								$person_name = '<span class="jpb-person-name">' . $name . '</span>';
								$output .= ! empty ( $link ) ? '<a ' . $target . ' href="' . $link . '">' . $person_name . '</a>' : $person_name;
							}
							if ($designation)
								$output .= '<span class="jpb-person-designation">' . $designation . '</span>';
							if ($social_icons)
								$output .= $social_icons;
						}
					}
			...
			$output .= '</div>'; // .jpb-person-image
		}

		if ($person_style_preset == 'layout4') {
			$output .= '</div>';
			$output .= '<div class="jpb-col-sm-7">';
			$output .= '<div class="jpb-person-addon-content-wrap">';
		}

		if ($person_style_preset !== 'layout2' && $person_style_preset !== 'layout3') {
			if ($name || $designation || $email) {
				$output .= '<div class="jpb-person-information">';
				if ($name) {
					$person_name = '<span class="jpb-person-name">' . $name . '</span>';
					$output .= ! empty ( $link ) ? '<a ' . $target . ' href="' . $link . '">' . $person_name . '</a>' : $person_name;
				}
				if ($designation)
					$output .= '<span class="jpb-person-designation">' . $designation . '</span>';
				if ($email)
					$output .= '<a href="mailto:' . $email . '" class="jpb-person-email">' . $email . '</a>';
				$output .= '</div>';
			}
		}
	...
	public function css() {
		...
		if (isset ( $settings->content_overlay_type )) {
			$settings->content_overlay_background_color = $settings->content_overlay_type === 'gradient' ? JpagebuilderCSSHelper::parseColor ( $settings, 'content_overlay_gradient' ) : $settings->content_overlay_color;
		}
		...
		if ((isset ( $settings->content_overlay_backdrop_filter ) && $settings->content_overlay_backdrop_filter) && ($settings->person_style_preset == 'layout2' || $settings->person_style_preset == 'layout3')) {
			$settings->content_overlay_backdrop_filter = $settings->content_overlay_backdrop_filter_webkit = JpagebuilderCSSHelper::parseBackDropFilter ( $settings, 'content_overlay_backdrop_filter', 'content_overlay_backdrop_filter_value' );
		}

		$personStyle = $cssHelper->generateStyle ( '.person-content-show-on-hover', $settings, [ 
				'content_overlay_background_color' => 'background',
				'content_overlay_backdrop_filter' => 'backdrop-filter',
				'content_overlay_backdrop_filter_webkit' => '-webkit-backdrop-filter'
		], false );
		$personImageStyle = $cssHelper->generateStyle ( '.jpb-person-image img', $settings, [ 
				'image_border_radius' => 'border-radius'
		] );
		$personPresetStyle = $cssHelper->generateStyle ( '.person-content-position-bottom-left .jpb-person-information', $settings, [ 
				'name_desig_bg' => 'background',
				'name_desig_padding' => 'padding'
		], false, [ 
				'name_desig_padding' => 'spacing'
		] );
		$nameStyle = $cssHelper->generateStyle ( '.jpb-person-name', $settings, [ 
				'name_color' => 'color'
		], false );
		$nameTypographyStyle = $cssHelper->typography ( '.jpb-person-name', $settings, 'name_typography', [ 
		...
		$designationStyle = $cssHelper->generateStyle ( '.jpb-person-designation', $settings, [ 
		...
		$designationTypographyStyle = $cssHelper->typography ( '.jpb-person-designation', $settings, 'designation_typography', [ 
		...
		$introTextStyle = $cssHelper->generateStyle ( '.jpb-person-introtext', $settings, [ 
				'introtext_color' => 'color'
		], false );
		$introTextTypographyStyle = $cssHelper->typography ( '.jpb-person-introtext', $settings, 'intro_typography', [ 
		...
		$socialIconStyle = $cssHelper->generateStyle ( '.jpb-person-social > li > a', $settings, [ 
				'social_icon_color' => 'color',
				'social_icon_fontsize' => 'font-size'
		], [ 
				'social_icon_color' => false
		] );
		$socialIconMarginStyle = $cssHelper->generateStyle ( '.jpb-person-social > li', $settings, [ 
				'social_icon_margin' => 'margin'
		], false, [ 
				'social_icon_margin' => 'spacing'
		] );
		$socialIconHoverStyle = $cssHelper->generateStyle ( '.jpb-person-social > li > a:hover', $settings, [ 
				'social_icon_hover_color' => 'color'
		], false );
		$contentStyle = $cssHelper->generateStyle ( '.jpb-person-addon-content-wrap', $settings, [ 
				'person_content_bg' => 'background-color',
				'person_content_padding' => 'padding'
		], false, [ 
				'person_content_padding' => 'spacing'
		] );

		$settings->alignment = JpagebuilderCSSHelper::parseAlignment ( $settings, 'alignment' );

		$alignment = $cssHelper->generateStyle ( '.jpb-addon.jpb-addon-person', $settings, [ 
				'alignment' => 'text-align'
		], false );
		$transformCss = $cssHelper->generateTransformStyle ( '.jpb-addon-person', $settings, 'transform' );
	...
	public static function getFrontendEditor() {
		...
		<# let alt_text = data?.image?.alt #>
			<style type="text/css">
				...
				#>
						#jpb-addon-{{ data.id }} .person-content-show-on-hover {
		...
		$output .= $lodash->unit ( 'border-radius', '.jpb-person-image img', 'data.image_border_radius', 'px', false );
		// Name Fallbacks
		...
		$output .= $lodash->typography ( '.jpb-person-name', 'data.name_typography', $nameTypographyFallbacks );
		...
		$output .= $lodash->typography ( '.jpb-person-designation', 'data.designation_typography', $designationTypographyFallbacks );
		// intro Fallbacks
		...
		$output .= $lodash->typography ( '.jpb-person-introtext', 'data.intro_typography', $introTypographyFallbacks );
		$output .= $lodash->color ( 'color', '.jpb-person-name', 'data.name_color' );
		$output .= $lodash->color ( 'color', '.jpb-person-designation', 'data.designation_color' );
		$output .= $lodash->color ( 'color', '.jpb-person-social > li > a', 'data.social_icon_color' );
		$output .= $lodash->color ( 'color', '.jpb-person-social > li > a:hover', 'data.social_icon_hover_color' );
		$output .= $lodash->unit ( 'font-size', '.jpb-person-social > li > a', 'data.social_icon_fontsize', 'px' );
		$output .= $lodash->spacing ( 'margin', '.jpb-person-social > li', 'data.social_icon_margin' );
		$output .= $lodash->spacing ( 'margin', '.jpb-person-designation', 'data.designation_margin' );
		$output .= '<# if (data.person_style_preset=="layout1") { #>';
		$output .= '<# if(data.name_desig_bg || data.name_desig_padding) { #>';
		$output .= $lodash->unit ( 'background', '.person-content-position-bottom-left .jpb-person-information', 'data.name_desig_bg', '', false );
		$output .= $lodash->spacing ( 'padding', '.person-content-position-bottom-left .jpb-person-information', 'data.name_desig_padding' );
		$output .= '<# } #>';
		$output .= '<# } #>';
		$output .= '<# if (data.person_style_preset=="layout4") { #>';
		$output .= $lodash->unit ( 'background', '.jpb-person-addon-content-wrap', 'data.person_content_bg', '', false );
		$output .= $lodash->spacing ( 'padding', '.jpb-person-addon-content-wrap', 'data.person_content_padding' );
		$output .= '<# } #>';
		...
		$output .= $lodash->alignment ( 'text-align', '.jpb-addon.jpb-addon-person', 'data.alignment' );
		$output .= $lodash->generateTransformCss ( '.jpb-addon-person', 'data.transform' );
		...
		$output .= '
			...
			let personName = `<span class="jpb-person-name jp-inline-editable-element" data-id=${data.id} data-fieldName="name" contenteditable="true">${data.name}</span>`;
			...
			if (isMenu || isPage || isUrl || isOldUrl) { 
				...
				personName = `<a href=${newUrl} target=${target} rel=${rel}>
								<span class="jpb-person-name jp-inline-editable-element" data-id=${data.id} data-fieldName="name" contenteditable="true">
									${data.name}
								</span>
							 </a>`;
			} 			
			#>
			<div class="jpb-addon jpb-addon-person {{ data.class}} {{content_position}}">
				<div class="jpb-addon-content">
				<# if(data.person_style_preset=="layout4"){ #>
					<div class="jpb-row jpb-no-gutter">
						<div class="jpb-col-sm-5">
				<# }
				var personImg = {}
				...
				#>
					<div class="jpb-person-image <# if(data.person_style_preset=="layout4"){ #> person-layout-4 <# } #>">
						<# if(personImg.src.indexOf("https://") == -1 && personImg.src.indexOf("http://") == -1){ #>
							<img class="jpb-img-responsive" style="display: inline-block;" src=\'{{ pagebuilder_base + personImg.src }}\' alt="{{ alt_text ?? data.name }}">
						<# } else { #>
							<img class="jpb-img-responsive" style="display: inline-block;" src=\'{{ personImg.src }}\' alt="{{ alt_text ?? data.name }}">
						<# } #>
							...
							<# if(data.person_style_preset=="layout1") { #>
								<# if(data.social_position == "after") { #>
									<# if(!_.isEmpty(data.introtext)) { #>
										<div class="jpb-person-introtext jp-inline-editable-element" data-id={{data.id}} data-fieldName="introtext" contenteditable="true">{{{ data.introtext }}}</div>
									<# } #>
								<# } #>
								...
								<# if ( data.facebook || data.twitter || data.youtube || data.linkedin || data.pinterest || data.flickr || data.dribbble || data.behance || data.instagram ) { #>
									<div class="jpb-person-social-icons">
										<ul class="jpb-person-social">
											{{{social_icon_list}}}
										</ul>
									</div>
								<# } #>
								...
								<# if(data.social_position == "before") { #>
									<# if(!_.isEmpty(data.introtext)) { #>
										<div class="jpb-person-introtext">{{{ data.introtext }}}</div>
									<# } #>
								<# } #>
							<# } #>													
							...
							<# if(data.person_style_preset=="layout2" || data.person_style_preset=="layout3"){ #>
								<# if(data.name || data.designation){ #>
									...
									<# if(!_.isEmpty(data.designation)) { #>
										<span class="jpb-person-designation jp-inline-editable-element" data-id={{data.id}} data-fieldName="designation" contenteditable="true">{{ data.designation}}</span>
									<# } #>
									<# if ( data.facebook || data.twitter || data.youtube || data.linkedin || data.pinterest || data.flickr || data.dribbble || data.behance || data.instagram ) { #>
										<div class="jpb-person-social-icons">
											<ul class="jpb-person-social">
				...
				#>
					...
					<div class="jpb-col-sm-7">
					<div class="jpb-person-addon-content-wrap">
				<# }
				if(data.person_style_preset!=="layout2" && data.person_style_preset!=="layout3"){
				#>
					<# if(data.name || data.designation || data.email ){ #>
						<div class="jpb-person-information">
							<# if(!_.isEmpty(data.name)) { #>
								{{{ personName }}}						
							<# } #>
							<# if(!_.isEmpty(data.designation)) { #>
							...
							<# if(!_.isEmpty(data.email)) { #>
								<a href="mailto:{{ data.email}}" class="jpb-person-email">{{ data.email}}</a>
							<# } #>
						</div>
					<# } #>
				<# } #>
