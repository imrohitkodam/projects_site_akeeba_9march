...
class JpagebuilderAddonPricelist extends JpagebuilderAddons {
	/**
	...
	public function render() {
		...
		if ($price_position == "content-bottom") {
			$content_alignment = 'jpb-text-alignment';
		}
		...
		// Output
		$output = '';
		$output .= '<div class="jpb-addon jpb-addon-pricelist ' . $class . '">';

		if ($add_number_or_image) {
			$output .= '<div class="pricelist-left-image">';

			if ($number_or_image_left == 'image') {
				$alt_text_fallback = ($title) ? $title : '';
				$alt_text = isset ( $image->alt ) ? $image->alt : $alt_text_fallback;
				$output .= '<img ' . ($placeholder ? 'class="jpb-element-lazy"' : '') . ' src="' . ($placeholder ? $placeholder : $image) . '" alt="' . $alt_text . '" ' . ($placeholder ? 'data-large="' . $image . '"' : '') . ' ' . ($image_width ? 'width="' . $image_width . '"' : '') . ' ' . ($image_height ? 'height="' . $image_height . '"' : '') . ' loading="lazy">';
		...
		if ($title) {
			$output .= '<div class="jpb-addon-title" style="display: block;">';
	...
	public function css() {
		...
		$priceListTypographyStyle = $cssHelper->typography ( '.jpb-addon-content', $settings, 'content_typography', [ 
		...
		$lineStyle = $cssHelper->generateStyle ( '.pricelist-line span', $settings, $lineProps, [ 
				'line_color' => false
		] );
		$lineBottomProps = [ 
		...
		$linePositionStyle = $cssHelper->generateStyle ( '.pricelist-line', $settings, [ 
		...
		$priceContentStyle = $cssHelper->generateStyle ( '.pricelist-price-content.bottom-of-content', $settings, [ 
				'price_top_gap' => 'margin-top',
		...
		$zeroPositionStyle = $cssHelper->generateStyle ( '.pricelist-point-zero', $settings, [ 
				'zero_position' => 'vertical-align'
		], false );
		$discountPriceStyle = $cssHelper->generateStyle ( '.pricelist-price.discounted-price', $settings, [ 
				'discount_price_position' => 'vertical-align'
		], false );
		$priceStyle = $cssHelper->generateStyle ( '.pricelist-price-content', $settings, [ 
				'price_color' => 'color'
		], false );

		$priceTypographyStyle = $cssHelper->typography ( '.pricelist-price-content', $settings, 'price_typography', [ 
				'font' => 'price_font_family',
				'size' => 'price_fontsize',
				'weight' => 'price_fontweight'
		] );
		$contentPositionStyle = $cssHelper->generateStyle ( '.jpb-text-alignment', $settings, [ 
				'content_position' => 'text-align'
		], false );
		$positionMap = [ 
				'left' => 'flex-start',
				'right' => 'flex-end',
		...
		$titlePositionStyle = $cssHelper->generateStyle ( '.pricelist-title-content', $settings, [ 
				'content_position_alt' => 'justify-content'
		], false );
		$tagStyle = $cssHelper->generateStyle ( '.pricelist-tag', $settings, [ 
		...
		if ($settings->add_number_or_image) {
			$numberOrImageStyle = $cssHelper->generateStyle ( '.pricelist-left-image', $settings, [ 
					'image_width' => [ 
							'-ms-flex: 0 0 %s',
							'flex: 0 0 %s',
							'max-width'
					],
					'image_gutter' => 'padding-right'
			], [ 
					'image_width' => '%'
			] );
			$settings->image_width = JpagebuilderAddonUtils::parseDeviceData ( $settings->image_width, JpagebuilderBase::$defaultDevice );

			if (! isset ( $settings->image_width )) {
				$settings->image_width = 15;
			}

			$settings->content_width = isset ( $settings->image_width_original ) ? $settings->image_width_original : $settings->image_width;

			if (is_object ( $settings->content_width )) {
				foreach ( $settings->content_width as $device => $value ) {
					if (! empty ( $value )) {
						$settings->content_width->$device = 100 - ( int ) $value;
					}
				}
			} else {
				$settings->content_width = 100 - ( int ) $settings->content_width;
			}

			$imageWidthGutterStyle = $cssHelper->generateStyle ( '.pricelist-text-content', $settings, [ 
					'content_width' => [ 
							'-ms-flex: 0 0 %s',
							'flex: 0 0 %s',
							'max-width: %s'
		...
		$imageBorderRadiusStyle = $cssHelper->generateStyle ( '.pricelist-left-number,.pricelist-left-image img', $settings, [ 
				'image_border_radius' => 'border-radius'
		] );
		$transformCss = $cssHelper->generateTransformStyle ( '.jpb-addon-pricelist', $settings, 'transform' );

		$numberStyle = $cssHelper->generateStyle ( '.pricelist-left-number', $settings, [ 
				'number_bg_color' => 'background-color',
				'number_color' => 'color',
				'number_top_padding' => 'padding-top',
				'number_bottom_padding' => 'padding-bottom'
		], [ 
				'number_bg_color' => false,
				'number_color' => false
		] );
		$numberTypographyStyle = $cssHelper->typography ( '.pricelist-left-number', $settings, 'number_typography', [ 
	...
	public static function getFrontendEditor() {
		...
		<#
			...
			let device_list = ["xl", "lg", "md", "sm", "xs"];
			let content_width = "";
			...
			if(_.isObject(data.image_width)) {
				...
				for(const device of device_list) {
					if(!data.image_width[device]) {
						continue;
					}

					content_width[device] = 100 - Number(data.image_width[device]);
				}
			} else {
				content_width = 100 - Number(data.image_width);
			}
		...
		$output .= '#jpb-addon-{{data.id}} .pricelist-price-content.bottom-of-content { display: block; }';
		...
		$output .= $lodash->flex ( '-ms-flex', '.pricelist-text-content', 'content_width', '%' );
		$output .= $lodash->flex ( 'flex', '.pricelist-text-content', 'content_width', '%' );
		$output .= $lodash->unit ( 'max-width', '.pricelist-text-content', 'content_width', '%' );
		...
		$output .= $lodash->generateTransformCss ( '.jpb-addon-pricelist', 'data.transform' );
		$output .= '
		
		</style>
		
		<div class="jpb-addon jpb-addon-pricelist {{data.class}}">
		<# if(data.add_number_or_image){ #>
			<div class="pricelist-left-image">
				<# if(data.number_or_image_left=="image") {
					...
					let alt_text = data?.image?.alt ?? data?.title;

					if(priceListImg.src.indexOf("http://") == -1 && priceListImg.src.indexOf("https://") == -1){ #>
						<img class="jpb-img-responsive" src=\'{{ pagebuilder_base + priceListImg.src }}\' alt="{{alt_text}}>
					<# } else { #>
						<img class="jpb-img-responsive" src=\'{{ priceListImg.src }}\' alt="{{alt_text}}>
					<# }
		...
		<# if (data.title) { #>
			<div class="jpb-addon-title" style="display: block;">
		...
		<div class="jpb-addon-content jp-editable-content" id="addon-text-{{data.id}}" data-id={{data.id}} data-fieldName="text">
			{{{data.text}}}
		</div>
