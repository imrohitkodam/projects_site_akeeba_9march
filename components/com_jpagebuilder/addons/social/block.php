...
class JpagebuilderAddonSocial extends JpagebuilderAddons {
	public function render() {
		...
		$icon_width = (isset ( $settings->icon_size ) && $settings->icon_size) ? '; width: ' . $settings->icon_size . '' : '';
		$icon_height = (isset ( $settings->icon_size ) && $settings->icon_size) ? '; height: ' . $settings->icon_size . ';' : '';
		...
		if (isset ( $settings->ui_subnav_items ) && count ( ( array ) $settings->ui_subnav_items )) {
			foreach ( $settings->ui_subnav_items as $key => $item ) {
				...
				list ( $link, $link_target ) = JpagebuilderAddonHelper::parseLink ( $item, 'link', [ 
						'new_tab' => 'target',
						'url' => 'url'
				] );
				$output .= '<div class="ui-item">';
				$output .= ($link) ? '<a class="' . $style . $icons_button_cls . '" href="' . $link . '"' . $link_target . '>' : '';
				$output .= '<span uk-icon="icon: ' . $brand . ($icons_button ? '' : $icon_width . $icon_height) . '"></span>';
				$output .= ($link) ? '</a>' : '';
				$output .= '</div>';
			}
		}
	...
	public static function getFrontendEditor() {
		...
		let text_alignment = data.alignment ? " uk-text-" + data.alignment : "";
		...
		let flex_alignment = data.alignment ? " uk-text-" + data.alignment : "";
		let flex_alignment_breakpoint = (data.alignment && data.text_breakpoint) ? "@" + data.text_breakpoint : "";
		let flex_alignment_fallback = (data.alignment && data.text_breakpoint && data.text_alignment_fallback) ? " uk-text-" + data.text_alignment_fallback : "";
		flex_alignment += flex_alignment_breakpoint + flex_alignment_fallback;

		let style = ( data.link_style ) ? "uk-"+data.link_style : "uk-icon-link";
		let gutter = ( data.gutter ) ? " uk-grid-column-"+data.gutter : "";
		...
		let icons_button     = ( data.icons_button ) ? 1 : "";
		let icons_button_cls = ( icons_button ) ? " uk-icon-button" : "";
		...
		<div class="ui-social{{ zindex_cls }}{{ general }}"{{{ animation }}}>

		<div class="uk-child-width-auto{{ gutter }}" uk-grid>
		<#
		_.each(data.ui_subnav_items, function (value){
			const isUrlObj = _.isObject(value?.link) && (!!value?.link?.url || !!value?.link?.page || !!value?.link?.menu);
			const isUrlString = _.isString(value?.link) && value?.link !== "";
			
			const isTarget = value?.link_open_new_window ? "_blank" : "";
			const urlObj = value?.link?.url ? value?.link : window.getSiteUrl(value?.link, isTarget);
			...
			let brand   = value.brand_name ? value.brand_name : "";
			#>
			<div class="ui-item">
			<# if (buttonUrl) { #>
				<a class="{{ style }}{{ icons_button_cls }}" href=\'{{ buttonUrl }}\' target=\'{{ target }}\' rel=\'{{ rel }}\'>
			<# } #>
			<span uk-icon="icon: {{ brand }}<# if(! icons_button) { #>{{ icon_width }}{{ icon_height }}<# } #>"></span> 
			<# if (buttonUrl) { #>
				</a>
			<# } #>
			</div>
