...
class JpagebuilderAddontotop extends JpagebuilderAddons {
	public function render() {
		...
		$link_title = (isset ( $settings->link_title ) && $settings->link_title) ? ' title="' . str_replace ( '"', '', $settings->link_title ) . '"' : '';
		$title_style = (isset ( $settings->title_style ) && $settings->title_style) ? ' uk-text-' . $settings->title_style : '';
		...
		$output = '<div class="ui-totop' . $zindex_cls . $general . '"' . $animation . '>';
		if ($title) {
			$output .= '<div class="uk-child-width-expand' . $title_breakpoint . $title_grid_cr . ' uk-flex-inline uk-flex-middle" uk-grid>';
			$output .= '<div>';
		}
		$output .= '<a href="#" class="ui-icon' . $text_color . '"' . $link_title . ' uk-totop uk-scroll></a>';
		if ($title) {
			$output .= '</div>';
			$output .= '<div class="uk-flex-first' . $title_breakpoint . ' uk-width-auto' . $title_breakpoint . '">';
			$output .= '<div class="el-title' . $title_style . '">' . $title . '</div>';
			$output .= '</div>';
			$output .= '</div>';
		}
	...
	public static function getFrontendEditor() {
		$output = '
		<#
		let text_color   = ( data.text_color ) ? " uk-text-"+data.text_color : "";
		...
		let link_title  = ( data.link_title ) ? \' title="\' + data.link_title.replace(/"/g, "") + \'"\' : "";
		let title_style = ( data.title_style ) ? " uk-text-"+ data.title_style : "";

		var title_breakpoint = ( data.title_breakpoint ) ? "@" + data.title_breakpoint : "";
		...
		<div class="ui-totop{{ zindex_cls }}{{ general }}"{{{ animation }}}>
		<# if ( title ) { #>
			<div class="uk-child-width-expand{{ title_breakpoint }}{{ title_grid_cr }} uk-flex-inline uk-flex-middle" uk-grid>
			<div>
		<# } #>
		<a href="#" class="ui-icon{{ text_color }}"{{{ link_title }}} uk-totop uk-scroll></a>
		<# if ( title ) { #>
			</div>
			<div class="uk-flex-first{{ title_breakpoint }} uk-width-auto{{ title_breakpoint }}">
			<div class="el-title{{ title_style }}">{{{ title }}}</div>
			</div>
			</div>
		<# } #>
