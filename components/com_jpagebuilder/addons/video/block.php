...
class JpagebuilderAddonVideo extends JpagebuilderAddons {
	/**
	...
	public function render() {
			...
			if (array_key_exists ( 'host', $video )) {
				switch ($video ['host']) {
					case 'youtu.be' :
						...
						$video_title = (isset ( $settings->video_title ) && $settings->video_title) ? $settings->video_title : Text::_ ( "COM_JPAGEBUILDER_ADDON_VIDEO_TITLE_DEFAULT_TEXT" );
						break;
		...
		$output .= '<div class="jpb-addon jpb-addon-video ' . $class . '">';
		$output .= ($title) ? '<' . $heading_selector . ' class="jpb-addon-title">' . $title . '</' . $heading_selector . '>' : '';
		if ($mp4_enable != 1) {
			$output .= '<div class="jpb-video-block jpb-embed-responsive jpb-embed-responsive-' . $aspect_ratio . '">';
			$output .= '<iframe class="jpb-embed-responsive-item' . ($placeholder ? ' jpb-element-lazy' : '') . '" ' . ($placeholder ? 'style="background:url(' . $placeholder . '); background-size: cover;"' : 'src="' . $src . '"') . ' ' . ($placeholder ? 'data-large="' . $src . '"' : '') . ' ' . ($video_title ? 'title="' . $video_title . '"' : '') . ' allow="accelerometer" webkitAllowFullScreen mozallowfullscreen allowFullScreen loading="lazy" ></iframe>';
			$output .= '</div>';
		} else {
			if ($mp4_video || $ogv_video) {
				$output .= '<div class="jpb-addon-video-local-video-wrap">';
				$output .= '<video class="jpb-addon-video-local-source' . ($placeholder ? ' jpb-element-lazy' : '') . '"' . ($video_loop != 0 ? ' loop' : '') . '' . ($autoplay_video != 0 ? ' autoplay' : '') . '' . ($show_control != 0 ? ' controls' : '') . '' . ($video_mute != 0 ? ' muted' : '') . ' ' . ($lazyload ? 'data-poster="' . $video_poster . '"' : ' poster="' . $video_poster . '"') . ' controlsList="nodownload" playsinline>';
				if (! empty ( $mp4_video )) {
	...
	public function css() {
		...
		$css = $cssHelper->generateTransformStyle ( '.jpb-addon-video', $settings, 'transform' );
	 ...
	 *
	 * @param array $embeddedParameter
	 *        	Array of embedded settings which will be added on vimeo player.
	 * @param string $src
