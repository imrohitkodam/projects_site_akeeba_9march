...
/**
 * JPageBuilder settings ajax loader
 *
 * @since version 2.0
 */
defined ( '_JEXEC' ) or die ( 'Restricted access' );
class JpbSettingsAjax {
	...
	private function get_inputs_output_html() {
		...
		if (count ( ( array ) $fieldsets )) {
			$output .= '<div class="jpagebuilder-fieldset">';
			$output .= '<ul class="jpagebuilder-nav jpagebuilder-nav-tabs">';
			foreach ( $fieldsets as $key => $value ) {
				$output .= '<li class="' . (($i === 0) ? "active" : "") . '"><a href="#jpagebuilder-tab-' . $key . '"  aria-controls="' . $key . '" data-toggle="tab">' . ucfirst ( $key ) . '</a></li>';
				$i ++;
			}
			$output .= '</ul>';
			$output .= '<div class="tab-content">';
			foreach ( $fieldsets as $key => $value ) {
				$output .= '<div class="tab-pane ' . (($j === 0) ? "active" : "") . '" id="jpagebuilder-tab-' . $key . '">';
	 ...
	 * @return string
	 */
	private function get_input_fields($fieldset = '', $title = '') {
		if (! $fieldset)
			return;
		...
		$output = '';
		foreach ( $options [$fieldset] as $key => $option ) {
			if (isset ( $option ['attr'] ) && is_array ( $option ['attr'] )) {
				...
				$output .= '<div class="jpagebuilder-parent-input-field" data-addon_title="' . $title . '">';
	...
	private function get_input_field_html($name, $options) {
		$class_name = 'JPageBuilderType' . ucfirst ( $options ['type'] );
	...
	private function get_grouped_input_fields($name, $options, $title = '') {
		...
		if (isset ( $options ['title'] ) && $options ['title'] != '') {
			$output .= '<div class="jpagebuilder-grouped-wrap jpagebuilder-parent-input-field jpagebuilder-has-group-title" data-addon_title="' . $title . '" data-field_name="' . $name . '">';
		} else {
			$output .= '<div class="jpagebuilder-grouped-wrap jpagebuilder-parent-input-field" data-addon_title="' . $title . '" data-field_name="' . $name . '">';
		}

		if (isset ( $options ['title'] ) && $options ['title'] != '') {
			$output .= '<h4 class="jpagebuilder-group-title">' . $options ['title'] . '</h4>';
		}

		$output .= '<a href="#" class="jpagebuilder-add-grouped-item jpagebuilder-btn jpagebuilder-btn-warning jpagebuilder-btn-sm"><i class="fa fa-plus"></i> Add New</a>';
		$output .= '<div class="jpagebuilder-grouped jpagebuilder-grouped-items grouped-' . $name . '" data-field_no="' . ($count - 1) . '">';
		for($i = 0; $i < $count; $i ++) {

			$output .= '<div class="jpagebuilder-grouped-item">';
			$output .= '<div class="jpagebuilder-repeatable-toggler">';
			$output .= '<span class="jpagebuilder-move-repeatable"><i class="fa fa-ellipsis-v"></i></span>';
			$output .= '<h4 class="jpagebuilder-repeatable-item-title">Item ' . ($i + 1) . '</h4>';
			$output .= '<span class="jpagebuilder-remove-grouped-item"><a href="#"><i class="fa fa-times"></i></a></span>';
			$output .= '<span class="jpagebuilder-clone-grouped-item"><a href="#"><i class="fa fa-clone"></i></a></span>';
			$output .= '</div>';

			$output .= '<div class="jpagebuilder-grouped-item-collapse" style="display:none;">';
			foreach ( $options ['attr'] as $key => $option ) {
				...
				$key = $name . '[' . $i . '][' . $key . ']';
				$output .= '<div class="jpagebuilder-repeatable-input-field">';
				$output .= $this->get_input_field_html ( $key, $option );
				$output .= '</div>';
			}
			$output .= '</div>';
