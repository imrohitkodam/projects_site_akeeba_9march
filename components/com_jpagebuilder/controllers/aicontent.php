...
class JpagebuilderControllerAicontent extends FormController {
	...
	public function uploadAiGeneratedImageFromUrl() {
		...
		$folder_root = $mediaParams->get ( 'file_path', 'files' ) . '/jpagebuilder/ai/';
		...
		if (($image->width > 300) || ($image->height > 225)) {
			...
			$created = $image->createThumb ( array (
					'300',
					'300'
			), $thumbDestPath, $base_name, $extension );
			...
			$report ['src'] = Uri::root ( true ) . '/' . $folder . '/_jpmedia_thumbs/' . $base_name . '.' . $extension;
			$thumb = $folder . '/_jpmedia_thumbs/' . $base_name . '.' . $extension;
