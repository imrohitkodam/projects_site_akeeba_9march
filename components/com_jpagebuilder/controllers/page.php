...
class JpagebuilderControllerPage extends FormController {
	...
	public function exitExitPage() {
		...
		$response = [ 
				'response' => Text::_ ( "COM_JPAGEBUILDER_ERROR_MSG" ),
				'status' => false,
				'code' => 500
		];
		...
		try {
			if ($table->checkIn ( $pageId )) {
				$response ['response'] = Uri::base () . 'index.php?option=com_jpagebuilder&view=dashboard&tmpl=component';
	...
	public function save($key = null, $urlVar = null) {
		...
		if (empty ( $recordId )) {
			...
			$authorised = $user->authorise ( 'core.edit', 'com_jpagebuilder' ) || $user->authorise ( 'core.edit', 'com_jpagebuilder.page.' . $recordId ) || ($user->authorise ( 'core.edit.own', 'com_jpagebuilder.page.' . $recordId ) && $data ['created_by'] == $user->id);
		}
		...
		if ($data === false) {
			...
			$url = $link . 'index.php?option=com_jpagebuilder&view=form&layout=edit&tmpl=component&id=' . $recordId . '&Itemid=' . $Itemid;
			$output ['redirect'] = $url;
			echo json_encode ( $output );
			die ();
		}
		...
		if (! $model->save ( $data )) {
			...
			$output ['redirect'] = $link . 'index.php?option=com_jpagebuilder&view=form&layout=edit&tmpl=component&id=' . $recordId . '&Itemid=' . $Itemid;
			echo json_encode ( $output );
			die ();
		}
	...
	public function deleteSection() {
		...
		die ( (string) $model->deleteSection ( $id ) );
	}
	...
	public function saveCode() {
		...
		try {
			...
			if ($codeCategory === 'section') {
				$table = '#__jpagebuilder_sections';
				$codeField = 'section';
			} elseif ($codeCategory === 'addon') {
				$table = '#__jpagebuilder_addons';
				$codeField = 'code';
			}

			// Check if title already exists in the specific table
			...
			if ($existingId) {
				...
				$response = [
					'status' => true,
					'data' => ucfirst($codeCategory) . ' updated successfully!'
				];
			} else {
				// Insert if title doesn't exist
				$data = new stdClass();
				$data->title = $title;
				$data->$codeField = $code;
	...
	public function deleteAddon() {
		...
		die ( (string) $model->deleteAddon ( $id ) );
	}
	...
	public function cancel($key = 'id') {
		parent::cancel ( $key );
		$return_url = Factory::getApplication ()->getInput()->get ( 'return_page', null, 'base64' );
	...
	public function addToMenu() {
		...
		$pageId = ( int ) $this->getInput()->get->get ( 'pageId', 0, 'int' );
		...
		$component_id = ComponentHelper::getComponent ( 'com_jpagebuilder' )->id;
		...
		if ($model->save ( $menuData )) {
			...
			$root = Uri::base ();
			$root = new Uri ( $root );
			$output ['redirect'] = Uri::base () . 'index.php?option=com_jpagebuilder&view=form&layout=edit&tmpl=component&id=' . $pageId . $menuItemId;
		} else {
			$output ['status'] = false;
	...
	public function getMenuParentItem() {
		...
		$results = array ();
		$menutype = $this->getInput()->get->get ( 'menutype', '' );
		$parent_id = $this->getInput()->get->get ( 'parent_id', 0 );
	...
	public function createNewPage() {
		...
		$title = trim ( htmlspecialchars ( $this->getInput()->post->get ( 'title', '', 'string' ) ) );
		$model = $this->getModel ( 'Form' );
		$id = $model->createNewPage ( $title );

		$root = Uri::base ();
		$root = new Uri ( $root );
		$redirect = Uri::base () . 'index.php?option=com_jpagebuilder&view=form&layout=edit&tmpl=component&id=' . $id;
	...
	public function deletePage() {
		...
		$user = Factory::getApplication()->getIdentity();
		$authorised = $user->authorise ( 'core.delete', 'com_jpagebuilder' );
		...
		$pageid = ( int ) $this->getInput()->post->get ( 'pageid', '', 'int' );
	...
	/**
	 * Get importing layout page information
	 *
	 * @return void
	 * @since 4.0.0
	 */
	public function importLayout() {
		...
		if (! empty ( $pageData )) {
			...
			$content->template = json_decode ( JpagebuilderHelperSite::sanitizeImportJSON ( $content->template ) );
	...
	public function savePage() {
			...
			foreach ( $data as &$row ) {
				foreach ( $row ['columns'] as &$column ) {
					foreach ( $column ['addons'] as &$addon ) {
						if (isset ( $addon ['type'] ) && ($addon ['type'] === 'jp_row' || $addon ['type'] === 'inner_row')) {
		...
		// $pageData->content = JpagebuilderEditorUtils::cleanXSS($data);
		$pageData->modified = Factory::getDate ()->toSql ();
		$pageData->modified_by = Factory::getApplication ()->getIdentity ()->id;
		$pageData->version = $version;

		/** @var JpagebuilderModelForm */
		$model = $this->getModel ( 'Form' );
		$pageModel = $this->getModel ();
		$oldPage = $model->getItem ( $id );
		$oldPage = JpagebuilderApplicationHelper::preparePageData ( $oldPage );
