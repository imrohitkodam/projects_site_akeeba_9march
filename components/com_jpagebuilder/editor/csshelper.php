...
/**
 * CSS method used in the block.php file.
 *
 * @since 4.0.0
 */
class JpagebuilderCSSHelper extends JpagebuilderHelperBase {
	/**
	 * The default device
	 *
	 * @var string
	...
	/**
	 * The font registry array.
	 * Registering all the fonts here.
	 *
	 * @var array
	 * @since 4.0.0
	 */
	 ...
	 *        	The font name which will be stored to the registry.
	 *        	
	 * @return void
	 * @since 4.0.0
	 */
	...
	private function purifyTypographyObject($obj) {
		...
		if (! \is_object ( $obj )) {
			...
			$newObject = JpagebuilderAddonHelper::initDeviceObject ();
	...
	public function loadGoogleFont(string $fontName) {
		...
		if (! empty ( $fontName )) {
			if (! \in_array ( $fontName, $systemFonts )) {
				$fontPath = '/media/com_jpagebuilder/assets/google-fonts/' . $fontName . '/stylesheet.css';
				...
				if (file_exists ( JPATH_ROOT . $fontPath )) {
					$wa->registerAndUseStyle('jpagebuilder.font-' . StringHelper::str_ireplace(' ', '-', $fontName) . '-stylesheet', 'media/com_jpagebuilder/assets/google-fonts/' . $fontName . '/stylesheet.css');
				} else {
					...
					$wa->registerAndUseStyle('jpagebuilder.font-' . StringHelper::str_ireplace(' ', '-', $fontName), $linkTagUrl);
				}
			}
		}
	}
	public function loadLocalFont(string $fontName) {
		$fontPath = '/media/com_jpagebuilder/assets/custom-fonts/' . $fontName . '/stylesheet.css';

		if (file_exists ( JPATH_ROOT . $fontPath )) {
			$wa = Factory::getApplication ()->getDocument ()->getWebAssetManager();
			$wa->registerAndUseStyle('jpagebuilder.font-' . StringHelper::str_ireplace(' ', '-', $fontName) . '-stylesheet', 'media/com_jpagebuilder/assets/custom-fonts/' . $fontName . '/stylesheet.css');
		}
	}
	 ...
	 *        	The settings object.
	 * @param string $prop
	 *        	The settings prop or the typography field name.
	 ...
	 *
	 * @param mixed $color
	 *        	Object or string value of the color.
	 *        	
	 ...
	 *        	The settings object.
	 * @param string $prop
	 *        	The alignment prop name.
	 * @param bool $flex
	 *        	If the alignment for the flex or not.
	 *        	
	...
	public static function parseAlignment($settings, $prop, $flex = false) {
		...
		$alignmentMap = [ 
				'jpb-text-center' => 'center',
				'jpb-text-left' => 'left',
				'jpb-text-right' => 'right'
		];
	 ...
	 *        	The addon settings.
	 * @param string $prop
	 *        	The box-shadow settings key inside the $settings object.
	 * @param boolean $isTextShadow
	 *        	Data comes from text-shadow or box shadow.
	 *        	
	 ...
	 *
	 * @param string $selector
	 *        	The selector string where to apply the styles.
	 * @param \stdClass $settings
	 *        	The settings object.
	 * @param array $propMap
	 *        	The property map. The structure of the array is [prop => cssProp | [cssProps]]
	 * @param mixed $unit
	 *        	The unit array. The structure of the array is [prop => unitValue | false]
	 * @param array $modifier
	 *        	The modifier array. This array says if any modifier is needed to apply
	 ...
	 * @param mixed $default
	 *        	The default values. This would be useful if we don't want to
	 *        	use the $settings->$prop value as a result but want
	 *        	user specific value as a value.
	 * @param boolean $important
	 *        	The important specifier says if the property need
	 *        	to append !important; at the end or not.
	 * @param string $static
	 *        	Provided any static style value. If provided that would be contacted.
	 *        	
	...
	public function generateStyle(string $selector, $settings, $propMap, $unit = 'px', $modifier = [ ], $default = null, $important = false, $static = ''): string {
		...
		foreach ( $propMap as $prop => $cssProp ) {
			...
			if (self::hasMultiDeviceSettings ( $settings->$prop )) {
				...
				if (isset ( $modifier [$primitiveProp] ) && $modifier [$primitiveProp] === 'spacing') {
					$multiDeviceData = JpagebuilderAddonHelper::generateSpacingObject ( $settings, $prop, $cssProp, $device );
				} else {
					$multiDeviceData = JpagebuilderAddonHelper::generateMultiDeviceObject ( $settings, $prop, $cssProp, $device, $_important, $_unit );
				}
				...
				if (isset ( $settings->$prop )) {
					if (isset ( $modifier [$primitiveProp] ) && $modifier [$primitiveProp] === 'spacing') {
						$_object = JpagebuilderAddonHelper::generateSpacingObject ( $settings, $prop, $cssProp, $device );
					...
					if (isset ( $val ) && $val !== '') {
						foreach ( $cssProp as $attr ) {
							...
							if (! empty ( $_object ) && ! empty ( $_object->$device )) {
								...
								$str .= (strpos ( $attr, '%1$s' ) || strpos ( $attr, '%s' )) !== false ? \sprintf ( $attr, $this->extractValue ( $settings->$prop ) ?? '') : $attr . ': ' . $this->extractValue ( $settings->$prop ) ?? '';
							}
		...
		if (! empty ( $media )) {
			$mediaStyle = array_map ( function ($key) use ($selector, $media) {
				$str = JpagebuilderAddonHelper::mediaQuery ( $key );
				$str .= $selector . "{";
	 ...
	 *        	The settings object
	 * @param string $prop
	 *        	The filter settings key inside the $settings object
	 *        	
	 * @return string
	 */
	 ...
	 *        	The backdrop-filter settings key inside the $settings object.
	 * @param string $propertyValue
	 *        	The backdrop settings value.
	 * @since 4.0.8
	 * @return string
	 */
	 ...
	 *
	 * @param object $fieldWidth
	 *        	given breakpoints of field width
	 *        	
	 * @return object
	 *
	 * @since 4.0.9
	...
	public function __construct(string $id, bool $force = false) {
		...
		self::$device = JpagebuilderBase::$defaultDevice;
	}
}
