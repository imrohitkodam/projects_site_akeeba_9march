 ...
 * @since 4.0.0
 */
final class JpagebuilderLodashlib extends JpagebuilderHelperBase {
	/**
	 * Device Size
	 *
	 * @var array
	...
	private function loadFonts(string $data, $isFallbacks = false): string {
		...
		$installedGoogleFonts = JpagebuilderHelperSite::getInstalledGoogleFonts ();

		$css = '<# var jpb_head = window.frames["jpagebuilder-view"].window.document.head; #>';
		$css .= '<# var jpb_doc  = window.frames["jpagebuilder-view"].window.document;#>';
		$css .= '<# var jpb_fontFamily = ' . $content . ' #>';
		$css .= '<# const jpb_fontType = ' . $data . '?.type' . ' ?? "google" #>';
		...
		$css .= '<#
				...
				if (!_.isEmpty(jpb_fontFamily) && !systemFonts.includes(jpb_fontFamily) && !_.isEmpty(jpb_fontType) && (jpb_fontType !== "google" || (jpb_fontType === "google" && !disableGoogleFonts))){
					let linkTagIdPrefix = "jpagebuilder-google-font-";
				
					let fontLink = jpb_head.querySelector(
						`#${linkTagIdPrefix}${jpb_fontFamily.toLowerCase().replace(/\s+/g, "_")}`
					);
				
					linkTagLink = `' . Uri::root () . 'media/com_jpagebuilder/assets/google-fonts/${jpb_fontFamily}/stylesheet.css`;
							
					if(jpb_fontType === "google") {
						const isFontInstalled = installed_google_fonts?.includes(jpb_fontFamily);
							
						if(!isFontInstalled) {
							linkTagLink = `https://fonts.googleapis.com/css?family=${jpb_fontFamily}:100,100italic,200,200italic,300,300italic,400,400italic,500,500italic,600,600italic,700,700italic,800,800italic,900,900italic&display=swap`;
						}
					}
							
					if(jpb_fontType === "local") {
						linkTagIdPrefix = "jpagebuilder-local-font-";
						fontLink = jpb_head.querySelector(
							`#${linkTagIdPrefix}${jpb_fontFamily.toLowerCase().replace(/\s+/g, "_")}`
						);
							
						linkTagLink = `' . Uri::root () . 'media/com_jpagebuilder/assets/custom-fonts/${jpb_fontFamily}/stylesheet.css`;
					}
								
					if (!fontLink) {
						const fontLink = jpb_doc.createElement("link");
						fontLink.id = linkTagIdPrefix + jpb_fontFamily.toLowerCase().replace(/\s+/g, "_");
						fontLink.href = linkTagLink;
						fontLink.rel  = "stylesheet";
						fontLink.type = "text/css";
						jpb_head.appendChild(fontLink);
					}
				} #>';
	...
	public function alignment(string $property, string $selector, string $data): string {
		...
		if (isset ( $property ) && isset ( $selector ) && isset ( $data )) {
			...
			$css [] = $property . ': ' . $this->inlineBlock ( $data . '[window.builderDefaultDevice]' ) . ';';
			$css [] = '<# } else { #>';
			$css [] = '<# if ( ' . $data . '== "jpb-text-center" ||  ' . $data . '== "center" ) { #>';
			$css [] = $property . ': center;';
			$css [] = '<# } else if ( ' . $data . '== "jpb-text-right" || ' . $data . '== "right" ) { #>';
			$css [] = $property . ': right;';
			$css [] = '<# } else if ( ' . $data . '== "jpb-text-left" || ' . $data . '== "left") { #>';
			$css [] = $property . ': left;';
			$css [] = '<# } #>';
			$css [] = '<# } #>';
			$css [] = '}';
	...
	public function flexAlignment(string $selector, string $data): string {
		...
		$css [] = '<# } #>';
		$css [] = '<# } else {#>';
		$css [] = '<# if (' . $data . '== "left" || ' . $data . ' == "jpb-text-left")  { #>';
		$css [] = '-webkit-box-pack: start;';
		$css [] = '-ms-flex-pack: start;';
		$css [] = 'justify-content: flex-start;';
		$css [] = '<# } else if (' . $data . '== "right" || ' . $data . ' == "jpb-text-right") { #>';
		$css [] = '-webkit-box-pack: end;';
		$css [] = '-ms-flex-pack: end;';
		$css [] = 'justify-content: flex-end;';
		$css [] = '<# } else if (' . $data . '== "center" || ' . $data . ' == "jpb-text-center") { #>';
		$css [] = '-webkit-box-pack: center;';
		$css [] = '-ms-flex-pack: center;';
		$css [] = 'justify-content: center;';
		$css [] = '<# } #>';
