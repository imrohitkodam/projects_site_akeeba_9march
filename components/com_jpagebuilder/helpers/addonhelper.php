 ...
 * @since 4.0.0
 */
final class JpagebuilderAddonHelper {
	 ...
	 *
	 * @param string $type
	 *        	The type value. Accepted values are 'string', 'number', 'boolean'
	 *        	
	 * @return \stdClass
	 * @since 4.0.0
	 */
	 ...
	 *        	If the CSS property need to append the !important keyword.
	 * @param string $unit
	 *        	The unit of the CSS property.
	 *        	
	 * @return \stdClass
	 * @since 4.0.0
	 */
		...
		if (\is_object ( $value )) {
					...
					if (\is_object ( $settings->$prop->$key )) {
						...
						if (preg_replace ( "@\s+@", '', $_value ) !== '') {
							...
							foreach ( $cssProp as $css ) {
								$data->$key = strpos ( $css, '%s' ) !== false ? \sprintf ( $css, $_value ) : $css . ': ' . ($_value ?? '');
								$data->$key .= $_unit ?? '';
						...
						if (preg_replace ( "@\s+@", '', $settings->$prop->$key ) !== '') {
							foreach ( $cssProp as $css ) {
								$data->$key .= strpos ( $css, '%s' ) !== false ? \sprintf ( $css, $settings->$prop->$key ) : $css . ': ' . ($settings->$prop->$key ?? '');
			...
			if (isset ( $settings->$prop )) {
				if (preg_replace ( "@\s+@", '', $settings->$prop ) !== '') {
					foreach ( $cssProp as $css ) {
						$data->$device .= strpos ( $css, '%s' ) !== false ? \sprintf ( $css, $settings->$prop ) : $css . ': ' . ($settings->$prop ?? '');
	 ...
	 *        	The settings object.
	 * @param string $type
	 *        	The spacing type, i.e. padding or margin
	 * @param string $device
	 *        	The device.
	 *        	
	 * @return \stdClass
	 ...
	 *        	The fallback settings property.
	 *        	
	 * @return array
	 * @since 4.0.0
	 */
		...
		if (! empty ( $link->type )) {
			switch ($link->type) {
				case 'menu' :
					...
					break;
				case 'page' :
					! empty ( $link->page ) ? $url = Uri::root ( true ) . '/index.php?option=com_jpagebuilder&view=page&id=' . $link->page : $url = '';
					break;
				case 'url' :
					$url = $link->url;
					break;
