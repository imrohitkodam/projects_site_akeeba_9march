...
abstract class JpagebuilderHelperArticles {
	private static function hasChildren($parent_id = 1) {
		...
		$query->select('*')
			  ...
			  ->where($db->quoteName('language')." IN (" . $db->quote(Factory::getApplication()->getLanguage()->getTag()).", ".$db->quote('*') . ")")
	...
	public static function getArticles( $count = 5, $ordering = 'latest', $catid = '', $include_subcategories = true, $post_format = '', $tagids = array() ) {
		...
		if (is_array($tagids) && count($tagids)) {
			...
			if ($tagId) {
				...
				$subQuery->select('DISTINCT content_item_id')
		...
		foreach ($items as &$item) {
			...
			$item->username = Factory::getContainer()->get('user.factory')->loadUserById($item->created_by)->name;
			$item->link 	= Route::_( Joomla\Component\Content\Site\Helper\RouteHelper::getArticleRoute($item->slug, $item->catid, $item->language) );
			...
			if(isset($feature_img) && $feature_img != NULL) {
				...
				//Small
				$small = JPATH_ROOT . '/' . dirname($featured_image) . '/' . File::stripExt($img_baseurl) .  '_small.' . pathinfo($img_baseurl, PATHINFO_EXTENSION);
				if(file_exists($small)) {
					$item->image_small = Uri::root(true) . '/' . dirname($featured_image) . '/' . File::stripExt($img_baseurl) . '_small.' . pathinfo($img_baseurl, PATHINFO_EXTENSION);
				}

				//Thumb
				$thumbnail = JPATH_ROOT . '/' . dirname($featured_image) . '/' . File::stripExt($img_baseurl) .  '_thumbnail.' . pathinfo($img_baseurl, PATHINFO_EXTENSION);
				if(file_exists($thumbnail)) {
					$item->image_thumbnail = Uri::root(true) . '/' . dirname($featured_image) . '/' . File::stripExt($img_baseurl) . '_thumbnail.' . pathinfo($img_baseurl, PATHINFO_EXTENSION);
				} else {
					$item->image_thumbnail = Uri::root(true) . '/' . $item->featured_image;
				}

				//Medium
				$medium = JPATH_ROOT . '/' . dirname($featured_image) . '/' . File::stripExt($img_baseurl) .  '_medium.' . pathinfo($img_baseurl, PATHINFO_EXTENSION);
				if(file_exists($medium)) {
					$item->image_medium = Uri::root(true) . '/' . dirname($featured_image) . '/' . File::stripExt($img_baseurl) . '_medium.' . pathinfo($img_baseurl, PATHINFO_EXTENSION);
				}

				//Large
				$large = JPATH_ROOT . '/' . dirname($featured_image) . '/' . File::stripExt($img_baseurl) .  '_large.' . pathinfo($img_baseurl, PATHINFO_EXTENSION);
				if(file_exists($large)) {
					$item->image_large = Uri::root(true) . '/' . dirname($featured_image) . '/' . File::stripExt($img_baseurl) . '_large.' . pathinfo($img_baseurl, PATHINFO_EXTENSION);
				}
			} else {
				...
				if(isset($gallery_imgs) && $gallery_imgs != NULL) {
					...
					if(isset($gallery_all_images) && is_array($gallery_all_images)){
						foreach ($gallery_all_images as $key=>$value) {
							...
							//Small
							$small = JPATH_ROOT . '/' . dirname($value) . '/' . File::stripExt($gallery_img_baseurl) .  '_small.' . pathinfo($gallery_img_baseurl, PATHINFO_EXTENSION);
							if(file_exists($small)) {
								$gallery_images[$key]['small'] = Uri::root(true) . '/' . dirname($value) . '/' . File::stripExt($gallery_img_baseurl) . '_small.' . pathinfo($gallery_img_baseurl, PATHINFO_EXTENSION);
							}

							//Thumbnail
							$thumbnail = JPATH_ROOT . '/' . dirname($value) . '/' . File::stripExt($gallery_img_baseurl) .  '_thumbnail.' . pathinfo($gallery_img_baseurl, PATHINFO_EXTENSION);
							if(file_exists($thumbnail)) {
								$gallery_images[$key]['thumbnail'] = Uri::root(true) . '/' . dirname($value) . '/' . File::stripExt($gallery_img_baseurl) . '_thumbnail.' . pathinfo($gallery_img_baseurl, PATHINFO_EXTENSION);
							}

							//Medium
							$medium = JPATH_ROOT . '/' . dirname($value) . '/' . File::stripExt($gallery_img_baseurl) .  '_medium.' . pathinfo($gallery_img_baseurl, PATHINFO_EXTENSION);
							if(file_exists($medium)) {
								$gallery_images[$key]['medium'] = Uri::root(true) . '/' . dirname($value) . '/' . File::stripExt($gallery_img_baseurl) . '_medium.' . pathinfo($gallery_img_baseurl, PATHINFO_EXTENSION);
							}

							//Large
							$large = JPATH_ROOT . '/' . dirname($value) . '/' . File::stripExt($gallery_img_baseurl) .  '_large.' . pathinfo($gallery_img_baseurl, PATHINFO_EXTENSION);
							if(file_exists($large)) {
								$gallery_images[$key]['large'] = Uri::root(true) . '/' . dirname($value) . '/' . File::stripExt($gallery_img_baseurl) . '_large.' . pathinfo($gallery_img_baseurl, PATHINFO_EXTENSION);
							}
						}
					}
