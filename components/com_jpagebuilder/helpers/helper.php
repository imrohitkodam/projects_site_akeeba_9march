...
class JpagebuilderHelperSite {
	...
	/**
	 * Load the addon structure from the config files.
	 *
	 * @return void
	 */
	private static function prepareAddonFieldStructures() {
		...
		foreach ( $addons as &$addon ) {
			$addon = JpagebuilderAddonsHelper::modernizeAddonStructure ( $addon );
			$addon ['addon_name'] = preg_replace ( '/^jpagebuilder_/i', '', $addon ['addon_name'] );
	...
	private static function getTemplate() {
		...
		$query->select ( 'template' )->from ( $db->quoteName ( '#__template_styles' ) )->where ( $db->quoteName ( 'client_id' ) . ' = 0' )->where ( $db->quoteName ( 'home' ) . ' = 1' );
	 ...
	 *
	 * @param int $size
	 *        	The size value ranged from 1 to 12.
	 *        	
		...
		if (! empty ( $row->columns )) {
			...
			foreach ( $row->columns as $i => &$column ) {
				...
				if (isset ( $column->settings )) {
					$column->settings = self::shiftResponsiveSettings ( $column->settings );
				}
				...
				if (! empty ( $column->addons )) {
					foreach ( $column->addons as $j => &$addon ) {
						...
						if (isset ( $addon->name ) && \in_array ( $addon->name, [ 
							...
							$repeatableKey = 'jp_' . $addon->name . '_item';
						...
						if (isset ( $addon->name ) && \in_array ( $addon->name, [ 
							...
							list ( $outerRows, $_addon ) = self::migrateDeepAddon ( $addon, 'jp_' . $addon->name . '_item', $row, $column );
							$addon = $_addon;
						...
						if (isset ( $addon->name ) && $addon->name === 'table_advanced') {
							...
							if (isset ( $addon->settings->jp_table_advanced_item )) {
								foreach ( $addon->settings->jp_table_advanced_item as $th => $thead ) {
									if (isset ( $thead->content ) && ! \is_array ( $thead->content )) {
										...
										$addon->settings->jp_table_advanced_item [$th]->content = [ ];
										$addon->settings->jp_table_advanced_item [$th]->content [] = $thead;
										...
										$addon->settings->jp_table_advanced_item [$th]->content = $contents;
									}
								}
							}
								...
								if (isset ( $tRow->table_advanced_item )) {
									foreach ( $tRow->table_advanced_item as $c => $tCell ) {
										if (isset ( $tCell->content ) && ! \is_array ( $tCell->content )) {
											...
											$addon->settings->table_advanced_item [$r]->table_advanced_item [$c]->content = [ ];
											$addon->settings->table_advanced_item [$r]->table_advanced_item [$c]->content [] = $td;
											...
											$addon->settings->table_advanced_item [$r]->table_advanced_item [$c]->content = $contents;
	...
	private static $defaultValueWithUnit = [ 
			'value' => '',
			'unit' => 'px'
	];
	...
	private static function reshapeSpacingValues($spacing, $forceModern = false) {
		...
		switch (count ( $spacing )) {
			case 0 :
				...
				break;
			case 1 :
				$newSpacing = $isLegacy ? array_fill ( 0, 4, $spacing [0] ) : array_replace ( $reshapedValue, $spacing );
				break;
			case 2 :
				$newSpacing = $isLegacy ? [ 
						$spacing [0],
	...
	private static function traverseSettingsToFixSpacingAnomalies($settings, $fieldTypes, $forceModern = false) {
		...
		if (! empty ( $settings )) {
			foreach ( $settings as $key => &$value ) {
				...
				if ($fieldType === null || (is_string ( $fieldType ) && ! in_array ( $fieldType, [ 
						'padding',
						'margin'
				] ))) {
					continue;
	...
	private static function parsingSpacingValues($settings, $type, $addonName = '') {
		...
		return self::traverseSettingsToFixSpacingAnomalies ( $settings, $fieldTypes, in_array ( $type, [ 
				'section',
				'column'
		] ) );
	}
	 ...
	 *
	 * @param string $text
	 *        	The json text of the page builder data.
	 *        	
				...
				if (! empty ( $row->columns )) {
					...
					foreach ( $row->columns as $i => &$column ) {
						...
						if (isset ( $rows [$key]->columns [$i]->settings )) {
							$rows [$key]->columns [$i]->settings = self::shiftResponsiveSettings ( $rows [$key]->columns [$i]->settings );
						}

						if (! empty ( $column->addons )) {
							foreach ( $column->addons as $j => &$addon ) {
								if (isset ( $rows [$key]->columns [$i]->addons [$j]->settings )) {
									$rows [$key]->columns [$i]->addons [$j]->settings = self::shiftResponsiveSettings ( $rows [$key]->columns [$i]->addons [$j]->settings );
								...
								if (isset ( $addon->name ) && \in_array ( $addon->name, [ 
									...
									list ( $outerRows, $addon ) = self::migrateDeepAddon ( $addon, 'jp_' . $addon->name . '_item', $row, $column );
									...
									if (isset ( $addon->settings->jp_table_advanced_item )) {
										foreach ( $addon->settings->jp_table_advanced_item as $th => $thead ) {
											if (isset ( $thead->content ) && ! \is_array ( $thead->content )) {
												...
												$thead = \json_decode ( \json_encode ( $thead ) );
												$rows [$key]->columns [$i]->addons [$j]->settings->jp_table_advanced_item [$th]->content = [ ];
												$rows [$key]->columns [$i]->addons [$j]->settings->jp_table_advanced_item [$th]->content [] = $thead;
											} elseif (isset ( $thead->content ) && \is_array ( $thead->content )) {
												$contents = [ ];
												...
												$rows [$key]->columns [$i]->addons [$j]->settings->jp_table_advanced_item [$th]->content = $contents;
											}
										}
									}
										...
										if (isset ( $tRow->table_advanced_item )) {
											foreach ( $tRow->table_advanced_item as $c => $tCell ) {
												if (isset ( $tCell->content ) && ! \is_array ( $tCell->content )) {
													...
													$rows [$key]->columns [$i]->addons [$j]->settings->table_advanced_item [$r]->table_advanced_item [$c]->content = [ ];
													$rows [$key]->columns [$i]->addons [$j]->settings->table_advanced_item [$r]->table_advanced_item [$c]->content [] = $td;
												} elseif (isset ( $tCell->content ) && \is_array ( $tCell->content )) {
													$contents = [ ];
													...
													$rows [$key]->columns [$i]->addons [$j]->settings->table_advanced_item [$r]->table_advanced_item [$c]->content = $contents;
	...
	public static function createRow(string $layout = '12', array $addons = [ ], $parent = null) {
		...
		$rowDefaultValues = JpagebuilderEditorUtils::getSectionSettingsDefaultValues ();
		$columnDefaultValues = JpagebuilderEditorUtils::getColumnSettingsDefaultValues ();
	...
	/**
	 * Remove jp_ from the addon name
	 *
	 * @return void
	 * @since 4.0.0
	 */
	...
	public static function loadLanguage($forceLoad = false) {
		...
		if (($option == 'com_jpagebuilder' && $view == 'form' && $id) || $forceLoad) {
			$lang->load ( 'com_jpagebuilder', JPATH_ADMINISTRATOR, null, true );
		}

		// Load template language file
		$lang->load ( 'tpl_' . $template, JPATH_SITE, null, true );

		if (($option === 'com_jpagebuilder' && $view !== 'page')) {
			require_once JPATH_ROOT . '/administrator/components/com_jpagebuilder/helpers/language.php';
		}
	}
	...
	public static function getSvgShapes() {
		$shape_path = JPATH_ROOT . '/components/com_jpagebuilder/assets/shapes';
	...
	public static function getSvgShapeCode($shapeName, $invert) {
		if ($invert) {
			$shape_path = JPATH_ROOT . '/components/com_jpagebuilder/assets/shapes/' . $shapeName . '-invert.svg';
		} else {
			$shape_path = JPATH_ROOT . '/components/com_jpagebuilder/assets/shapes/' . $shapeName . '.svg';
		}
	...
	public static function getPrettyText($sections) {
		if (! class_exists ( 'JpagebuilderAddonParser' )) {
			require_once JPATH_ROOT . '/components/com_jpagebuilder/editor/addonparser.php';
		}
		if (! class_exists ( 'JpagebuilderAddon' )) {
			require_once JPATH_ROOT . '/components/com_jpagebuilder/editor/classes/addon.php';
		}

		$content = JpagebuilderApplicationHelper::sanitizePageText ( $sections );
		$htmlContent = JpagebuilderAddonParser::viewAddons ( $content );
	...
	public static function addContainerMaxWidth() {
		...
		$wa->addInlineStyle( "@media(min-width: 1400px) {.jpb-row-container { max-width: " . $containerMaxWidth . "px; }}" );
	}
	...
	public static function getTemplateName() {
		...
		$query->select ( $db->quoteName ( [ 
				'template'
		] ) )->from ( $db->quoteName ( '#__template_styles' ) )->where ( $db->quoteName ( 'client_id' ) . ' = 0' )->where ( $db->quoteName ( 'home' ) . ' = 1' );
		$db->setQuery ( $query );
	...
	public static function getInstalledGoogleFonts() {
		...
		$query->select ( 'family_name' )->from ( $db->quoteName ( '#__jpagebuilder_fonts' ) )->where ( $db->quoteName ( 'type' ) . ' = ' . $db->quote ( 'google' ) );
	...
	public static function hasMultiDeviceSettings($value): bool {
		return isset ( $value->xl ) || isset ( $value->lg ) || isset ( $value->md ) || isset ( $value->sm ) || isset ( $value->xs );
	}
