...
if (! class_exists ( 'JpagebuilderCssParser' )) {
	require_once JPATH_ROOT . '/components/com_jpagebuilder/helpers/cssparser.php';
}

$selector_css = new FileLayout ( 'addon.css.selector', JPATH_ROOT . '/components/com_jpagebuilder/layouts' );
...
$addon_id = "#jpb-addon-" . $addon->id;
$addon_wrapper_id = "#jpb-addon-wrapper-" . $addon->id;
...
$units = [ 
		'global_text_color' => false,
		'global_border_color' => false,
		'global_boder_style' => false,
		'global_padding' => false
];
$modifier = [ 
		'global_padding' => 'spacing'
];
...
$settings->global_boxshadow = JpagebuilderCSSHelper::parseBoxShadow ( $settings, 'global_boxshadow' );

$boxShadowSelector = isset ( $addon->name ) && ($addon->name === 'button' || $addon->name === 'button_group') ? '.jpb-btn' : ':self';

$boxShadowStyle = $cssHelper->generateStyle ( $boxShadowSelector, $settings, [ 
		'global_boxshadow' => 'box-shadow'
], false );
...
if ($addon_css) {
	$inlineCSS .= $addon_id . " {\n" . $addon_css . "}\n";
	$inlineCSS .= $addon_id . " {\n" . $placeholder_bg_image . "}\n";
	$inlineCSS .= $addon_id . ".jpb-element-loaded {\n" . $lazy_bg_image . "}\n";
}
...
if (isset ( $settings->global_use_overlay ) && $settings->global_use_overlay && isset ( $settings->global_background_overlay ) && $settings->global_background_overlay && $settings->global_overlay_type === 'overlay_color') {
	$inlineCSS .= $addon_id . " .jpb-addon-overlayer { background-color: {$settings->global_background_overlay}; }\n";
}

if (isset ( $settings->global_use_overlay ) && $settings->global_use_overlay) {
	$inlineCSS .= $addon_id . " > .jpb-addon { position: relative; }\n";
}
	...
	if ($settings->global_background_type == 'image') {
		if (isset ( $settings->global_gradient_overlay ) && $settings->global_overlay_type == 'overlay_gradient') {
			...
			if (isset ( $settings->global_gradient_overlay->type ) && $settings->global_gradient_overlay->type == 'radial') {
				$inlineCSS .= $addon_id . ' .jpb-addon-overlayer {
...
if ($addon_link_hover_css) {
	$inlineCSS .= $addon_id . " a:hover,\n#jpb-addon-" . $addon->id . " a:focus,\n#jpb-addon-" . $addon->id . " a:active {\n" . $addon_link_hover_css . "}\n";
}
...
$titleStyle = $cssHelper->generateStyle ( '.jpb-addon-title', $settings, $propsMap, $units );
...
if (! empty ( $settings->title )) {
	...
	if ($addon->name !== $heading_addon_name) {
		$typography = $cssHelper->typography ( '.jpb-addon-title', $settings, 'title_typography', $typographyFallbacks );
		$titleStyle .= "\r\n" . $typography;
	}
}
...
// Selector
$inlineCSS .= $selector_css->render ( array (
		'options' => $settings,
		'addon_id' => $addon_id
) );
...
if (class_exists ( 'JpagebuilderCssParser' ) && isset ( $settings->global_custom_css ) && ! empty ( $settings->global_custom_css )) {
	if ($addon->name === "popover") {
		...
		// ["h3 {...", "h3 {..."] -> "#jpb-addon-{$addon['id']} h3 {...} #jpb-addon-{$addon['id']} h3 {...}"
	...
	$inlineCSS .= JpagebuilderCssParser::getCss ( $addon->name, $global_custom_css, $addon_id, $addon_wrapper_id );
}
