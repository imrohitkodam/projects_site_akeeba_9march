...
{
	$options->button_background_gradient = JpagebuilderCSSHelper::parseColor($options, 'button_background_gradient');
	$customProps = [
		'button_background_gradient' => 'border:none; background-image'
	];
	$customUnits = ['button_background_gradient' => false];

	$options->button_background_gradient_hover = JpagebuilderCSSHelper::parseColor($options, 'button_background_gradient_hover');
...
$buttonStyle = $cssHelper->generateStyle('#' . $id . '.jpb-btn-' . $btn_style, $options, $buttonProps, $buttonUnits);
$customStyle = $cssHelper->generateStyle('#' . $id . '.jpb-btn-custom', $options, $customProps, $customUnits, ['button_padding' => 'spacing']);
$hoverStyle = $cssHelper->generateStyle('#' . $id . '.jpb-btn-custom:hover', $options, $hoverProps, $hoverUnits);
...
$buttonTypography = $cssHelper->typography('#' . $id . '.jpb-btn-' . $btn_style, $options, 'button_typography', $fallback);
...
$linkHoverStyle = $cssHelper->generateStyle(
	' #' . $id . '.jpb-btn-link:hover, ' . '#' . $id . '.jpb-btn-link:focus',
	$options,
	[
