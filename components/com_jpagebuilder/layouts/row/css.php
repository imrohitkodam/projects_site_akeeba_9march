...
$rowIdSelector = '.jpagebuilder .page-content #' . $row_id;
$cssHelper = new JpagebuilderCSSHelper($rowIdSelector, true);
...
$cssHelper->setID($rowIdSelector . '.jpb-row-overlay', true);
...
$deviceList = array_filter(JpagebuilderAddonHelper::$deviceList, function ($size) {
	return $size !== JpagebuilderBase::$defaultDevice;
});
...
if (property_exists($options, 'background_type') && isset($options->background_type)) {
	...
	if (($options->background_type == 'image' && $background_image_src) || (isset($options->background_type) && $options->background_type == 'video')) {
		if ($lazyload) {
			...
			if (strpos($background_image_src, "http://") !== false || strpos($background_image_src, "https://") !== false) {
				...
				$lazy_bg_image .= 'background-image:url(' . JpagebuilderHelperSite::cleanPath($originalSrc) . ');';
			}
		} else {
			if (strpos($background_image_src, "http://") !== false || strpos($background_image_src, "https://") !== false) {
				...
				$style .= 'background-image:url(' . JpagebuilderHelperSite::cleanPath($original_src) . ');';
			}
		}
		...
		if (isset($options->background_size) && $options->background_size == 'custom') {
			if (isset($options->background_size_custom) && \is_object($options->background_size_custom)) {
				$backgroundSize = JpagebuilderAddonHelper::generateMultiDeviceObject($options, 'background_size_custom', 'background-size', $device, false, ($options->background_size_custom->unit ?? 'px'));
	...
	if (isset($options->background_position) && $options->background_position == 'custom') {
		$backgroundPosition = JpagebuilderAddonHelper::initDeviceObject();
	...
	if ($background_image_src || (isset($options->background_type) && $options->background_type == 'video')) {
		if ($lazyload) {
			...
			if (strpos($background_image_src, "http://") !== false || strpos($background_image_src, "https://") !== false) {
				...
				$original_src = Uri::base(true) . '/' . $background_image_src;
				$lazy_bg_image .= 'background-image:url(' .  JpagebuilderHelperSite::cleanPath($original_src) . ');';
			}
		} else {
			if (strpos($background_image_src, "http://") !== false || strpos($background_image_src, "https://") !== false) {
				$style .= 'background-image:url(' . $background_image_src . ');';
	...
	if ($style) {
		$row_styles .= '.jpagebuilder .page-content #' . $row_id . '{' . $style . '}';
	}
}
...
$mediaStyle = array_map(function ($size) use (
	...
	$str = '';
	$str .= JpagebuilderAddonHelper::mediaQuery($size);
...
if ($styleX) {
	$row_styles .= '.jpagebuilder .page-content #' . $row_id . '{' . $styleX . '}';
}
$mediaStyle = implode("\n", $mediaStyle);
...
if ($style) {
	$row_styles .= '.jpagebuilder .page-content #' . $row_id . '{' . $style . '}';
	$row_styles .= '.jpagebuilder .page-content #' . $row_id . '{' . $placeholder_bg_image . '}';
	$row_styles .= '.jpagebuilder .page-content #' . $row_id . '.jpb-element-loaded {' . $lazy_bg_image . '}';
}
...
if (isset($options->background_type)) {
	if ($options->background_type == 'image' || $options->background_type == 'video') {
		...
		if (isset($options->overlay) && $options->overlay && $options->overlay_type == 'overlay_color') {
			$row_styles .= '.jpagebuilder .page-content #' . $row_id . ' > .jpb-row-overlay {background-color: ' . $options->overlay . '}';
		}
		if (isset($options->gradient_overlay) && $options->gradient_overlay && $options->overlay_type == 'overlay_gradient') {
			...
			if (isset($options->gradient_overlay->type) && $options->gradient_overlay->type == 'radial') {
				$row_styles .= '.jpagebuilder .page-content #' . $row_id . ' > .jpb-row-overlay {
		...
		if (isset($options->pattern_overlay) && $options->pattern_overlay && $options->overlay_type == 'overlay_pattern') {
			if (is_object($options->pattern_overlay)) {
				if (strpos($options->pattern_overlay->src, "http://") !== false || strpos($options->pattern_overlay->src, "https://") !== false) {
					...
					$row_styles .= '.jpagebuilder .page-content #' . $row_id . ' > .jpb-row-overlay {
							background-image:url(' . JpagebuilderHelperSite::cleanPath($original_src) . ');
							background-attachment: scroll;
						}';
					if (isset($options->overlay_pattern_color)) {
						$row_styles .= '.jpagebuilder .page-content #' . $row_id . ' > .jpb-row-overlay {
...
if ((isset($options->title) && $options->title) || (isset($options->subtitle) && $options->subtitle)) {
	if (!empty($options->title)) {
		...
		if (!empty($options->title_fontsize)) {
			$titleFontSize = JpagebuilderAddonHelper::generateMultiDeviceObject($options, 'title_fontsize', ['font-size', 'line-height'], $device);
			$titleStyle .= $titleFontSize->$device;
		}

		if (!empty($options->title_position)) {
			$titleAlignment = JpagebuilderCSSHelper::parseAlignment($options, 'title_position');
		...
		$titleMarginTop = JpagebuilderAddonHelper::generateMultiDeviceObject($options, 'title_margin_top', 'margin-top', $device);
		$titleMarginBottom = JpagebuilderAddonHelper::generateMultiDeviceObject($options, 'title_margin_bottom', 'margin-bottom', $device);

		$titleStyle .= $titleMarginTop->$device;
		$titleStyle .= $titleMarginBottom->$device;

		$titleSelector = '.jpagebuilder .page-content #' . $row_id . ' .jpb-section-title .jpb-title-heading';
		...
		$row_styles .= !empty($titleStyle) ? '.jpagebuilder .page-content #' . $row_id . ' .jpb-section-title .jpb-title-heading {' . $titleStyle . '}' : '';
	...
	if (!empty($options->subtitle)) {
		...
		$subtitleFontSize = JpagebuilderAddonHelper::generateMultiDeviceObject($options, 'subtitle_fontsize', 'font-size', $device);
		$subtitleSelector = '.jpagebuilder .page-content #' . $row_id . ' .jpb-section-title .jpb-title-subheading';
		...
		$row_styles .= '.jpagebuilder .page-content #' . $row_id . ' .jpb-section-title .jpb-title-subheading {' . $subtitleFontSize->$device . '}';
		$row_styles .= '.jpagebuilder .page-content #' . $row_id . ' .jpb-section-title .jpb-title-subheading {' . $subTitleStyle . '}';
...
if (isset($options->fullscreen) && !$options->fullscreen && isset($options->container_width) && is_object($options->container_width) && isset($options->container_width->md) && $options->container_width->md != '') {
	$row_styles .= '@media (min-width: 1400px) {#' . $row_id . ' > .jpb-row-container { max-width:' . $options->container_width->md . 'px;}}';
} else if (isset($options->fullscreen) && !$options->fullscreen && isset($options->container_width) && !is_object($options->container_width)) {
	$row_styles .= '@media (min-width: 1400px) {#' . $row_id . ' > .jpb-row-container { max-width:' . $options->container_width . 'px;}}';
}
...
if (property_exists($options, 'no_gutter') && !$options->no_gutter) {
	if (!$options->no_gutter && isset($options->columns_gap) && is_object($options->columns_gap)) {
		$columnsGapMargin = JpagebuilderAddonHelper::initDeviceObject();
		$columnsGapPadding = JpagebuilderAddonHelper::initDeviceObject();
		...
		$rowContainerClass = (isset($options->fullscreen) && boolval($options->fullscreen)) ? ".jpb-container-inner" : ".jpb-row-container";
		$marginSelector = '#' . $row_id . ' > ' . $rowContainerClass . ' > .jpb-row ';
		$paddingSelector = '#' . $row_id . ' >  ' . $rowContainerClass . ' > .jpb-row > div';
		...
		// nested row
		$marginSelectorNested = '#' . $row_id . ' > .jpb-container-inner > .jpb-nested-row';
		$paddingSelectorNested = '#' . $row_id . ' > .jpb-container-inner > .jpb-nested-row > div';
