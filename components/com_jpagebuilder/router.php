...
class JpagebuilderRouterBase {
	public static function buildRoute(&$query) {
		...
		// Check again
		if ($menuItemGiven && isset ( $menuItem ) && $menuItem->component != 'com_jpagebuilder') {
			$menuItemGiven = false;
			unset ( $query ['Itemid'] );
		}
...
if(version_compare(JVERSION, '4', '>=')) {
	class JpagebuilderRouter extends Joomla\CMS\Component\Router\RouterBase {
		public function build(&$query) {
			$segments = JpagebuilderRouterBase::buildRoute ( $query );
			return $segments;
		}
		public function parse(&$segments) {
			$vars = JpagebuilderRouterBase::parseRoute ( $segments );
 ...
 */
function JpagebuilderBuildRoute(&$query) {
	$segments = JpagebuilderRouterBase::buildRoute ( $query );
 ...
 * @deprecated 4.0 Use Class based routers instead
 */
function JpagebuilderParseRoute(&$segments) {
	$vars = JpagebuilderRouterBase::parseRoute ( $segments );
