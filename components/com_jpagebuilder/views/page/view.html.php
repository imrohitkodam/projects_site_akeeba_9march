...
class JpagebuilderViewPage extends HtmlView {
	...
	function display($tpl = null) {
		...
		$this->canEdit = $user->authorise ( 'core.edit', 'com_jpagebuilder' ) || $user->authorise ( 'core.edit', 'com_jpagebuilder.page.' . $this->item->id ) || ($user->authorise ( 'core.edit.own', 'com_jpagebuilder.page.' . $this->item->id ) && $this->item->created_by == $user->id);
	...
	protected function _prepareDocument($title = '') {
		...
		$config_params = ComponentHelper::getParams ( 'com_jpagebuilder' );
		...
		$language = ($this->item->language == '*') ? Factory::getApplication ()->getLanguage ()->getTag () : $this->item->language;

		$doc->addCustomTag ( '<meta property="article:author" content="' . $this->item->author_name . '"/>' );
		$doc->addCustomTag ( '<meta property="article:published_time" content="' . $this->item->created_on . '"/>' );
		$doc->addCustomTag ( '<meta property="article:modified_time" content="' . $this->item->modified . '"/>' );
		...
		if (! $disable_og) {
			...
			$doc->addCustomTag ( '<meta property="og:url" content="' . Uri::getInstance ()->toString () . '" />' );

			if ($fb_app_id = $config_params->get ( 'fb_app_id', '' )) {
				$doc->addCustomTag ( '<meta property="fb:app_id" content="' . $fb_app_id . '" />' );
			}
		...
		if (! $disable_og && $og_image) {
			$doc->addCustomTag ( '<meta property="og:image" content="' . Uri::root () . $og_image . '" />' );
			$doc->addCustomTag ( '<meta property="og:image:width" content="1200" />' );
			$doc->addCustomTag ( '<meta property="og:image:height" content="630" />' );
		}

		$og_description = $this->item->og_description;
		if (! $disable_og && $og_description) {
			$doc->addCustomTag ( '<meta property="og:description" content="' . $og_description . '" />' );
		}
		...
		if (! $disable_tc) {
			...
			if ($config->get ( 'sitename', '' )) {
				$doc->addCustomTag ( '<meta name="twitter:site" content="' . htmlspecialchars ( $config->get ( 'sitename', '' ) ) . '" />' );
			}

			if ($og_description) {
				$doc->addCustomTag ( '<meta name="twitter:description" content="' . $og_description . '" />' );
			}

			if ($og_image) {
				$doc->addCustomTag ( '<meta name="twitter:image:src" content="' . Uri::root () . $og_image . '" />' );
			}
		}

		$meta_title = (isset ( $attribs->meta_title ) && $attribs->meta_title) ? $attribs->meta_title : '';
		...
		if ($menu) {
			if ($menu->getParams ()->get ( 'page_title' )) {
				$meta_title = $menu->getParams ()->get ( 'page_title' );
			}
		...
		if (! empty ( $meta_title )) {
			$this->setDocumentTitle($meta_title);
		}
