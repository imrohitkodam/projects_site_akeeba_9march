function qtc_place_order()
{
	...
	var callurl = Joomla.getOptions('system.paths').base+ "/index.php?option=com_quick2cart&task=createorder.qtc_place_order&tmpl=component";
	...
	techjoomla.jQuery.ajax({
		...
		success: function(data)
		{
			...
			{
				alert(result.status.charAt(0).toUpperCase() + result.status.slice(1)+" : "+result.message);
				location.reload();
			}
		}
	});
...
function qtc_update_product_price(prod_id, container_number)
{
	...
	var callurl = Joomla.getOptions('system.paths').base+ "/index.php?option=com_quick2cart&task=createorder.qtc_update_product_price&tmpl=component&prod_id="+prod_id+"&container_number="+container_number;
			...
			if ('1' in prod_info)
			{
				...
				for (var key in prod_info[1])
				{
					if (prod_info[1].hasOwnProperty(key))
					{
						...
						else if (prod_info[1][key].itemattributeoption_prefix == "-")
						{
							optionPrice = Number(optionPrice) - Number(prod_info[1][key].optionprice);
						}
					}
...
{
	var values  = techjoomla.jQuery('#createOrder').serialize();
	var callurl = Joomla.getOptions('system.paths').base+ "/index.php?option=com_quick2cart&task=createorder.qtc_update_order_price&tmpl=component";
...
function nextstep()
{
	...
	if (result)
	{
		...
		techjoomla.jQuery('#select-customer-tab').removeClass('active');
		techjoomla.jQuery('#select-product-tab').addClass('active');
...
function validateselectclient()
{
	...
	{
		Joomla.renderMessages({ 'error': [Joomla.JText._('COM_QUICK2CART_SELECT_CLIENT_ERROR_MSG')] });
		return false;
	}

	if (jQuery('input[name="shipping_address"]').length && (!jQuery('input[name="shipping_address"]:checked').val()))
	{
		Joomla.renderMessages({ 'error': [Joomla.JText._('COM_QUICK2CART_SELECT_SHIPPING_ADDRESS_ERROR_MSG')] });
		return false;
	}

	if(!jQuery('input[name="billing_address"]:checked').val())
	{
		Joomla.renderMessages({ 'error': [Joomla.JText._('COM_QUICK2CART_SELECT_BILLING_ADDRESS_ERROR_MSG')] });
		return false;
	}
...
function addClient(baseUrl, modalId = 'addClientModal')
{
...
function showAddresses()
{
	...
	var callurl = Joomla.getOptions('system.paths').base+ "/index.php?option=com_quick2cart&task=customer_addressform.getUserAddressList&tmpl=component&uid="+userId;
	...
	techjoomla.jQuery.ajax({
		...
		success: function(data){
			...
			techjoomla.jQuery('#system-message-container').empty();
...
function addAddress(baseUrl, modalId = 'addAddressModal')
{
	var userId      = techjoomla.jQuery("#qtcuser option:selected").val();
	var getModalUrl = jQuery('#' + modalId).attr('data-url');

	/*Update Modal Url append userid*/
	jQuery('#' + modalId).attr('data-url' , getModalUrl + userId);
	jQuery('#' + modalId).attr('data-width' , (window.innerWidth)/2);
	jQuery('#' + modalId).attr('data-height' , window.innerHeight);
	jQuery("#" + modalId).modal('show');
	jQuery('#' + modalId).attr('style' , 'display:block !important');
}

/* Function to open form for updating selected address for selected client*/
function editAddress(modalId = 'editAddressModal_', id)
{
	var userId = techjoomla.jQuery("#qtcuser option:selected").val();
	var getModalUrl = qtc_base_url+"index.php?option=com_quick2cart&view=customer_addressform&tmpl=component&id="+id+"&userid="+userId;
	...
	techjoomla.jQuery.ajax({
		...
		{
			jQuery('#' + modalId + id + " .modal-dialog").addClass('modal-dialog-scrollable');
			jQuery('#' + modalId + id + " .modal-content .modal-body").html(response);
		}
	});
}

/* Function to delete selected address for selected client*/
function deleteAddress(id, msg = '')
{
	var deleteFlag = confirm(deleteAddressMessage);

	if (deleteFlag == 1)
	{
		var callurl = Joomla.getOptions('system.paths').base+ "/index.php?option=com_quick2cart&task=customer_addressform.delete&tmpl=component&addressId="+id;
		...
		techjoomla.jQuery.ajax({
			...
			success: function(data){
				if (data == '1')
				{
					...
					setTimeout(function() {
						alert(msg ? msg : Joomla.JText._('COM_QUICK2CART_CUSTOMER_ADDRESS_DELETE_MSG'));
					}, 500);
				}
				else 
				{
					alert(Joomla.JText._('QTC_JERROR_ALERTNOAUTHORTODELETE'));
				}
			}
		});
	}
...
function qtcpopulateProducts(prod_container_num)
{
	...
	if (storeId != '')
	{
		var callurl = Joomla.getOptions('system.paths').base+ "/index.php?option=com_quick2cart&task=stores.getAllProductsFromStore&tmpl=component&storeId="+storeId;
		...
		techjoomla.jQuery.ajax({
			...
			success: function(data){
				...
				jQuery('#qtcorder_productdetails'+prod_container_num).trigger("chosen:updated");
				closeModal();
			}
		});
	}
...
function checklimit(prod_container_num,price,currency)
{
	...
	var quantity  = parseInt(techjoomla.jQuery('#qtc_prod_quantity'+prod_container_num).val());
	var min_val   = parseInt(techjoomla.jQuery('#qtc_prod_quantity'+prod_container_num).attr("min"));
	var max_val   = parseInt(techjoomla.jQuery('#qtc_prod_quantity'+prod_container_num).attr("max"));
	var lot_val   = parseInt(techjoomla.jQuery('#qtc_prod_quantity'+prod_container_num).attr("step"));
	...
	if (quantity != '')
	{
		...
		if (quantity%lot_val != 0)
		{
			alert(Joomla.JText._('COM_QUICK2CARET_SLAB_SHOULD_BE_MULT_MIN_QTY')+ lot_val);
