...
//@TODO use language constant for title=\"Add Options.\"
						...
						techjoomla.jQuery('#' + attributeContainerId + ' tr.clonedInput:last' ).children().last().replaceWith('<td><button type=\"button\" class=\"btnAdd btn btn-sm btn-mini btn-primary\" id=\"' + attributeContainerId + '\"  onclick=\"addopt(this.id)\" ><i class=\" icon-plus-2 icon22-white22 \"></i></button></td> ');
					...
					//	techjoomla.jQuery("#" + attributeContainerId + " .qtcRemoveOption ").hide();
						...
						techjoomla.jQuery('#'+attributeContainerId+'.btnAdd').replaceWith('<button type=\"button\" class=\"btn btn-sm btn-mini btn-danger qtcRemoveOption\"   onclick=\"techjoomla.jQuery(this).closest(\'tr\').remove();\" ><i class=\"' + qtcJsData.Q2C_ICON_TRASH +'\"></i></button> ');
	...
	else
	{
		...
		/* Replace last children delete button to plus */
		techjoomla.jQuery('#' + attributeContainerId + ' tr.clonedInput:last' ).children().last().replaceWith('<td><button type=\"button\" class=\"btnAdd btn btn-sm btn-mini btn-primary\" id=\"' + attributeContainerId + '\"  onclick=\"addopt(this.id)\" ><i class=\"icon-plus-2 icon-white\"></i></button></td> ');
...
function qtcClearStockFields(attributeContainerId)
{
	...
	/* Uncheck and show the is Compulsory field*/
	techjoomla.jQuery("#" + attributeContainerId + " .checkboxdiv").prop('checked', false);
