techjoomla.jQuery(document).ready(function()
{
	...
	if(jQuery('.q2c-wrapper.tjBs5').length)
	{
		jQuery('.q2c-wrapper.tjBs5 .sa_steps_parent #MyWizard ol.qtc-steps-ol li').each(function( index ) {
			var dataTarget = jQuery(this).attr('data-bs-target');
			if (dataTarget != undefined) {
				jQuery(this).attr('data-target', dataTarget);
			}
		});
	}
	...
	techjoomla.jQuery('#MyWizard').on('change', function(e, data)
	{
		...
		if(data.direction === 'next')
		{
			if (stepId == 'qtc_billing')
			{
				...
				if (techjoomla.jQuery('input[name="shipping_address"]').length !== undefined && techjoomla.jQuery('input[name="shipping_address"]').length > 0)
				{
				...
				if (techjoomla.jQuery('input[name="billing_address"]').length  !== undefined && techjoomla.jQuery('input[name="billing_address"]').length > 0)
				{
			...
			if (stepId == 'qtc_shippingStep')
			{
				itemlLevelShppingRestricted = document.getElementById('itemlLevelshppingrestricted').value;

				if (itemlLevelShppingRestricted == '1')
				{
					alert(Joomla.JText._('COM_QUICK2CART_ITEM_LEVEL_SHIP_METHODS_RESTRICTED'));
					return false;
				}
			}
			...
			techjoomla.jQuery.ajax({
				url: Joomla.getOptions('system.paths').base+'/index.php?option=com_quick2cart&view=cartcheckout&task=cartcheckout.qtc_autoSave&tmpl=component&stepId='+stepId,
				type: 'POST',
				//async:false,
				data:values,
				dataType: 'json',
	...
	techjoomla.jQuery('#MyWizard').on('changed', function(e, data)
	{
		...
		if(stepthisactive == techjoomla.jQuery("#qtc-steps li").last().attr('id'))
		{
			...
			if(stepthisactive == "qtc_summaryAndPay" ){
				...
				techjoomla.jQuery('#ad_payHtmlDiv div.form-actions input[type="submit"]').addClass('float-end');
			}
...
/*
	...
	techjoomla.jQuery('#btnWizardPrev').on('click', function() {
		techjoomla.jQuery('#btnWizardPrev').addClass('clicked');
		techjoomla.jQuery('#btnWizardNext').removeClass('clicked');
		techjoomla.jQuery('#MyWizard').wizard('previous');
	});

	techjoomla.jQuery('#btnWizardNext').on('click', function()
	{
		techjoomla.jQuery('#btnWizardNext').addClass('clicked');
		techjoomla.jQuery('#btnWizardPrev').removeClass('clicked');
