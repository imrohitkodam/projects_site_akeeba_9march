 ...
 * @since  1.6
 */
class Quick2cartControllerCreateOrder extends FormController
{
	/**
	...
	public function qtc_place_order()
	{
		...
		if (empty($orderData->userId))
		{
			...
			$orderStatus->message = Text::_('COM_QUICK2CART_SELECT_CLIENT_ERROR_MSG');
		...
		if ($isShippingEnabled && empty($shippingAddressId))
		{
			$orderStatus->status  = "error";
			$orderStatus->message = Text::_('COM_QUICK2CART_SELECT_SHIPPING_ADDRESS_ERROR_MSG');
			echo json_encode($orderStatus);
			jexit();
		}
		...
		{
			$orderStatus->status  = "error";
			$orderStatus->message = Text::_('COM_QUICK2CART_SELECT_BILLING_ADDRESS_ERROR_MSG');
			echo json_encode($orderStatus);
			jexit();
		}
