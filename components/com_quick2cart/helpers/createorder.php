...
class CreateOrderHelper
{
	...
	public function qtc_place_order($orderData)
	{
		...
		if (empty($orderData))
		{
			$orderStatus->message = Text::_('COM_QUICK2CART_INVALID_DATA_FOR_PLACE_ORDER_API');

			return $orderStatus;
		}

		$qtcshiphelper = new Qtcshiphelper;
		$profieId      = $qtcshiphelper->getItemsShipProfileId($orderData->products_data[0]['product_id']);
		...
		if ($shippingMode == 'itemLevel' && $restrictItemLevelShipping == 1)
		{
			if (empty($shipMeths))
			{
				$orderStatus->message = Text::_('COM_QUICK2CART_ITEM_LEVEL_SHIP_METHODS_RESTRICTED');
				return $orderStatus;
			}
		}
		...
		$orderDetails->cdate = Factory::getDate()->toSQL();
		$orderDetails->mdate = Factory::getDate()->toSQL();

		// Price without coupon
		$order_original_amount = $this->calculateOrderPrice($orderData->products_data);
		$orderDetails->amount = $order_original_amount;
		...
		$orderDetails->customer_note = isset($orderData->customer_note) ? $orderData->customer_note : '';

		//Set payment gatway
		$orderDetails->processor = $orderData->processor;
		...
		if (!empty($orderId))
		{
			...
			{
				$orderStatus->status = "error";
				$orderStatus->message = Text::_('ERR_CONFIG_SAV_LOGIN');
		...
		else
		{
			...
			foreach ($orderData->products_data as $product_data)
			{
				...
				if (!empty($product_data['att_option']))
				{
					...
					$attriItemsInfo                = $this->getItemAttributeDetails($product['product_attributes']);

					if (!empty($attriItemsInfo))
					{
						$product['product_attribute_names'] = $this->getItemsSelectedAttributeName($product_data['att_option'], $attriItemsInfo);
					}
				}
			...
			if ($isShippingEnabled)
			{
				...
				// If not allowed to place order return here
				if (!empty($shippingDetails) && $shippingDetails['allowToPlaceOrder'] == 0)
				{
					$shippingMode = $params->get('shippingMode', 'itemLevel');

					if ($shippingMode == "orderLeval")
					{
						$enabled = PluginHelper::isEnabled('qtcshipping', 'qtc_pinwise_shipping');

						if ($enabled)
						{
							$orderStatus->status  = "error";
							$orderStatus->message = Text::_($shippingDetails['detailMsg']);
							$orderStatus->dberror = $db->stderr();
			...
			if (property_exists($orderData, 'deliveryfromdate') && isset($orderData->deliveryfromdate))
			{
				$orderDetails->deliveryfromdate = $orderData->deliveryfromdate;
			}

			if (property_exists($orderData, 'deliverytodate') && isset($orderData->deliverytodate))
			{
				$orderDetails->deliverytodate = $orderData->deliverytodate;
			}

			// If allowed to place order first insert it then apply shipping changes
			...
			if (!empty($orderStatus->order_id))
			{
				...
				$this->qtc_add_order_items($orderData, $orderStatus->order_id);
			}
			...
			if ($isShippingEnabled && $orderStatus->status == 'success')
			{
				if (!empty($shippingDetails) && $shippingDetails['allowToPlaceOrder'] == 1 && !empty($shippingDetails['charges']))
				{
			...
			$result = $app->triggerEvent("onAfterQ2cOrderPlace", array($order_obj, $data));

			// @DEPRICATED
			$result = $app->triggerEvent("onAfterQ2cOrder", array($order_obj, $data));
	 ...
	 *
	 * @param   object  $address  array of address info
	 *
	...
	public function qtc_add_billing_address($address, $orderId)
	{
		...
		{
			$status->status = "error";
			$status->message = Text::_('COM_QUICK2CART_INVALID_ADDRESS_OR_ORDER');
		...
		if ($bill_id)
		{
			...
			{
				$status->status = "error";
				$status->message = Text::_('COM_QUICK2CART_INVALID_ADDRESS');
	 ...
	 *
	 * @param   Object|ARRAY  $orderData    array of orders data
	 *
	 * @param   INT    $orderId      order id
	 *
	...
	public function qtc_add_order_items($orderData, $orderId, $orderItemId = 0)
	{
		...
		foreach ($products_data as $key => $product_data)
		{
			...
			if (!empty($prod_info[1]))
			{
				foreach ($prod_info[1] as $attri_info)
				{
					...
					{
						$orderItemData->product_attributes_price += $attri_info['optionprice'];
					}
					else
					{
						$orderItemData->product_attributes_price -= $attri_info['optionprice'];
			...
			$orderItemData->cdate = Factory::getDate()->toSQL();
			$orderItemData->mdate = Factory::getDate()->toSQL();
			$orderItemData->status = 'P';

			if (!empty($product_data['att_option']))
			{
				$orderItemData->attributeOptions = $product_data['att_option'];
			}
		...
		foreach ($ordeItemList as $key => $orderItem)
		{
			...
			if (!empty($prod_info[1]))
			{
				if (!empty($product_data['att_option']))
				{
					$this->qtc_add_order_items_attributeoptions($order_item_id, $prod_info[1], '0', $orderItem->attributeOptions);
				}
				else
				{
	...
	public function qtc_add_order_items_attributeoptions($order_item_id, $attributes_info, $orderitemattribute_id = 0, $attributeData = '')
	{
		...
		foreach ($attributes_info as $key => $attribute_info)
		{
			...
			$itemAttributeOptionDetails->orderitemattribute_price = $attribute_info['optionprice'];
	 ...
	 *
	 * @param   object  $taxDetails   array of tax details
	 *
	 * @param   INT    $orderId      order id
	 *
	...
	public function qtc_apply_taxes($taxDetails, $orderId, $orderItemId = '0')
	{
		...
		* */
		...
		if (!empty($taxDetails->order_tax_details))
		{
			...
			$orderDetails->amount = ($orderDetails->original_amount - $orderDetails->coupon_discount) + $orderDetails->order_tax + $orderDetails->order_shipping;
				...
				if (!empty($item_tax_detail))
				{
					...
					if (!empty($item_tax_amount))
					{
						...
						if (!empty($itemResult))
						{
							...
							$itemResult->product_final_price = ($itemResult->original_price - $itemDiscount) + $itemResult->item_tax + $shippingCharges;
	 ...
	 * @param   INT  $orderId  order id
	 *
	 * @return  Object  Object of order info
	 *
	 * @since	2.5.5
	 */
	...
	public function calculateProductPrice($product_data)
	{
		...
		if (!empty($prod_info[1]))
		{
			foreach ($prod_info[1] as $attri_info)
			{
				if ($attri_info['itemattributeoption_prefix'] == '+')
				{
					$basePrice += $attri_info['optionprice'];
				}
				else
				{
					$basePrice -= $attri_info['optionprice'];
		...
		if (!empty($shippingDetails['itemShipMethDetail']))
		{
			foreach ($shippingDetails['itemShipMethDetail'] as $shippingDetail)
			{
				...
				if (!empty($itemResult))
				{
					...
					$itemDiscount = empty($itemResult->discount)?'0':$itemResult->discount;
					$itemResult->product_final_price = ($itemResult->original_price  - $itemDiscount) + $itemResult->item_shipcharges + $taxCharges;
					$db = Factory::getDbo();
					$db->updateObject('#__kart_order_item', $itemResult, 'order_item_id');
				}
			}
	...
	protected function updateAsLastUsedBillingAddress($addressId, $userId)
	{
		...
		foreach ($userAddresses as $address)
		{
			...
			$result = Factory::getDbo()->updateObject('#__kart_customer_address', $address, 'id');
		}
	}
