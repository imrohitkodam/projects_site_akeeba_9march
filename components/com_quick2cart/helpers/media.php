...
class Qtc_MediaHelper
{
	...
	public function __construct()
	{
		...
		$this->sa_config['video_gallery']              = $params->get('video_gallery');
		$this->sa_config['videoExtensions']            = $params->get('videoExtensions');
		$this->sa_config['max_video_file_size']        = $params->get('max_video_file_size');
	}
	...
	public function check_max_size($file_size)
	{
		...
		$max_image_size   = $this->sa_config['image_size'] * 1024;

		if ($this->sa_config['video_gallery'])
		{
			$max_image_size = $this->sa_config['max_video_file_size'] * 1048576;
		}
	 ...
	 * @param   String|Array  $file_extension  File extension
	 *
	 * @return  array  $return
	 */
	public function check_media_type_group($file_type, $file_extension)
	{
		$media_type_group           = (!empty($file_type)) ? explode("/", $file_type)[0] : '';
		$return                     = array();
		...
		$uploadMediaAllowedExtension = array('gif', 'png', 'jpeg', 'jpg');

		if($this->sa_config['video_gallery'] && (!empty($this->sa_config['videoExtensions'])))
		{
			$videoExtensions = explode(",", $this->sa_config['videoExtensions']);

			if (!empty($videoExtensions))
			{
				$uploadMediaAllowedExtension = array_merge($uploadMediaAllowedExtension, $videoExtensions);
			}
		}

		// Check is file extension allows 
		if (!in_array($file_extension, $uploadMediaAllowedExtension))
		{
			$supportedMediaExtension = 'gif, jpeg, jpg, png';

			if ($this->sa_config['video_gallery'] && (!empty($this->sa_config['videoExtensions'])))
			{
				$supportedMediaExtension = $supportedMediaExtension . ', ' . $this->sa_config['videoExtensions'];
			}

			$return['allowed'] = 0;
			$return['msg']     = Text::sprintf('COM_QUICK2CART_FILE_TYPE_NOT_ALLOWED', $supportedMediaExtension);

			return $return;
		}

		$fileMimeType = $this->getMimeType($file_extension);

		if (empty($fileMimeType) || $fileMimeType == null)
		{
			$return['allowed'] = 0;
			$return['msg']     = Text::_('COM_QUICK2CART_INVALID_FILE_UPLOADED');
	 ...
	 * @param   int  $colorR                       [description]
	 * @param   int  $colorG                       [description]
	 * @param   int  $colorB                       [description]
	 * @param   int  $new_media_width              [description]
	 * @param   int  $new_media_height             [description]
	 * @param   int  $blank_height                 [description]
	 * @param   int  $top_offset                   [description]
		...
		$top_offset, $media_extension, $file_name_without_extension, $max_zone_height = null)
	{
		...
		if ($filesize > 0)
		{
			...
			$new_media_width = $new_media_width ? $new_media_width : 0;
			$blank_height = $blank_height ? $blank_height : 0;

			if (!$blank_height && !$new_media_width)
			{
				$errorList[] = Text::_('FILE_EMPTY');
			}
	 ...
	 * @param   null|bool|int|float|string  $chunkSize  [each chunk size (in bytes)]
	 * @param   null|bool|int|float|string  $totalSize  [original file size (in bytes)]
	 *
	 * @return  [type]              [description]
	 */
