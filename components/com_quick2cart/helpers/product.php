...
class ProductHelper
{
	...
	public function getAllFeturedProducts($store_id = '', $prod_cat = '', $limit = '4')
	{
		...
		$query->join('LEFT', $db->qn('#__kart_store', 's') . ' ON ' . $db->qn(name: 's.id') . ' = ' . $db->quoteName('i.store_id'));
		$query->where($db->quoteName('s.live') . "=1");
	...
	public function getSimilarProdFromCat($cat, $item_id, $parent = '', $limit = 4)
	{
		...
		$db    = Factory::getDbo();
		$query = "SELECT i.* FROM #__kart_items as i " . $where . ' ORDER BY RAND() LIMIT 0 , ' . $limit;
		$db->setQuery($query);
	...
	public function prodFromSameStore($store_id, $item_id, $parent = '', $limit = 4)
	{
		...
		$where = (count($where) ? ' WHERE ' . implode(' AND ', $where) : '');
		$db    = Factory::getDbo();
		$query = "SELECT i.* FROM #__kart_items as i " . $where . ' LIMIT 0 ,' . $limit;
		$db->setQuery($query);
	...
	{
		$db    = Factory::getDbo();
		$query = 'SELECT DISTINCT(oi.item_id),i.*
	...
	public function getUserProducts($userid, $limit = '2', $client = "com_quick2cart")
	{
		...
		$query->select("i.*");
		$query->from($db->quoteName('#__kart_items', 'i'));
		$query->join('INNER', $db->quoteName('#__kart_store', 's') . ' ON (' . $db->quoteName('i.store_id') . ' = ' . $db->quoteName('s.id') . ')');
		$query->where($db->quoteName('s.owner') . ' = ' . (int) $userid);
		$query->where($db->quoteName('i.display_in_product_catlog') . ' =1');
		$query->order($db->quoteName('i.cdate') . ' DESC');
	...
	public function delWholeAttributeOption($itemattributeoption_id)
	{
		...
		if (!empty($itemattributeoption_id))
		{
			...
			$attributeOptionData = $this->getAttributeOptionDetails($itemattributeoption_id);
			BaseDatabaseModel::addIncludePath(JPATH_SITE . '/components/com_quick2cart/models', 'attributes');
			$attributesModel     = BaseDatabaseModel::getInstance('attributes', 'Quick2cartModel');
			$attributeData       = $attributesModel->getAttributeData($attributeOptionData['itemattribute_id']);
			$productData         = $attributesModel->getItemDetail('', '', $attributeData->item_id);
	...
	public function deleteWholeProduct($item_id)
	{
		$storeHelper         = new StoreHelper;
		$comquick2cartHelper = new comquick2cartHelper;
		$productData         = $this->getItemCompleteDetail($item_id);
		$storeOwner          = $storeHelper->getStoreOwner($productData->store_id);
		$isOwner             = $comquick2cartHelper->checkOwnership($storeOwner);
		$db                  = Factory::getDbo();
		...
		if ($isOwner)
		{
			...
			$productTable = Table::getInstance('Product', 'Quick2cartTable', array('dbo', $db));
			$productTable->load(array('item_id' => $item_id));

			$data = $productTable->getProperties();

			// On before product delete
			PluginHelper::importPlugin("actionlog");
			Factory::getApplication()->triggerEvent("onBeforeQ2cDeleteProduct", array($data));
			...
			$status = $quick2cartModelAttributes->deleteItem($item_id);

			if ($status)
			{
				// On after product delete
				PluginHelper::importPlugin("actionlog");
				Factory::getApplication()->triggerEvent("onAfterQ2cDeleteProduct", array($data));
			}
	...
	{
		if (!empty($item_id) && !empty($image_path))
		{
			$db    = Factory::getDbo();
	 ...
	 *
	 * @param   INT  $item_id    item id
	 * @param   array  $attIdList  attribute Id List
	 *
	 * @return status
	 */
	 ...
	 * @param   array  $optionids  option id
	 *
	 * @return status
	 */
	...
	public function getAttrFieldTypeHtml($data)
	{
		...
		//  if checked
		{
			$select_opt[] = HTMLHelper::_('select.option', "", "");
		}
		...
		foreach ($atri_options as $atri_option)
		{
			...
			$select_opt[] = HTMLHelper::_('select.option', $atri_option->itemattributeoption_id, $opt_str);
				$returnHtml   = HTMLHelper::_(
				'select.genericlist', $select_opt, $field_name, 'class="q2c_AttoptionsMaxWidth {$parent}-{$product_id}_options"
				onchange= "updateCartItemsAttribute(\'' . $cart_id . '\',\'' . $product_id . '\')"',
		...
		return $returnHtml;
		/*    if (!in_array($data['fieldType'],$userData)) {
		echo HTMLHelper::_('select.genericlist', $select_opt, 'attri_option', "  class='span2 {$parent}-{$product_id}_options'", 'value', 'text', '',false);
		}*/
	}
	...
	{
		$fields[] = HTMLHelper::_('select.option', 'Select', Text::_('QTC_ADDATTRI_SELECT_FIELD'));
		$fields[] = HTMLHelper::_('select.option', 'Textbox', Text::_('QTC_ADDATTRI_TEXT_FIELD'));
	...
	public function saveProdMediaDetails($media_detail, $item_id, $deleteOldRec = 1)
	{
		...
		foreach ($media_detail as $media)
		{
			...
			$file->file_display_name = !empty($media['name']) ? $media['name'] : Text::_('COM_QUICK2CRT_DEFAULT_DOWNLODABLE_FILE');
			$file->item_id           = $item_id;
			$file->state             = isset($media['status']) ? true : false;
			$file->purchase_required = 0;

			if (array_key_exists("purchaseReq",$media) && isset($media['purchaseReq']))
			{
				$file->purchase_required = 1;
			}
			...
			if (empty($media['file_id']) && !empty($media['mediaFilePath']))
			{
				...
				if (!Folder::exists($newDestination))
				{
					$status = Folder::create($newDestination);
				}

				// Move a file from temporary location to newlocation
				$newDestination = $newDestination . "/" . $fileName;
				$uploadSuccess  = File::move($tempSource, $newDestination);
			}
			...
			try
			{
				$db->$action('#__kart_itemfiles', $file, 'file_id');
			}
	...
	public function deleteProductMediaFile($mediaIds)
	{
		...
		foreach ($mediaIds as $media)
		{
			...
			$query = $db->getQuery(true);
			$query->select('*');
			$query->from('#__kart_itemfiles');
			$query->where('file_id' . ' = ' . $media);
			$db->setQuery($query);
			$mediaData = $db->loadObject();
			$productData = $this->getItemCompleteDetail($mediaData->item_id);
			$storeOwner  = $storeHelper->getStoreOwner($productData->store_id);
			$isOwner     = $comquick2cartHelper->checkOwnership($storeOwner);
			...
			{
				$query = $db->getQuery(true);
				$query->delete($db->quoteName('#__kart_itemfiles'));
				$query->where($db->quoteName('file_id') . ' = ' . $db->quote($media));
				$db->setQuery($query);
				...
				try
				{
					...
					if ($status && !empty($mediaData->filePath))
					{
						$productHelper->deleteFile($delFromDir . "/" . $mediaData->filePath);
					}
				}
				catch (RuntimeException $e)
				{
					Factory::getApplication()->enqueueMessage(Text::_('COM_QUICK2CART_DB_EXCEPTION_WARNING_MESSAGE'), 'error');
				}
			}
			else
			{
	...
	public function showMediaDownloadLink($linkData)
	{
		...
		. "\" $event title=\""
		. Text::_("COM_QUICK2CART_PROD_PG_DOWNLOADLINK_TITLE") . "\">" . ucfirst($linkData['linkName']) . "</a> ";
	...
	public function getMediaDownloadLinkHref($fileid, $extraUrlPrams = '')
	{
		...
		$link = Uri::root() . substr(Route::_($b_lk . $fileid . $extraUrlPrams . '&Itemid=' . $storecp_Itemid), strlen(Uri::base(true)) + 1);
	...
	public function addEntryInOrderItemFiles($order_id)
	{
		...
		foreach ($result as $res)
		{
			...
			foreach ($mediaDetail as $media)
			{
				...
				$query->from($db->quoteName('#__kart_orderItemFiles'));
				$query->where($db->quoteName('order_item_id') . ' = ' . $pfile->order_item_id);
				$query->where($db->quoteName('product_file_id') . ' = ' . $pfile->product_file_id);
				$query->order('id');
				$db->setQuery($query);
				...
				if (empty($pfile->expirary_date))
				{
					$pfile->expirary_date = '0000-00-00 00:00:00';
				}
				...
				{
					echo Text::_("COM_QUICK2CART_ERROR_WHILE_ASSIGNING_MEIDA_FILE_TO_ORDER") . $db->stderr();
	...
	public function mediaFileAuthorise($file_id, $strorecall, $guest_email, $order_item_id)
	{
		...
		$user = Factory::getUser();
		$orderDetails = $comquick2cartHelper->getorderinfo($orderid);
		$info = $orderDetails['order_info']['0'];
	...
	public function validDownload($file_id, $order_item_id)
	{
		...
		$query->select('*');
		$query->from($db->quoteName('#__kart_orderItemFiles'));
		$query->where($db->quoteName('order_item_id') . ' = ' . $order_item_id);
		$query->where($db->quoteName('product_file_id') . ' = ' . $file_id);
		$query->order('id');
		$db->setQuery($query);
	 ...
	 * @return  html
	 */
	public function download($file, $filename_direct = '', $extern = '', $exitHere = 1, $watermark = false)
	{
		$filename  = basename($file);
		$tempFiles = array();

		if ($watermark && strtolower(File::getExt($filename)) == 'pdf')
		{
			$params = ComponentHelper::getParams('com_quick2cart');
			JLoader::import('tjfpdi.tjwatermark', JPATH_LIBRARIES);
			$tjwatermark = new tjwatermark;
			$wmText = $params->get('wm_text', '', 'STRING');

			if ($wmText != '')
			{
				$tjwatermark->setFontSize($params->get('wm_text_font_size', '20', 'INT'));
				$tjwatermark->setFontStyle($params->get('wm_text_font_style', 'Helvetica', 'STRING'));
				$tjwatermark->setFontColor($params->get('wm_text_font_color'));
				$tjwatermark->setPosition($params->get('wm_text_position', 'center', 'STRING'));
				$wmText = $this->processWmText($wmText, $filename);
				$file = $tjwatermark->textOnPdf($file, $wmText);
				$tempFiles[] = $file;
			}

			if ($params->get('wm_image', '', 'STRING') != '')
			{
				$tjwatermark->setPosition($params->get('wm_image_position', 'center', 'STRING'));
				$file = $tjwatermark->imageOnPdf($file, JPATH_SITE . '/' . $params->get('wm_image', '', 'STRING'));
				$tempFiles[] = $file;
			}
		}
	...
	public function DisplayAvarageRating($product_url, $id, $title)
	{
		...
		PluginHelper::importPlugin('content', 'jlike_quick2cart');

		$grt_response = Factory::getApplication()->triggerEvent('onDisplayRatingAvarage', array('com_quick2cart.productpage', $jlikeparams)
			);
	...
	public function DisplayjlikeButton($product_url, $id, $title)
	{
		...
		PluginHelper::importPlugin('content', 'jlike_quick2cart');
		$grt_response = Factory::getApplication()->triggerEvent('onBeforeDisplaylike', array('com_quick2cart.productpage', $jlikeparams)
		);
		...
		foreach ($curr as $currKey => $value)
		{
			...
			if ($app->isClient('administrator'))
			{
				...
				else
				{
					...
					<input type="number" class="form-control input-mini" placeholder="" aria-label="" aria-describedby="" name="' . $name . '['.  $value . ']'. '" value="' . number_format($currentFieldValue, 2) . '">
					<span class="input-group-text" id="basic-addon1">' . $currtext . '</span>
					</div>';
				}
			}
			else
			{
				if ($bsVersion == 'bs5')
				{
					$html .= '<div class="input-group mb-3" >' . '<input type="number" name="' . $name . '[' . $value . ']"
					size="" id="" value="' . number_format($currentFieldValue, 2) . '"
					class=" form-control ' . $currFieldClass . '">' . '<div class="input-group-text">' . $currtext . '</div>' . '</div>';
				}
				elseif($bsVersion == 'bs3')
				{
					$html .= '<div class="input-group  curr_margin " >' . '<input type="number" name="' . $name . '[' . $value . ']" value="' . number_format($currentFieldValue, 2) . '"class=" form-control ' . $currFieldClass . '">' . '<span class="input-group-addon" id="basic-addon1">' . $currtext . '</span>' . '</div>';
				}
			}
		}
	...
	public function isAllowedToDelProduct($item_id)
	{
		...
		if (!empty($entryList))
		{
			...
			$errMsg = Text::sprintf('COM_QUICK2CART_DEL_PROD_FOUND_ORDER_AGAINST', $item_id, $entryListStr);
	 ...
	 *
	 * @param   object   $itemdetail  itemdetail.
	...
	public function isInStockProduct($itemdetail, $userBuyQty = "")
	{
		...
		$chkLotMinMax         = $params->get('minmax_quantity', 0);
		...
		if ($chkLotMinMax)
		{
			...
			if (!empty($userBuyQty))
			{
				if ($usestock && ($stock != null || $stock != ''))
				{
					$buybutton_status = ($userBuyQty < min($stock, $max_qty) && $userBuyQty >= $min_qty) ? 1 : 0;
				}
				else
				{
					$buybutton_status = ($userBuyQty < $max_qty && $userBuyQty >= $min_qty) ? 1 : 0;
				}
			}
		}
	 ...
	 * @since   2.2
	 *
	 * @return   null|bool|int|float|string|array count
	 */
	 ...
	 * @since   2.2.2
	 *
	 * @return  OBJECT|boolean item detail, false if some error occurred
	 */
	 ...
	 *
	 * @param   array  $store_list  The list of the store id.
	 *
	 ...
	 * @param   CHAR  $status    order status
	 *
	 * @return null
	 */
	public function pushProductPurchaseActivity($order_id, $status)
	{
		$mainHelper = new comquick2cartHelper;
		$params = ComponentHelper::getParams('com_quick2cart');
		...
		{
			$orderDetails = $mainHelper->getOrderInfo($order_id);
		}
		...
		if ($streamBuyProd && $socialintegration != 'none')
		{
			if ($status == 'C')
			{
				...
				if ($orderDetails['order_info'][0]->user_id)
				{
					$user = Factory::getUser($orderDetails['order_info'][0]->user_id);
				}
				else
				{
					$user = Factory::getUser();
				...
				foreach ($orderDetails['items'] as $item)
				{
					...
					$action = 'buyproduct';
					$prodLink = '<a class="" href="' . $mainHelper->getProductLink($item->item_id, 'detailsLink', 1) . '">' . $item->order_item_name . '</a>';

					$storeInfo[$item->store_id] = $mainHelper->getSoreInfo($item->store_id);

					$storeBaseLink = 'index.php?option=com_quick2cart&view=vendor&layout=store&store_id=';

					$storeLink = '<a class="" href="' . Uri::root() .
					substr(Route::_($storeBaseLink . $item->store_id), strlen(Uri::base(true)) + 1) . '">' . $storeInfo[$item->store_id]['title'] . '</a>';

					$originalMsg = Text::sprintf('QTC_ACTIVITY_BUY_PROD', $prodLink, $storeLink);
					$title = '{actor} ' . $originalMsg;

					// According to integration create social lib class obj.
					$libclass = $mainHelper->getQtcSocialLibObj();
	...
	/**
	 * Function to process watermark text
	 *
	 * @param   STRING  $wmText    Watermark text
	 * @param   STRING  $filename  File name
	 *
	 * @return STRING
	 */
	private function processWmText($wmText, $filename)
	{
		// If no tag in the wmtext then no need to process the text
		if (strpos($wmText, '{{') === false && strpos($wmText, '}}') === false)
		{
			return $wmText;
		}
		...
		$db   = Factory::getDbo();
		$data = new stdClass;
		JLoader::import('components.com_quick2cart.tables.itemfile', JPATH_ADMINISTRATOR);
		$itemFileTable = Table::getInstance('ItemFile', 'Quick2cartTable', array('dbo', $db));
		$itemFileTable->load(array('filePath' => $filename));

		$data->file = $itemFileTable;

		if (empty($itemFileTable->file_id))
		{
			return $wmText;
		}

		JLoader::import('components.com_quick2cart.tables.product', JPATH_ADMINISTRATOR);
		$itemTable = Table::getInstance('Product', 'Quick2cartTable', array('dbo', $db));
		$itemTable->load($itemFileTable->item_id);
		$data->product = $itemTable;

		JLoader::import('components.com_quick2cart.tables.store', JPATH_ADMINISTRATOR);
		$storeTable = Table::getInstance('Store', 'Quick2cartTable', array('dbo', $db));
		$storeTable->load($itemTable->store_id);
		$data->store = $storeTable;
		$data->user = Factory::getUser();
		$data->date = HTMLHelper::date('now', Text::_("COM_QUICK2CART_DIGITAL_PROD_WATERMARK_DATE_FORMAT"));

		// On before processing the wm text
		PluginHelper::importPlugin("quick2cart");
		$app->triggerEvent("onBeforeQ2cWmTextProcess", array($wmText, $data));
		...
		while ($tmpWmText != '')
		{
			$start = strpos($tmpWmText, '{{');
			$end = strpos($tmpWmText, '}}');

			if (is_numeric($start) && is_numeric($end) && $start < $end)
			{
				$start += 2;
				$tempTag = substr($tmpWmText, $start, $end - $start);
				$tags[$tempTag] = $tempTag;

				$tmpWmText = str_replace('{{' . $tempTag . '}}', '', $tmpWmText);
			}
			else
			{
				$tmpWmText = '';
		...
		if (!empty($tags))
		{
			...
			{
				$tmpTagKey = explode('.', $tagKey);
				$tmpData = $data;

				foreach ($tmpTagKey as $tg)
				{
					if (isset($tmpData->$tg))
					{
						$tmpData = $tmpData->$tg;
				...
				if ((is_string($tmpData) || is_numeric($tmpData)))
				{
					$tags[$tagKey] = $tmpData;
				}
				else
				...
				$wmText = str_replace('{{' . $tagKey . '}}', $tags[$tagKey], $wmText);
			}
		}

		// On after processing the wm text
		PluginHelper::importPlugin("quick2cart");
		$app->triggerEvent("onAfterQ2cWmTextProcess", array($wmText, $data));
