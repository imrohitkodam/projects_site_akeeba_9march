...
class ZoneHelper
{
	...
	public function getZoneStoreId($zone_id)
	{
		...
		$query->select($db->quoteName(array('store_id')))->from('#__kart_zone')->where('id=' . (int) $zone_id);
	 ...
	 *
	 * @since   2.2
	 * @return   Array array of zone with zone_id and name
	 */
	...
	public function isAllowedToDelZone($zone_id)
	{
		...
		if (!empty($taxEntry))
		{
			$errMsg = Text::sprintf('COM_QUICK2CART_ZONES_DEL_TAXRATE_AGAINST_ZONES', $zone_id);
			...
			<div class="<?php echo Q2C_WRAPPER_CLASS; ?>" >
				<div class="well" >
					<div class="alert alert-error alert-danger">
						<span ><?php echo Text::_('COM_QUICK2CART_TAXTRATES_S_STORE_NOT_FOUND');?> </span>
					</div>
				</div>
			</div>
			<?php
	...
	public function showUnauthorizedMsg()
	{
		...
		<div class="<?php	echo Q2C_WRAPPER_CLASS; ?>" >
			<div class="well" >
				<div class="alert alert-error alert-danger">
					<span ><?php echo Text::_('COM_QUICK2CART_TAXTRATES_NOT_AUTHORIZED'); ?> </span>
				</div>
			</div>
		</div>
		<?php
