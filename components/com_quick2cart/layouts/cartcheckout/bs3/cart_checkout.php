...
<div class="qtc_chekout_cartdetailWrapper  broadcast-expands" ><!-- LM Removed qtcAddBorderToWrapper -->
	...
	<div class="table-responsive">
		<table class="table table-checkout qtc-table table-hover">
				...
				foreach ($data->cart as $cart)
				{
					...
					<tr class="qtcborderedrow">
						...
						<td style="<?php echo $showqty_style; ?>" class="cartitem_qty rightalign" >
							...
							<input id ="quantity_field_<?php echo $cart['id'];?>" class="cart_fields pull-right input qtc-input-small" id="cart_count" name="<?php echo 'cartDetail[' . $cart['id'] . '][cart_count]' ?>" type="number" value="<?php echo $cart['qty'];?>"  maxlength="3" onchange="updateCartItemsAttribute('<?php echo $cart['id'];?>', '<?php echo $cart['item_id']; ?>')">
			...
			<td colspan = "<?php echo $col; ?>">
				...
				<span id = "cop_tr">
					<span class="input-group">
						<input type="text" class="form-control input input-medium"   id = "coupon_code" name="cop" value="<?php echo $ccode ?>"    placeholder="<?php echo Text::_('QTC_CUPCODE');?>"/>
			...
			<tr class="dis_tr qtcborderedrow highlightedrow" >
				...
				<td colspan = "<?php echo ($col + 1);?>" class="cartitem_tprice_label rightalign">
					...
					(
						...
						{
							?>
							<small><strong><?php echo Text::_('QTC_DISCOUNT_CODE') . " : " . $ccode . " "; ?> </strong>
							</small>
							<?php
						}
						?>
		...
		?>
		<button type="button" class="btn btn-danger btn-sm" onclick="emptycart(<?php echo $msg_order_js . ",'" . $baseUrl."'"; ?>);" ><i class="<?php echo Q2C_ICON_TRASH; ?> <?php echo Q2C_ICON_WHITECOLOR; ?> <?php echo Q2C_ICON_WHITECOLOR; ?>"></i>&nbsp;<?php echo Text::_('QTC_BTN_EMPTY_CART')?></button>
</div>
