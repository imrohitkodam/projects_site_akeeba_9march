...
?>
	<tr class="highlightedrow active">
		<td class="">
			<div>
			<strong><?php echo Text::_('MOD_QUICK2CART_PROMOTION_DICOUNT');?></strong>
			</div>
				(
				<?php
				if (!empty($ccode))
				{
					?>
					<small><strong><?php echo Text::_('COM_MOD_QUICK2CART_CUPCODE') . " : " . $ccode . " "; ?> </strong>
					</small>
					<?php
				}
				?>
	...
	<tr class="highlightedrow active">
		<td class="">
			<b>
				<?php echo Text::_("MOD_QUICK2CART_AMT_AFTER_PROMOTION");?>
			</b>
		</td>
		<td class="">
		<b>
...
<tr class="active">
	<td colspan="2">
		...
		{
		?>
		<div class="input-group modulecartview" id="mod_applyCopWrapper">
			<input type="text" class="form-control qtc_coupon_input_box" id="cart_coupon_code" name="cop" value="" placeholder="<?php echo Text::_('COM_MOD_QUICK2CART_CUPCODE');?>" style="max-width:180px;" />
              <span class="input-group-btn">
                  <button  class="btn btn-default"  onclick="cart_applycoupon('<?php echo Text::_('COM_MOD_QUICK2CARTENTER_COP_COD')?>', '.modulecartview #cart_coupon_code')"><?php echo Text::_('COM_MOD_QUICK2CART_APPLY');?></button>
              </span>
          </div>
		...
		<span class="label label-success qtcHandPointer" onclick="remove_cop()">
			<?php echo Text::_("MOD_QUICK2CART_REMOVE_COP");?>
