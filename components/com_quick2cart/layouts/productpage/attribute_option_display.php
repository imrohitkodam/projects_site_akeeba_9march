...
$chekForStock          = ($usestock == 1 && $outofstock_allowship == 0) ? 1 : 0;
...
foreach ($atri_options as $atri_option)
{
	...
	if (in_array($data['fieldType'], $userData))
	{
		...
		$required = ($data['attribute_compulsary']) ? " required " : '';
		$returnHtml = "<input type='text' name='" . $attribute_selectbox_name . "' class='form-control input input-small " . $parent . '-' . $product_id . '_UserField' . "' " . $required . ">";
	}
	else
	{
...
if (!in_array($data['fieldType'], $userData))
{
	...
	$required = ($data['attribute_compulsary']) ? " required " : '';
	?>
	<select class="form-select <?php echo (JVERSION > '4.0.0') ? '' : 'q2c_AttoptionsMaxWidth'; echo $required .' ' . $parent; ?>-<?php echo $product_id ?>_options" onchange="<?php echo $selectFieldOnchangeEvent;?>" name="<?php echo $selectFieldName; ?>">
	<?php
		foreach ($select_opt as $op_key => $option)
		{
			...
			if ((isset($option->disabled) && $option->disabled == 1))
			{
				$isdisabled = 'disabled';
			}
			?>
				<option value="<?php echo $option->value ?>" <?php echo $isdisabled?><?php echo $selected ?> >
