 ...
 * @since       2.2
 */
class Quick2cartModelCouponForm extends FormModel
{
	/**
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_quick2cart.edit.coupon.data', array());
	...
	public function save($data)
	{
		...
		if ($coupon_id)
		{
			...
			if ($exists)
			{
				...
				try
				{
					$db->updateObject('#__kart_coupon', $obj, 'id');
				}
		...
		else
		{
			try
			{
				$db->insertObject('#__kart_coupon', $obj, 'id');
			}
			catch (\RuntimeException $e)
			{
				$this->setError($e->getMessage());
