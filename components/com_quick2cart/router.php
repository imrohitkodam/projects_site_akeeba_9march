 ...
 * @since       1.0.0
 */
class Quick2CartRouter extends RouterBase
{
	private  $views = array('adduserform', 'createorder', 'productpage', 'shipprofileform', 'vendor',
			...
			'product', 'shipping', 'taxrates', 'zone');
	...
	public function build(&$query)
	{
		...
		if (($menuItem) && isset($menuItem->query['view']) && isset($query['view']))
		{
			...
			{
				// For my customers view - menu can be created it also needs store id or user id to filter orders on ther views
				if ($query['view'] == 'orders' && $query['layout'] == 'customerdetails')
				{
					return $segments;
				}
		...
		if (isset($view) && !in_array($view, $this->special_views))
		{
			$segments[] = $view;
		}
		...
		if ($view == 'vendor')
		{
			if (($query['layout'] == 'store' || $query['layout'] == 'createstore' || $query['layout'] == 'storeinfo' || $query['layout'] == 'cp'))
			{
		...
		if (isset($query['layout']) && $view == 'promotion')
		{
			$segments[] = $query['layout'];
			unset($query['layout']);
		}
		...
		/* Handle normal views */
		if ($view == 'promotion' || $view == 'taxrateform' || $view == 'taxprofileform' || $view == 'shipprofileform' || $view == 'zoneform')
		{
			if (isset($query['id']))
			{
			...
			if ($view == 'zoneform' && isset($query['zonerule_id']))
			{
				$segments[] = (INT) $query['zonerule_id'];
				unset($query['id']);
			}
		}
	...
	public function parse(&$segments)
	{
		...
		if ($count == 1)
		{
			$categoryTable = Table::getInstance('Category', 'JTable', array('dbo', $db));
		...
		else
		{
			...
			elseif (($storeId = $this->_getStoreRow($segments[0])->id) && ($categoryTable->load(array('alias' => $segments[1], 'extension' => 'com_quick2cart'))))
			{
					$vars['view'] = 'vendor';
					$vars['store_id'] = $storeId;
					$vars['layout'] = 'store';
			...
			else
			{
				...
				switch ($segments[0])
				{
					...
					case 'taxrateform':
						...
						if (isset($segments[1]))
						{
							$taxrateid  = array_shift($segments);
					...
					case 'zoneform':
						if (count($segments) > 2)
						{
							...
							$vars['layout'] = 'setrule_' . $bsVersion;

							if (isset($segments[1]))
							{
								$vars['id']          = $segments[2];
								$vars['zonerule_id'] = $segments[3];
								array_shift($segments);
							}
						}
						else
					...
					case 'shipprofileform':
						$vars['layout'] = (count($segments) == 3) ? $segments[1] : 'default';

						if ((count($segments) == 3) && isset($segments[2]))
						{
							$defaultid = array_shift($segments);
							$vars['id'] = array_shift($segments);
						}

						if ((count($segments) == 2) && isset($segments[1]))
						{
							$defaultid = array_shift($segments);
							$vars['id'] = array_shift($segments);
						}
				...
				array_shift($segments);
				if (!empty($segments) && in_array($segments[0], $this->views_needing_tmpl))
				{
					$vars['tmpl'] = 'component';
				}
			}
	...
	{
		$db    = Factory::getDbo();
		$table = Table::getInstance('Store', 'Quick2cartTable', array('dbo', $db));
