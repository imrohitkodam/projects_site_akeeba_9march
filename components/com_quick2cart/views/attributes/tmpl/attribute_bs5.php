...
function addopt(){
		...
		techjoomla.jQuery('.qtc_attributeOpTable tbody .btnAdd').last().replaceWith('<button type=\"button\" class=\"btn btn-sm btn-danger qtcRemoveOption\" id=\"btnRemove'+num+'\"  onclick=\"techjoomla.jQuery(this).closest(\'tr\').remove();\" ><i class=\"' + qtcJsData.Q2C_ICON_TRASH + ' \"></i></button> ');
...
function removeopt(elem,id)
{
	...
	if (1){
		techjoomla.jQuery.ajax({
			url: Joomla.getOptions('system.paths').base + '/index.php?option=com_quick2cart&controller=attributes&task=delattributeoption&tmpl=component&opt_id='+opt_id,
			type: 'GET',
...
<form method='POST' name='adminForm' class="form-validate" action='index.php' id='no-more-tables'>
	...
	<legend><?php echo Text::_('QTC_ADD_ATTRI');?></legend>
	<div class="">
		<div class=" ">
			<table class="table  table-condensed" >
			<thead>
				<tr>
					<th width="42%" align="left"><label for="atrri_name_id"><?php echo Text::_( 'QTC_ADDATTRI_NAME' ); ?></label></th>
					<th width="32%"	align="left"><?php echo Text::_( 'QTC_ADDATTRI_FIELD_TYPE_TO_USE' ); ?></th>
					<th width="%"	align="left"><?php echo Text::_( 'QTC_ATT_COMPALSARY_CK' ); ?> </th>
				</tr>
			</thead>
			<tbody>
				<td data-title="<?php echo Text::_('QTC_ADDATTRI_NAME');?>">
					<input id="atrri_name_id" class="form-control-sm bill required" type="text" value="<?php echo (isset($this->itemattribute_name))? htmlentities($this->itemattribute_name):''; ?>" maxlength="250"  name="att_detail[attri_name]" title="<?php echo Text::_('QTC_ADDATTRI_NAME_DESC')?>">
				...
				<td data-title="<?php echo Text::_('QTC_ADDATTRI_FIELD_TYPE_TO_USE');?>">
					<?php
					$fields = array();
					...
					echo HTMLHelper::_('select.genericlist', $fields, "att_detail[fieldType]", 'class="no_chzn form-select-sm  qtcfieldType" onChange="qtc_fieldTypeChange('.$fnparam.')"', "value", "text",$default);
				?>
				</td>
				<td data-title="<?php echo Text::_('QTC_ATT_COMPALSARY_CK');?>">
					<?php
						$qtc_ck_att="checked";
...
<!--
	<div class="">
		<div class="col-md-12">
			<div class="span4">
				<input id="atrri_name_id" class="input-small bill required" type="text" value="<?php echo (isset($this->itemattribute_name))?$this->itemattribute_name:''; ?>" maxlength="250"  name="att_detail[attri_name]" title="<?php echo Text::_('QTC_ADDATTRI_NAME_DESC')?>">
				<input type="hidden" name="att_detail[product_id]" value="<?php echo $pid ?>" />
				<input type="hidden" name="att_detail[client]" value="<?php echo $client ?>" />
			</div>
...
-->
	...
	<div class="form-group">
		<h4><?php echo Text::_('QTC_ADDATTRI_NAME')?></h4>
		<div class="col-sm-10">
			<input id="atrri_name_id" class="input-medium bill required" type="text" value="<?php echo (isset($this->itemattribute_name))?$this->itemattribute_name:''; ?>" maxlength="250"  name="attri_name" title="<?php echo Text::_('QTC_ADDATTRI_NAME_DESC')?>">
	</div>
	</div>
	<div class="form-group">
		...
		?>
			<input type="checkbox" name="iscompulsary_attr" autocomplete="off" <?php echo $qtc_ck_att;?> > <h5><?php echo Text::_('QTC_ATT_COMPALSARY_CK')?></h5	>
		</label>
	</div>
	-->
	<?php
	...
	?>
<table class="table table-condensed item_attris_opt qtc_attributeOpTable" style="<?php echo $tableDisplay; ?>"><!-- the table width is fixed to 450px -->
	<thead>
		<tr>
			<th width="140px" align="left"><b><?php echo Text::_( 'QTC_ADDATTRI_OPTNAME' ); ?> </b></th>
			<th width="40px"	align="left"><b><?php echo Text::_( 'QTC_ADDATTRI_OPTPREFIX' ); ?></b> </th>
			<th width="550px"	align="left"><b><?php echo Text::_( 'QTC_ADDATTRI_OPTVAL' ); ?></b> </th>
			<th width="130px"	align="left"><b><?php echo Text::_( 'QTC_ADDATTRI_OPTORDER' ); ?></b> </th>
	...
	?>
		<tr class="form-group form-inline clonedInput" id="attri_opts<?php echo $k; ?>" >
			<td data-title="<?php echo Text::_('QTC_ADDATTRI_OPTNAME');?>">
				...
				<input type="text" class="form-control-sm" name="att_detail[attri_opt][<?php echo $k; ?>][name]" placeholder="<?php echo Text::_('QTC_ADDATTRI_OPTNAME')?>" value="<?php echo (isset($this->attribute_opt[$k]->itemattributeoption_name)) ? htmlentities($this->attribute_opt[$k]->itemattributeoption_name) : ''; ?>">
			</td>
			<td data-title="<?php echo Text::_('QTC_ADDATTRI_OPTPREFIX');?>">
				...
				echo HTMLHelper::_('select.genericlist', $addpre_select, "att_detail[attri_opt][$k][prefix]", 'class="form-select-sm"   ', "value", "text", $addpre_val);
				?>
			</td>
			<td data-title="<?php echo Text::_('QTC_ADDATTRI_OPTVAL');?>">
				...
				<div class='qtc_currencey_textbox input-group form-group'  >
					...
					foreach($curr as $value)    // key contain 0,1,2... // value contain INR...
					{
						...
						?>
					<div class="input-group input-group-sm curr_margin " >
						<input type='text' name="att_detail[attri_opt][<?php echo $k; ?>][currency][<?php echo $value; ?>]" size='1' id='' value="<?php echo ((isset($currvalue[0]['price']))?$currvalue[0]['price'] : ''); ?>" class=" currtext form-control-sm" Onkeyup="checkforalpha(this,46,<?php echo $entered_numerics; ?>);">
			...
			<td data-title="<?php echo Text::_('QTC_ADDATTRI_OPTORDER');?>">
				<input type="text" Onkeyup="checkforalpha(this,'',<?php echo $entered_numerics; ?>);" onchange="qtc_ispositive(this)"	 id="" class="qtc-attribute-Option-order form-control-sm" name="att_detail[attri_opt][<?php echo $k; ?>][order]" placeholder="<?php echo Text::_('QTC_ADDATTRI_OPTORDER')?>" value="<?php echo (isset($this->attribute_opt[$k]->ordering))?$this->attribute_opt[$k]->ordering:$k+1; ?>">
			...
			<td>
				...
				if ($k == $lastkey_opt)
				{ ?>
					<button type="button" class="btnAdd btn btn-sm btn-primary"  onclick="addopt();"><i class="<?php echo QTC_ICON_PLUS;?>"></i></button>
				<?php
				}
				else
				{ ?>
					<button type="button" class="btn btn-sm btn-danger" id="btnRemove<?php echo $k; ?>" onclick="techjoomla.jQuery(this).closest('tr').remove();" ><i class="<?php echo Q2C_ICON_TRASH; ?>"></i></button>
				<?php
				} ?>
			</td>
		</tr>
	...
	<div class="">
		...
		<input class="float-end ms-2 btn btn-default validate" type="button" onclick="qtc_closePopUp();" value="<?php echo Text::_('QTC_ADDATTRI_CANCEL')?>" >		<?php echo HTMLHelper::_( 'form.token' ); ?>
		<input class="float-end ms-2 btn btn-success validate" type="submit" onclick="submitbutton('attributes.save');" value="<?php echo Text::_('QTC_ADDATTRI_SAVE')?>" >		<?php echo HTMLHelper::_( 'form.token' ); ?>
