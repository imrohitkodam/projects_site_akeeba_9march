...
$add_link        = $qtc_base_url.'index.php?option=com_quick2cart&view=attributes&layout=attribute_bs5&tmpl=component&pid='.$pid.'&client='.$client;
...
<script>
 	...
 	{
		// Need to check why this added while page load then also reload perform
	...
	function toggleAddAttributeListModal(itemAttributeId = '')
	{
		if (itemAttributeId == '')
		{
			...
			jQuery('#addAttributeListModal').attr('style' , 'display:block !important');
		}
		else
		{
			...
			jQuery('#editAttributeListModal_'+itemAttributeId).attr('style' , 'display:block !important');
		}
	}
</script>
<div class="<?php echo Q2C_WRAPPER_CLASS; ?>" >
	...
	if( $pid && $client )
	{
		...
		<script type="text/javascript">
			function EditAttribute(att_id,pid)
			{
					...
					url: Joomla.getOptions('system.paths').base + '/index.php?option=com_quick2cart&task=attributes.EditAttribute&tmpl=component&att_id='+att_id+'&pid='+pid,
					type: 'GET',
					dataType: 'json',
			...
			function AddNewAttribute(pid,count)
			{
				...
				techjoomla.jQuery.ajax({
					url: Joomla.getOptions('system.paths').base + '/index.php?option=com_quick2cart&task=attributes.AddNewAttribute&tmpl=component&pid='+pid+'&count='+rowCount,
					type: 'GET',
					dataType: 'json',
					success: function(data)
					{
			...
			function deleteAttribute(dellink,pid)
			{
				if (confirm("<?php echo Text::_( 'COM_QUICK2CART_ATTRIBUTE_DELET_CONFIRM' ); ?>") == true)
				{
					...
					techjoomla.jQuery.ajax({
						url: Joomla.getOptions('system.paths').base + '/index.php?option=com_quick2cart&task=attributes.delattribute&tmpl=component&pid='+pid+'&attr_id='+dellink,
						type: 'GET',
						dataType: 'json',
					...
					if(!empty($attributes) )
					{
						...
						{ ?>
							<tr class="<?php echo "att_".$attributes->itemattribute_id; ?>">
								...
								<td>
									<button
										class="btn btn-sm btn-primary qtcAddAttributeLink"
										type="button"
									...
									?>
									<button type="button" class="btn  btn-sm btn-danger "  onclick="deleteAttribute('<?php echo $attributes->itemattribute_id;?>','<?php echo $item_id; ?>' )">
	...
	?>
	<button class="btn btn-sm btn-primary qtcAddAttributeLink" type="button" onclick="toggleAddAttributeListModal();" data-target="#addAttributeListModal">
