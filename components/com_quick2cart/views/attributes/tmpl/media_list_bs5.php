...
if( !empty($pid) && $client )
{
	...
	if(!empty($item_id))
	{
		...
		$addMediaLink   = $qtc_base_url.'index.php?option=com_quick2cart&view=attributes&layout=media_bs5&tmpl=component&item_id='.$item_id;
	}
	?>
	<script type="text/javascript">
		...
		function toggleAddMediaListModal(itemAttributeId = '')
		{
			if (itemAttributeId == '')
			{
				...
				jQuery('#addMediaListModal').attr('style' , 'display:block !important');
			}
			else
			{
				...
				jQuery('#editMediaListModal_'+itemAttributeId).attr('style' , 'display:block !important');
			}
		}
	</script>
							...
							<td>
								...
								<?php
									echo HTMLHelper::_(
										...
										array(
											'title'		 => Text::_('QTC_EDIT_MEDIA_FILES'),
											'url'        => $edit_link,
											'modalWidth' => '80',
											'bodyHeight' => '70',
											'width'      => '100px',
...
?>
<button class="btn btn-sm btn-primary qtcAddMediaLink" type="button" onclick="toggleAddMediaListModal();" data-target="#addMediaListModal">
