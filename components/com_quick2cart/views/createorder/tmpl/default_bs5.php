...
$itemid = $this->comquick2cartHelper->getitemid('index.php?option=com_quick2cart&view=product');
?>
<script>
	var deleteAddressMessage = Joomla.Text._('COM_QUICK2CART_DELETE_ADDRESS');
	techjoomla.jQuery(window).on('load',function (){
	...
	techjoomla.jQuery(document).ready(function ()
	{
		...
		techjoomla.jQuery('.nav-tabs li').click(function()
		{
			...
			{
				alert(Joomla.Text._('COM_QUICK2CART_SELECT_CLIENT_ERROR_MSG'));
				return false;
			}
		});
	});
	...
	function qtcaddMoreProduct()
	{
		...
		clone.find('#qtcorder_productdetails0_chosen').remove();
		clone.find('#qtcorder_productdetails0store_id_chosen').remove();
		...
		techjoomla.jQuery("#qtc_remove_Product_button"+i).append('<a class="btn btn-danger btn-sm" onclick="qtcremoveProduct('+i+');" title="'+Joomla.Text._('COM_Q2C_REMOVE_TOOLTIP')+'"><i class=" <?php echo QTC_ICON_MINUS; ?> "></i>');
	...
	function qtcGetProductShippingDetails(productId, prod_container_num)
	{
		...
		var callurl  = Joomla.getOptions('system.paths').base + "/index.php?option=com_quick2cart&task=product.loadProductShippingDetails&tmpl=component&prodId="+productId+"&qty="+qty+"&tamt="+tamt+"&shipping="+shipping+"&billing="+billing;
		...
		techjoomla.jQuery.ajax({
			...
			success: function(data)
			{
				...
				techjoomla.jQuery.each(shipping_info, function(i, item)
				{
					...
					{
						flagForChecked++;
						var check = (flagForChecked == 1) ? " checked='true'" : "";
						...
						if (itemz.hasOwnProperty('totalShipCost'))
						{
							...
							techjoomla.jQuery("#qtc_product_shipping_details"+prod_container_num).append('<div class="col-lg-3"><label class="radio"><input type="radio" name="itemshipMeth['+prod_container_num+']['+item.item_id+']" value="'+itemz.methodId+'" aria-invalid="false" '+check+'><span>'+currency+'&nbsp;'+itemz.totalShipCost+'</span>&nbsp;&nbsp;&nbsp;'+itemz.name+'</label></div>');
						}
					});
				});
			}
	...
	function qtcgetProductDetails(prod_container_num)
	{
		...
		if (productId != '')
		{
			var callurl = Joomla.getOptions('system.paths').base + "/index.php?option=com_quick2cart&task=product.loadProductDetails&tmpl=component&prodId="+productId+"&prod_container_num="+prod_container_num+"&renderer_view=createorder";
			...
			techjoomla.jQuery.ajax({
				...
				success: function(data)
				{
					...
					techjoomla.jQuery('#qtc_prod_quantity'+prod_container_num).attr("step", prod_info.lot);
					...
					{
						techjoomla.jQuery('#qtc_product_attributes'+prod_container_num).append("<div class='col-lg-4 prod_attribute_options' name='attribute_options[]'><div>"+prod_info.attribute_html[key].name+"</div>"+prod_info.attribute_html[key].html+"<br><br></div>");
					}
...
<div class="<?php echo Q2C_WRAPPER_CLASS; ?> container-fluid">
	<form name="createOrder" id="createOrder" class="form-validate form-horizontal" method="post">
		...
		<div class="row">
			<div class="col-lg-12">
				<h1><strong><?php echo Text::_("COM_QUICK2CART_CREATE_ORDER_VIEW_TITLE");?></strong></h1>
			</div>
		</div>
		<div class="row">
			<div class="col-lg-12 alert alert-info"><?php echo Text::_("COM_QUICK2CART_CREATE_ORDER_VIEW_INFO");?></div>
		</div>
		<div class="row qtc-createorder-buttons">
			<div class="col-sm-12">
				<a class="btn btn-primary col-lg-3 float-end ms-2 me-2 opacity-75 text-light"
					onclick="addClient('<?php echo Uri::root();?>', 'addClientModal');jQuery('#addClientModal').removeClass('hide')">
					<i class="fa fa-user">&nbsp;</i>
					<?php echo Text::_("COM_QUICK2CART_CREATE_ORDER_ADD_CLIENT");?>
				</a>
				
				<a class="btn btn-primary col-lg-3 float-end opacity-75 text-light"
					target="_blank"
					href="<?php echo Route::_( 'index.php?option=com_quick2cart&view=product&Itemid=' . $itemid); ?>">
					<i class=" icon-plus-2 icon22-white22 ">&nbsp;</i>
					<?php echo Text::_("COM_QUICK2CART_POINTS_ADD_PRODUCT_TITLE");?>
				</a>

				<?php
				$addClientUrl = Uri::root() . "index.php?option=com_quick2cart&view=registration&layout=addnewuser&tmpl=component";
				echo HTMLHelper::_(
					'bootstrap.renderModal',
					'addClientModal',
					array(
						'url'        => $addClientUrl,
						'title'      => Text::_('COM_QUICK2CART_CREATE_ORDER_ADD_CLIENT'),
						'width'      => '100%',
						'height'     => '350px',
						'modalWidth' => '80',
						'bodyHeight' => '60'
		...
		<div class="row">
			<div class="col-sm-12">
				<ul class="nav nav-tabs" role="tablist">
					<li id="select-customer-tab" class="nav-item active">
						<a href="#qtctab_selectclient" data-bs-toggle="tab" class="nav-link active" role="tab" aria-controls="select-customer" aria-selected="true">
							<?php echo Text::_( "COM_QUICK2CART_SELECT_CUSTOMER"); ?>
						</a>
					</li>
					<li id="select-product-tab" class="nav-item">
						<a href="#qtctab_addproduct" data-bs-toggle="tab" class="nav-link" role="tab" aria-controls="select-product" aria-selected="false">
							<?php echo Text::_( "COM_QUICK2CART_SELECT_PRODUCT"); ?>
						</a>
					</li>
				</ul>
				<div class="tab-content">
					<div class="tab-pane active" id="qtctab_selectclient">
						<div id="qtc_order_error" class="row">&nbsp;</div>
						<div class="form-group row">
							<div class="form-label col-md-2">
								<?php echo Text::_("COM_QUICK2CART_SELECT_CUSTOMER"); ?>
							</div>
							<div class="col-md-4">
								<?php echo HTMLHelper::_('select.genericlist', $this->users, "qtcuser", 'class="form-select ad-status input-medium required" size="1" required onchange="showAddresses();" name="qtcuser"', "id", "username");?>
							</div>
						</div>
						<div class="clearfix">&nbsp;</div>
						<div id="qtc_user_addresses" class="">
							<div class="qtc_user_addresses_wrapper row"></div>
						</div>
						<div class="row">
							<div class="col-sm-12">
								<div class="qtcadd_address_button center">
									<a class="btn btn-success text-light" onclick="addAddress('<?php echo JUri::root();?>', 'addAddressModal'); jQuery('#addAddressModal').removeClass('hide')">
										<?php echo JText::_('COM_QUICK2CART_ADD_CUSTOMER_ADDRESS');?>
									</a>
									<?php
							...
							<div class="col-sm-12">
								<div class="qtc_next_button float-end" >
									<a class="btn btn-success text-light" onclick="nextstep()"><?php echo Text::_('QTC_NEXT_BUY');?></a>
								</div>
							</div>
						</div>
					</div>
						...
						<div id="qtc_add_product_wrapper">
							<div id="qtc_product_clone">
								<div id="qtc_product_div0" class="qtc_product_div">
									<div class="row">
										<div class="col-lg-3">
											<label id="qtcorder_productdetails_label0" for="qtcorder_productdetails0store_id">
												<?php echo Text::_("COM_QUICK2CART_SELET_STORE"); ?>
											</label>
											<div>
												<?php echo HTMLHelper::_('select.genericlist', $this->stores, "qtcorder_productdetails[0][store_id]", 'class="form-select ad-status input-medium required qtcorder_select_chosen" required="required" size="1" onchange="qtcpopulateProducts(\'0\');"', "store_id", "title");?>
											</div>
										</div>
										<div class="col-lg-3 qtc_select_product_select_div0">
											<label id="qtc_select_product_select0" for="qtcorder_productdetails0" class="">
												<?php echo Text::_("COM_QUICK2CART_SELECT_PRODUCT"); ?>
											</label>
											<select name="qtcorder_productdetails[0][product_id]" id="qtcorder_productdetails0" class="required qtcorder_select_chosen form-select " required="required" onchange="qtcgetProductDetails('0')">
												<option selected="selected"><?php echo Text::_('COM_QUICK2CART_SELECT_PRODUCT')?></option>
											</select>
										</div>
										<div class="col-lg-3 center qtc_prod_price_div">
											<label for="qtc_prod_price0"><?php echo Text::_("QTC_CART_PRICE");?></label>
											<div>
												<span id="qtc_prod_price0" class="input form-control">-</span>
											</div>
										</div>
										<div class="col-lg-3">
											<label for="qtc_prod_quantity0"><?php echo Text::_("QTC_PRODUCT_QTY");?></label>
											<input
												id="qtc_prod_quantity0"
												min="0"
												max="0"
												step="0"
												name="qtcorder_productdetails[0][product_quantity]"
												class="input form-control"
												type="number"
												value="">
										</div>
										<div id="qtc_remove_Product_button0" class="col-lg-1"></div>
										<div class="row mt-2">
											<div id="qtc_product_attributes0" class="col-lg-12 row"></div>
										</div>
										<div id="qtc_product_shipping_details_div0" class="hidden row qtc_product_shipping_title_div">
<!--
											<h4 class="col-lg-12"><?php echo Text::_("COM_QUICK2CART_CHOOSE_YOUR_DELIVARY_OPTION");?></h4>
-->
										</div>
										<div id="qtc_product_shipping_details0" class="row qtc_product_shipping_method_div"></div>
									</div>
								</div>
							</div>
						</div>
						<h3>
							<div class="float-start qtc-total-order-price">
								<label for="qtc_prod_total0" class="form-label mb-0 me-1">
									<?php echo Text::_("QTC_TOTALPRICE");?> :
								</label>
								<span id="qtc_prod_total_currency0"></span>
								<span id="qtc_prod_total0" class="ms-1">-</span>
								<input type="hidden" name="qtc_product_total[0][ptotal]" value='0' />
							</div>
						</h3>
						<div class="clearfix"></div>
						<div class="row">
							<div class="col-lg-12 alert alert-info">
								<?php echo Text::_("COM_QUICK2CART_CREATE_ORDER_VIEW_ORDER_INFO");?>
							</div>
						</div>
						<div class="qtc-create-order-buttons center">
							<a onclick="qtcaddMoreProduct();" class="btn btn-sm btn-primary text-white" title="Add More">
								<i class=" icon-plus-2 icon22-white22 "></i><?php echo Text::_('COM_QUICK2CART_CREATE_ORDER_ADD_MORE_PRODUCTS');?>
							</a>
							<a onclick="qtc_place_order();" class="btn btn-sm btn-success text-white">
