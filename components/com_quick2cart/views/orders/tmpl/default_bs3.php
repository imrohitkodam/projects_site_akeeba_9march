		...
		if($orders_site)
		{
			if($vendor_order_view==1)
			{
				...
				if (!empty($this->storeReleatedView))
				{
					...
					{
						?>
						<h1><strong><?php echo Text::_('QTC_STORE_CUS_ORDERS')?> </strong></h1>
						<?php
					}
				}
			}
			...
			{
				?>
				<div class="page-header"><h1><strong><?php echo Text::_('QTC_MYORDERS')?>&nbsp;</strong></h1></div>
				<?php
			}
		}
		?>
		...
		<div class="">
			<div id="qtc-filter-bar" class="qtc-btn-toolbar">
				...
				<div class="qtc-btn-group pull-right hidden-xs q2c-btn-wrapper">
					<?php
					echo HTMLHelper::_('select.genericlist', $this->sstatus, "search_select", 'class=""  onchange="document.adminForm.submit();" name="search_select"',"value", "text", $this->lists['search_select']);
					?>
				</div>
						...
						foreach ($result as $orders)
						{
							...
							<tr class="">
								...
								<td class="qtc_pending_action q2c_width_30 small" data-title="<?php echo Text::_('QTC_ORDER_STATUS');?>">
									...
									if ((!($orders_site) || !empty($this->storeReleatedView)))
									{
										...
										<div class="row" >
											<span class="pull-left">
												<label class="radio-inline af-mb-5" for="add_note_chk_<?php echo $orders->id;?>">
											...
											<span class="pull-left">
												<label class="radio-inline af-mb-5" for="notify_chk_<?php echo $orders->id;?>">
									...
									if (!($orders_site))
									{
										echo HTMLHelper::_('select.genericlist', $validStatuses, "pstatus", 'class="pad_status input-medium pull-left" size="1"', "value", "text", $orders->status, 'pstatus' . $orders->id);
									}
									else if(!empty($this->storeReleatedView))
									{
										...
										echo "&nbsp;" . HTMLHelper::_('select.genericlist', $temp_vendorstatus, "vendor_orderItemStatus", 'class="pad_status  pull-left" size="1" ;" autocomplete="off"', "value", "text", $orders->status, 'pstatus' . $orders->id);
									}
									else
									{
										?>
									...
									{ ?>
										<a class="btn btn-primary btn-sm validate" href="<?php echo Uri::base().substr(Route::_('index.php?option=com_quick2cart&view=orders&layout=order&orderid=' . $orders->id . '&Itemid=' . $this->orderDetailItemId . '&paybuttonstatus=1'), strlen(Uri::base(true)) + 1) . '#complete-order';?>">
									...
									if (!($orders_site))
									{ ?>
										<button type="button" class="btn btn-primary btn-sm af-ml-5" onClick="updateOrderStatus(<?php echo $orders->id; ?>, this);">
