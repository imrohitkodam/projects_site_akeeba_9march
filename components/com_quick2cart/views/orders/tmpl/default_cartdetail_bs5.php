							...
							<td colspan="<?php echo (($showoptioncol == 1) ? "5" : "4");?>" style="<?php echo ($orders_email) ? $emailStyle_td . "padding-left:5px;": '';?>">
								...
								if (empty($orders_email) && ($view != "cartcheckout"))
								{
									...
									<div class="invoice-pdf qtcHandPointer" style="float:right;">
										<a href="<?php echo Uri::root().substr(Route::_('index.php?option=com_quick2cart&view=orders&layout=invoice_bs5&orderid=' .  $this->orderinfo->id. '&tmpl=component&store_id=' . $order->store_id . $streLinkPrarm . '&Itemid=' . $invoiceItemid),strlen(Uri::base(true))+1); ?>"
										target="_blank">
