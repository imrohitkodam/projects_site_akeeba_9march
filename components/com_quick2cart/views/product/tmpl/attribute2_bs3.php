...
<div class="container-fluid">
				...
				<tbody>
					...
					<tr class="attributeTrHeading">
						<td width="33%" align="left" data-title="<?php echo Text::_('QTC_PROD_SEL_ATTRIBUTE');?>">
							...
							echo HTMLHelper::_('select.genericlist', $attributeSetOptions, "att_detail[". $i . "][global_attribute_set]",'class="qtcAttriSetType" sutocomplete="off" style="" onChange="qtcLoadArribute(this.id, ' . $i . ', qtcJsData)" ','value','text',$selectedAttrSet, "qtcAttriSetTypeId" . (string) $i);
							?>
						...
						<td	align="left" data-title="<?php echo Text::_('COM_QUICK2CART_IS_STOCKABLE_UNIT');?>" class="<?php echo ($stockableAttrId || $qtc_stck_att  || $productType != 2) ? "qtc_hideEle" : ''?>">
							<?php
							$qtc_stck_att="";
					...
					<tr>
						<td data-title="<?php echo Text::_('QTC_ADDATTRI_NAME');?>">
							<input id="atrri_name" name="att_detail[<?php echo $i; ?>][attri_name]" class="input-medium bill qtc_attrib " type="text" value="<?php echo (isset($this->allAttribues[$i]->itemattribute_name)) ? htmlentities($this->allAttribues[$i]->itemattribute_name) : ''; ?>" maxlength="250"  title="<?php echo Text::_('QTC_ADDATTRI_NAME_DESC')?>">
							<!-- Hidded field -->
							<input id="atrri_name_id" name="att_detail[<?php echo $i; ?>][attri_id]" class="input-medium bill " type="hidden" value="<?php echo (isset($this->allAttribues[$i]->itemattribute_id))?$this->allAttribues[$i]->itemattribute_id:''; ?>" maxlength="250"  title="<?php echo Text::_('QTC_ADDATTRI_NAME_DESC')?>">
						</td>
						<td data-title="<?php echo Text::_('QTC_ADDATTRI_FIELD_TYPE_TO_USE');?>" style="<?php echo $hideFieldsForStockableAttribute ?>">
							...
							<div >
								<?php echo HTMLHelper::_('select.genericlist', $fields, "att_detail[$i][fieldType]", 'class="no_chzn qtcfieldType input-small" data-chosen="qtc" onChange="qtc_fieldTypeChange('.$fnparam.')"', "value", "text",$default);?>
							</div>
						</td>
						<td data-title="<?php echo Text::_('QTC_ATT_COMPALSARY_CK');?>" style="<?php echo $hideFieldsForStockableAttribute ?>">
							<label class="qtcAttrCheckbox" >
								...
								?>
								<input type="checkbox" class="checkboxdiv" name="att_detail[<?php echo $i; ?>][iscompulsary_attr]" id="att_detail[<?php echo $i; ?>][iscompulsary_attr]" autocomplete="off" <?php echo $qtc_ck_att;?> >
							</label>
						</td>
					</tr>
				</tbody>
	...
	<div class="qtcMarginTop10px">
				...
				?>
					<tr class="form-inline clonedInput" id="attri_opts<?php echo $k; ?>" >
						...
						<td data-title="<?php echo Text::_('QTC_PROD_STATUS');?>">
							...
							echo HTMLHelper::_('select.genericlist', $option_status, "att_detail[$i][attri_opt][$k][state]", 'class="chzn-done qtc-attribute-option-state"', "value", "text", $defOptionStatus);
							?>
						</td>
						<td class="<?php echo $stockRelatedFildClass; ?> qtcStockSkufields" data-title="<?php echo Text::_('QTC_ADDATTRI_STOCK');?>">
							...
							<input type="number" class="qtcAttriOptionStock" name="att_detail[<?php echo $i; ?>][attri_opt][<?php echo $k; ?>][stock]" Onkeyup="" onchange=""	 id=""   placeholder="<?php echo Text::_('QTC_ADDATTRI_STOCK')?>" value="<?php echo isset($childProdDetail->stock) ? $childProdDetail->stock : '' ?>">
						</td>
						<td class="<?php echo $stockRelatedFildClass; ?> qtcStockSkufields" data-title="<?php echo Text::_('QTC_ADDATTRI_SKU');?>">
							<input type="text" class="qtcAttriOptionSku" name="att_detail[<?php echo $i; ?>][attri_opt][<?php echo $k; ?>][sku]" Onkeyup="" onchange=""	 id="" class=""  placeholder="<?php echo Text::_('QTC_ADDATTRI_SKU')?>" value="<?php echo !empty($childProdDetail->sku) ? $childProdDetail->sku : '' ?>">
						</td>
						<td data-title="<?php echo Text::_('QTC_ADDATTRI_OPTPREFIX');?>">
							...
							echo HTMLHelper::_('select.genericlist', $addpre_select, "att_detail[$i][attri_opt][$k][prefix]", 'class="chzn-done qtc-attribute-option-prefix" data-chosen="qtc"', "value", "text", $addpre_val);
						?>
						</td>
						...
						<td data-title="<?php echo Text::_('QTC_ADDATTRI_OPTORDER');?>">
							<input type="text" name="att_detail[<?php echo $i; ?>][attri_opt][<?php echo $k; ?>][order]" Onkeyup="checkforalpha(this,'',<?php echo $entered_numerics; ?>);" onchange="qtc_ispositive(this)"	 id="" class="qtc-attribute-Option-order"  placeholder="<?php echo Text::_('QTC_ADDATTRI_OPTORDER')?>" value="<?php echo (isset($this->attribute_opt[$k]->ordering))?$this->attribute_opt[$k]->ordering:1; ?>">
						</td>
		...
		<div class="">
			<?php $hideAddGlobalOptionFields = ($selectedAttrSet && $this->allAttribues[$i]->global_attribute_id) ? 0 : 1;?>
