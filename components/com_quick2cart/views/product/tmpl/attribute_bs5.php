...
$attributeSetOptions[] = HTMLHelper::_('select.option',0, Text::_('COM_QUICK2CART_ADDATTRI_CUSTOM_ATTRIBUTE'));

if (!empty($this->attributeSetList))
{
	$attributeSetOptions[] = HTMLHelper::_('select.option', '<OPTGROUP>', Text::_('COM_QUICK2CART_SEL_FROM_GLOBAL_ATTRIBUTE_SET'));

	foreach ($this->attributeSetList as $attriSet)
	{
		$attributeSetOptions[] = HTMLHelper::_('select.option', $attriSet->id, $attriSet->attribute_name);
	}

	$attributeSetOptions[] = HTMLHelper::_('select.option', '</OPTGROUP>');
}
?>
<script type="text/javascript">
	...
	var qtcJsData = {
		...
		qtcLoadArributefunction_URL: Joomla.getOptions('system.paths').base + "/index.php?option=com_quick2cart&task=product.loadGlobalAttriOptions&tmpl=component",
		urfnName : function() {
			return '';
		}
	};
	...
	*/
	...
	function addClone(rId,rClass)
	{
		...
		var removeButton="<div class='com_qtc_remove_button float-start col-xs-12 col-md-1'>";
		removeButton+="<button class='btn btn-sm btn-danger' type='button' id='remove"+num+"'";
		...
		techjoomla.jQuery('#'+newelementid+ ' table tr').each(function()
		{
			...
			{ // avoid first tr (which is used for giving heading to column)
				...
				var addOptionBtn=" <button type='button' id='"+newelementid+"' class='btnAdd btn btn-sm btn-primary'  onclick='addopt(this.id);'><i class='" + iconplus +"  '></i></button> ";
	...
	*/
	function addopt(attribute_container_id)
	{
		...
		techjoomla.jQuery('#'+current_attribute+' .btnAdd').last().replaceWith('<button type=\"button\" class=\"btn btn-sm btn-danger qtcRemoveOption\" id=\"btnRemove'+num+'\" title=\"Joomla.Text._(\"QTC_REMOVE_MORE_OPTION_TITLE\");\" onclick=\"techjoomla.jQuery(this).closest(\'tr\').remove();\" ><i class=\"' + qtcJsData.Q2C_ICON_TRASH + ' \"></i></button> ');
		...
		<div class="">
			<div class="">
				<div class="alert alert-info ">
					<?php echo Text::_('COM_QUICK2CART_ADD_ATTRIBUTE_FIELD_DETAILS');?>
				</div>
			...
			?>
			<div id=<?php echo $attribute_container_id ?> class="qtc_container row af-mt-5 mt-2 qtc_att_hide" >
				<div class="col-md-11">
					<div class="com_qtc_repeating_block well well-small col-xs-12 col-md-11  com_qtc_img_border">
						<?php
						// For hidden attribute template
						if ( !empty($productType) && $productType == 2)
						{
							$stockCheckboxClass = !empty($stockableAttrId) ? "qtc_hideEle": '';
							$stockRelatedFildClass = "qtc_hideEle";
						}
						else
						{
						...
						$att_list_path = $comquick2cartHelper->getViewpath('product','attribute2_bs5',"SITE","SITE");
						ob_start();
			...
			<?php
				...
				for($i=1;$i<=$count;$i++) // for each attribute
				{
					...
					?>
					<div id=<?php echo $attribute_container_id ?> class="qtc_container row af-mt-5 mt-2">
						<div class="col-md-11">
							<div class="com_qtc_repeating_block well well-small col-xs-12 col-md-11 com_qtc_img_border">
								<?php
						...
						<div class="col-md-1">
							<div class='com_qtc_remove_button float-start col-xs-12 col-md-1 '>
								<button class='btn btn-sm btn-danger ' type='button' id='remove<?php echo $i;?>'
									onclick="removeClone('qtc_container<?php echo $i;?>','qtc_container');" title="<?php echo Text::_('COM_QUICK2CRT_REMOVE_TOOLTIP');?>" >
			...
			<div class="com_qtc_add_button float-start  col-xs-12 col-md-offset-11 col-md-1">
				<button class="btn btn-sm btn-primary mt-2" type="button" id='add'
