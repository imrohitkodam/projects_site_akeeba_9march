...
$fileUploadMode        = $this->params->get('eProdUploadMode',1);
$eProdUExpiryMode      = $this->params->get('eProdUExpiryMode','epboth');
$allowedFileExtensions = $this->params->get('allowedFileExtensions');
$eProdMaxFileLimit     = $this->params->get('eProdMaxFileLimit',5);
$allowedFileExtensions = $this->params->get('allowedFileExtensions',"zip,doc,docx,pdf,xls,txt,gz,gzip,rar,jpg,gif,tar.gz,xlsx,pps,csv,bmp,epg,ico,odg,odp,ods,odt,png,ppt,swf,xcf,wmv,avi,mkv,mp3,ogg,flac,wma,fla,flv,mp4,wav,aac,mov,epub,png,jpg");
$extArr                = explode(',', $this->params->get('allowedFileExtensions'));
...
<script type="text/javascript">
	...
	var maxAllowedMedia  = <?php echo $eProdMaxFileLimit-1  ?>;
	var r                = new Array();
	var fileUpload_maxFileSize=<?php echo $this->params->get('eProdMaxSize')*1024*1024;?>;
	var fileUpload_acceptFileTypes = [<?php
			$arr=explode(',', $this->params->get('allowedFileExtensions'));
			echo "'". implode("','", $arr) ."'" ;
				?>];
	var qtcUploadTarget = '<?php echo Uri::root();?>index.php?option=com_quick2cart&task=product.mediaUpload&tmpl=component';
	...
	function addCloneMedia(rId,rClass)
	{
		...
		{
			var num=media_current_id + 1;

			// CREATE REMOVE BUTTON
			var removeButton="<div class='col-md-12'>";
			removeButton+="<button class='btn btn-sm btn-danger float-end' type='button' id='remove"+num+"'";
			removeButton+="onclick=\"removeClone('"+rId+num+"','"+rClass+"');\" title=\"<?php echo Text::_('COM_QUICK2CRT_REMOVE_TOOLTIP');?>\" >";
			...
			newElem.find('.qtcMedUploadModeWrapper').removeClass('af-d-none');
			newElem.find('.qtcMedUploadWrapper').removeClass('af-d-none');

			//HIDE product link
			newElem.find('.qtcMediaProdLink').addClass('af-d-none');
		...
		{
			alert("<?php echo Text::sprintf('COM_QUICK2CART_MEDIA_REACHED_MAX_ADD_MEDIALIMIT',$eProdMaxFileLimit); ?>");
		}
	}
	...
	function changeUploadMethod(val,medNum)
	{
		if (val == 'upload')
		{
			...
			techjoomla.jQuery("#qtc_progress-barWrapper"+medNum).removeClass('af-d-none');

			// hide filePath textbox
			techjoomla.jQuery("#ajax_upload_hidden"+medNum).addClass('af-d-none');
		}
		else
		{
			// hide file field
			techjoomla.jQuery("#qtcMediaFile"+medNum).hide();
			techjoomla.jQuery("#qtc_progress-barWrapper"+medNum).addClass('af-d-none');

			// show filePath textbox
			techjoomla.jQuery("#ajax_upload_hidden"+medNum).removeClass('af-d-none');
		}
	}
	...
	function qtcProdMedUpload(index)
	{
		...
		str = str +  obj+".on('fileSuccess', function(file){ " + "var len = file.chunks.length ; " + 
			"if (jQuery.parseJSON(file.chunks[len-1].xhr.response).validate.error === 1)  "+
			" { "+
				 ' alert("<?php echo Text::_('COM_QUICK2CART_FILE_UPLOAD_ERROR_MSG', true); ?>"); ' +
			"}" +
			"else "+
			"{  "+
			"	jQuery('#ajax_upload_hidden"+index+ "').val( jQuery.parseJSON(file.chunks[len-1].xhr.response).fileUpload.filePath ); "+
				"setTimeout(function () {" + 
					'alert("<?php echo Text::_('COM_QUICK2CART_UPLOAD_SUCCESS_MSG', true); ?>"); ' +
				"}, 1000);" +
			"} "+
		"}); " +
		";  ";
...
<div class="">
	<!-- Allowed file types -->
	<?php $allowedFileExtensionsMSG = str_replace(",", ", ", $allowedFileExtensions);?>
	<div class="row-fluid">
		<div class="alert alert-info">
			<span>
				<?php echo Text::sprintf("COM_QUICK2CRT_MEDIA_SUPPORTED_FILE_TYPES",$allowedFileExtensionsMSG);?>
				<b><?php echo Text::sprintf("COM_QUICK2CRT_MEDIA_LIMIT_MSG",$eProdMaxFileLimit);?>  </b>
			</span>
		</div>
	</div>
	<div class="row">
		...
		<div id=<?php echo $attribute_container_id ?> class="qtc_mediaContainer qtc_media_hide " >
			<div class="com_qtc_media_repeat_block well well-small col-md-11 com_qtc_img_border">
				...
				// CHECK for view override
					...
					$att_list_path       = $comquick2cartHelper->getViewpath('product','media_bs5',"SITE","SITE");
					ob_start();
		...
		for($m=1; $m<=$mediaCount+1  && $m < $eProdMaxFileLimit+1; $m++) // for each attribute
		{
			...
			<div id=<?php echo $attribute_container_id ?> class="qtc_mediaContainer">
				...
				<div class='col-md-12'>
					<button class='btn btn-sm btn-danger float-end' type='button' id='remove<?php echo $m;?>'
						onclick="removeClone('qtc_mediaContainer<?php echo $m;?>','qtc_mediaContainer');" title="<?php echo Text::_('COM_QUICK2CRT_REMOVE_TOOLTIP');?>" >
		...
		<div class=" col-md-1 ">
			...
			title="<?php echo Text::_('COM_QUICK2CRT_ADDMORE_TOOLTIP');?>">
