...
$isFavorite = $product->isFavorite($itemDetailObj->item_id, Factory::getUser()->id);
$user       = Factory::getUser();
$isLoggedIn = $user->guest ? '1' : '0';

?>

<div class="qtc-prod-pin-inner mt-1 rounded-1">
	<div class="qtc-prod-pin-header ">
		<div id="wishlistButton<?php echo $itemDetailObj->item_id;?>" class="ms-3 btn p-0 border-0" 
			data-product-id="<?php echo $itemDetailObj->item_id; ?>"
			data-logged-in="<?php echo $isLoggedIn; ?>"
			data-user-id="<?php echo $user->id; ?>"
			data-ajax-url="<?php echo Uri::root(); ?>index.php?option=com_quick2cart&task=productpage.toggleFavourite"
			onclick="toggleFavourite(event)">
			<svg xmlns="http://www.w3.org/2000/svg" class="ms-1 mt-2 cursor-pointer" width="25" height="25" viewBox="0 0 20 16">
				<path id="heartPath<?php echo $itemDetailObj->item_id;?>" 
					d="M8.695 16.682C4.06 12.382 1 9.536 1 6.065 
					1 3.219 3.178 1 5.95 1c1.566 0 3.069.746 4.05 1.915C10.981 1.745 
					12.484 1 14.05 1 16.822 1 19 3.22 19 6.065c0 3.471-3.06 6.316-7.695 
					10.617L10 17.897l-1.305-1.215z" 
					fill="<?php echo $isFavorite ? '#ff4343' : '#c2c2c2';?>" class="heart-path" stroke="" fill-rule="evenodd" opacity=".9">
				</path>
			</svg>
		</div>
	...
	<div class="qtc-prod-img-cover <?php if (empty($qtcTeaserShowBuyNowBtn)){ echo 'poos';} ?>">
		<a title="<?php echo htmlentities($data['name']);?>" href="<?php echo $product_link; ?>">
			...
			{
			?>
				<div class="productimg qtc-prod-img" style="background-image: url('<?php echo htmlentities($img) ; ?>'); height:<?php echo !empty($pinHeight) ? $pinHeight : 200;?>px"></div>
			<?php
			}
			else
			{
	...
	<div class="qtc-prod-footer-cover">
		<div class="qtc-prod-name-cover">
			<strong>
				<a title="<?php echo htmlentities($data['name']);?>" href="<?php echo $product_link; ?>" class="qtc-cv-prod-name text-dark fs-6 fw-bold text-decoration-none">
					<?php echo $prodname;?>
				</a>
			</strong>
		</div>
		<div class="qtc-prod-price-cover">
			<?php
			$discount_present = ($params->get('usedisc') && isset($item_price['discount_price']) && !is_null($item_price['discount_price']) && $item_price['discount_price'] > 0) ? 1 : 0;
			$p_price = (!is_null($item_price['discount_price']) && $item_price['discount_price'] >0) ? $item_price['discount_price'] : $item_price['price'];
			?>
			<?php
			if ($discount_present == 1)
			{
			?>
				<span class="qtc-offer-price mt-1">
			...
			?>
				<span class='qtcproductdiscount d-block w-50 mx-auto mt-2' title= "<?php echo Text::sprintf('QTC_PERCENT_OFF',round($discount_percent)."%");?>">
				...
				<div style="<?php echo $qtyDivStyle;?>" class="form-inline <?php echo $layout_to_load == 'fixed_layout' ? 'q2c-inline-block' : 'q2c-inline-grid'; ?>">
					...
					<div class="form-group q2c-table-cell-center q2c-item-to-cart-section-<?php echo $data['product_id'];?>" <?php echo $showQtyToCartSecStyle;?>>
						<div class="q2c-display-table">
							<div class="form-group q2c-table-cell-center">
								<div class="input-group" >
									<span class="input-group-text q2c-padding-5px"
										for="<?php echo $textboxid . $showQtyToCartSecStyleSuffix; ?>">
					...
					<div class="form-group q2c-table-cell-center q2c-item-to-cart-section-<?php echo $data['product_id'];?>" <?php echo $showQtyToCartSecStyle;?>>
						...
						<button class="btn btn-sm btn-success q2c-small-buy-button <?php echo $layout_to_load != 'fixed_layout' ? ' mt-2 ' : ' '; ?>" type="button" <?php echo $disableBuyBtn; ?> onclick="qtc_addtocart('<?php echo $fun_param; ?>');">
		...
		?>
		<div class="clearfix"></div>
		<?php $options_str = implode(',', $attribure_option_ids); ?>
	</div>
