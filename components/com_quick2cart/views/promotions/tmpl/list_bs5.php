...
<div id="filter-bar" class="js-stools">
	<div class="js-stools-container-selector btn-group float-start filter-search">
		...
		<button class="btn btn-secondary hasTooltip" id="clear-search-button" type="button" title="<?php echo Text::_('JSEARCH_FILTER_CLEAR'); ?>">
			<i class="fa fa-remove"></i>
		</button>
	</div>
	<div class="btn-group float-end hidden-xm q2c-btn-wrapper">
		<label for="limit" class="element-invisible">
			<?php echo Text::_('JFIELD_PLG_SEARCH_SEARCHLIMIT_DESC'); ?>
		</label>
		<?php echo $this->pagination->getLimitBox(); ?>
	</div>
	<div class="btn-group float-end hidden-phone q2c-btn-wrapper">
		<?php echo HTMLHelper::_('select.genericlist', $this->storeList, "filter_store", 'class="form-select input-medium" size="1" onchange="document.adminForm.submit();" name="filter_store"', "id", "title", $this->state->get('filter.store'));
		?>
	</div>
</div>
<div class="clearfix"></div>
<div id="no-more-tables">
	<table class="table table-striped table-hover border table-bordered mt-4" id="promotionList">
		<?php
		if (!empty($this->items))
		{
		?>
			...
			?>
				<tr class="row<?php echo $i % 2; ?>">
					...
					<td data-title="<?php echo Text::_('COM_QUICK2CART_PROMOTIONS_NAME');?>">
						<?php if ($canEdit) : ?>
						<a href="<?php echo Route::_('index.php?option=com_quick2cart&view=promotion&layout=default&id='.(int) $item->id . '&Itemid=' . $promotionsItemId); ?>"><?php echo $item->name; ?></a>
						<?php else: ?>
							<?php echo $item->name; ?>
						<?php endif; ?>
					</td>
