...
<script type="text/javascript">
	...
	function deleteShipProfileMethod(methodId,delBtn)
	{
		...
		techjoomla.jQuery.ajax({
			type : "POST",
			url : Joomla.getOptions('system.paths').base + "/index.php?option=com_quick2cart&view=<?php echo $actionViewName; ?> &task=<?php echo $actionControllerName; ?>.deleteShipProfileMethod&tmpl=component",
			data : data,
	...
	function qtcLoadPlgMethods()
	{
		...
		techjoomla.jQuery.ajax({
				type : "POST",
				url : Joomla.getOptions('system.paths').base + "/index.php?option=com_quick2cart&view=<?php echo $actionViewName; ?>&task=<?php echo $actionControllerName; ?>.qtcLoadPlgMethods&tmpl=component",
				data : data,
				dataType: "json",
				beforeSend: function()
				{
    ...
    function qtc_addShipMethod()
    {
		...
		techjoomla.jQuery.ajax({
					type : "POST",
					url : Joomla.getOptions('system.paths').base + "/index.php?option=com_quick2cart&view=<?php echo $actionViewName; ?>&task=<?php echo $actionControllerName; ?>.addShipMethod&tmpl=component",
					data : data,
					dataType: "json",
					beforeSend: function()
					{
					...
					<div class="form-group">
						...
						<div class="controls"><?php echo $this->form->getInput('store_id'); ?>
							<span class="help-block">
								...
								<?php echo Text::_('COM_QUICK2CART_SHIPPROFILE_CAN_NOT_CAHGNE_STORE_MSG'); ?></span>
						</div>
					</div>
					<?php
				}
