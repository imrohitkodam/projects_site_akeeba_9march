...
<div class="<?php echo Q2C_WRAPPER_CLASS; ?> my-stores">
	<form method="post" name="adminForm" id="adminForm" class="form-validate">
		...
		<div id="filter-bar" class="js-stools">
			...
			<div class="btn-group float-end hidden-xs btn-wrapper">
				<label for="limit" class="element-invisible">
					<?php echo Text::_('COM_QUICK2CART_SEARCH_SEARCHLIMIT_DESC'); ?>
				</label>
				<?php echo $this->pagination->getLimitBox(); ?>
			</div>
			<div class="btn-group float-end hidden-xs btn-wrapper af-mr-5">
				<?php echo HTMLHelper::_('select.genericlist', $this->statuses, "filter_published", 'class="form-select input-medium" onchange="document.adminForm.submit();" name="filter_published"', "value", "text", $this->state->get('filter.state'));?>
			</div>
		</div>
		...
		{ ?>
			...
			<table class="table table-striped table-bordered table-responsive table-hover border" id="storeList">
				<thead class="table-primary border table-bordered">
					<tr>
						...
						<?php if (isset($this->items[0]->published)) { ?>
							<th class="q2c_width_1 nowrap center fw-bold">
								<?php echo HTMLHelper::_('grid.sort', 'JSTATUS', 'published', $listDirn, $listOrder); ?>
							</th>
						<?php } ?>
						<th class='q2c_width_15 fw-bold'>
