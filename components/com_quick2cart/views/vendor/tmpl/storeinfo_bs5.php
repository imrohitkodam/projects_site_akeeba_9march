...
$integrate_with = $this->params->get('integrate_with','none', 'string');
				...
				<div class=" col-xs-12">
					<legend>
						...
						if (!empty($this->editstoreBtn) && (!empty($this->store_id)))
						{
							...
							echo "<button type='button' title=".Text::_( 'SA_EDIT' )." class='btn  btn_margin float-end btn-sm' onclick=\"window.open('".Route::_("index.php?option=com_quick2cart&view=vendor&layout=createstore&store_id=".$storeid."&Itemid=".$createstore_Itemid)."')\" >
						...
						<div class="col-xs-12">
							<p>
								...
								else
								{
									...
									$storeinfo_link     = Route::_('index.php?option=com_quick2cart&view=vendor&layout=storeinfo&Itemid='.$vendor_Itemid.'&store_id='.$sinfo['id'].'&tmpl=component');
									$description_length = strlen($sinfo['description'] );
									$limit              = $this->params->get("storeDescriptionLimit",10);
									...
									if ($limit < $description_length)
									{
										$modalConfig = array('width' => '600px', 
											'height' => '600px', 
											'title' => Text::_('QTC_VENDER_STORE_INFO'), 
											'closeButton' => true, 
											'modalWidth' => 80, 
											'bodyHeight' => 70);
										...
										?>
										<a href="javascript:;" title="<?php echo Text::_('QTC_READMORE')?>" data-bs-target="#qtcRemoveModal" data-bs-toggle="modal" class="qtc_modal" title="<?php echo Text::_('QTC_READMORE'); ?>">
