		...
		techjoomla.jQuery.ajax({
			...
			success : function(response)
			{
				...
				if (response.error != 1)
				{
					...
					var q = "'";
					var url = Joomla.getOptions('system.paths').base+"/index.php?option=com_quick2cart&view=zoneform&layout=setrule_bs5&id="+zone_id+"&zonerule_id="+zoneRuleId+"&tmpl=component";
			...
			<form action="<?php echo Route::_('index.php?option=com_quick2cart&layout=edit&id=' . (int) $this->item->id); ?>" method="post" enctype="multipart/form-data" name="zoneForm" id="zoneForm" class="form-validate">
					...
					<div class=" form-horizontal">
						...
						<div class="row">
							<div class="col-sm-5 col-xs-12">
								...
								echo HTMLHelper::_('select.genericlist',$options,'qtc_ZoneCountry','class="form-select col-sm-12 col-xs-12"  aria-invalid="false" autocomplete="off" onchange=\'qtc_generateState("jform[qtc_state_id]","jform_qtc_state_id")\' ','value','text',$default,'qtc_ZoneCountry');
								?>
							</div>
								...
								if (!empty($this->geozonerules))
								{
									...
									{ ?>
										<tr>
											...
											<td>
												<button class="btn btn-sm btn-primary" type="button" onclick="toggleSetZoneRuleModal('<?php echo $rule->id;?>');">
													<?php echo Text::_('COM_QUICK2CART_ZONERULE_EDIT'); ?>
												</button>
												<?php
													$setZoneRulelink = Route::_('index.php?option=com_quick2cart&view=zoneform&layout=setrule_bs5&id=' . $this->item->id . '&zonerule_id=' . $rule->id . '&tmpl=component');
													echo HTMLHelper::_(
				...
				<div class="float-end">
					...
					<?php if(!empty($this->item) && $this->item->get('id')){?>
						<button type="button" class="btn btn-secondary validate" title="<?php echo Text::_('COM_QUICK2CART_COMMON_SAVE_AND_CLOSE'); ?>" onclick="qtcsubmitAction('saveAndClose');">
