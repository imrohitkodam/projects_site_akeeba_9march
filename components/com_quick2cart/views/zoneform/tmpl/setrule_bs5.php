...
<script type="text/javascript">
	...
	*/
	...
	function qtcUpdateZoneRule()
	{
		...
		techjoomla.jQuery.ajax({
			type : "POST",
			url : Joomla.getOptions('system.paths').base + "/index.php?option=com_quick2cart&task=zoneform.updateZoneRule&tmpl=component",
			data : data,
			dataType: "json",
			success : function(response)
			{
...
<div class = "<?php echo Q2C_WRAPPER_CLASS; ?> container-fluid">
	<form action="<?php echo Route::_('index.php?option=com_quick2cart&layout=edit&id=' . (int) $previousId ); ?>" method="post" enctype="multipart/form-data" name="adminForm" id="zone-form" class="form-validate">
		<div class="">
			<legend><?php echo Text::_('COM_QUICK2CART_ZONE_COUNTRIES_AND_ZONES'); ?></legend>
			<div class="">
				...
				<div class="col-sm-4 col-xs-12">
					...
					echo $this->dropdown = HTMLHelper::_('select.genericlist',$options,'qtc_ZoneCountry','class="form-select col-sm-12 col-xs-12"  required="" aria-invalid="false"   autocomplete="off" onchange=\'qtc_generateState("jform[qtc_state_id]","jform_qtc_state_id")\' ','value','text',$default,'qtc_ZoneCountry');
					?>
				</div>
				</div>
				<div class="form-group">
					...
					<span id="qtcStateContainer">
						...
						if ($this->getRegionList)
						{
							...
							// now we must generate the select list and echo that
							 echo $stateList = HTMLHelper::_('select.genericlist', $options, 'jform[qtc_state_id]', '  class="form-select" autocomplete="off"', 'value', 'text',$default_region,'jform_qtc_state_id');
						}
						?>
					</span>
				</div>
				...
				<div class="col-sm-4 col-xs-12">
					<span >
						<input
							...
							onClick="qtcUpdateZoneRule(<?php echo $this->rule_id; ?>)"/>
					</span>
				</div>
			</div>
			...
			<div class="qtcMarginTop10px">
				<div class="error alert alert-danger qtcError" style="display: none;">
					...
					<i class="<?php echo QTC_ICON_REMOVE; ?> float-end" style="align: right;" onclick="jQuery(this).parent().fadeOut();"> </i>					
				</div>
			</div>
