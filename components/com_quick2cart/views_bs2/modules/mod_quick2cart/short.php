		...
		<table class="table table-condensed ">
			<!-- detailed view of modulecart Table -->
				...
				<thead class="qtc_cart_module_head">
					<tr class="qtcborderedrow">
						...
						<th class="rightalign"><?php echo Text::_('QTC_MOD_PRICE');?></th>
					</tr>
				</thead>
				<?php
				*/
			...
			<tbody class="qtc_modulebody">
				...
				if (!empty($cart))
				{
					...
					<tr>
						<td><?php echo Text::_('QTC_MOD_CART_ITEMS');?></td>
					...
					?>
					<tr>
						<td><strong> <?php echo Text::_('MOD_QUICK2CART_SUBTOTAL_TOTALPRICE');?></strong> </td>
					...
					echo $response = $layout->render($layoutData);
					// <!-- FOr promotion End -->
					...
					<tr class="active">
						<td colspan="2">
							<button class="btn btn-danger btn-sm btn_margin pull-left float-start span6" onclick="emptycart(<?php echo $msg_order_js . ",'" . $baseUrl."'"; ?>);" >
								<i class="icon-trash icon22-white22"></i>&nbsp;<?php echo Text::_('QTC_MOD_EMPTY_CART')?>
							</button>

							<?php
							$Itemid = $comquick2cartHelper->getitemid('index.php?option=com_quick2cart&view=cart');
							$ckout_btname=(isset($ckout_text))?$ckout_text:Text::_('QTC_CHKOUT'); ?>
							<button class="btn btn-primary btn-sm btn_margin pull-right span6" onclick="window.open('<?php echo Route::_('index.php?option=com_quick2cart&view=cartcheckout&Itemid='.$Itemid);?>','_self')" >
