...
<form name='qtcSetRateform' id="qtcSetRateform"  method="post" >
	<legend id="" ><?php echo Text::_("PLG_QTC_DEFAULT_ZONESHIPPING_SET_RATEFOR_") ?>&nbsp; <?php echo !empty($shipMethDetail['name']) ? ucfirst($shipMethDetail['name']) : ''; ?>
		...
		<span class="pull-right" >
			<?php
			 $backlink = Route::_("index.php?option=com_quick2cart&view=shipping&layout=list&plugview=default&extension_id=" . $extension_id . "&Itemid=" . $itemid . "&methodId=" . $methodId . '');
			 ?>
			<button type="button" onClick="location.href='<?php echo $backlink; ?>'" title="<?php echo Text::_("PLG_QTC_DEFAULT_ZONESHIPPING_SHIPTAXPROFILE_SETRATE_BACK_TITLE"); ?>" class="btn  btn-primary " >
			...
			<tbody>
				<tr>
					<td>
						...
						echo HTMLHelper::_('select.genericlist',$options,"zone_id",'class=""  required="" aria-invalid="false" size="1" ','value','text',$default,'zone_id');
						?>
					</td>
					...
					<td>
						<div class="input-append curr_margin ">
							<input type="text" name="rangeFrom" size="" id="qtc_shipping_range_start" value="" class=" input-mini " placeholder="<?php echo Text::_("PLG_QTC_DEFAULT_ZONESHIPPING_FROM_HINT"); ?>">
							<?php
						...
						<div class="input-append curr_margin ">
							<input type="text" name="rangeTo" size="" id="qtc_shipping_range_end" value="" class=" input-mini " placeholder="<?php echo Text::_("PLG_QTC_DEFAULT_ZONESHIPPING_TO_HINT"); ?>">
							<?php
							if ($showWeightUnite ==1 )
							{?>
							<span class="add-on "><?php echo $weightUniteSymbol; ?></span>
			...
			foreach ($rateList as $rate)
			{
				...
				<tr id="qtcRateId_<?php echo $rate['rateId'];?>">
					...
					<td id="qtcActionTd_<?php echo $qtcRateId;?>" >
						...
						href="<?php echo Route::_($editRateLink); ?>" class="modal qtc_modal">
							<input type="button" value="<?php echo Text::_("PLG_QTC_DEFAULT_ZONESHIPPING_EDIT"); ?>" 	class=" btn btn-primary">
						</a> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;

						<input onclick="qtcDeleteShipRate(<?php echo $rate['rateId']; ?>, this);" class="btn btn-danger" type="button" value="<?php echo Text::_("PLG_QTC_DEFAULT_ZONESHIPPING_DELETE"); ?>">
					</td>
				</tr>
