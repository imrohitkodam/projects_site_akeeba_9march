...
<?php
		...
		if(!empty($getMediaDetail))
		{
			...
			{ ?>
					<tr class="<?php echo "file_".$media['file_id']; ?>">
						...
						<td>
							<a  rel="{handler: 'iframe', size: {x: window.innerWidth-450, y: window.innerHeight-150}, onClose: function(){EditFile('<?php echo $media['file_id'];?>','<?php echo $item_id; ?>');}}" class="btn btn-sm btn-primary modal qtc_modal" href="<?php echo $edit_link; ?> "> <i class="<?php echo $qtc_icon_edit; ?> <?php echo Q2C_ICON_WHITECOLOR; ?>"></i>
							</a>
							<button type="button" class="btn btn-sm btn-danger "  onclick="deleteMediFile('<?php echo $media['file_id'];?>','<?php echo $item_id; ?>' )"><i class="<?php echo Q2C_ICON_TRASH; ?> <?php echo Q2C_ICON_WHITECOLOR; ?>"></i></button>
	...
	<a rel="{handler: 'iframe', size: {x: window.innerWidth-350, y: window.innerHeight-150}, onClose: function(){AddNewMedia(<?php echo $fparam; ?>);}}" class="btn btn-primary btn-sm <?php echo ($button_dis == ""?'modal':$button_dis)?> " href="<?php echo ($button_dis == ""?$addMediaLink:"javascript:void(0);")?> qtcAddMediaLink" style="display: inline;">
