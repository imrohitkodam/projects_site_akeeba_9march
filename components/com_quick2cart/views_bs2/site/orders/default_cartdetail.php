...
{ ?>
	<div class="qtcPadding1111">
		...
		if ($orders_email)
		{ ?>
			<h4 <?php echo $emailstyle;?>><?php echo Text::_('QTC_ORDER_DETAILS');?></h4>
			<?php
		}
		elseif ($orders_site)
		{ ?>
    ...
    border-spacing:0px;";
		...
		<div class="table-responsive" id='no-more-tables'>
			<table width="100%" class="table table-condensed table-bordered qtc-table" style="<?php echo ($orders_email) ? $emailStyle_Table : '';?>" cellspacing="0">
				<!-- 	<tr> <td><h4 <?php echo $emailstyle;?> ><?php echo Text::_('QTC_ORDER_DETAILS');?></h4></td> </tr> -->
				<thead>
				<tr class="hidden-xs hidden-sm" style="<?php echo ($orders_email) ? $emailStyle_tr : '';?>">
					<th class="cartitem_num" width="5%" align="left" style="<?php echo ($orders_email) ? $emailStyle_th : '';?>"> <?php echo Text::_('QTC_NO');?></th>
					<!--			<th><?php //echo JHtml::_( 'grid.sort', JText::_('GETWAY'),'processor', $this->lists['order_Dir'], $this->lists['order']);?></th> -->
					<th class="cartitem_name" align="left" style="<?php echo ($orders_email) ? $emailStyle_th : '';?>" ><?php echo Text::_('QTC_PRODUCT_NAM');?></th>
					<?php

					if ($showoptioncol == 1)
					{ ?>
					<th class="cartitem_opt" align="left" style="<?php echo ($orders_email) ? $emailStyle_th : '';?>" ><?php echo Text::_('QTC_PRODUCT_OPTS');?></th>
					<?php
					}?>
					<th class="cartitem_qty rightalign" width="5%"  style="<?php echo ($orders_email) ? $emailStyle_th : '';?>" ><?php echo Text::_('QTC_PRODUCT_QTY');?></th>
					<th class="cartitem_price rightalign" width="15%" style="<?php echo ($orders_email) ? $emailStyle_th. $emailStyle_priceNdTotPrice: '';?>"
						><?php echo Text::_('QTC_PRODUCT_PRICE');?></th>
					<th class="cartitem_tprice rightalign"  width="15%"
						 style="<?php echo ($orders_email) ? $emailStyle_th . $emailStyle_priceNdTotPrice : '';?>"><?php echo Text::_('QTC_PRODUCT_TPRICE');?></th>
				</tr>
				</thead>
				<?php
				...
				foreach ($this->orderitems as $order)
				{
							...
							<tr class="<?php echo $qtc_store_row_classes;?>" style="<?php echo !empty($qtc_store_row_style) ? $qtc_store_row_style : '';?>">
								...
								<td colspan="<?php echo (($showoptioncol == 1) ? "5" : "4");?>" style="<?php echo ($orders_email) ? $emailStyle_td . "padding-left:5px;": '';?>">
									...
									<div class="invoice-pdf" style="float:right;">
										<a href="<?php echo Uri::root().substr(Route::_('index.php?option=com_quick2cart&view=orders&layout=invoice&orderid=' .  $this->orderinfo->id. '&tmpl=component&store_id=' . $order->store_id . $streLinkPrarm . '&Itemid=' . $invoiceItemid),strlen(Uri::base(true))+1); ?>"
										target="_blank">
					...
					<tr class="row0" style="<?php echo ($orders_email) ? $emailStyle_tr : '';?>">
						...
						<td class="cartitem_name" data-title="<?php echo Text::_('QTC_PRODUCT_NAM');?>" style="<?php echo ($orders_email) ? $emailStyle_td : '';?>">
							...
							{ ?>
								<a href="<?php	echo $product_link;?>"><?php echo htmlspecialchars($order->order_item_name, ENT_COMPAT, 'UTF-8');?></a>
							<?php
							}
								...
								if (!empty($item_shipDetail['name']))
								{
									?>
										<span>
											<strong>
												<br />
												<?php echo Text::_('COM_QUICK2CART_ORDER_SHIP_METH') . ": " ?> </strong>
						...
						<td class="cartitem_qty rightalign" data-title="<?php echo Text::_('QTC_PRODUCT_QTY');?>" style="<?php echo ($orders_email) ? $emailStyle_td : '';?>"><?php echo $order->product_quantity;?></td>
				...
				<tr style="<?php echo ($orders_email) ? $emailStyle_tr : '';?>">
					<td colspan="<?php echo $col;?>" class="cartitem_tprice_label hidden-xs hidden-xs rightalign" align="left" style="<?php echo ($orders_email) ? $emailStyle_td . $emailStyle_priceNdTotPrice: '';?>"><strong><?php echo Text::_('QTC_PRODUCT_TOTAL');?></strong></td>
							...
							<strong><?php echo Text::_('COM_QUICK2CART_PROMOTION_DICOUNT');?></strong>
								...
								if (!empty($discount_detail) )
								{
									...
									?>
										...
										{
											?>
											<small><strong><?php echo Text::_('QTC_DISCOUNT_CODE') . " : " . $dis_detail->coupon_code . " "; ?> </strong>
											</small>
											<?php
										}
										?>
				...
				if ($calledStoreview == 1 && $layout !="invoice")
				{
					...
					<!--
					<tr style="<?php echo ($orders_email) ? $emailStyle_tr : '';?>">
						<td colspan="<?php echo $col;?>" class="cartitem_tprice_label hidden-xs hidden-xs rightalign" style="<?php echo ($orders_email) ? $emailStyle_td  . $emailStyle_priceNdTotPrice: '';?>" align="left"><strong><?php echo sprintf(Text::_('QTC_COMMISSION_CUT_NET_TOT'), '(' . $commission . '%)');?> 	</strong></td>
						<td class="cartitem_tprice rightalign" style="<?php echo ($orders_email) ? $emailStyle_td  . $emailStyle_priceNdTotPrice : '';?>"  data-title="<?php echo Text::_('QTC_COMMISSION_CUT_NET_TOT');?>" >
							<span id="cop_discount">
