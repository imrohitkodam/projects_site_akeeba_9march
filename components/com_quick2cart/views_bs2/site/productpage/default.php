					...
					<div class="row-fluid">
						...
						<div class="row-fluid">
							<div class="span8">
								...
								?>
								<?php
								if (property_exists($this->item, 'tags') && (!empty($this->item->tags->itemTags)))
								{
									?>
									<div class="tags">
										...
										{
											?>
											<span class="badge tag-2 tag-list<?php echo $key; ?>" itemprop="keywords">
												<?php echo $tag->title;?>
											</span>
											<?php
										}
						...
						<div class="form-horizontal" id="<?php echo $this->item_id;?>_item" style="width: auto;">
							...
							if ($this->attributes)
							{
								foreach ($this->attributes as $attribute)
								{
									...
									<div class="control-group">
										<label class="control-label ">
											<strong><?php echo $requiredLabel . ' ' . $attribute->itemattribute_name; ?></strong>
										</label>
										<?php
										$productHelper = new productHelper();
...
-->
								...
								/*if (empty($showqty) )
								{
									?>
									<div class="controls">
										<button class="btn btn-sm btn-success qtc_buyBtn_style" type="button"
											onclick="qtc_addtocart('<?php echo $fun_param; ?>');">
												<i class="<?php echo QTC_ICON_CART;?>"></i> <?php echo Text::_('QTC_ITEM_BUY');?>
										</button>
									</div>
				...
				<div class="" style="<?php echo $WhoBought_style; ?>">
					...
					<ul class="thumbnails qtc_ForLiStyle">
						...
						foreach ($this->peopleWhoBought as $data)
						{
							...
							{
								echo "</ul>";
								echo '<a href="index.php?option=com_quick2cart&view=productpage&layout=users&itemid=' . $this->item_id . '&tmpl=component" class="modal qtc_modal" rel="{size: {x: 700, y: 500}, handler:\'iframe\'}">' . Text::_('COM_QUICK2CART_SHOW_MORE') . '</a>';
								break;
							}
						}
						?>
