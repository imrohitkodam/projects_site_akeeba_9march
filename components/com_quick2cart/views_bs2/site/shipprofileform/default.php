...
<script type="text/javascript">
	...
	function deleteShipProfileMethod(methodId,delBtn)
	{
		...
		techjoomla.jQuery.ajax({
			type : "POST",
			url :"<?php echo Uri::base();?>index.php?option=com_quick2cart&view=shipprofileform&task=shipprofileform.deleteShipProfileMethod&tmpl=component",
			data : data,
	...
	function qtcLoadPlgMethods()
	{
		...
		techjoomla.jQuery.ajax({
				type : "POST",
				url :"<?php echo Uri::base();?>index.php?option=com_quick2cart&view=shipprofileform&task=shipprofileform.qtcLoadPlgMethods&tmpl=component",
				data : data,
				dataType: "json",
				beforeSend: function()
				{
    ...
    function qtc_addShipMethod()
    {
		...
		{
			techjoomla.jQuery('#qtcErrorContentDiv').html("<?php echo Text::_('COM_QUICK2CART_S_SHIPPLUGIN_INVALID_SELECTION'); ?>");
		...
		techjoomla.jQuery.ajax({
					type : "POST",
					url :"<?php echo Uri::base();?>index.php?option=com_quick2cart&view=shipprofileform&task=shipprofileform.addShipMethod&tmpl=component",
					data : data,
					dataType: "json",
					beforeSend: function()
					{
					...
					success : function(response)
					{
						...
						if (response.error == 0)
						{
							...
							var delLink = '<input onclick="deleteShipProfileMethod('+
										shipProfileMethodId+',this);" class="btn btn-danger" type="button" value="<?php echo Text::_('COM_QUICK2CART_PROFILERULE_DELETE'); ?>">';
	...
	<form id="shipprofileform" name="shipprofileform"   method="post" class="form-validate" enctype="multipart/form-data">
		...
		<div class="row-fluid">
			...
			<fieldset class="adminform">
				<legend><?php echo Text::_('COM_QUICK2CART_SHIPPROFILE'); ?></legend>
					...
					<div class="control-group">
						...
						<div class="controls"><?php echo $this->form->getInput('store_id'); ?>
							<span class="help-block">
								...
								<?php echo Text::_('COM_QUICK2CART_TAXPROFILE_CAN_NOT_CAHGNE_STORE_MSG'); ?></span>
						</div>
					</div>
					<?php
				}
				...
				<div class="row-fluid">
					...
					<div class="span4">
						<span id="qtcShipMethContainer">
							...
							echo $this->dropdown = HTMLHelper::_('select.genericlist',$options,'qtc_shipMethod','class=""  aria-invalid="false" data-chosen="qtc"   autocomplete="off" ','value','text',$default,'qtc_shipMethod');
							?>
						</span>
				...
				foreach ($this->shipMethods as $meths)
				{
					...
					<tr>
						...
						<td>
							...
							href="index.php?option=com_quick2cart&view=shipprofileform&layout=setrule&id=<?php echo $meths->shipprofile_id;?>&shipmethId=<?php echo $meths->id; ?>&tmpl=component" class="modal qtc_modal">
								<input type="button" value="<?php echo Text::_('COM_QUICK2CART_SHIPPROFIL_METH_EDIT'); ?>" 	class=" btn btn-primary">
							</a>
		...
		<div class="form-horizontal">
			...
			<?php if ($this->item->id): ?>
				<button type="button" class="btn  validate" title="<?php echo Text::_('COM_QUICK2CART_COMMON_SAVE_AND_CLOSE'); ?>" onclick="qtcsubmitAction('saveAndClose');">
					<?php echo Text::_('COM_QUICK2CART_COMMON_SAVE_AND_CLOSE'); ?>
				</button>
			<?php endif; ?>
			 <a href="<?php echo Route::_('index.php?option=com_quick2cart&task=shipprofileform.cancel&id='.$this->item->id); ?>&Itemid=<?php echo $store_cp_itemid; ?>" class="btn " title="<?php echo Text::_('COM_QUICK2CART_CANCEL_ITEM'); ?>">
