				...
				<div class="span12">
					<legend>
						...
						if (empty($this->editstoreBtn))
						{
							...
							$result = Factory::getApplication()->triggerEvent('onProductDisplaySocialOptions', array($this->storeDetailInfo['id'], 'com_quick2cart.vendor.storeinfo', $sinfo['title'], $storeLink));
						...
						{
							?>
							<p style="font-size: 13px;"><?php echo Text::sprintf('COM_QUICK2CART_CREATED_BY',$profile_path); ?></p>
							<?php
						}
						?>
					</legend>
					...
					<span>
						...
						?>
						<img align="right" class='img-rounded img-polaroid qtc_putmargin5px' src="<?php echo $img;?>" alt="<?php echo  Text::_('QTC_IMG_NOT_FOUND') ?>"/>

						<p>
							<?php
							$enableEditor = $params->get('enable_editor', 0);
							...
							else
							{
								...
								if ($limit < $description_length)
								{
									// SHOE READ MORE LINK
									?>
									<a title="<?php echo Text::_('QTC_READMORE')?>" class="modal qtc_modal" rel="{handler: 'iframe', size: {x: 720, y: 500}, onClose: function(){}}" class="modal qtc_modal " href="<?php echo $storeinfo_link;?>">
						...
						<?php
							...
							<address class="span4 qtcWordWrap">
								<strong><?php echo Text::_('VENDER_ADDRESS'); ?></strong>
								<br/>
								<span><?php echo $addStr; ?></span>
							</address>
						<?php } ?>
