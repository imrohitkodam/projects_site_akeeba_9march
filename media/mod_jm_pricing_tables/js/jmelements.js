(function($){
	...
	var JMElements = function(id, fields, lang, element_field){
		...
		this.cancel_btn.click(function(e){
			e.preventDefault();
			self.clearForm();
			$('#jform_params_highlight1').prop('checked', false);
			$("label[for='jform_params_highlight1']").removeClass('active');
			$('#jform_params_highlight0').prop('checked', false);
			$("label[for='jform_params_highlight0']").removeClass('active');
			self.toggleFormFields(0);
			$('#' + self.id + '_items').find('.jm-elements-edit-icon').removeClass('active');
			return false;
		});
