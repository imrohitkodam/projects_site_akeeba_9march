...
HTMLHelper::_('stylesheet','modules/mod_q2cfilters/assets/css/bootstrap-slider.css');
HTMLHelper::_('script','modules/mod_q2cfilters/assets/js/bootstrap-slider.js');
HTMLHelper::_('stylesheet','modules/mod_q2cfilters/assets/css/q2cfilters.css');
...
$selectedFilters = explode(',',$jinput->input->get('attributeoption', '', 'string'));
...
if (isset($filters))
{
	...
	{ ?>
		<br>
			<div class="center">
				<a class="btn btn-small btn-info qtc-mobile-filter-apply" onclick='qtcfiltersubmitmobile()'>
					<?php echo Text::_('MOD_Q2CFILTERS_APPLY_FILTERS');?>
				</a>
				<a class="btn btn-small btn-info" onclick='q2c_clearfilters()'>
					<?php echo Text::_('MOD_Q2CFILTERS_CLEAR_FILTERS');?>
				</a>
			</div>
		<?php
	}
...
?>
<b class="q2c-price-slider">
	<?php echo Text::_('MOD_Q2CFILTERS_PRICE_FILTER_RANGE');?>
</b>
<br>
<input
	id="q2c-price-filter-slider"
	type="text"
	style="width:100%"
	class="q2c-price-slider"
	value="" data-slider-min="<?php echo $priceRange['min'];?>"
	data-slider-max="<?php echo $priceRange['max'];?>"
	data-slider-step="5"
	data-slider-value="[<?php echo $min_price;?>, <?php echo $max_price +1;?>]"/><br><br>
...
<div class="<?php echo Q2C_WRAPPER_CLASS . ' ' . $params->get('moduleclass_sfx');?>">
	<form action="" method="post" name="filterForm" id="filterForm">
		<div id="qtcFilterWrapperDiv " style="<?php echo $filterStylings?>">
			<?php
			if (isset($filters))
			{
				foreach ($filters as $filterName => $filter)
				{
					...
					if (!empty($filter))
					{
						$layout = new FileLayout(str_replace('.php','',$filter->renderer), $basePath = JPATH_ROOT .'/components/com_quick2cart/layouts/globalattribute/renderer');
						$fieldHtml = $layout->render($filter);
						?>
						<div><?php echo $fieldHtml;?></div>
						<?php
					}
				}
			}
