		...
		<table class="table table-condensed table-mod-cart qtc-table">
			<tbody class="qtc_modulebody">
				...
				if (!empty($cart))
				{
					...
					$layout                 = new FileLayout("module.cart.bs3.promodetail");
					...
					<tr class="active">
						<td colspan="2">
							<button class="btn btn-danger btn-sm btn_margin pull-left" onclick="emptycart(<?php echo $msg_order_js . ",'" . $baseUrl."'"; ?>);" >
								<i class="icon-trash icon-white"></i>&nbsp;<?php echo Text::_('QTC_MOD_EMPTY_CART')?>
							</button>

							<?php
							$Itemid       = $comquick2cartHelper->getitemid('index.php?option=com_quick2cart&view=cart');
							$ckout_btname = (isset($ckout_text))?$ckout_text:Text::_('QTC_CHKOUT'); ?>
							<button class="btn btn-primary btn-sm btn_margin pull-right" onclick="window.open('<?php echo Route::_('index.php?option=com_quick2cart&view=cartcheckout&Itemid='.$Itemid);?>','_self')" >
							...
							<input
								type="hidden"
								name="qtc_cartDiffItemCount"
								id="qtc_cartDiffItemCount"
								value="<?php echo count($cart);?>">
						</td>
					</tr>
					<?php
