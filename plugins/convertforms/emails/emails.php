...
class plgConvertFormsEmails extends CMSPlugin
{
    /**
     * Form Object
     *
    ...
    public function onConvertFormsBackendFormPrepareFieldset($fieldset_name, &$fieldset)
    {
        ...
        if ((bool) Factory::getConfig()->get('mailonline'))
        {
            return;
        }
        ...
        $warning = '
            <div class="alert alert-error">
                ...
                Text::_('PLG_CONVERTFORMS_EMAILS_ERROR_MAIL_SENDING_DISABLED') . '
            </div>';
	...
	public function onContentBeforeSave($context, $form, $isNew)
	{
        ...
        foreach ($params->emails as $key => $email)
        {
            ...
            $error = Text::_('COM_CONVERTFORMS_EMAILS') . ' #' . $keyToID . ' - ';
            ...
            foreach ($options as $key => $option)
            {
                ...
                foreach ($optionValues as $optionValue)
                {
                    ...
                    if (is_string($result))
                    {
                        $form->setError($error . Text::_($option[0]) . ' - ' . $result);
                        return false;
                        break;
                    }
                }
    ...
    private function validateOption($string, $validateAsEmail = true, $required = true)
    {
        ...
        {
            return Text::sprintf('PLG_CONVERTFORMS_EMAILS_ERROR_BLANK', $string);
        }
        ...
        if (!empty($result[1]) && count($result[1]) > 0)
        {
            foreach ($result[1] as $key => $match)
            {
                ...
                {
                    return Text::sprintf('PLG_CONVERTFORMS_EMAILS_ERROR_UNKNOWN_SMART_TAG', $result[0][$key]);
                    break;
                }
            }
        ...
        if ($validateAsEmail && !empty($string))
        {
            ...
            {
                return Text::sprintf('PLG_CONVERTFORMS_EMAILS_ERROR_INVALID_EMAIL_ADDRESS', $string);
            }
        }
