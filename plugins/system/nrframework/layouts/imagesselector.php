	...
	for ($i = 0; $i < count($images); $i++)
	{
		...
		if ($pro_items)
		{
			foreach ($pro_items as $item)
			{
				...
				$isPro = true;
				$class = 'is-pro';
				$atts = 'data-pro-only="' . ($label ? Text::_($label) : Text::_('NR_THIS_STYLE')) . '"';
			}
		}
		...
		?>
		<div class="nr-images-selector-item image<?php echo $class ? ' ' . $class : ''; ?>"<?php echo $atts; ?> title="<?php echo Text::_($label); ?>">
			<?php if ($isPro): ?>
