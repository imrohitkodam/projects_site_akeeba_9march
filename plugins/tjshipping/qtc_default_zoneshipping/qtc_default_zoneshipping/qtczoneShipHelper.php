...
class QtczoneShipHelper
{
	...
	public function _viewHandler($jinput, $plugview = 'default')
	{
		...
		return $qtczoneShipHelper->$viewhandler($jinput);
	}
	...
	public function _shipViewHandler_default($jinput)
	{
		...
		switch ($plugtask)
		{
			case 'publish':
					...
					$actionStatus['statusMsg']  = Text::_('PLG_QTC_DEFAULT_ZONESHIPPING_METH_PUBLISHED_SUCCESS');
			break;

			case 'unpublish':
					$qtcshiphelper->changeShipMethState($jinput, 0);
					$actionStatus['urlPramStr'] = 'plugview=default';
					$actionStatus['statusMsg']  = Text::_('PLG_QTC_DEFAULT_ZONESHIPPING_METH_UNPUBLISHED_SUCCESS');
			break;
			...
			case 'delete':
					...
					if (!empty($status))
					{
						$actionStatus['statusMsg'] = Text::_('PLG_QTC_DEFAULT_ZONESHIPPING_METH_DELETED_SUCCESS');
					}
			break;
		}
		...
		switch ($plugtask)
		{
			...
			case 'qtcshipMethodSave':
				...
				if (!empty($shipMethId))
				{
					...
					$actionStatus['statusMsg'] = Text::_('PLG_QTC_DEFAULT_ZONESHIPPING_METH_SAVED');
				}
				else
				{
					// Not saved
					$shipMethId = $jinput->get('methodId', 0, 'INT');
					$actionStatus['urlPramStr'] = $actionStatus['urlPramStr'] . "&methodId=" . $shipMethId;
					$actionStatus['statusMsg'] = Text::_('PLG_QTC_DEFAULT_ZONESHIPPING_METH_NOTSAVED');
				}
			break;
	...
	public function _qtcLoadViewData($plugLayout = 'default', $jinput = '')
	{
		...
		switch ($plugLayout)
		{
			...
			case 'default': $data = $qtczoneShipHelper->ViewDataForShipmethListView($jinput);
			break;
		}
	...
	{
		$shipForm             = array();
		$shipForm['methodId'] = $shipMethId = $jinput->get('methodId', 0, 'INT');
		$qtczoneShipHelper    = new qtczoneShipHelper;
	...
	public function getShipTypeName($methType)
	{
		...
		{
			case 3:
				return  Text::_('PLG_QTC_DEFAULT_ZONESHIPPING_STORE_ITEM');
			break;

			case 1:
				return  Text::_('PLG_QTC_DEFAULT_ZONESHIPPING_STORE_QTY');
			break;

			case 2:
				return  Text::_('PLG_QTC_DEFAULT_ZONESHIPPING_STORE_WEIGHT');
			break;
		}
	}
	...
	public function getShipMethodChargeDetail($vars)
	{
		...
		$comp_params = ComponentHelper::getParams('com_quick2cart');
	...
	public function getFieldHtmlForShippingType($fieldData)
	{
		...
		switch ($shipping_type)
		{
			...
			$maxFieldHtml .= '<p class="text-info">' . Text::_("PLG_QTC_DEFAULT_ZONESHIPPING_MAXIMUM_AMT_HELP") . '</p>';
			$fieldLable = Text::_("PLG_QTC_DEFAULT_ZONESHIPPING_PRICE_LABLE");
			...
			$minFieldHtml   = '<input id="' . $minFieldId . '" name="' . $minFieldName . '" class=" form-control required validate-name" type="text" value="' . $minFieldAmount . '">';
			...
			$maxFieldHtml   = '<input id="' . $maxFieldId . '" name="' . $maxFieldName . '" class="form-control required validate-name" type="text" value="' . $maxFieldAmount . '">
			<p class="text-info">' . Text::_("PLG_QTC_DEFAULT_ZONESHIPPING_MAXIMUM_AMT_HELP") . '</p>';

				switch ($shipping_type)
				{
					case 1;
						$fieldLable = Text::_("PLG_QTC_DEFAULT_ZONESHIPPING_QTY_LABLE");
					break;

					case 2;
						$fieldLable = Text::_("PLG_QTC_DEFAULT_ZONESHIPPING_WT_LABLE");
					break;
		...
		$return['minFieldLable'] = Text::sprintf("PLG_QTC_DEFAULT_ZONESHIPPING_MIN_LMIT", $fieldLable);
		$return['maxFieldLable'] = Text::sprintf("PLG_QTC_DEFAULT_ZONESHIPPING_MAX_LMIT", $fieldLable);
