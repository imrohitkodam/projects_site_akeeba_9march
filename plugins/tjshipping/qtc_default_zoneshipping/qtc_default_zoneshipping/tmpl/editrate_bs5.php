...
function qtcUpdateShipMethRates()
{
	...
	techjoomla.jQuery('.qtc_handle_rates_validate input').each(function(){
		if (techjoomla.jQuery(this).val() == '')
		{
			var msg = "<?php echo Text::_('PLG_QTC_DEFAULT_ITEM_DATA_SAVED_SUCCESSFULLY'); ?>"
			alert(msg);
	...
	techjoomla.jQuery.ajax({
		url :"<?php echo Uri::base();?>index.php?option=com_quick2cart&task=shipping.qtcHandleShipAjaxCall&plugview=setrates&extension_id=" +extension_id +"&methodId="+ shipMethodId +"&rateId="+ rateId + '&tmpl=component',
		type: 'POST',
		async:false,
		data:values,
		dataType: 'json',
...
<form name='qtcEditRateform' id="qtcEditRateform" method="post" >
	<legend id="" >
		<?php echo Text::_("PLG_QTC_DEFAULT_ZONESHIPPING_UPDATE_SHIP_RATE") ?>&nbsp; <?php echo !empty($shipMethDetail['name']) ? ucfirst($shipMethDetail['name']) : ''; ?>
	</legend>
	<div class="">
		<table class=" table table-striped">
			...
			<tbody>
				<tr>
					...
					?>
						<td>
							<input type="number" id="qtc_shipping_range_start" class="form-control" name="rangeFrom" value="<?php echo isset($shipFormData['rateDetail']['rangeFrom']) ? $shipFormData['rateDetail']['rangeFrom'] : '';?>" size="8" />
							<br/>
							<?php echo Text::_("PLG_QTC_DEFAULT_ZONESHIPPING_TO"); ?>
							<br/>
							<input type="number" id="qtc_shipping_range_end" class="form-control" name="rangeTo" value="<?php echo isset($shipFormData['rateDetail']['rangeTo']) ? $shipFormData['rateDetail']['rangeTo'] : '';?>" size="8" />
						</td>
					<?php
					}
					?>
					...
					<td>
						<?php $fn_para =  $extension_id . "," . $rateId . ",'".  Uri::base() . "'" ;?>
						<input class="btn btn-primary" type="button" onclick="qtcUpdateShipMethRates()" value="<?php echo Text::_("PLG_QTC_DEFAULT_ZONESHIPPING_UPDATE_RATE"); ?>"
						class="button" />
					</td>
				</tr>
			</tbody>
...
<div class='row'>
	<div class="error alert alert-danger qtcError" style="display: none;">
		...
		<i class="icon-cancel float-end" style="align: right;" onclick="techjoomla.jQuery(this).parent().fadeOut();"> </i> <br />
		<hr />
