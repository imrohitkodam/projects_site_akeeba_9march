...
function qtcAddShipMethRates()
{
	...
	techjoomla.jQuery.ajax({
		url :"<?php echo Uri::base();?>index.php?option=com_quick2cart&task=shipping.qtcHandleShipAjaxCall&plugview=setrates_bs5&extension_id=" +extension_id +"&methodId="+ shipMethodId + '&tmpl=component',
		type: 'POST',
		async:false,
		data:values,
		dataType: 'json',
		...
		success: function(response)
		{
			...
			var editHref       = "<?php echo Uri::base();?>index.php?option=com_quick2cart&view=shipping&layout=list&plugview=editrate_bs5&extension_id=" +extension_id +"&methodId="+ shipMethodId + "&rateId=" + rateId + "&tmpl=component";
			...
			var delLink        = '<input onclick="qtcDeleteShipRate('+
						rateId+',this);" class="btn btn-danger btn-sm" type="button" value="<?php echo Text::_("PLG_QTC_DEFAULT_ZONESHIPPING_DELETE"); ?>">';
...
<form name='qtcSetRateform' id="qtcSetRateform"  method="post" class="container-fluid">
	...
	<div class="no-more-tables" id="qtc_shipping_list">
		<table class=" table table-striped">
			<thead>
				<tr>
					<th width="10%"><?php echo Text::_("PLG_QTC_DEFAULT_ZONESHIPPING_ZONES"); ?></th>
					<?php
					...
					?>
					<th width="15%"><?php echo Text::_("PLG_QTC_DEFAULT_ZONESHIPPING_SHIP_COST"); ?></th>
					<th width="15%"><?php echo Text::_("PLG_QTC_DEFAULT_ZONESHIPPING_HANDLE_FEE"); ?></th>
					<th width=""></th>
				</tr>
			</thead>
			<tbody>
				<tr>
					<td style="width:30%" data-title="<?php echo Text::_('PLG_QTC_DEFAULT_ZONESHIPPING_ZONES');?>">
						<?php
						$default   = 0;
						$options   = array();
						...
						echo HTMLHelper::_('select.genericlist',$options,"zone_id",'class="form-select col-sm-6"  required="" aria-invalid="false" size="1" ','value','text',$default,'zone_id');
						?>
					</td>
					...
					if ($shipMethDetail['shipping_type'] !=3)
					{
						...
						?>
						<td data-title="<?php echo Text::_('PLG_QTC_DEFAULT_ZONESHIPPING_RANGE');?>">
							<div class="input-group">
								<input
									type="number"
									name="rangeFrom"
									size=""
									id="qtc_shipping_range_start"
									value=""
									class="form-control"
									placeholder="<?php echo Text::_("PLG_QTC_DEFAULT_ZONESHIPPING_FROM_HINT"); ?>">
								<?php
							...
							<br/><?php echo Text::_("PLG_QTC_DEFAULT_ZONESHIPPING_TO"); ?><br/>
							...
							<div class="input-group">
								<input
									...
									name="rangeTo"
									size=""
									id="qtc_shipping_range_end"
									value=""
									class="form-control"
									placeholder="<?php echo Text::_("PLG_QTC_DEFAULT_ZONESHIPPING_TO_HINT"); ?>">
								<?php
								if ($showWeightUnite ==1 )
								{
									?>
					...
					?>
					<td class="qtc_ship_rates_validate" data-title="<?php echo Text::_('PLG_QTC_DEFAULT_ZONESHIPPING_SHIP_COST');?>">
						<?php echo $productHelper->getMultipleCurrFields($name = 'shipCost');?>
					</td>
					<td class="qtc_handle_rates_validate" data-title="<?php echo Text::_('PLG_QTC_DEFAULT_ZONESHIPPING_HANDLE_FEE');?>">
						<?php echo $productHelper->getMultipleCurrFields($name = 'handleCost');?>
					</td>
					<td>
						<input
							class="btn btn-primary"
							type="button"
							onclick="qtcAddShipMethRates()"
							value="<?php echo Text::_("PLG_QTC_DEFAULT_ZONESHIPPING_ADD_RATE"); ?>"
						class="button" />
					</td>
				</tr>
			</tbody>
				...
				?>
					<tr id="qtcRateId_<?php echo $rate['rateId'];?>">
						...
						<td id="qtcActionTd_<?php echo $qtcRateId;?>" >
							<?php $editRateLink = Route::_("index.php?option=com_quick2cart&view=shipping&layout=list&plugview=editrate&extension_id=" . $extension_id . "&Itemid=" . $itemid . "&methodId=" . $methodId . "&rateId=" . $rate['rateId'] . '&tmpl=component');?>

							<button class="btn btn-sm btn-success" type="button" onclick="toggleShippingZoneRateModal('<?php echo $rate['rateId'];?>');">
							...
							?>
							<input onclick="qtcDeleteShipRate(<?php echo $rate['rateId']; ?>, this);" class="btn btn-sm btn-danger" type="button" value="<?php echo Text::_("PLG_QTC_DEFAULT_ZONESHIPPING_DELETE"); ?>">
						</td>
					</tr>
				<?php
				}
